 ...
 * \sa AlphaSetMax
 */
unsigned char CxImage::AlphaGetMax() const
{
 ...
 */
void CxImage::AlphaSetMax(unsigned char nAlphaMax)
{
 ...
 */
void CxImage::AlphaSet(unsigned char level)
{
...
{
	if (pAlpha==NULL) {
		pAlpha = (unsigned char*)cxalloc(head.biWidth * head.biHeight);//malloc(head.biWidth * head.biHeight);
...
bool CxImage::AlphaCopy(CxImage &from)
{
	...
	if (pAlpha==NULL) pAlpha = (unsigned char*)cxalloc(head.biWidth * head.biHeight);//malloc(head.biWidth * head.biHeight);
 ...
 */
void CxImage::AlphaSet(const long x,const long y,const unsigned char level)
{
 ...
 */
unsigned char CxImage::AlphaGet(const long x,const long y)
{
 ...
 * \author ***bd*** 2.2004
 */
unsigned char* CxImage::AlphaGetPointer(const long x,const long y)
{
	if (pAlpha && IsInside(x,y)) return pAlpha+x+y*head.biWidth;
	return 0;
}
 ...
 * \author ***bd*** 2.2004
 */
unsigned char CxImage::BlindAlphaGet(const long x,const long y)
{
#ifdef _DEBUG
...
{
	RGBQUAD c;
	for(unsigned short ip=0; ip<head.biClrUsed;ip++){
		c=GetPaletteColor((unsigned char)ip);
		c.rgbReserved=0;
		SetPaletteColor((unsigned char)ip,c);
	}
}
////////////////////////////////////////////////////////////////////////////////
/**
	...
	if (head.biBitCount==24){
		for(long y=0; y<head.biHeight; y++){
			for(long x=0; x<head.biWidth; x++){
				...
				a1 = 256-a;
				c.rgbBlue = (unsigned char)((c.rgbBlue * a + a1 * info.nBkgndColor.rgbBlue)>>8);
				c.rgbGreen = (unsigned char)((c.rgbGreen * a + a1 * info.nBkgndColor.rgbGreen)>>8);
				c.rgbRed = (unsigned char)((c.rgbRed * a + a1 * info.nBkgndColor.rgbRed)>>8);
...
{
	if (!pAlpha) return false;
	unsigned char* pAlpha2 = (unsigned char*)cxalloc(head.biWidth * head.biHeight);//malloc(head.biWidth * head.biHeight);
