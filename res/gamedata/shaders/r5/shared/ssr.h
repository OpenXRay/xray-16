	...
	float roughnessBlur)
{
	...
	for (int i = 1; i <= steps; ++i)
	{
		...
		if (IsSkyDepth(rawDepth) || IsHudPixel(rawDepth, 0.0))
		{
#ifdef SSR_ACCEPT_SKY
			if (t > tMin * 1.25 && !IsHudPixel(rawDepth, 0.0))
			{
				hitUV = uv;
				hitConf = 0.06;
				hit = true;
...
{
	float3 N = normalize(lerp(normalize(normal), float3(0.0, 1.0, 0.0), 0.72));
	if (N.y < 0.0)
		N = -N;
	float3 V = normalize(position - eye_position);
	float3 R = reflect(V, N);
	...
	const float tMin = 1.25;

	float3 origin = position + float3(0.0, 1.0, 0.0) * 0.15;
	float waterCamDist = length(position - eye_position);
	float2 startUV = 0.0;
	float startW = 1.0;
	...
	for (int i = 1; i <= steps; ++i)
	{
		...
		if (length(scenePos - eye_position) < waterCamDist - 0.15)
		{
			prevUV = uv;
			tPrev = t;
			continue;
		...
		float dHit = 0.0;
		float sHit = SampleSceneViewZ(hitUV, dHit);
		if (IsSkyDepth(dHit) || IsHudPixel(dHit, 0.0))
			break;
		float3 hitPos = ReconstructWorld(hitUV, dHit);
		if (hitPos.y < position.y - 0.2)
			break;
		if (length(hitPos - eye_position) < waterCamDist - 0.15)
			break;
	...
	float roughnessBlur)
{
	...
	for (int i = 1; i <= steps; ++i)
	{
		...
		if (IsSkyDepth(rawDepth) || IsHudPixel(rawDepth, 0.0))
		{
			if (t > tMin * 1.5 && !IsHudPixel(rawDepth, 0.0))
			{
				hitUV = uv;
				hitConf = 0.05;
				hit = true;
