...
void main(uint3 dispatchID : SV_DispatchThreadID)
{
    ...
    float guideMark = RestirLoadTex4(t_WorldPos, pixel, giSize, fullSize).w;
    float classifyMark = RestirLoadTex4(t_ClassifyWorldPos, pixel, giSize, fullSize).w;
    if (depth <= 0.0 || SkipRtSurfLighting(classifyMark, guideMark)) {
        if (g_Mode == 1)
            u_SceneColor[pixel] = RestirLoadTex4(t_SceneColorIn, pixel, giSize, fullSize);
        u_FilteredDiffuse[pixel] = 0;
        u_FilteredSpecular[pixel] = 0;
        return;
    }

    float4 nPack = RestirLoadTex4(t_Normal, pixel, giSize, fullSize);
    ...
    [unroll] for (int iy = -2; iy <= 2; ++iy) {
        [unroll] for (int ix = -2; ix <= 2; ++ix) {
            ...
            {
                float nd = RestirLoadDepth(t_Depth, uint2(npD), giSize, fullSize);
                if (nd > 0.0 && SameHudSurfClass(guideMark, RestirLoadTex4(t_WorldPos, uint2(npD), giSize, fullSize).w))
                {
                    float3 nN = normalize(RestirLoadTex4(t_Normal, uint2(npD), giSize, fullSize).xyz);
                    float3 nD = SoftClampFirefly(t_NoisyDiffuse.Load(int3(npD, 0)).rgb, max(centerLumaD, 0.35), 8.0);
                    ...
                    w *= max(wGeo, 0.05);
                    float wD = w * exp(-abs(Luma(nD) - centerLumaD) * 0.35);
                    sumD += nD * wD;
                    wSumD += wD;
                }
            }
            ...
            if (npS.x < 0 || npS.y < 0 || npS.x >= (int)g_ScreenSize.x || npS.y >= (int)g_ScreenSize.y)
                continue;
            float ndS = RestirLoadDepth(t_Depth, uint2(npS), giSize, fullSize);
            if (ndS <= 0.0)
                continue;
            if (!SameHudSurfClass(guideMark, RestirLoadTex4(t_WorldPos, uint2(npS), giSize, fullSize).w))
                continue;

            float3 nNs = normalize(RestirLoadTex4(t_Normal, uint2(npS), giSize, fullSize).xyz);
            float3 nS = SoftClampFirefly(t_NoisySpecular.Load(int3(npS, 0)).rgb, max(centerLumaS, 0.35), 10.0);
    ...
    float3 filteredD = (wSumD > 1e-4) ? (sumD / wSumD) : SoftClampFirefly(noisyD, max(centerLumaD, 0.35), 8.0);
    float3 filteredS = (wSumS > 1e-4) ? (sumS / wSumS) : SoftClampFirefly(noisyS, max(centerLumaS, 0.35), 10.0);
    filteredD = clamp(filteredD, nbMinD * 0.25, nbMaxD * 3.0 + 0.02);
    ...
    if (g_Mode == 1) {
        float3 ambientBase = RestirLoadTex4(t_SceneColorIn, pixel, giSize, fullSize).rgb;
