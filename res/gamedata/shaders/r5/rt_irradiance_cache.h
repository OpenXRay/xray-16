    ...
    float envScale)
{
    ...
    return e.irradiance * fade * envScale;
}

float3 QueryIrradianceCache(
    StructuredBuffer<IrradianceCacheEntry> cache,
    float3 pos,
    float cellSize,
    uint cacheSize,
    uint frameIndex,
    ...
    float envScale)
{
    ...
    if (e.stamp == 0)
        return 0;
    uint age = frameIndex >= e.stamp ? frameIndex - e.stamp : 0;
    if (age > maxAge)
        return 0;
    float fade = 1.0 - saturate((float)age / (float)max(maxAge, 1u));
    return e.irradiance * fade * envScale;
    ...
    float envScale)
{
    ...
    float3 canon = irradiance / max(envScale, 1e-4);
    float3 blended = canon;
    if (prev.stamp != 0 && (frameIndex - prev.stamp) < 64u)
        blended = lerp(prev.irradiance, canon, 0.35);
