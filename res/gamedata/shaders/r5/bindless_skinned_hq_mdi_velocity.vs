...
VelocityVSOut main(VS_INPUT v)
{
    ...
    float3 currWorld = mul(rec.world, skinning_pos(v.P, mdi_curr_bone(rec.boneOffset, boneIdx))).xyz;
    float4x4 prevW = rec.prevValid ? rec.prevWorld : rec.world;
    float3 prevWorld = mul(prevW, skinning_pos(v.P, mdi_prev_bone(rec, boneIdx))).xyz;
