...
RWStructuredBuffer<uint> g_BucketCursor : register(u0);
RWStructuredBuffer<uint> g_Items : register(u1);
...
void main(uint3 dtID : SV_DispatchThreadID)
{
    ...
    {
        uint pos;
        InterlockedAdd(g_BucketCursor[baseB + uint(y * VSM_BUCKET_AXIS + x)], 1u, pos);
        if (pos < g_ItemCap)
        {
            g_Items[g_ItemBase + pos] = idx;
        }
        else
        {
            uint d;
