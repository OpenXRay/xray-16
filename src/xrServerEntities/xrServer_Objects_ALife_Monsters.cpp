...
void setup_location_types_section(GameGraph::TERRAIN_VECTOR& m_vertex_types, CInifile const* ini, LPCSTR section)
{
    ...
    for (; I != E; ++I)
    {
        ...
        if (N != GameGraph::LOCATION_TYPE_COUNT) continue;
    ...
    if (!m_vertex_types.empty()) return;
...
CSE_ALifeTraderAbstract::CSE_ALifeTraderAbstract(LPCSTR caSection)
{
    ...
    m_dwMoney = 0;
    if (pSettings->line_exist(caSection, "money")) m_dwMoney = pSettings->r_u32(caSection, "money");
...
void CSE_ALifeTraderAbstract::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion > 19) {
        ...
        if (m_wVersion > 62) tNetPacket.r_u32(m_dwMoney);
        ...
        if (m_wVersion > 77) m_trader_flags.assign(tNetPacket.r_u32());
        ...
        if (m_wVersion > 85) tNetPacket.r_s32(m_community_index);
        if (m_wVersion > 86) {
...
#pragma todo("Dima to Yura, MadMax : Remove that hacks, please!")
    ...
    if (m_SpecificCharacter.size()) return m_SpecificCharacter;
    ...
    else
    {
        ...
        for (int i = 0; i <= CSpecificCharacter::GetMaxIndex(); i++)
        {
            ...
            if (!char_info.data()->m_Class.size() || class_found) {
                ...
                if (spec_char.data()->m_bDefaultForCommunity) m_DefaultCharacters.push_back(id);
...
#ifdef XRGAME_EXPORTS
                        int* count = NULL;
                        if (ai().get_alife()) count = ai().alife().registry(specific_characters).object(id, true);
...
void CSE_ALifeTraderAbstract::set_specific_character(shared_str new_spec_char)
{
    ...
    if (m_SpecificCharacter.size()) {
        if (ai().get_alife()) ai().alife().registry(specific_characters).remove(m_SpecificCharacter, true);
    }
#endif
    ...
    if (selected_char.Visual()) {
        ...
        VERIFY(visual);
        if (xr_strlen(selected_char.Visual()) > 0) visual->set_visual(selected_char.Visual());
    }
    ...
    if (NO_COMMUNITY_INDEX == m_community_index) {
        ...
        if (creature) creature->s_team = selected_char.Community().team();
    }
    ...
    //----
    if (NO_RANK == m_rank) m_rank = selected_char.Rank();

    if (NO_REPUTATION == m_reputation) m_reputation = selected_char.Reputation();
...
void CSE_ALifeTrader::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if ((m_wVersion > 35) && (m_wVersion < 118)) tNetPacket.r_u32();
...
void CSE_ALifeCustomZone::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion > 102) tNetPacket.r_u32(m_owner_id);
...
void CSE_ALifeAnomalousZone::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if ((m_wVersion > 38) && (m_wVersion < 113)) tNetPacket.r_float();
...
{
    if (pSettings->line_exist(caSection, "visual")) set_visual(pSettings->r_string(caSection, "visual"));
...
void CSE_ALifeCreatureAbstract::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion > 18) tNetPacket.r_float(fHealth);

    if (m_wVersion < 115) fHealth /= 100.0f;

    if (m_wVersion < 32) visual_read(tNetPacket, m_wVersion);
    o_model = o_torso.yaw;
    ...
    if (m_wVersion > 94) set_killer_id(tNetPacket.r_u16());

    o_torso.pitch = o_Angle.x;
    o_torso.yaw = o_Angle.y;

    if (m_wVersion > 115) tNetPacket.r_u64(m_game_death_time);
}
...
void CSE_ALifeMonsterAbstract::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion > 72) {
        ...
        if (m_wVersion > 73) tNetPacket.r_stringZ(m_in_space_restrictors);
    }
...
void CSE_ALifeCreatureActor::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    if (m_wVersion < 21) {
        ...
        tNetPacket.r_u8(s_squad);
        tNetPacket.r_u8(s_group);
        if (m_wVersion > 18) set_health(tNetPacket.r_float());
        if (m_wVersion >= 3) visual_read(tNetPacket, m_wVersion);
    }
    else
    {
        inherited1::STATE_Read(tNetPacket, size);
...
void CSE_ALifeMonsterRat::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion <= 5) set_health(tNetPacket.r_float());
...
{
    inherited1::STATE_Read(tNetPacket, size);
    if (m_wVersion >= 68) inherited2::STATE_Read(tNetPacket, size);

    if (m_wVersion >= 109) tNetPacket.r_u16(m_spec_object_id);
}
...
void CSE_ALifeHumanAbstract::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if ((m_wVersion >= 110) && (m_wVersion < 112)) tNetPacket.r(&m_smart_terrain_id, sizeof(m_smart_terrain_id));
}
...
void CSE_ALifeHumanStalker::STATE_Read(NET_Packet& tNetPacket, u16 size)
{
    ...
    if (m_wVersion > 67) inherited2::STATE_Read(tNetPacket, size);

    if ((m_wVersion > 90) && (m_wVersion < 111)) tNetPacket.r_u8();
}
