    ...
    EPatrolRouteType tPatrolPathStop, bool bRandom, u32 index)
{
	...
	m_path				= GlobalEnv.AISpace->patrol_paths().path(m_path_name,true);
...
const Fvector &CPatrolPathParams::point	(u32 index) const
{
	...
	if (!m_path->vertex(index)) {
		GlobalEnv.ScriptEngine->script_log(LuaMessageType::Error,"Can't get information about patrol point number %d in the patrol way %s",index,*m_path_name);
