...
TEMPLATE_SPECIALIZATION
inline void CHashFixedVertexManager::add_opened(Vertex& vertex)
{
    vertex.opened() = 1;
}

TEMPLATE_SPECIALIZATION
inline void CHashFixedVertexManager::add_closed(Vertex& vertex)
{
    vertex.opened() = 0;
}
...
TEMPLATE_SPECIALIZATION
inline bool CHashFixedVertexManager::is_opened(const Vertex& vertex) const
{
...
TEMPLATE_SPECIALIZATION
inline bool CHashFixedVertexManager::is_closed(const Vertex& vertex) const
{
    ...
    Vertex& vertex, const Index& vertex_id)
{
    ...
    {
        if (index_vertex->m_next) index_vertex->m_next->m_prev = nullptr;
    ...
    if (!_vertex || _vertex->m_path_id != current_path_id() || _vertex->m_hash != index) _vertex = nullptr;
