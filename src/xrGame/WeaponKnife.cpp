...
void CWeaponKnife::LoadFireParams(LPCSTR section)
{
    ...
    xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPower_2[egdMaster]);
    _GetItem(*s_sHitPowerCritical_2, 0, buffer); //первый параметр - это хит для уровня игры мастер
    xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPowerCritical_2[egdMaster]);
    ...
    if (num_game_diff_param > 1) //если задан второй параметр хита
    {
        ...
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPower_2[egdVeteran]);
    }
    if (num_game_diff_param > 2) //если задан третий параметр хита
    {
        _GetItem(*s_sHitPower_2, 2, buffer); //то вычитываем его для уровня сталкера
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPower_2[egdStalker]);
    }
    if (num_game_diff_param > 3) //если задан четвёртый параметр хита
    {
        _GetItem(*s_sHitPower_2, 3, buffer); //то вычитываем его для уровня новичка
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPower_2[egdNovice]);
    }
    ...
    if (num_game_diff_param > 1) //если задан второй параметр хита
    {
        ...
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPowerCritical_2[egdVeteran]);
    }
    if (num_game_diff_param > 2) //если задан третий параметр хита
    {
        _GetItem(*s_sHitPowerCritical_2, 2, buffer); //то вычитываем его для уровня сталкера
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPowerCritical_2[egdStalker]);
    }
    if (num_game_diff_param > 3) //если задан четвёртый параметр хита
    {
        _GetItem(*s_sHitPowerCritical_2, 3, buffer); //то вычитываем его для уровня новичка
        xr_from_chars(buffer, buffer + xr_strlen(buffer), fvHitPowerCritical_2[egdNovice]);
    }
