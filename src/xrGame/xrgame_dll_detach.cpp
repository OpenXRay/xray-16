...
void init_game_globals()
{
    ...
    {
        CInfoPortion::InitInternal(pSettingsOpenXRay->read_if_exists<bool>("gameplay", "crash_on_fail_CInfoPortion", psGameMode.is_any(ClearSkyMode | ShadowOfChernobylMode)), true);
        CEncyclopediaArticle::InitInternal(pSettingsOpenXRay->read_if_exists<bool>("gameplay", "crash_on_fail_CEncyclopediaArticle", psGameMode.is(ShadowOfChernobylMode)), true);
