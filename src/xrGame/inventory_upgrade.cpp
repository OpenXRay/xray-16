...
void Upgrade::construct(const shared_str& upgrade_id, Group& parental_group, Manager& manager_r)
{
    ...
    const char* prereq_functor_str = pSettings->r_string(id(), "prereq_functor"); // prerequisites_functor
    //	const char* tooltip_functor_str	= pSettings->r_string( id(), "prereq_tooltip_functor" );
...
const char* Upgrade::get_prerequisites() { return m_prerequisites(); }
