...
void stalker_movement_manager_smart_cover::update(u32 time_delta)
{
    ...
    if (m_non_animated_loophole_change) non_animated_change_loophole();
...
{
    if (!object().g_Alive()) return (MotionID());
...
void stalker_movement_manager_smart_cover::on_animation_end()
{
    ...
    if (!m_target.cover()) {
        if (!m_current.cover()) unbind_global_selector();
...
void stalker_movement_manager_smart_cover::reach_enter_location(u32 const& time_delta)
{
    ...
    if (target_params().cover()->can_fire()) {
        ...
        if (!inventory_item) {
            if (!object().CObjectHandler::goal_reached()) return;
...
void stalker_movement_manager_smart_cover::enter_smart_cover(u32 const& time_delta)
{
    ...
    if (m_entering_smart_cover_with_animation) return;
...
bool stalker_movement_manager_smart_cover::default_behaviour() const
{
    ...
    VERIFY(m_target_selector);
    if (m_target_selector->callback()) return (m_default_behaviour);

    if (m_current.cover_fire_object()) return (true);

    if (m_current.cover_fire_position()) return (true);
...
{
    if (current_params().cover()) return (true);

    if (entering_smart_cover_with_animation()) return (true);
...
void stalker_movement_manager_smart_cover::remove_links(IGameObject* object)
{
    ...
    if (m_target.cover_fire_object() == object) m_target.cover_fire_object(0);

    if (m_current.cover_fire_object() == object) m_current.cover_fire_object(0);
}