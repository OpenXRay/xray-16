...
CMapLocation::CMapLocation(LPCSTR type, u16 object_id)
{
    ...
    if (!IsGameTypeSingle()) m_cached.m_LevelName = Level().name();
}
...
bool CMapLocation::Update()  // returns actual
{
    ...
    if (m_owner_se_object || (!IsGameTypeSingle() && pObject)) {
        ...
        if (IsGameTypeSingle()) CalcLevelName();
...
void CMapLocation::UpdateSpot(CUICustomMap* map, CMapSpot* sp)
{
    if (map->MapName() == GetLevelName()) {
        ...
        if (b_pointer) UpdateSpotPointer(map, GetSpotPointer(sp));
    }
...
void CMapLocation::UpdateMiniMap(CUICustomMap* map)
{
    ...
    if (!sp) return;
    if (SpotEnabled()) UpdateSpot(map, sp);
}
...
bool CRelationMapLocation::Update()
{
    ...
    else
    {
        ...
        if (pEntAlive) bAlive = !!pEntAlive->g_Alive();
    }
    shared_str sname;
    ...
    if (m_b_visible == false && vis_res == true) m_minimap_spot->ResetXformAnimation();
...
{
    if (IsVisible() && m_b_minimap_visible) inherited::UpdateMiniMap(map);
}

void CRelationMapLocation::UpdateLevelMap(CUICustomMap* map)
{
    if (IsVisible() && m_b_levelmap_visible) inherited::UpdateLevelMap(map);
}
