    ...
    u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
    ...
    if (!m_owner->is_item_in_group(kill_weapon_id, ammunition_group::gid_hand_grenades)) return;

    if (!tmp_local_player->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) return;
    ...
    if (!killer_actor || !victim_actor) return;
