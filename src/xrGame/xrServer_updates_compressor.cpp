...
server_updates_compressor::server_updates_compressor()
{
    ...
    if (!IsGameTypeSingle()) init_compression();
...
{
    if (m_acc_buff.w_tell() > 2) flush_accumulative_buffer();
    ...
    if (g_sv_write_updates_bin) {
        if (!dbg_update_bins_writer) create_update_bin_writer();
