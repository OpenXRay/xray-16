...
bool CCoverEvaluatorBase::inertia(Fvector const& position, float radius)
{
    ...
    if (time_criteria && radius_criteria) return true;

    if (!m_stalker) return false;
    ...
    if (!m_stalker->can_fire_right_now()) return false;
...
void CCoverEvaluatorCloseToEnemy::evaluate_cover(const CCoverPoint* cover_point, float weight)
{
    ...
    if ((enemy_distance <= m_min_distance) && (m_current_distance > enemy_distance)) return;

    if ((enemy_distance >= m_max_distance) && (m_current_distance < enemy_distance)) return;

    if (enemy_distance >= m_current_distance + m_deviation) return;
    ...
    if (enemy_distance >= m_best_value) return;
...
bool CCoverEvaluatorBest::threat_on_the_way(Fvector const& cover_position) const
{
    ...
    if (start_to_cover_magnitude < EPS_L) return (false);
    ...
    if (angle >= PI_DIV_6) return (false);

    if (projection > start_to_cover_magnitude * 1.5f) return (false);
...
void CCoverEvaluatorBest::evaluate_cover(const CCoverPoint* cover_point, float weight)
{
    ...
    if (m_use_smart_covers_only && !cover_point->m_is_smart_cover) return;
    ...
    if (threat_on_the_way(cover_point->position())) return;
    ...
    if ((value == m_best_value) && (cover_point > m_selected)) return;
...
void CCoverEvaluatorBest::evaluate_smart_cover(smart_cover::cover const* smart_cover, float const& weight)
{
	...
	return;
#else
    if (!m_can_use_smart_covers) return;
...
{
    if (m_start_position.distance_to(cover_point->position()) <= m_min_distance) return;
    ...
    float cover_value = _min(high_cover_value, low_cover_value);
    if (cover_value >= m_best_value) return;
...
void CCoverEvaluatorAmbush::evaluate_cover(const CCoverPoint* cover_point, float weight)
{
    ...
    if (my_distance <= m_min_enemy_distance) return;
