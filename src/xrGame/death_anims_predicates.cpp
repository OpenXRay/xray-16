    ...
    LPCSTR message, type_motion::edirection dr, const CEntityAlive& ea, const SHit& H, const MotionID& m)
{
    ...
    if (m.valid()) motion_name = KA->LL_MotionDefName_dbg(m).first;
...
class type_motion0 : public type_motion
{
    ...
    {
        m = MotionID();
        if (H.initiator() != Level().CurrentControlEntity()) return false;

        VERIFY(ea.Visual());
        IKinematics* K = ea.Visual()->dcast_PKinematics();
        VERIFY(K);
        if (!is_bone_head(*K, H.bone())) return false;

        // CAI_Stalker* s = ea.cast_stalker	();
        CCharacterPhysicsSupport* chs = ea.character_physics_support();
        if (!chs || chs->Type() == CCharacterPhysicsSupport::etBitting) return false;
        ...
        if (stalker_speed < min_stalker_speed) return false;
        ...
        if (stalker_velocity_dir.dotproduct(dir_to_actor) < front_angle_cos) return false;

        if (type_motion::front != type_motion::dir(ea, H, angle)) return false;
        Fvector p;
        if (Fvector().sub(H.initiator()->Position(), global_hit_position(p, ea, H)).magnitude() > 30.f) return false;
...
bool is_snipper(u16 weaponID)
{
    ...
    if (!WM) return false;
    if (!WM->IsZoomed()) return false;
    // if( !WM->SingleShotMode() )
    // return false;
    if (!WM->IsScopeAttached()) return false;
...
void death_anims::setup(IKinematicsAnimated* k, LPCSTR section, CInifile const* ini)
{
    ...
    anims[6] =
        (new type_motion3())->setup(k, ini, section, "kill_headshot");  // 4.	Хедшот (по вероятности), кроме 5 (4)
