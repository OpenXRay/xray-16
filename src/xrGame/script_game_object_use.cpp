...
Fvector CScriptGameObject::GetCurrentDirection()
{
	...
	if (!obj) {
		ai().script_engine().script_log		(LuaMessageType::Error,"Script Object : cannot access class member GetCurrentDirection!");
...
void CScriptGameObject::Kill					(CScriptGameObject* who)
{
	...
	if (!l_tpEntity) {
		ai().script_engine().script_log			(LuaMessageType::Error,"%s cannot access class member Kill!",*object().cName());
		return;
	}
...
ALife::ERelationType CScriptGameObject::GetRelationType	(CScriptGameObject* who)
{
	...
	if (!l_tpEntityAlive1) {
		ai().script_engine().script_log			(LuaMessageType::Error,"%s cannot access class member GetRelationType!",*object().cName());
		return ALife::eRelationTypeDummy;
	}
	
	CEntityAlive		*l_tpEntityAlive2 = smart_cast<CEntityAlive*>(&who->object());
	if (!l_tpEntityAlive2) {
		ai().script_engine().script_log			(LuaMessageType::Error,"%s cannot apply GetRelationType method for non-alive object!",*who->object().cName());
		return ALife::eRelationTypeDummy;
	}
...
IC	T	*CScriptGameObject::action_planner()
{
	...
	if (!manager) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CAI_Stalker : cannot access class member action_planner!");
		return							(0);
	}
...
void CScriptGameObject::set_enemy_callback	(const luabind::functor<bool> &functor)
{
	...
	if (!monster) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CCustomMonster : cannot access class member set_enemy_callback!");
		return;
	}
...
void CScriptGameObject::set_const_force(const Fvector &dir,float value,u32 time_interval)
{
	...
	if(!shell){
		ai().script_engine().script_log				(LuaMessageType::Error,"set_const_force : object %s has no physics shell!",*object().cName());
		return;
	}
//#ifdef DEBUG
