...
void CAI_Stalker::renderable_Render()
{
    ...
    if (!already_dead()) CInventoryOwner::renderable_Render();

#ifdef DEBUG
    if (g_Alive()) {
        if (psAI_Flags.test(aiAnimationStats)) animation().add_animation_stats();
    }
#endif // DEBUG
}
...
{
    if (!m_take_items_enabled && smart_cast<CInventoryItem*>(O)) return (false);

    if (O == this) return (false);

    if (!inherited::feel_touch_contact(O)) return (false);
...
bool CAI_Stalker::feel_touch_on_contact(IGameObject* O)
{
    ...
    if ((O->GetSpatialData().type | STYPE_VISIBLEFORAI) != O->GetSpatialData().type) return (false);
...
void CAI_Stalker::feel_touch_delete(IGameObject* O)
{
    ...
    if (i == m_ignored_touched_objects.end()) return;
