...
void CStateMonsterAttackMoveToHomePointAbstract::select_target()
{
    ...
    if (m_target_node != u32(-1)) squad->unlock_cover(m_target_node);

    for (u32 i = 0; i < 5; ++i)
    {
        m_target_node = object->Home->get_place_in_cover();
        if (m_target_node != self_node) break;
        m_target_node = u32(-1);
    }
...
{
    if (m_target_node == u32(-1)) {
        if (current_time() > m_selected_target_time + 500) select_target();
    }
    else
    {
        float const dist_to_target = object->Position().distance_to_xz(m_target_pos);
        if (dist_to_target < 2) select_target();
    }
...
{
    if (!object->at_home()) return true;

    if (!object->run_home_point_when_enemy_inaccessible()) return false;

    if (!object->enemy_accessible()) return true;
...
{
    if (!object->at_home()) return false;

    if (object->run_home_point_when_enemy_inaccessible()) {
        if (!object->enemy_accessible()) return false;
    }
