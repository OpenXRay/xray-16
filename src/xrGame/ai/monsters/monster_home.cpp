    ...
    if (!m_path) {
        ...
        if (result == u32(-1)) {
            ...
            if (result == u32(-1)) {
                ...
                if (result == u32(-1)) return m_object->ai_location().level_vertex_id();
            }
        }
    }
    else
...
{
    if (!m_path) {
        if (!ai().level_graph().valid_vertex_id(m_level_vertex_id)) return true;
    ...
    else
    {
        ...
        for (u32 i = 0; i < m_path->vertex_count(); i++)
        {
            ...
            if (!ai().level_graph().valid_vertex_id(level_vertex)) continue;
