...
bool CTelekineticObject::init(CTelekinesis* tele, CPhysicsShellHolder* obj, float s, float h, u32 ttk, bool rot)
{
    ...
    if (object->m_pPhysicsShell) object->m_pPhysicsShell->set_ApplyByGravity(FALSE);
...
void CTelekineticObject::raise(float step)
{
    ...
    if (OnServer()) (object->m_pPhysicsShell->get_ElementByStoreOrder(0))->applyGravityAccel(dir);
...
void CTelekineticObject::fire_t(const Fvector& target, float time)
{
    ...
    if (sound_throw._handle()) sound_throw.play_at_pos(object, object->Position());
...
{
    if (time_raise_started + RAISE_MAX_TIME < Device.dwTimeGlobal) return true;
