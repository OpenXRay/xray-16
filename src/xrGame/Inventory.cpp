...
void CInventory::Take(CGameObject* pObj, bool bNotActivate, bool strict_placement)
{
    ...
    if (!strict_placement) pIItem->m_ItemCurrPlace.type = eItemPlaceUndefined;
    ...
    case eItemPlaceBelt:
        ...
        if (!result) pIItem->m_ItemCurrPlace.type = eItemPlaceUndefined;
#ifdef DEBUG
    ...
    case eItemPlaceSlot:
        ...
        if (!result) pIItem->m_ItemCurrPlace.type = eItemPlaceUndefined;
#ifdef DEBUG
        if (!result) Msg("cant slot in slot item %s", *pIItem->object().cName());
#endif
        break;
    }
...
bool CInventory::Slot(u16 slot_id, PIItem pIItem, bool bNotActivate, bool strict_placement)
{
    ...
    if (ItemFromSlot(slot_id) == pIItem) return false;
    ...
    {
        if (it_ruck != m_ruck.end()) m_ruck.erase(it_ruck);
        if (it_belt != m_belt.end()) m_belt.erase(it_belt);
    }

    bool in_slot = InSlot(pIItem);
    if (in_slot && (pIItem->CurrSlot() != slot_id)) {
        if (GetActiveSlot() == pIItem->CurrSlot()) Activate(NO_ACTIVE_SLOT);
...
void CInventory::Activate(u16 slot, bool bForce)
{
    ...
    if (slot != NO_ACTIVE_SLOT) tmp_item = ItemFromSlot(slot);
...
//		Msg("--- There's no need to activate slot [%d], next active slot is [%d]", slot, m_iNextActiveSlot);
    ...
    if (slot != NO_ACTIVE_SLOT && !m_slots[slot].CanBeActivated()) return;
...
//			Msg("--- Inventory owner [%s]: send deactivate item [%s]", m_pOwner->Name(), active_item->NameItem());
        ...
        {
            if (tmp_item) tmp_item->ActivateItem();
...
PIItem CInventory::Get(LPCSTR name, bool bSearchRuck) const
{
    ...
    {
        PIItem pIItem = *it;
        if (!xr_strcmp(pIItem->object().cNameSect(), name) && pIItem->Useful()) return pIItem;
    }
    return NULL;
}
...
PIItem CInventory::Get(CLASS_ID cls_id, bool bSearchRuck) const
{
    ...
    {
        PIItem pIItem = *it;
        if (pIItem->object().CLS_ID == cls_id && pIItem->Useful()) return pIItem;
    }
    return NULL;
}
...
PIItem CInventory::Get(const u16 id, bool bSearchRuck) const
{
    ...
    {
        PIItem pIItem = *it;
        if (pIItem->object().ID() == id) return pIItem;
    }
    return NULL;
}
...
u32 CInventory::dwfGetSameItemCount(LPCSTR caSection, bool SearchAll)
{
    ...
    for (TIItemContainer::iterator l_it = l_list.begin(); l_list.end() != l_it; ++l_it)
    {
        ...
        if (!xr_strcmp(l_pIItem->object().cNameSect(), caSection)) ++l_dwCount;
    }
...
bool CInventory::bfCheckForObject(ALife::_OBJECT_ID tObjectID)
{
    ...
    {
        PIItem l_pIItem = *l_it;
        if (l_pIItem->object().ID() == tObjectID) return (true);
    }
    return (false);
}
...
CInventoryItem* CInventory::get_object_by_id(ALife::_OBJECT_ID tObjectID)
{
    ...
    {
        PIItem l_pIItem = *l_it;
        if (l_pIItem->object().ID() == tObjectID) return (l_pIItem);
    }
    return (0);
}
...
bool CInventory::Eat(PIItem pIItem)
{
    ...
    if (!pItemToEat->UseBy(entity_alive)) return false;
...
bool CInventory::CanPutInSlot(PIItem pIItem, u16 slot_id) const
{
    ...
    if (slot_id == HELMET_SLOT) {
        ...
        if (pOutfit && !pOutfit->bIsHelmetAvaliable) return false;
    }

    if (slot_id != NO_ACTIVE_SLOT && NULL == ItemFromSlot(slot_id)) return true;
    ...
    if (m_bSlotsUseful) {
        ...
        for (; I <= E; ++I)
        {
            ...
            if (item && (!for_trade || item->CanTrade())) {
                if (!SlotIsPersistent(I) || item->BaseSlot() == GRENADE_SLOT) items_container.push_back(item);
            }
        }
    }
}
...
bool CInventory::isBeautifulForActiveSlot(CInventoryItem* pIItem)
{
    ...
    {
        PIItem item = ItemFromSlot(I);
        if (item && item->IsNecessaryItem(pIItem)) return (true);
    }
    return (false);
}
...
void CInventory::TryDeactivateActiveSlot()
{
    ...
    if ((ActiveSlot == NO_ACTIVE_SLOT) && (NextActiveSlot == NO_ACTIVE_SLOT)) return;
