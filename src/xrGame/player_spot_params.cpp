...
{
u32 calculate_spots(game_PlayerState* ps)
{
    return ps->m_iRivalKills - (ps->m_iTeamKills * 2) - ps->m_iSelfKills + (ps->af_count * 3);
};

u32 player_spots_counter::get_u32_param()
{
...
u32 player_spots_with_top_enemy_divider::get_top_enemy_player_score()
{
    game_PlayerState* tmp_local_player = m_owner->get_local_player();
    if (!tmp_local_player)
        return 0;
...
float player_spots_with_top_enemy_divider::get_float_param()
{
