...
void CActor::IR_OnKeyboardPress(int cmd)
{
    ...
    case kWPN_FIRE:
    {
        ...
        if (inventory().ActiveItem() && (slot == INV_SLOT_3 || slot == INV_SLOT_2)) mstate_wishful &= ~mcSprint;
        //-----------------------------
        if (OnServer()) {
            NET_Packet P;
...
void CActor::IR_OnMouseMove(int dx, int dy)
{
    ...
    if (iitem && iitem->cast_hud_item()) iitem->cast_hud_item()->ResetSubStateTime();
...
bool CActor::use_Holder(CHolderCustom* holder)
{
    ...
    else
    {
        ...
        if (smart_cast<CCar*>(holder)) b = use_Vehicle(holder);
...
void CActor::ActorUse()
{
    ...
    if (!m_pUsableObject || m_pUsableObject->nonscript_usable()) {
        ...
        u16 element = BI_NONE;
        if (object) element = (u16)RQ.element;
...
void CActor::OnNextWeaponSlot()
{
    ...
    if (ActiveSlot == NO_ACTIVE_SLOT) ActiveSlot = KNIFE_SLOT;
...
{
    if (m_input_external_handler) return m_input_external_handler->mouse_scale_factor();
...
{
    // clear state
    if (handler) mstate_wishful = 0;

    // release fire button
    if (handler) IR_OnKeyboardRelease(kWPN_FIRE);
...
void CActor::SwitchNightVision()
{
    CWeapon* wpn1 = NULL;
    CWeapon* wpn2 = NULL;
    if (inventory().ItemFromSlot(INV_SLOT_2)) wpn1 = smart_cast<CWeapon*>(inventory().ItemFromSlot(INV_SLOT_2));

    if (inventory().ItemFromSlot(INV_SLOT_3)) wpn2 = smart_cast<CWeapon*>(inventory().ItemFromSlot(INV_SLOT_3));
    ...
    for (; it != it_e; ++it)
    {
        ...
        if (torch) {
            if (wpn1 && wpn1->IsZoomed()) return;

            if (wpn2 && wpn2->IsZoomed()) return;
