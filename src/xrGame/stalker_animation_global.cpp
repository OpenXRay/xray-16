...
void CStalkerAnimationManager::global_play_callback(CBlend* blend)
{
    ...
    if (!manager.m_global_callback) return;
...
{
    if (!object().critically_wounded()) return (MotionID());

    if (global().animation()) return (global().animation());
...
{
    if (m_global_selector) return (m_global_selector(animation_movement_controller));

    animation_movement_controller = false;

    if (eMentalStatePanic != object().movement().mental_state()) return (global_critical_hit());

    if (fis_zero(object().movement().speed(object().character_physics_support()->movement()))) return (MotionID());
