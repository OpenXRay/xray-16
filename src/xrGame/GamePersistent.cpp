...
{
    if (m_pMainMenu->IsActive()) m_pMainMenu->Activate(false);
...
void CGamePersistent::WeathersUpdate()
{
    if (g_pGameLevel && !g_dedicated_server) {
        ...
        if (ambient_particles && !ambient_particles->IsPlaying()) CParticlesObject::Destroy(ambient_particles);
    }
}
...
void CGamePersistent::OnFrame()
{
    ...
    if (0 == Device.dwFrame % 200) CUITextureMaster::FreeCachedShaders();
    ...
    if (!m_pMainMenu->IsActive()) m_pMainMenu->DestroyInternal(false);
    ...
    if (!Device.Paused()) Engine.Sheduler.Update();

    // update weathers ambient
    if (!Device.Paused()) WeathersUpdate();
...
{
    if (E == eQuickLoad) {
        if (Device.Paused()) Device.Pause(FALSE, TRUE, TRUE, "eQuickLoad");
        ...
        if (g_tutorial2) g_tutorial2->Stop();
...
{
    if (CurrentGameUI()) CurrentGameUI()->UIMainIngameWnd->OnSectorChanged(sector);
}
