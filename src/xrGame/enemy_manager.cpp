...
{
    if (!entity_alive->g_Alive()) return (false);

    if ((entity_alive->spatial.type & STYPE_VISIBLEFORAI) != STYPE_VISIBLEFORAI) return (false);

    if ((m_object->ID() == entity_alive->ID()) || !m_object->is_relation_enemy(entity_alive)) return (false);
...
float CEnemyManager::evaluate(const CEntityAlive* object) const
{
    ...
    if (actor) m_ready_to_save = false;
    ...
    if (wounded) {
        if (m_stalker && m_stalker->agent_manager().enemy().assigned_wounded(object, m_stalker)) return (0.f);
    ...
    // if we see object
    if (m_object->memory().visual().visible_now(object)) penalty -= 1000.f;
...
bool CEnemyManager::expedient(const CEntityAlive* object) const
{
    ...
    if (ai().ef_storage().m_pfExpediency->dwfGetDiscreteValue()) return (true);

    if (m_object->memory().hit().hit(ai().ef_storage().non_alife().enemy())) return (true);
    return (false);
}
...
void CEnemyManager::remove_links(IGameObject* object)
{
    ...
    if (m_last_enemy == object) m_last_enemy = 0;

    if (m_selected == object) m_selected = 0;
}
...
bool CEnemyManager::change_from_wounded(const CEntityAlive* current, const CEntityAlive* previous) const
{
    ...
    if (!current_stalker) return (false);

    if (current_stalker->wounded()) return (false);

    const CAI_Stalker* previous_stalker = smart_cast<const CAI_Stalker*>(previous);
    if (!previous_stalker) return (false);

    if (!previous_stalker->wounded()) return (false);
...
void CEnemyManager::process_wounded(bool& only_wounded)
{
    ...
    for (; I != E; ++I)
    {
        ...
        if (stalker && stalker->wounded()) continue;
...
bool CEnemyManager::need_update(const bool& only_wounded) const
{
    ...
    if (!selected()->g_Alive()) return (true);

    if (!m_object->is_relation_enemy(selected())) return (true);

    if (enable_enemy_change() && !m_object->memory().visual().visible_now(selected())) return (true);

    if (only_wounded) return (false);

    const CAI_Stalker* stalker = smart_cast<const CAI_Stalker*>(selected());
    if (stalker && stalker->wounded()) return (true);
    ...
    if (last_hit_time && (last_hit_time > m_last_enemy_change)) {
        ...
        if (alive_enemy && m_object->is_relation_enemy(alive_enemy)) return (true);
    }
...
void CEnemyManager::try_change_enemy()
{
    ...
    if (!need_update(only_wounded)) return;
    ...
    if (selected() != previous_selected) m_object->on_enemy_change(previous_selected);
}
