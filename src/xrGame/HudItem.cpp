...
void CHudItem::renderable_Render()
{
    ...
    else
    {
        ...
        else if (object().H_Parent())
        {
            ...
            if (owner->attached(self)) on_renderable_Render();
        }
    }
}
...
{
    if (HudItemData()) g_player_hud->detach_item(this);
    StopCurrentAnimWithoutCallback();
}
...
attachable_hud_item* CHudItem::HudItemData()
{
    ...
    if (!g_player_hud) return hi;

    hi = g_player_hud->attached_item(0);
    if (hi && hi->m_parent_hud_item == this) return hi;
