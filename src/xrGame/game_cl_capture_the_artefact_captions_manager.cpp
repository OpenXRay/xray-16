...
void CTAGameClCaptionsManager::ShowInProgressCaptions()
{
    ...
    if (parent_game_object->HasTimeLimit()) parent_game_ui->SetTimeMsgCaption(timelimit_message);

    IGameObject* control_entity = Level().CurrentControlEntity();
    if (!control_entity) return;
    ...
    if (ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD)) {
        ...
        {
            if (m_can_show_payspawn) parent_game_ui->SetPressJumpMsgCaption("mp_press_jump2pay_spaw");
        }
        return;
    }
}
...
{
    if (!parent_game_object || !parent_game_ui) return;
...
void CTAGameClCaptionsManager::SetTimeLimit(u32 time_limit, u32 current_time)
{
    ...
    {
        if (parent_game_ui) parent_game_ui->SetTimeMsgCaption("00:00:00");
    }
}