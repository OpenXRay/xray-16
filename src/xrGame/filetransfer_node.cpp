...
bool memory_writer_reader::make_data_packet(NET_Packet& packet, u32 chunk_size)
{
    ...
    if (!elapsed) return (m_writer_pointer == m_writer_max_size);
...
{
    if ((Device.dwTimeGlobal - m_last_chunksize_update_time) < 1000) return;
    ...
    else  // peak is reached
    {
        ...
        if ((Device.dwTimeGlobal - m_last_chunksize_update_time) < 3000) return;
...
{
    if (opened()) return (m_reader->tell() == m_reader->size());
