...
void Root::highlight_hierarchy(shared_str const& upgrade_id)
{
    ...
    for (; ib != ie; ++ib)
    {
        ...
        {
            if (pSettingsOpenXRay->read_if_exists<bool>("ui", "cs_upgrades_highlights", psGameMode.is(ClearSkyMode))) // XXX Clear Sky upgrades: find a dynamic, universal solution
                (*ib)->highlight_up();
            (*ib)->highlight_down();
            return;
        }
