...
bool CSpaceRestrictionBridge::on_border(const Fvector& position) const
{
    ...
    if ((I == E) || (ai().level_graph().vertex(*I)->position().xz() != pos.xz())) return (false);
    ...
    for (; I != E; ++I)
    {
        if (ai().level_graph().vertex(*I)->position().xz() != pos.xz()) break;

        if (*I == vertex_id) return (true);
    }
...
bool CSpaceRestrictionBridge::out_of_border(const Fvector& position)
{
    ...
    if (!ai().level_graph().valid_vertex_id(level_vertex_id)) return (true);
