...
{
    if (!inventory_item->useful_for_NPC()) return (false);

    if (CLSID_DEVICE_PDA == inventory_item->object().CLS_ID) {
        CPda* pda = smart_cast<CPda*>(inventory_item);
        VERIFY(pda);
        if (pda->GetOriginalOwnerID() == current_owner_id) return (false);
    }
...
u32 CAI_Stalker::fill_items(CInventory& inventory, CGameObject* old_owner, ALife::_OBJECT_ID new_owner_id)
{
    ...
    for (; I != E; ++I)
    {
        if (!tradable_item(*I, old_owner->ID())) continue;
...
IC void CAI_Stalker::buy_item_virtual(CTradeItem& item)
{
    ...
    if (m_current_trader) m_current_trader->set_money(m_current_trader->get_money() + item.m_item->Cost(), true);
}
...
void CAI_Stalker::attach_available_ammo(CWeapon* weapon)
{
    ...
    for (; I != E; ++I)
    {
        if (m_total_money < (*I).m_item->Cost()) continue;
        ...
        ++count;
        if (count >= MAX_AMMO_ATTACH_COUNT) break;
    }
}
...
void CAI_Stalker::choose_weapon(ALife::EWeaponPriorityType weapon_priority_type)
{
    ...
    for (; I != E; ++I)
    {
        ...
        case ALife::eWeaponPriorityTypeKnife:
        {
            ...
            if (I->m_item->m_ItemCurrPlace.type == eItemPlaceSlot) current_value += 10.0f;
        ...
        {
            if ((6 != j) && (7 != j) && (8 != j) && (9 != j) && (11 != j) && (12 != j)) continue;
...
void CAI_Stalker::choose_detector()
{
    ...
    if (best_detector) buy_item_virtual(*best_detector);
}
...
void CAI_Stalker::update_sell_info()
{
    ...
    for (; I != E; ++I)
    {
        if (!tradable_item(*I, ID())) m_temp_items.push_back(CTradeItem(*I, ID(), ID()));
    }
}

bool CAI_Stalker::can_sell(CInventoryItem* item)
{
    if (READ_IF_EXISTS(pSettings, r_bool, cNameSect(), "is_trader", false)) return (tradable_item(item, ID()));
...
{
    if (!g_Alive()) return (trade_parameters().enabled(CTradeParameters::action_show(0), item->object().cNameSect()));
...
{
    if (item == new_weapon) return (true);
...
bool CAI_Stalker::enough_ammo(const CWeapon* new_weapon) const
{
    ...
    for (; I != E; ++I)
    {
        ...
        ++ammo_box_count;
        if (ammo_box_count >= enough_ammo_box_count) return (true);
    }
...
{
    if (non_conflicted(item, new_weapon)) return (false);

    const CWeapon* weapon = smart_cast<const CWeapon*>(item);
    VERIFY(weapon);

    bool current_weapon_enough_ammo = enough_ammo(weapon);
    if (current_weapon_enough_ammo && !new_wepon_enough_ammo) return (true);

    if (!current_weapon_enough_ammo && new_wepon_enough_ammo) return (false);

    if (!fsimilar(weapon->GetCondition(), new_weapon->GetCondition(), .05f))
        return (weapon->GetCondition() >= new_weapon->GetCondition());

    if (weapon->ef_weapon_type() != new_weapon->ef_weapon_type()) return (weapon->Cost() >= new_weapon->Cost());

    u32 weapon_rank = get_rank(weapon->cNameSect());

    if (weapon_rank != (u32)new_weapon_rank) return (weapon_rank >= (u32)new_weapon_rank);
...
bool CAI_Stalker::can_take(CInventoryItem const* item)
{
    ...
    TIItemContainer::iterator E = inventory().m_all.end();
    for (; I != E; ++I)
        if (conflicted(*I, new_weapon, new_weapon_enough_ammo, new_weapon_rank)) return (false);
...
void CAI_Stalker::remove_personal_only_ammo(const CInventoryItem* item)
{
    ...
    for (; I != E; ++I)
    {
        ...
        for (; i != e; ++i)
        {
            if ((*i)->object().ID() == weapon->ID()) continue;

            const CWeapon* temp = smart_cast<const CWeapon*>(*i);
            if (!temp) continue;

            if (std::find(temp->m_ammoTypes.begin(), temp->m_ammoTypes.end(), *I) == temp->m_ammoTypes.end()) continue;
        ...
        {
            if (xr_strcmp(*I, (*i)->object().cNameSect())) continue;
...
{
    if (non_conflicted(item, new_weapon)) return;
...
void CAI_Stalker::on_after_take(const CGameObject* object)
{
    ...
    if (!READ_IF_EXISTS(pSettings, r_bool, cNameSect(), "use_single_item_rule", true)) return;
