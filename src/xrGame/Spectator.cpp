    ...
    if (GameID() != eGameIDSingle) {
        if (Game().local_player && ((Game().local_player->GameID == ID()) || Level().IsDemoPlay())) {
            if (cam_active != eacFreeFly) {
                if (m_pActorToLookAt && !m_pActorToLookAt->g_Alive()) cam_Set(eacFreeLook);
                if (!m_pActorToLookAt) {
...
void CSpectator::IR_OnKeyboardPress(int cmd)
{
    ...
    case kWPN_FIRE:
    {
        if ((cam_active != eacFreeFly) || (!m_pActorToLookAt)) {
            ...
            if (cam_active == eacFirstEye && m_pActorToLookAt) FirstEye_ToPlayer(m_pActorToLookAt);
        }
    }
    break;
    case kWPN_ZOOM:
    {
        ...
        if (!PS->testFlag(GAME_PLAYER_FLAG_SPECTATOR)) {
            ...
            while (!found)
            {
                ...
                if (new_camera == (eacMaxCam - 1)) break;
...
void CSpectator::net_Destroy()
{
    ...
    if (!g_dedicated_server) Level().MapManager().OnObjectDestroyNotify(ID());
}
...
bool CSpectator::SelectNextPlayerToLook(bool const search_next)
{
    ...
    if (PPCount > 0) {
        ...
        if (tmp_state) m_last_player_name = tmp_state->getName();
        return true;
    };
    return false;
};
