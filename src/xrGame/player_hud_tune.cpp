...
{
#ifndef MASTER_GOLD
    if (!is_attachable_item_tuning_mode()) return;
...
void attachable_hud_item::debug_draw_firedeps()
{
    ...
    if (hud_adj_mode == 5 || hud_adj_mode == 6 || hud_adj_mode == 7 || bForce) {
        ...
        if (hud_adj_mode == 5 || bForce) render.draw_aabb(fd.vLastFP, 0.005f, 0.005f, 0.005f, color_xrgb(255, 0, 0));

        if (hud_adj_mode == 6) render.draw_aabb(fd.vLastFP2, 0.005f, 0.005f, 0.005f, color_xrgb(0, 0, 255));
...
void player_hud::tune(Ivector _values)
{
    ...
    {
        if (hud_adj_mode == 8 && (values.z)) _delta_pos += (values.z > 0) ? 0.001f : -0.001f;

        if (hud_adj_mode == 9 && (values.z)) _delta_rot += (values.z > 0) ? 0.1f : -0.1f;
    }
    else
    {
...
{
    if (!hud_adj_mode) return;

    LPCSTR _text = NULL;
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT) && hud_adj_mode)
        _text = "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot 5-fire_point 6-fire_2_point "
...
{
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT)) {
        if (dik == DIK_NUMPAD0) hud_adj_mode = 0;
        if (dik == DIK_NUMPAD1) hud_adj_mode = 1;
        if (dik == DIK_NUMPAD2) hud_adj_mode = 2;
        if (dik == DIK_NUMPAD3) hud_adj_mode = 3;
        if (dik == DIK_NUMPAD4) hud_adj_mode = 4;
        if (dik == DIK_NUMPAD5) hud_adj_mode = 5;
        if (dik == DIK_NUMPAD6) hud_adj_mode = 6;
        if (dik == DIK_NUMPAD7) hud_adj_mode = 7;
        if (dik == DIK_NUMPAD8) hud_adj_mode = 8;
        if (dik == DIK_NUMPAD9) hud_adj_mode = 9;
    }
    if (pInput->iGetAsyncKeyState(DIK_LCONTROL)) {
        if (dik == DIK_NUMPAD0) hud_adj_item_idx = 0;
        if (dik == DIK_NUMPAD1) hud_adj_item_idx = 1;
    }
}
