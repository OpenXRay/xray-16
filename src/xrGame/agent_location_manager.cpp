...
struct CRemoveOldDangerCover
{
    ...
    IC bool operator()(const CAgentLocationManager::CDangerLocationPtr& location) const
    {
        if (!location->useful()) {
            ...
            for (; I != E; ++I)
            {
                if (!location->mask().test(m_members->mask(&(*I)->object()))) continue;
...
bool CAgentLocationManager::suitable(CAI_Stalker* object, const CCoverPoint* location, bool use_enemy_info) const
{
    ...
    for (; I != E; ++I)
    {
        if ((*I)->object().ID() == object->ID()) continue;

        if (!(*I)->cover()) {
            if (this->object().member().registered_in_combat(&(*I)->object())) continue;

            if ((*I)->object().Position().distance_to_sqr(location->position()) <= _sqr(5.f)) return (false);
...
void CAgentLocationManager::add(CDangerLocationPtr location)
{
    ...
    for (; I != E; ++I)
    {
        if (!location->mask().test(object().member().mask(&(*I)->object()))) continue;
    ...
    if (danger->m_interval < location->m_interval) danger->m_interval = location->m_interval;

    if (danger->m_radius < location->m_radius) danger->m_radius = location->m_radius;
}
...
float CAgentLocationManager::danger(const CCoverPoint* cover, CAI_Stalker* member) const
{
    ...
    for (; I != E; ++I)
    {
        if (Device.dwTimeGlobal > (*I)->m_level_time + (*I)->m_interval) continue;

        if (!(*I)->mask().test(mask)) continue;

        float distance = 1.f + (*I)->position().distance_to(cover->position());
        if (distance > (*I)->m_radius) continue;
