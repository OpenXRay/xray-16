...
{
    if (!!abstract.m_spawn_flags.is(CSE_Abstract::flSpawnInfiniteCount)) return (false);
...
{
    if (!!abstract.m_spawn_flags.is(CSE_Abstract::flSpawnOnSurgeOnly)) return (false);
...
{
    if (vertex->edges().empty()) return (spawned_item(vertex->data()->object(), objects));

    SPAWN_GRAPH::const_iterator I = vertex->edges().begin();
    SPAWN_GRAPH::const_iterator E = vertex->edges().end();
    for (; I != E; ++I)
        if (spawned_item(m_spawns.vertex((*I).vertex_id()), objects)) return (true);
...
{
    if (!abstract.m_spawn_flags.is(CSE_Abstract::flSpawnIfDestroyedOnly)) return (false);

    if (spawned_item(abstract, objects)) return (true);
    ...
    SPAWN_GRAPH::CVertex* vertex, SPAWN_IDS& spawns, ALife::_TIME_ID game_time, SPAWN_IDS& objects)
{
    ...
    if (probability >= accumulator * group_probability) return;
...
{
    VERIFY(vertex);
    if (!can_spawn(vertex->data()->object(), game_time, objects)) return;
    ...
    SPAWN_GRAPH::const_iterator E = vertex->edges().end();
    for (; I != E; ++I)
        if (randF(1.f) < (*I).weight()) fill_new_spawns(m_spawns.vertex((*I).vertex_id()), spawns, game_time, objects);
}
