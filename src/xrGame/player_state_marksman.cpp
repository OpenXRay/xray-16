    ...
    u16 killer_id, u16 target_id, u16 weapon_id, std::pair<KILL_TYPE, SPECIAL_KILL_TYPE> kill_type)
{
    ...
    if (!m_owner->is_item_in_group(weapon_gid, ammunition_group::gid_sniper_rifels)) return;
    ...
    if (!m_owner->is_item_in_group(victim_weapon_gid, ammunition_group::gid_sniper_rifels)) return;
