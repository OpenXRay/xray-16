...
void game_cl_ArtefactHunt::TranslateGameMessage(u32 msg, NET_Packet& P)
{
    ...
    case GAME_EVENT_ARTEFACT_TAKEN: // ahunt
    {
        ...
        if (m_reward_generator) m_reward_generator->OnPlayerTakeArtefact(pPlayer);
    ...
    case GAME_EVENT_ARTEFACT_DROPPED: // ahunt
    {
        ...
        if (m_reward_generator) m_reward_generator->OnPlayerDropArtefact(pPlayer);
    ...
    case GAME_EVENT_ARTEFACT_ONBASE: // ahunt
    {
        ...
        if (m_reward_generator) m_reward_generator->OnPlayerBringArtefact(pPlayer);
    ...
    case GAME_EVENT_ARTEFACT_SPAWNED: // ahunt
    {
        ...
        if (CurrentGameUI()) CurrentGameUI()->CommonMessageOut(Text);
        if (m_reward_generator) m_reward_generator->OnArtefactSpawned();
    ...
    case GAME_EVENT_ARTEFACT_DESTROYED: // ahunt
    {
        ...
        if (pObj && xr_strlen(m_Eff_Af_Disappear)) PlayParticleEffect(m_Eff_Af_Disappear.c_str(), pObj->Position());
        //-------------------------------------------
        if (CurrentGameUI()) CurrentGameUI()->CommonMessageOut(Text);
    }
    break;
...
void game_cl_ArtefactHunt::SetScore()
{
    ...
    if (Level().CurrentViewEntity() && m_game_ui) {
        ...
        if (ps && m_game_ui) m_game_ui->SetFraglimit(ps->frags(), artefactsNum);
    }
}
...
bool game_cl_ArtefactHunt::CanBeReady()
{
    ...
    if (pCurBuyMenu && !pCurBuyMenu->IsShown()) ClearBuyMenu();
...
void game_cl_ArtefactHunt::UpdateMapLocations()
{
    ...
    if (local_player) {
        if (!artefactID) {
            if (old_artefactID) Level().MapManager().RemoveMapLocationByObjectID(old_artefactID);
        }
        else
        {
            if (!artefactBearerID) {
...
bool game_cl_ArtefactHunt::NeedToSendReady_Spectator(int key, game_PlayerState* ps)
{
        ...
        (local_player->money_for_round + m_iSpawn_Cost) >= 0)
    {
        ...
        if (m_bTeamSelected && m_bSkinSelected) m_game_ui->m_pBuySpawnMsgBox->ShowDialog(true);
...
void game_cl_ArtefactHunt::OnSpawn(IGameObject* pObj)
{
    ...
    if (pArtefact) {
        if (xr_strlen(m_Eff_Af_Spawn)) PlayParticleEffect(m_Eff_Af_Spawn.c_str(), pObj->Position());
    };
}
