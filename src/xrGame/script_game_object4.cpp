...
void CScriptGameObject::set_sound_mask	(u32 sound_mask)
{
	CCustomMonster				*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster)
		ai().script_engine().script_log					(LuaMessageType::Error,"CSoundPlayer : cannot access class member set_sound_mask!");
	else {
...
int  CScriptGameObject::active_sound_count		(bool only_playing)
{
	CCustomMonster				*monster = smart_cast<CCustomMonster*>(&object());
	if (!monster) {
		ai().script_engine().script_log		(LuaMessageType::Error,"CGameObject : cannot access class member active_sound_count!");
		return								(-1);
	}
	else
...
CSightParams CScriptGameObject::sight_params	()
{
	CAI_Stalker						*stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker) {
		ai().script_engine().script_log			(LuaMessageType::Error,"CAI_Stalker : cannot access class member sight_params!");
...
bool CScriptGameObject::critically_wounded		()
{
	...
	if (!custom_monster) {
		ai().script_engine().script_log	(LuaMessageType::Error,"CCustomMonster : cannot access class member critically_wounded!");
		return							(false);
	}
...
void CScriptGameObject::start_particles(LPCSTR pname, LPCSTR bone)
{
	...
	if(K->LL_GetBoneVisible(play_bone))
		...
		ai().script_engine().script_log	(LuaMessageType::Error,"Cant start particles, bone [%s] is not visible now", bone);
}
...
void CScriptGameObject::stop_particles(LPCSTR pname, LPCSTR bone)
{
	...
	if(K->LL_GetBoneVisible(play_bone))
		...
		ai().script_engine().script_log	(LuaMessageType::Error,"Cant stop particles, bone [%s] is not visible now", bone);
}