...
IC u32 pack(const float& unpacked_value, const u32 bit_count)
{
    ...
    if (bit_count > 1)
        if (result == 0 && unpacked_value != 0) result += 1;
...
void actor_mp_state_holder::read(NET_Packet& packet)
{
    ...
    if (!total_bit_count) return;

    output |= u32(packet.r_u8()) << 0;

    if (total_bit_count > 8) output |= u32(packet.r_u8()) << 8;

    if (total_bit_count > 16) output |= u32(packet.r_u8()) << 16;

    if (total_bit_count > 24) output |= u32(packet.r_u8()) << 24;
