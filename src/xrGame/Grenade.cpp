...
{
    if (IsGameTypeSingle() && (GetState() == eReady || GetState() == eThrow)) OnStateSwitch(eIdle);
}
...
void CGrenade::PutNextToSlot()
{
    ...
    if (smart_cast<CInventoryOwner*>(H_Parent()) && m_pInventory) {
        ...
        else
        {
            ...
            if (pActor) pActor->OnPrevWeaponSlot();
        }
...
bool CGrenade::NeedToDestroyObject() const
{
    ...
    if (TimePassedAfterIndependant() > m_dwGrenadeRemoveTime) return true;
