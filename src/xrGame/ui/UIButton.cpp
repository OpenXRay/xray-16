...
bool CUIButton::OnKeyboardAction(SDL_Scancode dik, EUIMessages keyboard_action)
{
...
bool CUIButton::IsAccelerator(int iAccel) const
{
    ...
    if (!res)
    {
        res = ((m_uAccelerator[2] != -1) ? is_binded((EGameActions)GetAccelerator(2), (SDL_Scancode) iAccel) : false) ||
            ((m_uAccelerator[3] != -1) ? is_binded((EGameActions)GetAccelerator(3), (SDL_Scancode) iAccel) : false);
    }
    return res;
}
