...
bool CUIFixedScrollBar::OnMouseAction(float x, float y, EUIMessages mouse_action)
{
    ...
    case WINDOW_MOUSE_MOVE:
    {
        ...
        if (box_rect.in(cursor_pos)) cursor_over = true;
...
{
    if (mouse_btn == MOUSE_1 && OnMouseDownEx()) return true;
...
{
    if (pWnd == m_DecButton) {
        if (msg == BUTTON_CLICKED || msg == BUTTON_DOWN) TryScrollDec();
    }
    else if (pWnd == m_IncButton)
    {
        if (msg == BUTTON_CLICKED || msg == BUTTON_DOWN) TryScrollInc();
    }
    else if (pWnd == m_ScrollBox)
    {
        if (msg == SCROLLBOX_MOVE) {
            ...
            if (m_bIsHorizontal) {
                ...
                if (GetMessageTarget()) GetMessageTarget()->SendMessage(this, SCROLLBAR_HSCROLL);
            }
            else
            {
                SetPosScrollFromView(m_ScrollBox->GetWndPos().y, m_ScrollBox->GetHeight(), GetWidth());
                if (GetMessageTarget()) GetMessageTarget()->SendMessage(this, SCROLLBAR_VSCROLL);
            }
        }
    }
