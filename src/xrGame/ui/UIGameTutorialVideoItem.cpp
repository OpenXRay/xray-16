...
void CUISequenceVideoItem::Start()
{
    ...
    if (m_flags.test(etiNeedPauseSound)) Device.Pause(TRUE, FALSE, TRUE, "videoitem_start");
...
{
    if (m_flags.test(etiStoredCursorState)) GetUICursor().Show();

    if (!m_flags.test(etiCanBeStopped) && !bForce && IsPlaying()) return false;
    ...
    if (m_flags.test(etiNeedPauseSound)) Device.Pause(FALSE, FALSE, TRUE, "videoitem_stop");
