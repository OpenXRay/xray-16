...
void CMMSound::Init(CUIXml& xml_doc, LPCSTR path)
{
    ...
    if (check_file(xml_doc.Read(_path, 0, ""))) m_whell.create(xml_doc.Read(_path, 0, ""), st_Effect, sg_SourceType);
...
{
    if (m_whell._handle() && !m_whell._feedback()) m_whell.play(NULL, sm_Looped | sm_2D);
}

void CMMSound::whell_Stop()
{
    if (m_whell._feedback()) m_whell.stop();
}

void CMMSound::whell_Click()
{
    if (m_whell_click._handle()) m_whell_click.play(NULL, sm_2D);
}
...
void CMMSound::music_Update()
{
    ...
    if (0 == m_music_stereo._feedback()) music_Play();
}
