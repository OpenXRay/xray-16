...
game_PlayerState* game_sv_GameState::get_eid(u16 id)  // if exist
{
    ...
    if (tmp_client) return tmp_client->ps;
    return NULL;
}
...
void* game_sv_GameState::get_client(u16 id)  // if exist
{
    ...
    struct client_searcher
    {
        ...
        {
            xrClientData* tmp_client = static_cast<xrClientData*>(client);
            if (!tmp_client || !tmp_client->ps) return false;
...
struct player_exporter
{
    ...
    void __stdcall export_players(IClient* client)
    {
        ...
        if (to_ps == curr_ps) curr_ps->setFlag(GAME_PLAYER_FLAG_LOCAL);
    ...
    if (FS.exist(fn_game, "$level$", "level.game")) {
        ...
        if (0 != (O = F->open_chunk(RPOINT_CHUNK))) {
            for (int id = 0; O->find_chunk(id); ++id)
            {
                ...
                type = O->r_u8();
                GameType = O->r_u16();
                if (type == rptItemSpawn) O->r_stringZ(rp_profile);
...
game_sv_GameState::~game_sv_GameState()
{
    if (!g_dedicated_server) ai().script_engine().remove_script_process(ScriptProcessor::Game);
...
void game_sv_GameState::OnEvent(NET_Packet& tNetPacket, u16 type, u32 time, ClientID sender)
{
    ...
    case GAME_EVENT_CREATE_PLAYER_STATE:
    {
        ...
        if (g_dedicated_server && (CL == m_server->GetServerClient())) break;
...
public:
    ...
    bool __stdcall PredicateDelVictim(GameEvent* const ge)
    {
        ...
        case GAME_EVENT_PLAYER_HITTED:
        {
            ...
            if (id_entity_for == id_entity_victim) ret_val = true;
            ge->P.r_seek(tmp_pos);
        }
        break;
        };
...
public:
    ...
    {
        if (client == m_exclude) return false;

        xrClientData* tmp_cl = static_cast<xrClientData*>(client);
        if (!tmp_cl || !tmp_cl->ps) return false;
...
void game_sv_GameState::GenerateNewName(char const* old_name, char* dest, u32 const dest_size)
{
    ...
    {
        if (*currc == suffix_symbol) break;

        --currc;
    }
    if (*currc != suffix_symbol) currc = old_name + old_name_size - 1;
