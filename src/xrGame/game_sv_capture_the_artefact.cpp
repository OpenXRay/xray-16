...
void game_sv_CaptureTheArtefact::OnPlayerDisconnect(ClientID id_who, pstr Name, u16 GameID)
{
    ...
    auto te = teams.end();
    auto artefactOwnerTeam = std::find_if(teams.begin(), te, [&](const TeamPair& tp) constexpr
    {
...
void game_sv_CaptureTheArtefact::ProcessPlayerDeath(game_PlayerState* playerState)
{
    ...
    auto te = teams.end();
    auto childArtefactTeam = std::find_if(teams.begin(), te, [&](const TeamPair& tp) constexpr
    {
...
BOOL game_sv_CaptureTheArtefact::OnTouch(u16 eid_who, u16 eid_target, BOOL bForced)
{
    ...
    auto te = teams.end();
    auto artefactOfTeam = std::find_if(teams.begin(), te, [&](const TeamPair& tp) constexpr
    {
    ...
    if (artefactOfTeam != te)
    {
        ...
        if (artefactOfTeam->first == ps_who->team)
        {
            ...
            if (!tempArtefact->o_Position.similar(tempRPoint.P, g_sv_cta_artefactsBaseRadius))
            {
                ...
                auto it = std::find_if(teams.begin(), te, [&](const TeamPair& tp) constexpr {
