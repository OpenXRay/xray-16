...
{
    if (object().movement().current_params().cover() != object().movement().target_params().cover()) return (false);
...
{
    if (!m_object->m_object->movement().current_params().cover()) return (false);

    if (!m_object->m_object->movement().current_params().cover_loophole()) return (false);
...
{
    if (!m_object->movement().current_params().cover_loophole()) return (false);
...
{
    if (!m_object->m_object->movement().default_behaviour()) return (true);

    if (m_object->m_object->movement().current_params().cover_fire_position()) return (true);

    if (!m_object->m_object->movement().enemy_in_fov()) return (false);
...
{
    if (!m_object->stay_idle()) return (false);
