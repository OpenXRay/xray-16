...
{
    if (!m_accessible_neighbour_border_actual) prepare_accessible_neighbour_border(restriction, out_restriction);
...
IC bool CSpaceRestrictionAbstract::accessible_neighbours(T& restriction, u32 level_vertex_id, bool out_restriction)
{
    ...
    for (; I != E; ++I)
    {
        ...
        if (!ai().level_graph().valid_vertex_id(current)) continue;

        if (restriction->inside(current, !out_restriction) != out_restriction) continue;
...
IC void CSpaceRestrictionAbstract::prepare_accessible_neighbour_border(T& restriction, bool out_restriction)
{
    ...
    for (; I != E; ++I)
        if (accessible_neighbours(restriction, *I, out_restriction)) m_accessible_neighbour_border.push_back(*I);
}
