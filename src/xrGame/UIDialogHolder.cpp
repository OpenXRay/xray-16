...
void CDialogHolder::StartMenu(CUIDialogWnd* pDialog, bool bDoHideIndicators)
{
    ...
    if (pDialog->NeedCursor()) GetUICursor().Show();
...
void CDialogHolder::StopMenu(CUIDialogWnd* pDialog)
{
    ...
    if (!TopInputReceiver() || !TopInputReceiver()->NeedCursor()) GetUICursor().Hide();
}
...
void CDialogHolder::DoRenderDialogs()
{
    ...
    {
        if ((*it).enabled && (*it).wnd->IsShown()) (*it).wnd->Draw();
    }
}
...
{
    if (!m_input_receivers.empty()) return m_input_receivers.back().m_item;
    return NULL;
};
...
void CDialogHolder::OnFrame()
{
    ...
    // else
    {
        xr_vector<dlgItem>::iterator it = m_dialogsToRender.begin();
        for (; it != m_dialogsToRender.end(); ++it)
            if ((*it).enabled && (*it).wnd->IsEnabled()) (*it).wnd->Update();
    }
...
bool CDialogHolder::IR_UIOnKeyboardPress(int dik)
{
    ...
    if (dik == MOUSE_1 || dik == MOUSE_2 || dik == MOUSE_3) {
        ...
        if (TIR->OnMouseAction(cp.x, cp.y, action)) return true;
    }

    if (TIR->OnKeyboardAction(dik, WINDOW_KEY_PRESSED)) return true;
...
bool CDialogHolder::IR_UIOnKeyboardRelease(int dik)
{
    ...
    if (TIR->OnKeyboardAction(dik, WINDOW_KEY_RELEASED)) return true;
...
bool CDialogHolder::IR_UIOnKeyboardHold(int dik)
{
    ...
    if (TIR->OnKeyboardHold(dik)) return true;
