...
Root* Manager::add_root(shared_str const& root_id)
{
    ...
    if (get_root(root_id))
    {
        Msg("! Try add the existent upgrade_root for inventory item <%s>!", root_id.c_str());
    }
#endif
...
Upgrade* Manager::add_upgrade(shared_str const& upgrade_id, Group& parent_group)
{
    ...
    {
        Msg("! Try add the existent upgrade (%s), in group <%s>. Such upgrade is in group <%s> already!",
...
Property* Manager::add_property(shared_str const& property_id)
{
    ...
    {
        Msg("! Try add the existent upgrade property <%s>!", property_id.c_str());
    }
#endif
...
{
    R_ASSERT2_CURE(pSettings->section_exist(item_id), make_string("Inventory item [%s] does not exist!", item_id.c_str()),
    {
        return false;
    });
...
void Manager::load_all_properties()
{
    ...
    {
        Msg("! Can't load upgrades properties: section [%s] does not exist!", properties_section);
        return;
    }

    if (pSettings->line_count(properties_section) == 0)
    {
        Msg("! Can't load upgrades properties: section [%s] is empty!", properties_section);
        return;
    }
    ...
    if (g_upgrades_log == 1)
    {
        Msg("# Upgrades properties of inventory items loaded.");
    }
}
...
void Manager::test_all_upgrades(CInventoryItem& item)
{
    ...
    R_ASSERT2_CURE(root_p,
        make_string("Upgrades for item <%s> (id = %d) does not exist!", item.m_section_id.c_str(), item.object_id()), { return; });
...
Upgrade* Manager::upgrade_verify(shared_str const& item_section, shared_str const& upgrade_id)
{
...
#ifndef MASTER_GOLD
    auto root_p = get_root(item_section);
    if (!root_p)
    ...
    if (root_p && !root_p->contain_upgrade(upgrade_id))
...
{
    Upgrade* upgrade_p = get_upgrade(upgrade_id);
    R_ASSERT2_CURE(upgrade_p, make_string("Upgrade <%s> does not exist!", upgrade_id.c_str()), { return false; });
...
LPCSTR Manager::get_upgrade_by_index(CInventoryItem& item, Ivector2 const& index)
{
    ...
    {
        if (Upgrade* upgrade = root_p->get_upgrade_by_index(index))
        {
            return upgrade->id_str();
        }
    }

    Msg("! Upgrade with index <%d,%d> in inventory item [%s] does not exist!", index.x, index.y, item.m_section_id.c_str());
    return nullptr;
}
