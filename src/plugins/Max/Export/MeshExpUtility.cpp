...
static BOOL CALLBACK DefaultDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
    case WM_INITDIALOG: U.Init(hWnd); break;

    case WM_DESTROY: U.Destroy(hWnd); break;

    case WM_COMMAND:
        switch (LOWORD(wParam))
        {
        case IDC_CLOSE: U.iu->CloseUtility(); break;
        case IDC_EXPORT_AS_OBJECT: U.ExportObject(); break;
        case IDC_EXPORT_AS_LWO: U.ExportLWO(); break;
        case IDC_SKIN_EXPORT: U.ExportSkin(); break;
        case IDC_SKIN_KEYS_EXPORT: U.ExportSkinKeys(); break;
        }
        break;
...
BOOL MeshExpUtility::BuildObject(CEditableObject*& exp_obj, LPCSTR m_ExportName)
{
    ...
    if (bResult)
    {
        ...
        ELog.Msg(mtInformation, "Object '%s' contains: %d points, %d faces", exp_obj->GetName(),
            exp_obj->GetVertexCount(), exp_obj->GetFaceCount());
    }
    else
    {
        xr_delete(exp_obj);
