...
public:
    ...
    //
    bool convertToNormalMap(unsigned char* inImagePtr, unsigned int width, unsigned int height,
        OutFormatType outputPixelFormat, float bumpScale = 1.0, unsigned char* outImagePtr = NULL);

    // Convert the normal map texture to its corresponding heightfield texture
    //
    bool convertToHeightMap(unsigned char* inImagePtr, unsigned int width, unsigned int height,
        OutFormatType outputPixelFormat, float heightScale = 1.0, unsigned char* outImagePtr = NULL);

protected:
    // Convert the heightfield to the normal map texture in place
    // (replace the input texture with the normal map version)
    //
    bool convertToNormalMap_InPlace(unsigned char* inImagePtr, unsigned int width, unsigned int height,
