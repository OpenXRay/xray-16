...
MStatus CXRayMtl::initialize()
{
    ...
    */
    ...
    aLightData = lAttr.create("lightDataArray", "ltd", aLightDirection, aLightIntensity, aLightAmbient, aLightDiffuse,
        aLightSpecular, aLightShadowFraction, aPreShadowIntensity, aLightBlindData);
    ...
    for (int count = 1; count <= numLights; count++)
    {
        ...
        else
        {
            ...
            if (currentLight.child(aLightSpecular).asBool())
            {
                ...
                specLightDirection =
                    MRenderUtil::maximumSpecularReflection(blindData, lightDirection, point, surfaceNormal, direction);
