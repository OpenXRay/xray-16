...
public:
    ...
    // >>>> sampling
    void SetSamplingOn(BOOL on) { pb_sampling->SetValue(std2_ssampler_enable, 0, on != 0); }
    BOOL GetSamplingOn()
    {
        Interval iv;
        BOOL on;
    ...
    void SetSamplingQuality(float quality) { pb_sampling->SetValue(std2_ssampler_qual, 0, quality); }
