...
IC float Min(Color& c)
{
    return Min(c.r, c.g, c.b);
}
IC float Max(float a, float b)
{
    return (a < b) ? b : a;
}
...
IC float Max(Color& c)
{
    return Max(c.r, c.g, c.b);
}

IC float LBound(float x, float min = 0.0f)
{
    return x < min ? min : x;
}
...
IC float UBound(float x, float max = 1.0f)
{
    return x > max ? max : x;
}
...
static TCHAR* mtlChannelInternalNames[STD2_NMAX_TEXMAPS] = {
    _T("bumpMap"), _T("reflectionMap"), _T("refractionMap"), _T("displacementMap"), _T(""), _T(""), _T(""), _T(""),
...
static int mtlChannelType[STD2_NMAX_TEXMAPS] = {
    BUMP_CHANNEL, REFL_CHANNEL, REFR_CHANNEL, DISP_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL,
    UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL,

    UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL,
    UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL,
    UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL, UNSUPPORTED_CHANNEL,
    UNSUPPORTED_CHANNEL,
};
...
public:
    void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) // set from v
    {
        ...
        break;
        case std2_wire: m->SetFlag(NEWSTDMTL_WIRE, v.i); break;
        case std2_two_sided: m->SetFlag(NEWSTDMTL_2SIDE, v.i); break;
        case std2_face_map: m->SetFlag(NEWSTDMTL_FACEMAP, v.i); break;
        case std2_faceted: m->SetFlag(NEWSTDMTL_FACETED, v.i); break;
        }
    }
    ...
    void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval& valid) // get into v
    {
        ...
        case std2_shader_by_name:
        {
            ...
            if (pCD) v.s = (TCHAR*)pCD->ClassName();
        }
        break;
        }
    }
...
// shader parameters
static ParamBlockDesc2 std2_shader_blk(std2_shader, _T("newShaderParameters"), 0, &stdmtl2CD,
    P_AUTO_CONSTRUCT + P_AUTO_UI, SHADER_PB_REF,
    // rollout
    IDD_DMTL_SHADER4, IDS_KE_SHADER, 0, 0, &shaderDlgProc,
    // params
    std2_shader_type, _T("shaderType"), TYPE_INT, 0, IDS_JW_SHADERTYPE, p_default, 3, p_ui, TYPE_INTLISTBOX, IDC_SHADER,
    0, p_accessor, &shaderPBAccessor, end, std2_wire, _T("wire"), TYPE_BOOL, 0, IDS_DS_WIREFRAME, p_default, FALSE,
    p_ui, TYPE_SINGLECHEKBOX, IDC_WIRE, p_accessor, &shaderPBAccessor, end, std2_two_sided, _T("twoSided"), TYPE_BOOL,
    0, IDS_JW_TWOSIDED, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX, IDC_2SIDE, p_accessor, &shaderPBAccessor, end,
    std2_face_map, _T("faceMap"), TYPE_BOOL, 0, IDS_JW_FACEMAP, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX,
    IDC_FACE_MAP, p_accessor, &shaderPBAccessor, end, std2_faceted, _T("faceted"), TYPE_BOOL, 0, IDS_KE_FACETED,
    p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX, IDC_FACETED, p_accessor, &shaderPBAccessor, end, std2_shader_by_name,
    _T("shaderByName"), TYPE_STRING, 0, IDS_JW_SHADERBYNAME, p_accessor, &shaderPBAccessor, end, end);
...
public:
    void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) // set from v
    {
        ...
        case std2_opacity:
            m->opacity = v.f;
            if (m->pShader->GetParamDlg()) m->pShader->GetParamDlg()->UpdateOpacity();
            break;
        case std2_filter_color: m->filter = *v.p; break;
        case std2_falloff_type: m->SetFlag(NEWSTDMTL_FALLOFF_OUT, v.i == 1); break;
        case std2_falloff_amnt: m->opfall = v.f; break;
        case std2_ior: m->ioRefract = v.f; break;

        case std2_wire_size: m->wireSize = v.f; break;
        case std2_wire_units: m->SetFlag(NEWSTDMTL_WIRE_UNITS, v.i == 1); break;

        case std2_apply_refl_dimming: m->dimReflect = v.i; break;
        case std2_dim_lvl: m->dimIntens = v.f; break;
        case std2_refl_lvl: m->dimMult = v.f; break;
        }
    }
};
...
// extended parameters
static ParamBlockDesc2 std2_extended_blk(std2_extended, _T("extendedParameters"), 0, &stdmtl2CD,
    ...
    p_ui, TYPE_TEXMAPBUTTON, IDC_MAPON_FI, p_accessor, &extendedPBAccessor, end, std2_falloff_type,
    _T("opacityFallOffType"), TYPE_INT, 0, IDS_JW_FALLOFFTYPE, p_default, 0, p_range, 0, 1, p_ui, TYPE_RADIO, 2,
    IDC_TF_IN, IDC_TF_OUT, p_accessor, &extendedPBAccessor, end, std2_falloff_amnt, _T("opacityFallOff"),
    TYPE_PCNT_FRAC, P_ANIMATABLE, IDS_DS_FALLOFF, p_default, 0.0, p_range, 0.0, 100.0, p_ui, TYPE_SPINNER, EDITTYPE_INT,
    IDC_TF_EDIT, IDC_TF_SPIN, 0.1, p_accessor, &extendedPBAccessor, end, std2_ior, _T("ior"), TYPE_FLOAT, P_ANIMATABLE,
    IDS_DS_IOR, p_default, 1.5, p_range, 0.0, 10.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_IOR_EDIT, IDC_IOR_SPIN,
    0.01, p_accessor, &extendedPBAccessor, end, std2_wire_size, _T("wireSize"), TYPE_FLOAT, P_ANIMATABLE, IDS_DS_WIRESZ,
    p_default, 1.0, p_range, 0.0, 100.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_WIRE_EDIT, IDC_WIRE_SPIN, 1.0,
    p_accessor, &extendedPBAccessor, end, std2_wire_units, _T("wireUnits"), TYPE_INT, 0, IDS_JW_WIREUNITS, p_default, 0,
    p_range, 0, 1, p_ui, TYPE_RADIO, 2, IDC_PIXELS, IDC_UNITS, p_accessor, &extendedPBAccessor, end,
    std2_apply_refl_dimming, _T("applyReflectionDimming"), TYPE_BOOL, 0, IDS_JW_APPLYREFDIM, p_default, FALSE, p_ui,
    TYPE_SINGLECHEKBOX, IDC_DIM_REFL, p_accessor, &extendedPBAccessor, end, std2_dim_lvl, _T("dimLevel"), TYPE_FLOAT,
    P_ANIMATABLE, IDS_DS_DIMLEV, p_default, 0.0, p_range, 0.0, 1.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_DIM_AMT,
    IDC_DIM_AMTSPIN, 0.01, p_accessor, &extendedPBAccessor, end, std2_refl_lvl, _T("reflectionLevel"), TYPE_FLOAT,
    P_ANIMATABLE, IDS_DS_DIMMULT, p_default, 1.0, p_range, 0.1, 10.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_DIM_MULT,
...
public:
    void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) // set from v
    {
        ...
        break;
        case std2_ssampler_qual: m->GetPixelSampler()->SetQuality(v.f); break;
        case std2_ssampler_enable:
            m->GetPixelSampler()->SetEnable(v.i);
            m->SetFlag(NEWSTDMTL_SSAMP_ON, v.i);
            break;

        case std2_ssampler_adapt_threshold: m->GetPixelSampler()->SetAdaptiveThreshold(v.f); break;
        ...
        break;
        case std2_ssampler_subsample_tex_on: m->GetPixelSampler()->SetTextureSuperSampleOn(v.i); break;
        ...
        break;
        case std2_ssampler_param0: m->GetPixelSampler()->SetOptionalParam(0, v.f); break;
        case std2_ssampler_param1: m->GetPixelSampler()->SetOptionalParam(1, v.f); break;
        }
...
static ParamBlockDesc2 std2_sampling_blk(std2_sampling, _T("samplingParameters"), 0, &stdmtl2CD,
    P_AUTO_CONSTRUCT + P_AUTO_UI, SAMPLING_PB_REF,
    // rollout
    IDD_DMTL_SAMPLING3, IDS_KE_SAMPLING, 0, APPENDROLL_CLOSED, &samplingDlgProc,
    // params
    std2_ssampler, _T("sampler"), TYPE_INT, 0, IDS_JW_PIXELSAMPLER, p_default, 0, p_ui, TYPE_INTLISTBOX,
    IDC_PIX_SAMPLER, 0, p_accessor, &samplingPBAccessor, end, std2_ssampler_qual, _T("samplerQuality"), TYPE_FLOAT,
    P_ANIMATABLE, IDS_JW_SAMPLERQUAL, p_default, 0.5, p_range, 0.0, 1.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT,
    IDC_SAMPLEQUALITY_EDIT, IDC_SAMPLEQUALITY_SPIN, 0.01, p_accessor, &samplingPBAccessor, end, std2_ssampler_enable,
    _T("samplerEnable"), TYPE_BOOL, P_ANIMATABLE, IDS_JW_SAMPLERENABLE, p_default, FALSE, p_ui, TYPE_SINGLECHEKBOX,
    IDC_SUPER_SAMP, p_accessor, &samplingPBAccessor, end, std2_ssampler_adapt_threshold, _T("samplerAdaptThreshold"),
    TYPE_FLOAT, 0, IDS_KE_SAMPLERADAPTTHRESH, p_default, 0.1, p_range, 0.0, 1.0, p_ui, TYPE_SPINNER, EDITTYPE_FLOAT,
    IDC_THRESHOLD_EDIT, IDC_THRESHOLD_SPIN, 0.001, p_accessor, &samplingPBAccessor, end, std2_ssampler_adapt_on,
    _T("samplerAdaptOn"), TYPE_BOOL, 0, IDS_KE_SAMPLERADAPTON, p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_ADAPT_ON,
    p_accessor, &samplingPBAccessor, end, std2_ssampler_subsample_tex_on, _T("subSampleTextureOn"), TYPE_BOOL, 0,
    IDS_KE_SUBSAMPLE_TEX_ON, p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_SAMPLE_TEX, p_accessor, &samplingPBAccessor,
    end, std2_ssampler_advanced, _T("samplerAdvancedOptions"), TYPE_BOOL, 0, IDS_KE_SAMPLERADVANCED, p_default, TRUE,
    p_ui, TYPE_SINGLECHEKBOX, IDC_ADVANCED_BUTTON, p_accessor, &samplingPBAccessor, end, std2_ssampler_by_name,
    _T("samplerByName"), TYPE_STRING, 0, IDS_JW_SAMPLERBYNAME, p_accessor, &samplingPBAccessor, end,
    std2_ssampler_param0, _T("UserParam0"), TYPE_FLOAT, 0, IDS_KE_SAMPLER_PARAM0, p_default, 0.0, p_range, 0.0, 1.0,
    p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_PARAM0_EDIT, IDC_PARAM0_SPIN, 0.01, p_accessor, &samplingPBAccessor, end,
    std2_ssampler_param1, _T("UserParam1"), TYPE_FLOAT, 0, IDS_KE_SAMPLER_PARAM1, p_default, 0.0, p_range, 0.0, 1.0,
    p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_PARAM1_EDIT, IDC_PARAM1_SPIN, 0.01, p_accessor, &samplingPBAccessor, end,
    end);
...
public:
    void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) // set from v
    {
        ...
        switch (id)
        {
        case std2_eshader_type: m->eshaderId = (m->GetEShader(v.i)) ? v.i : -1; break;
        case std2_cshader_type: m->cshaderId = (m->GetCShader(v.i)) ? v.i : -1; break;
        case std2_gamemtl_type: m->gamemtlId = (m->GetGameMtl(v.i)) ? v.i : -1; break;
    ...
    void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval& valid) // get into v
    {
        ...
        switch (id)
        {
        case std2_eshader_by_name: { v.s = m->GetEShader(m->eshaderId);
        }
        break;
        case std2_cshader_by_name: { v.s = m->GetCShader(m->cshaderId);
        }
        break;
        case std2_gamemtl_by_name: { v.s = m->GetGameMtl(m->gamemtlId);
        }
        break;
        }
    }
...
// shader parameters
static ParamBlockDesc2 std2_xray_blk(std2_xray, _T("Parameters"), 0, &stdmtl2CD, P_AUTO_CONSTRUCT + P_AUTO_UI,
    XRAY_PB_REF,
    // rollout
    IDD_DMTL_XRAY, IDS_KE_XRAY, 0, APPENDROLL_CLOSED, &xrayDlgProc,
    // params
    std2_eshader_type, _T("EShaderType"), TYPE_INT, 0, IDS_JW_ESHADERTYPE, p_default, 0, p_ui, TYPE_INTLISTBOX,
    IDC_ESHADER, 0, p_accessor, &xrayPBAccessor, end, std2_cshader_type, _T("CShaderType"), TYPE_INT, 0,
    IDS_JW_CSHADERTYPE, p_default, 0, p_ui, TYPE_INTLISTBOX, IDC_CSHADER, 0, p_accessor, &xrayPBAccessor, end,
    std2_gamemtl_type, _T("GameMtlType"), TYPE_INT, 0, IDS_JW_GAMEMTLTYPE, p_default, 0, p_ui, TYPE_INTLISTBOX,
    IDC_GAMEMTL, 0, p_accessor, &xrayPBAccessor, end, std2_eshader_by_name, _T("EShaderByName"), TYPE_STRING, 0,
    IDS_JW_ESHADERBYNAME, p_accessor, &xrayPBAccessor, end, std2_cshader_by_name, _T("CShaderByName"), TYPE_STRING, 0,
    IDS_JW_CSHADERBYNAME, p_accessor, &xrayPBAccessor, end, std2_gamemtl_by_name, _T("GameMtlByName"), TYPE_STRING, 0,
...
public:
    ...
    ~ResourceDelete()
    {
        if (hLockButtons) ImageList_Destroy(hLockButtons);
    }
};
...
public:
    ...
    void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t) // set from v
    {
        ...
        case std2_maps:
        {
            ...
            {
                if (m->maps->txmap[tabIndex].amtCtrl != NULL) m->maps->DeleteReference(2 * tabIndex);
                macroRecorder->Disable();
        ...
        case std2_map_amnts:
        {
            ...
            KillRefmsg kill(m->killRefmsg);
            if (m->maps->txmap[tabIndex].amtCtrl) m->maps->txmap[tabIndex].amtCtrl->SetValue(t, &v.f);
            m->maps->txmap[tabIndex].amount = v.f;
            if (tabIndex == m->stdIDToChannel[ID_BU]) m->mInRQ = chooseReshade[m->mInRQ][IReshading::RR_NeedPreshade];
        }
        break;
    ...
    void Get(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t, Interval& valid) // set from v
    {
        ...
        switch (id)
        {
        case std2_map_enables: v.i = m->maps->txmap[tabIndex].mapOn; break;
        case std2_maps: v.r = m->maps->txmap[tabIndex].map; break;
        case std2_map_amnts:
...
static ParamBlockDesc2 std_maps_blk(std_maps, _T("maps"), 0, &stdmtl2CD, P_AUTO_CONSTRUCT + P_AUTO_UI, MAPS_PB_REF,
    ...
    // params
    std2_map_enables, _T("mapEnables"), TYPE_BOOL_TAB, STD2_NMAX_TEXMAPS, 0, IDS_JW_MAPENABLES, p_default, FALSE, p_ui,
    TYPE_SINGLECHEKBOX, IDC_USEMAP_0, IDC_USEMAP_1, IDC_USEMAP_2, IDC_USEMAP_3, IDC_USEMAP_4, IDC_USEMAP_5,
    IDC_USEMAP_6, IDC_USEMAP_7, IDC_USEMAP_8, IDC_USEMAP_9, IDC_USEMAP_10, IDC_USEMAP_11, IDC_USEMAP_12, IDC_USEMAP_13,
    IDC_USEMAP_14, IDC_USEMAP_15, IDC_USEMAP_16, IDC_USEMAP_17, IDC_USEMAP_18, IDC_USEMAP_19, IDC_USEMAP_20,
    IDC_USEMAP_21, IDC_USEMAP_22, IDC_USEMAP_23, p_accessor, &mapsPBAccessor, end, std2_maps, _T("maps"),
    TYPE_TEXMAP_TAB, STD2_NMAX_TEXMAPS, P_NO_REF, IDS_JW_MAPS, p_ui, TYPE_TEXMAPBUTTON, IDC_MAP_0, IDC_MAP_1, IDC_MAP_2,
    ...
    IDC_AMTEDIT_9, IDC_AMTSPIN_9, IDC_AMTEDIT_10, IDC_AMTSPIN_10, IDC_AMTEDIT_11, IDC_AMTSPIN_11, IDC_AMTEDIT_12,
    IDC_AMTSPIN_12, IDC_AMTEDIT_13, IDC_AMTSPIN_13, IDC_AMTEDIT_14, IDC_AMTSPIN_14, IDC_AMTEDIT_15, IDC_AMTSPIN_15,
    IDC_AMTEDIT_16, IDC_AMTSPIN_16, IDC_AMTEDIT_17, IDC_AMTSPIN_17, IDC_AMTEDIT_18, IDC_AMTSPIN_18, IDC_AMTEDIT_19,
    IDC_AMTSPIN_19, IDC_AMTEDIT_20, IDC_AMTSPIN_20, IDC_AMTEDIT_21, IDC_AMTSPIN_21, IDC_AMTEDIT_22, IDC_AMTSPIN_22,
    IDC_AMTEDIT_23, IDC_AMTSPIN_23, 1.0, p_accessor, &mapsPBAccessor, end, std2_mp_ad_texlock, _T("adTextureLock"),
    TYPE_BOOL, 0, IDS_JW_ADTEXLOCK, p_default, TRUE, p_ui, TYPE_CHECKBUTTON, IDC_LOCK_TEX01, p_accessor,
    &mapsPBAccessor, end, end);

#ifndef DESIGN_VER
// std2_dynamics parameters
static ParamBlockDesc2 std2_dynamics_blk(std2_dynamics, _T("dynamicsParameters"), 0, &stdmtl2CD,
    P_AUTO_CONSTRUCT + P_AUTO_UI, DYNAMICS_PB_REF,
    // rollout
    IDD_DMTL_DYNAM, IDS_DS_DYNAMICS, 0, APPENDROLL_CLOSED, NULL,
    // params
    std2_bounce, _T("bounce"), TYPE_FLOAT, P_ANIMATABLE, IDS_DS_BOUNCE, p_default, 1.0, p_range, 0.0, 1.0, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_BOUNCE_EDIT, IDC_BOUNCE_SPIN, 0.01, end, std2_static_friction,
    _T("staticFriction"), TYPE_FLOAT, P_ANIMATABLE, IDS_DS_STATFRIC, p_default, 0.0, p_range, 0.0, 1.0, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_STATFRIC_EDIT, IDC_STATFRIC_SPIN, 0.01, end, std2_sliding_friction,
    _T("slidingFriction"), TYPE_FLOAT, P_ANIMATABLE, IDS_DS_SLIDFRIC, p_default, 0.0, p_range, 0.0, 1.0, p_ui,
    TYPE_SPINNER, EDITTYPE_FLOAT, IDC_SLIDFRIC_EDIT, IDC_SLIDFRIC_SPIN, 0.01, end, end);
#endif
...
static ParamVersionDesc oldNewStdMtlVersions[NEWSTDMTL_NUMOLDVER + 1] = {ParamVersionDesc(stdmtlPB, 8, 0),
    ParamVersionDesc(stdmtlPB, 9, 1), ParamVersionDesc(stdmtlPB, 9, 2), ParamVersionDesc(stdmtlPB, 10, 3),
    ParamVersionDesc(stdmtlPB, 11, 4), ParamVersionDesc(stdmtlPB, 14, 5), ParamVersionDesc(stdmtlPB, 15, 6),
    ParamVersionDesc(stdmtlPB, 15, 7), ParamVersionDesc(stdmtlPB, 16, 8), ParamVersionDesc(stdmtlPB, 17, 9)};
...
{
    if (id == IID_IReshading) return (IReshading*)(this);
...
ParamDlg* XRayMtl::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
    ...
    pShaderDlg = pShader->CreateParamDialog(
        NULL, hwMtlEdit, imp, this, ((flags & NEWSTDMTL_ROLLUP1_OPEN) ? 0 : APPENDROLL_CLOSED));
...
BOOL XRayMtl::SetDlgThing(ParamDlg* dlg)
{
    ...
    if (dlg == pShaderDlg) {
        ...
        if ((oldClassId == newClassId) || ((newParams & STD_BASIC2_DLG) && (oldParams & STD_BASIC2_DLG))) {
            assert(pShaderDlg);
        ...
        else
        {
            ...
            // make a new one
            pShaderDlg = pShader->CreateParamDialog(
                oldPanel, curHwmEdit, curImp, this, (flags & NEWSTDMTL_ROLLUP1_OPEN ? 0 : APPENDROLL_CLOSED));
            pShader->SetParamDlg(pShaderDlg);
...
{
    if (shaderList.Count() == 0) LoadShaderList();
...
{
    if (samplerList.Count() == 0) LoadSamplerList();
...
public:
    ...
    RefResult NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message)
    {
        if (message == REFMSG_TARGET_DELETED) {
            if (hTarget == rtarget) rtarget = NULL;
        }
        return REF_SUCCEED;
    }
...
public:
    ...
    {
        if (message == REFMSG_TARGET_DELETED) {
            if (hTarget == saveSampler) saveSampler = NULL;
        }
        return REF_SUCCEED;
    }
    void DeleteThis()
...
{
    Texmap* t = (*maps)[nMap].map;
    if (t == NULL) return TSTR(GetString(IDS_KE_NONE));
...
void XRayMtl::UpdateLockADTex(BOOL passOn)
{
    ...
    if (pShader->SupportStdParams() & STD_PARAM_LOCKADTEX) {
        if (passOn) {
            ...
            if (pmap != NULL && pShader->GetParamDlg()) pShader->GetParamDlg()->LoadDialog(TRUE);
        }
        if (pmap != NULL) {
...
void XRayMtl::Update(TimeValue t, Interval& valid)
{
    ...
    if (!ivalid.InInterval(t)) {
        ...
        if (IsShaderInUI()) pShader->GetParamDlg()->UpdateOpacity();
        ...
        if (pShader) pShader->Update(t, ivalid);

        for (int i = 0; i < STD2_NMAX_TEXMAPS; i++)
            if (MAPACTIVE(i)) maps->txmap[i].Update(t, ivalid);
    }
    valid &= ivalid;
}
...
void XRayMtl::ShuffleShaderParams(Shader* newShader, Shader* oldShader)
{
    ...
    if (oldCD != NULL && oldCD->InternalName() != NULL) oldCD2 = (ClassDesc2*)oldCD;
    if (newCD != NULL && newCD->InternalName() != NULL) newCD2 = (ClassDesc2*)newCD;
...
ULONG XRayMtl::Requirements(int subMtlNum)
{
    ...
    if (opacity != 1.0f || ((*maps)[opacChan].map && MAPACTIVE(opacChan) && Active())) req |= MTLREQ_TRANSP_IN_VP;

    for (int i = 0; i < STD2_NMAX_TEXMAPS; i++)
    {
        if (MAPACTIVE(i)) req |= (*maps)[i].map->Requirements(subMtlNum);
    }
    if (MAPACTIVE(bumpChan)) {
        ULONG bmpreq = (*maps)[bumpChan].map->Requirements(subMtlNum);
        if (bmpreq & MTLREQ_UV) req |= MTLREQ_BUMPUV;
        if (bmpreq & MTLREQ_UV2) req |= MTLREQ_BUMPUV2;
    }
    if (flags & NEWSTDMTL_WIRE) req |= MTLREQ_WIRE;
    if (flags & NEWSTDMTL_2SIDE) req |= MTLREQ_2SIDE;
    //	if (pixelSampler && pixelSampler->GetEnable() )
    if (flags & NEWSTDMTL_SSAMP_ON) req |= MTLREQ_SUPERSAMPLE;
...
RefResult XRayMtl::NotifyRefChanged(Interval changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message)
{
    switch (message)
    {
    ...
    case REFMSG_CHANGE:
        ...
        if (hTarget != pShader) // && hTarget != maps)
        {
            if (changing_param != -1) {
                ...
                if (changing_param == std2_opacity && cpb == pb_extended && pShader && pShader->GetParamDlg())
...
Class_ID XRayMtl::ClassID()
{
    return XRAYMTL_CLASS_ID;
}
...
void XRayMtl::SetFilter(Color c, TimeValue t)
{
    ...
    int opacChan = stdIDToChannel[ID_OP];
    if (opacity != 1.0f || opfall != 0.0f || MAPACTIVE(opacChan)) NotifyChanged();
}
...
void XRayMtl::SetWireSize(float v, TimeValue t)
{
    ...
    if (flags & NEWSTDMTL_WIRE) NotifyChanged();
}
...
Color XRayMtl::GetFilter()
{
    return filter;
}
float XRayMtl::GetTexmapAmt(int imap)
{
    return maps->txmap[imap].amount;
}
...
BOOL XRayMtl::MapEnabled(int i)
{
    return maps->txmap[i].mapOn;
}
...
float XRayMtl::GetDynamicsProperty(TimeValue t, int mtlNum, int propID)
{
    ...
    switch (propID)
    {
    case DYN_BOUNCE: pb_dynamics->GetValue(std2_bounce, t, val, ivalid); return val;
    case DYN_STATIC_FRICTION: pb_dynamics->GetValue(std2_static_friction, t, val, ivalid); return val;
    case DYN_SLIDING_FRICTION: pb_dynamics->GetValue(std2_sliding_friction, t, val, ivalid); return val;
...
void XRayMtl::SetDynamicsProperty(TimeValue t, int mtlNum, int propID, float value)
{
    ...
    switch (propID)
    {
    case DYN_BOUNCE: pb_dynamics->SetValue(std2_bounce, t, value); break;
    case DYN_STATIC_FRICTION: pb_dynamics->SetValue(std2_static_friction, t, value); break;
    case DYN_SLIDING_FRICTION: pb_dynamics->SetValue(std2_sliding_friction, t, value); break;
...
void XRayMtl::OldVerFix(int loadVer)
{
    ...
    if (loadVer < 10) {
        ...
        IParamBlock* newParamBlk =
            UpdateParameterBlock(stdmtlPB, NPARAMS_O, oldPB, stdmtl2PB, NPARAMS, NEWSTDMTL_PBVERSION);
    ...
    pb_extended->SetValue(
        std2_opacity_type, 0, (GetFlag(NEWSTDMTL_ADD_TRANSP) ? 2 : GetFlag(NEWSTDMTL_FILT_TRANSP) ? 0 : 1));
    ...
    if (loadVer < 8) {
        ...
        if (MAPACTIVE(ID_OP)) {
            if (maps->txmap[ID_OP].amount != 1.0f) SetOpacity(0.0f, 0);
        }

        if (stdParams & STD_PARAM_SELFILLUM)
            if (MAPACTIVE(ID_SI)) {
                if (maps->txmap[ID_SI].amount != 1.0f) pShader->SetSelfIllum(0.0f, 0);
            }
...
IOResult XRayMtl::Load(ILoad* iload)
{
    ...
    while (IO_OK == (res = iload->OpenChunk()))
    {
        ...
        case MTL_HDR_CHUNK:
            ...
            ivalid.SetEmpty();
            break;
        case NEWSTDMTL_FLAGS_CHUNK: res = iload->Read(&flags, sizeof(flags), &nb); break;
        case NEWSTDMTL_SHADING_CHUNK: res = iload->Read(&shaderId, sizeof(shaderId), &nb); break;
        ...
        break;
        case NEWSTDMTL_VERS_CHUNK: res = iload->Read(&version, sizeof(version), &nb); break;
        ...
        break;
        case NEWSTDMTL_DIM_REFLECT: dimReflect = TRUE; break;
        }
        iload->CloseChunk();
        if (res != IO_OK) return res;
    }
    ...
    if (version <= FINAL_PARAMBLOCK_v1_VERSION) {
        iload->RegisterPostLoadCallback(
            new ParamBlockPLCB((ParamVersionDesc*)oldNewStdMtlVersions, NEWSTDMTL_NUMOLDVER, &stdMtlVersion, this, 0));
        iload->RegisterPostLoadCallback(new NewStdMtl2UpdateCB(this, version));
        if (version < 12) iload->RegisterPostLoadCallback(new NewStdMtl2BumpFixCB(this));
        iload->SetObsolete();
    }
...
void XRayMtl::PreShade(ShadeContext& sc, IReshadeFragment* pFrag)
{
    ...
    if (bumped) sc.SetNormal(sc.OrigNormal());
}
...
void XRayMtl::PostShade(ShadeContext& sc, IReshadeFragment* pFrag, int& nextTexIndex, IllumParams*)
{
    ...
    if (pFrag->NChannels() > 0) {
        ...
        // do refraction map
        if ((*maps)[refrChan].map) ip.hasComponents |= HAS_REFRACT_MAP;
...
void XRayMtl::Shade(ShadeContext& sc)
{
    ...
    if (sc.mode == SCMODE_SHADOW) {
        // Opacity mapping;
        if (DOMAP(trChan)) opac = (*maps)[trChan].LerpEvalMono(sc, opac);
...
void XRayMtl::SetupGfxMultiMaps(TimeValue t, Material* mtl, MtlMakerCallback& cb)
{
    ...
    if (tx[0]) {
        ...
        if (ti.tiling[0] == GW_TEX_NO_TILING || ti.tiling[1] == GW_TEX_NO_TILING) needDecal = TRUE;
    ...
    )
{
    ...
    if (channelID < 0 || channelID >= NumSubTexmaps()) return false;
...
bool XRayMtl::EvalChannel(ShadeContext& sc, int channelID, Color& outClr)
{
    ...
    if (sc.doMaps) {
        if (channelID == StdIDToChannel(ID_AM) && (flags & NEWSTDMTL_LOCK_ADTEX) == NEWSTDMTL_LOCK_ADTEX) {
            ...
            if (diffChanID >= 0 && SubTexmapOn(diffChanID) && (diffChanType & SKIP_CHANNELS) == 0) {
...
{
    int id = StdIDToChannel(ID_RL);
    if (!ShouldEvalSubTexmap(sc, id)) return false;
...
bool XRayMtl::ShouldEvalSubTexmap(ShadeContext& sc, int id)
{
    if (id >= 0 && id < NumSubTexmaps() && sc.doMaps && SubTexmapOn(id)) {
        ...
        if (texMap != NULL && texMap->IsOutputMeaningful(sc)) return true;
    }
