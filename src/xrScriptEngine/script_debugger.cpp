...
{
    if (Active()) _SendMessage(DMSG_CLOSE_CONNECTION, 0, 0);
...
{
    if (Active()) DebugBreak(szFile, nLine);
}
...
bool CScriptDebugger::HasBreakPoint(const char* fileName, s32 lineNum)
{
    ...
    for (u32 i = 0; i < m_breakPoints.size(); i++)
    {
        ...
        if (bp.nLine == lineNum && xr_strlen(bp.fileName) == xr_strlen(sFileName) && !stricmp(*bp.fileName, sFileName))
            return true;
    }
    return false;
}
