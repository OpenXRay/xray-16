...
{
    xr_unordered_multimap<u32, std::pair<u32, shared_str>> mtex;
    mtex.reserve(m_textures.size());

    // sort
    for (const auto& t : m_textures)
    {
        u32 m = t.second->flags.MemoryUsage;
        shared_str n = t.second->cName;
        mtex.emplace(m, std::make_pair(t.second->dwReference, n));
    }

    // dump
    for (const auto& t : mtex)
        Msg("* %4.1f : [%4d] %s", float(t.first) / 1024.f, t.second.first, t.second.second.c_str());
}
