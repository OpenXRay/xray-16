...
class FSRUpscaleBackend final : public IUpscaleBackend
{
    bool m_ready = false;
public:
    bool Init(nvrhi::IDevice*) override
    {
        ...
        m_ready = true;
        Msg("* [Upscale] FSR backend initialized");
#else
        m_ready = false;
        Msg("* [Upscale] FSR SDK not compiled in (XRAY_USE_FSR)");
#endif
        return m_ready;
    }
    void Shutdown() override { m_ready = false; }
    bool IsAvailable() const override { return m_ready; }
    UpscaleBackendType GetType() const override { return UpscaleBackendType::FSR; }
    bool SupportsFG() const override { return m_ready; }
...
IUpscaleBackend* CreateFSRUpscaleBackend() { return new FSRUpscaleBackend(); }
...
IUpscaleBackend* CreateUpscaleBackendAuto()
{
    ...
    if (ps_r_upscale == 2) {
        ...
        if (dlss->Init(GEnv.Backend ? GEnv.Backend->GetDevice() : nullptr) && dlss->IsAvailable())
            return dlss;
        delete dlss;
        Msg("! [Upscale] DLSS unavailable, falling back to FSR");
        ps_r_upscale = 1;
    }
    if (ps_r_upscale == 1) {
        auto* fsr = CreateFSRUpscaleBackend();
        if (fsr->Init(GEnv.Backend ? GEnv.Backend->GetDevice() : nullptr) && fsr->IsAvailable())
            return fsr;
        delete fsr;
        Msg("! [Upscale] FSR unavailable");
        ps_r_upscale = 0;
    }
