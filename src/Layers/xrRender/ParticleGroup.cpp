...
BOOL CPGDef::Load(IReader& F)
{
    ...
    const bool dontCalcTimeLimit = m_fTimeLimit > 0.0f;
...
void CParticleGroup::SItem::Stop(BOOL def_stop)
{
    ...
    for (auto& it : _children_related)
        static_cast<CParticleEffect*>(it)->Stop(def_stop);

    for (auto& it : _children_free)
        static_cast<CParticleEffect*>(it)->Stop(def_stop);
...
void CParticleGroup::SItem::OnFrame(u32 u_dt, const CPGDef::SEffect& def, Fbox& box, bool& bPlaying)
{
    ...
    if (!_children_related.empty())
    {
        for (auto& it : _children_related)
        {
            CParticleEffect* effect = static_cast<CParticleEffect*>(it);
            if (effect)
            {
                effect->OnFrame(u_dt);
                if (effect->IsPlaying())
...
void CParticleGroup::OnFrame(u32 u_dt)
{
    if (m_Def && m_RT_Flags.is(flRT_Playing))
    {
        ...
        for (auto e_it = m_Def->m_Effects.cbegin(); e_it != m_Def->m_Effects.cend(); ++e_it)
        {
            const auto particleEffect = *e_it;

            if (particleEffect->m_Flags.is(CPGDef::SEffect::flEnabled))
            {
                VERIFY(items.size() == m_Def->m_Effects.size());
                SItem& item = items[e_it - m_Def->m_Effects.begin()];

                if (item.IsPlaying())
                {
                    if (ct <= particleEffect->m_Time1 && ct + f_dt >= particleEffect->m_Time1)
                        item.Stop(particleEffect->m_Flags.is(CPGDef::SEffect::flDefferedStop));
                }
                else
                {
                    if (!m_RT_Flags.is(flRT_DefferedStop))
                        if (ct <= particleEffect->m_Time0 && ct + f_dt >= particleEffect->m_Time0)
                            item.Play();
                }
            }
        }
        ...
        for (auto item = items.begin(); item != items.end(); ++item)
            item->OnFrame(u_dt, *m_Def->m_Effects[item - items.begin()], box, isPlaying);

        if (m_RT_Flags.is(flRT_DefferedStop) && !isPlaying)
