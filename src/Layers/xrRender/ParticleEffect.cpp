...
{
    auto processParticle = [&](u32 i, Particle& m)
    {
        float sina = 0.0f, cosa = 0.0f;

        // Should be max or some particles won't play
    ...
    if (count <= MAX_COUNT)
    {
        PARALLEL_FOR(u32, 0, count, i, processParticle(i, particles[i]))
    }
    else
    {
        for (u32 i = 0; i < count; i++)
            processParticle(i, particles[i]);
    }
}
