...
void EnsureGlowResources(nvrhi::IDevice* nv, GlowPassState& state)
{
    ...
    state.inputLayout = cache.GetOrCreateInputLayout("GlowBillboard", attribs, std::size(attribs), state.vs, nv);

    nvrhi::RenderState rs;
    rs.blendState.targets[0].setBlendEnable(true);
    rs.blendState.targets[0].setSrcBlend(nvrhi::BlendFactor::One);
