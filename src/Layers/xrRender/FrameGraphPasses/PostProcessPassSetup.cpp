...
void EnsurePPPipeline(nvrhi::IDevice* nv, PostProcessPassState& st, nvrhi::Format outFmt)
{
    ...
    {
        Msg("! [PostProcess] shaders missing — PP disabled");
        st.initialized = true;
        return;
    }
    ...
    PostProcessPassState& state)
{
    ...
    if (!target->u_need_PP())
    {
        ...
        {
            if (g_pGamePersistent->m_pGShaderConstants->m_blender_mode.x < 0.5f)
                return ldrInput;
        }
        else
            return ldrInput;
    ...
    auto& pd = fg.addCallbackPass<PassData>(
        ...
        [](const PassData& data, const FrameGraph& graph, fg::RenderContext* ctx) {
            ...
            if (data.target->u_need_CM())
            {
                auto& cm0 = data.target->get_color_map_manager().get_cmap(0);
                auto& cm1 = data.target->get_color_map_manager().get_cmap(1);
                if (cm0 && cm0->surface_get_native())
                    grad0 = static_cast<nvrhi::ITexture*>(cm0->surface_get_native());
                if (cm1 && cm1->surface_get_native())
                    grad1 = static_cast<nvrhi::ITexture*>(cm1->surface_get_native());
            }
