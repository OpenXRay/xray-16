...
constexpr u32 kWetPipeVersion = 28;
...
void InitializeWetSurfacesPass(nvrhi::IDevice* device, WetSurfacesPassState& state)
{
    ...
    state.wetTexResolved = false;
    state.waterRippleTex = nullptr;
    state.waterFallTex = nullptr;
    state.puddlesPerlinTex = nullptr;
    ...
    {
        Msg("! [WetSurfaces] Failed to load apply shader vs=%d apply=%d — wet disabled",
            vs.handle && vs.reflection, applyPs.handle && applyPs.reflection);
        state.initialized = true;
    ...
    if (patchPs.handle && patchPs.reflection)
    {
        state.patchLayout = cache.GetOrCreateBindingLayoutFromReflection(
            "WetPatch_v27", *vs.reflection, *patchPs.reflection, device);
    }
    else
    {
    ...
    makePipe(applyPs, state.layout, state.pipeline, "WetApply_v28_ActorWet");
    makePipe(patchPs, state.patchLayout, state.patchPipeline, "WetPatch_v28_ActorWet");
    makePipe(writePs, state.writeNormalLayout, state.writeNormalPipeline, "WetWriteNormal_v28");

    state.initialized = true;
    state.pipeVersion = kWetPipeVersion;
    if (state.pipeline && state.layout)
    {
        Msg("* [WetSurfaces] Rebuild v28: apply=%d patch=%d write=%d",
    ...
    const WetSurfacesExtras& extras)
{
    ...
    (void)fg.addCallbackPass<WetSurfacesDrawData>(
        "WetSurfaces",
        [inputs, patched, outColor, outNormal, doWriteNormal, hasRainSM, hasSceneReflection,
         width, height, &passState, extras](
            FrameGraph& builder, PassHandle passHandle, WetSurfacesDrawData& data)
        {
        ...
        [](const WetSurfacesDrawData& data, const FrameGraph& fgGraph, fg::RenderContext* ctx)
        {
            ...
            if (!ps->wetTexResolved)
            {
                ...
                nvrhi::ITexture* waterFall = loadTex("water" DELIMITER "water_normal");
                if (!waterFall)
                    waterFall = cache.GetDummyContactHistory(nvDevice);
                ps->waterFallTex = waterFall;
                ps->waterRippleTex = waterFall;
                ps->puddlesPerlinTex = waterFall;
                ps->wetTexResolved = true;
            }
            nvrhi::ITexture* waterRipple = ps->waterRippleTex;
            nvrhi::ITexture* waterFall = ps->waterFallTex;
            nvrhi::ITexture* puddlesPerlin = ps->puddlesPerlinTex;
            if (!waterRipple)
            ...
            if (ps->patchPipeline && ps->patchLayout && patchRefl)
            {
                ...
                bsb.ConstantBuffer("WetConstants", wetCB)
                    ...
                    .Texture("g_SkyOpen", data.extras.skyOpenTex
                        ? data.extras.skyOpenTex
                        : ReSTIRMemoryManager::Instance().GetPlaceholderTex());
