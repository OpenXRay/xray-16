    ...
    auto& passData = fg.addCallbackPass<PassData>(
        ...
        [](const PassData& data, const FrameGraph& graph, fg::RenderContext* ctx) {
            ...
            auto drawSet = [&](const BindlessDrawSet& set) {
                ...
                nvrhi::IBuffer* batchIndices = set.compactBatchIndicesBuffer;
                nvrhi::IBuffer* materialIDs = set.compactMaterialIDBuffer;
                nvrhi::IBuffer* drawArgs = set.compactDrawArgsBuffer;
                nvrhi::IBuffer* countBuffer = set.compactCountBuffer;
