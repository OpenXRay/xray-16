...
ref_constant R_constant_table::get(pcstr S, u16 type /*= u16(-1)*/) const
{
    ...
    if (type == u16(-1))
    {
        it = std::lower_bound(table.cbegin(), table.cend(), S, [](const ref_constant& C, cpcstr S) constexpr
        {
    ...
    {
        it = std::find_if(table.cbegin(), table.cend(), [&](const ref_constant& constant) constexpr
        {
