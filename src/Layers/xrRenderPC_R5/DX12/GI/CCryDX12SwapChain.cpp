...
void CCryDX12SwapChain::AcquireBuffers()
{
    ...
    std::vector<DX12::Resource*> resources;
    ...
    for (int i = 0; i < desc.BufferCount; ++i)
    {
        ...
        if (S_OK == swapChain->GetBuffer(i, IID_PPV_ARGS(&resource12)))
        {
    ...
    _Out_  void** ppSurface)
{
    ...
    {
        R_ASSERT2(Buffer < m_BackBuffers.size(), "Invalid buffer index");
...
HRESULT STDMETHODCALLTYPE CCryDX12SwapChain::Present1(
    UINT SyncInterval, UINT Flags, const DXGI_PRESENT_PARAMETERS* pPresentParameters)
{
    DX12_FUNC_LOG
...
UINT STDMETHODCALLTYPE CCryDX12SwapChain::GetCurrentBackBufferIndex(void)
{
    DX12_FUNC_LOG
    return m_SwapChain->GetCurrentBackBufferIndex();
}

HRESULT STDMETHODCALLTYPE CCryDX12SwapChain::ResizeBuffers1(UINT BufferCount, UINT Width, UINT Height,
