...
inline UINT64 UpdateSubresources(
    ...
    _In_ const D3D12_BOX* pDstBox) noexcept
{
    // Minor validation
#if defined(_MSC_VER) || !defined(_WIN32)
    ...
    {
        const CD3DX12_TEXTURE_COPY_LOCATION Dst(pDestinationResource, FirstSubresource);
        const CD3DX12_TEXTURE_COPY_LOCATION Src(pIntermediate, pLayouts[0]);
    ...
    _In_ const D3D12_BOX* pDstBox) noexcept
{
    ...
    const UINT64 Result = UpdateSubresources(pCmdList, pDestinationResource, pIntermediate, FirstSubresource, NumSubresources, RequiredSize, pLayouts, pNumRows, pRowSizesInBytes, pSrcData, pDstBox);
    ...
    _In_ const D3D12_BOX* pDstBox) noexcept
{
...
#else
    D3D12_RESOURCE_DESC tmpDesc;
    auto& Desc = *pDestinationResource->GetDesc(&tmpDesc);
#endif
    ID3D12Device* pDevice = nullptr;
    pDestinationResource->GetDevice(IID_ID3D12Device, reinterpret_cast<void**>(&pDevice));
    pDevice->GetCopyableFootprints(&Desc, FirstSubresource, NumSubresources, IntermediateOffset, pLayouts, pNumRows, pRowSizesInBytes, &RequiredSize);
    ...
    const UINT64 Result = UpdateSubresources(pCmdList, pDestinationResource, pIntermediate, FirstSubresource, NumSubresources, RequiredSize, pLayouts, pNumRows, pRowSizesInBytes, pResourceData, pSrcData, pDstBox);
    HeapFree(GetProcessHeap(), 0, pMem);
    return Result;
}
