...
namespace hash
{
    ...
    //
    //=========================================================================
    constexpr crc32::crc32(std::string_view view)
        : m_value{ 0 }
    {
        if (!view.empty())
        {
    ...
    namespace internal
    {
        ...
        inline static constexpr hash::crc32 CompileTimeCrc32 = hash::crc32(CrcValue);
        ...
        constexpr void Crc32Set(const CharType* data, size_t size, bool forceLowerCase, u32& value)
        {
            ...
            else
            {
                ...
                if (size)
                {
                    if (forceLowerCase)
                    {
                        do
                        {
                            ...
                            {
                                hash = ComputeCrc32Octet(hash, (static_cast<uint8_t>(*buf)) + 'a' - 'A');
                                buf++;
                            }
                            else
                            {
                                hash = ComputeCrc32Octet(hash, static_cast<uint8_t>(*buf++));
                            }
                        } while (--size);
                    }
                    else
                ...
                value = hash ^ 0xffffffffL;     /* (instead of ~c for 64-bit machines) */
            }
        }
    }

    IC void hash::crc32::Set(const void* data, size_t size, bool forceLowerCase)
    {
    ...
    // crc32 - Add
    //=========================================================================
    constexpr void crc32::Add(std::string_view view)
    {
        if (!view.empty())
        {
            size_t len = view.size();
            u32 hash = static_cast<u32>(crc32(view));
            Combine(hash, len);
        }
    }
    ...
    IC void crc32::Add(const void* data, size_t size, bool forceLowerCase)
    {
    ...
    // crc32 - Combine
    //=========================================================================
    constexpr void crc32::Combine(u32 hash, size_t len)
    {
