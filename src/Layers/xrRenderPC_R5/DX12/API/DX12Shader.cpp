...
namespace DX12
{
    ...
    Shader* Shader::Create(Device* device, const D3D12_SHADER_BYTECODE& byteCode)
    {
        ID3D12ShaderReflection* shaderReflection = NULL;

        // Reflect shader internals
        if (S_OK !=
            D3DReflectDXILorDXBC(byteCode.pShaderBytecode, byteCode.BytecodeLength, IID_ID3D12ShaderReflection,
