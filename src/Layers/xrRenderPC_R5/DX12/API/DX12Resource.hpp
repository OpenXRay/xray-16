...
namespace DX12
{
    ...
    public:
        ...
        struct InitialData
        {
            ...
            ~InitialData()
            {
                for (D3D12_SUBRESOURCE_DATA& data : m_SubResourceData)
                {
                    if (data.pData)
                    {
                        delete[] data.pData;
...
#define DX12_FREETHREADED_RESOURCES
        ...
        inline const std::atomic<u64> (& GetFenceValues(const int type))[CMDQUEUE_NUM]
        {
            return m_FenceValues[type];
        }

        inline void MaxFenceValues(std::atomic<u64> (&rFenceValues)[CMDTYPE_NUM][CMDQUEUE_NUM], const int type)
        {
    ...
    protected:
        ...
        _smart_ptr<ID3D12Resource> m_pD3D12Resource;  // can be replaced by MAP_DISCARD, which is deprecated

        SwapChain* m_pSwapChainOwner;

        // Potentially changes on every resource-use
        ResourceStates m_States;
        mutable std::atomic<u64> m_FenceValues[CMDTYPE_NUM][CMDQUEUE_NUM];

        // Used when using the resource the first time
        std::unique_ptr<InitialData> m_InitialData;
    };
}
