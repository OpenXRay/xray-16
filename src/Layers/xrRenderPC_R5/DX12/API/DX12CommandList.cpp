...
namespace DX12
{
    ...
    void CommandList::SetDescriptorTables(CommandMode commandMode, D3D12_DESCRIPTOR_HEAP_TYPE eType)
    {
        ...
        for (u32 rootParameterIdx = 0, tableIdx = 0; rootParameterIdx < layout.m_NumRootParameters; ++rootParameterIdx)
        {
    ...
    void CommandList::BindVertexBufferView(const ResourceView& view, INT offset, const _range_t<UINT>& bindRange, UINT bindStride, D3D12_VERTEX_BUFFER_VIEW (&heap)[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT])
    {
    ...
    void CommandList::CopySubresource(Resource& dstResource, UINT dstSubResource, UINT x, UINT y, UINT z, Resource& srcResource, UINT srcSubResource, const D3D12_BOX* srcBox)
    {
        ...
        else if (dstResource.GetDesc().Dimension != D3D12_RESOURCE_DIMENSION_BUFFER)
        {
            ...
            R_ASSERT4(dstSubResource < CCryDX12DeviceContext::MAX_SUBRESOURCES, "Too many sub resources: (sub ID %d requested, %d allowed)",  std::to_string(dstSubResource).c_str(),  std::to_string(CCryDX12DeviceContext::MAX_SUBRESOURCES).c_str());
    ...
    void CommandList::UpdateSubresourceRegion(Resource& resource, UINT subResource, const D3D12_BOX* box, const void* data, UINT rowPitch, UINT slicePitch)
    {
        ...
        R_ASSERT2(resourceStates.m_AnnouncedState == -1, "Recycled resource should not be in split state");
    ...
    void CommandListPool::ScheduleCommandLists()
    {
        ...
        {
            _smart_ptr<CommandList> pCmdList = m_LiveCommandLists.front();
        ...
        for (uint32_t t = 0; t < m_LiveCommandLists.size(); ++t)
        {
            _smart_ptr<CommandList> pCmdList = m_LiveCommandLists[t];
        ...
        {
            _smart_ptr<CommandList> pCmdList = m_BusyCommandLists.front();
    ...
    void CommandListPool::CreateOrReuseCommandList(_smart_ptr<CommandList>& result)
    {
    ...
    void CommandListPool::AcquireCommandList(_smart_ptr<CommandList>& result)
    {
    ...
    void CommandListPool::ForfeitCommandList(_smart_ptr<CommandList>& result, bool bWait)
    {
        static CryCriticalSectionNonRecursive csThreadSafeScope;
        CryAutoLock<CryCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
        _smart_ptr<CommandList> pWaitable = result;
    ...
    void CommandListPool::AcquireCommandLists(uint32_t numCLs, _smart_ptr<CommandList>* results)
    {
        static CryCriticalSectionNonRecursive csThreadSafeScope;
        CryAutoLock<CryCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
        ...
        for (uint32_t i = 0; i < numCLs; ++i)
        {
            CreateOrReuseCommandList(results[i]);
        }
    }

    void CommandListPool::ForfeitCommandLists(uint32_t numCLs, _smart_ptr<CommandList>* results, bool bWait)
    {
        static CryCriticalSectionNonRecursive csThreadSafeScope;
        CryAutoLock<CryCriticalSectionNonRecursive> lThreadSafeScope(csThreadSafeScope);
        _smart_ptr<CommandList> pWaitable = results[numCLs - 1];
