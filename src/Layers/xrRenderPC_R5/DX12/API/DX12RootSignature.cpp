...
namespace DX12
{
    ...
    void PipelineLayout::MakeDescriptorTable(
        u32 rangeCursorEnd,
        u32& rangeCursorBegin,
        u32 offsetEnd,
        u32& offsetBegin,
        D3D12_DESCRIPTOR_HEAP_TYPE eHeap,
        D3D12_SHADER_VISIBILITY visibility)
    {
        const u32 numRanges = rangeCursorEnd - rangeCursorBegin;
        if (numRanges > 0)
        {
        ...
        if (bindings.m_ConstantBuffers.m_DescriptorCount)
        {
            ...
            for (size_t rangeIdx = 0; rangeIdx < ranges.size(); ++rangeIdx)
            {
                ...
                else if (!range.m_bUsed)
                {
                    ...
                    for (u32 rootParameterIdx = rangeBegin; rootParameterIdx < rangeEnd; ++rootParameterIdx)
                    {
    ...
    void PipelineLayout::AppendDescriptorTables(Shader* pShader, EShaderStage shaderStage)
    {
        ...
        u32 rangeCursorBegin = m_DescRangeCursor;
        u32 rangeCursorEnd = m_DescRangeCursor;
        u32 descriptorBegin = m_NumDescriptors;
        u32 descriptorEnd = m_NumDescriptors;
        ...
        if (bindings.m_ConstantBuffers.m_DescriptorCount)
        {
            ...
            for (size_t rangeIdx = 0; rangeIdx < ranges.size(); ++rangeIdx)
            {
                ...
                for (size_t shaderRegister = rangeBegin; shaderRegister < rangeEnd; ++shaderRegister)
                {
                    ...
                    ZeroMemory(&bindingLayout, sizeof(ResourceLayoutBinding));
        ...
        if (DX12_DESCRIPTORTABLE_MERGERANGES_MODE != 0)
        {
            MakeDescriptorTable(rangeCursorEnd, rangeCursorBegin, descriptorEnd, descriptorBegin, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, visibility);
        }
        ...
        u32 lastSamplers = m_NumDynamicSamplers;
        u32 currentSamplers = m_NumDynamicSamplers;
    ...
    void PipelineLayout::DebugPrint() const
    {
        ...
        for (u32 p = 0, r = 0; p < m_NumRootParameters; ++p)
        {
    ...
    bool RootSignature::Init(CommandMode commandType)
    {
...

        {
            ...
            if (S_OK != GetDevice()->GetD3D12Device()->CreateRootSignature(m_nodeMask, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(), IID_PPV_ARGS(&rootSign)))
            {
