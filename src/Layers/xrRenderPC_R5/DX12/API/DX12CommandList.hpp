...
namespace DX12
{
    ...
    public:
        ...
        void AcquireCommandList(_smart_ptr<CommandList> & result);
        void ForfeitCommandList(_smart_ptr<CommandList> & result, bool bWait = false);
        void AcquireCommandLists(uint32_t numCLs, _smart_ptr<CommandList> * results);
        void ForfeitCommandLists(uint32_t numCLs, _smart_ptr<CommandList> * results, bool bWait = false);
    ...
    public:
        ...
        inline void WaitForFenceOnGPU(const std::atomic<u64> (&fenceValues)[CMDQUEUE_NUM])
        {
            ...
            if (!m_rCmdFences.IsCompleted(fenceValuesMasked))
            {
                DX12_LOG("Waiting for GPU fences (type: %d) [%d,%d] -> [%d,%d]",
                    ...
                    static_cast<u64>(fenceValues[CMDQUEUE_COPY    ]),
                    static_cast<u64>(fenceValues[CMDQUEUE_GRAPHICS]));
#ifdef DX12_STATS
                m_NumWaitsGPU += 2;
#endif // DX12_STATS
        ...
        inline bool IsCompleted(const std::atomic<u64> (&fenceValues)[CMDQUEUE_NUM]) const
        {
    ...
    private:
        ...
        void CreateOrReuseCommandList(_smart_ptr<CommandList> & result);
        ...
        typedef std::deque<_smart_ptr<CommandList>> TCommandLists;
...
#define CLCOUNT_SETSTATE    0
        ...
        void ClearState(const DX12::PipelineState* pipeline) 
        {
            m_CommandList->ClearState(pipeline->GetD3D12PipelineState());
        }
        ...
        inline void TrackResourceSRVUsage(Resource& resource) { TrackResourceUsage(resource, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, CMDTYPE_WRITE, CMDTYPE_READ); }
        inline void TrackResourceUAVUsage(Resource& resource) { TrackResourceUsage(resource, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, CMDTYPE_ANY, CMDTYPE_ANY); }
        inline void TrackResourceDSVUsage(Resource& resource) { TrackResourceUsage(resource, D3D12_RESOURCE_STATE_DEPTH_WRITE, CMDTYPE_ANY, CMDTYPE_WRITE); }
        inline void TrackResourceDRVUsage(Resource& resource) { TrackResourceUsage(resource, D3D12_RESOURCE_STATE_DEPTH_READ, CMDTYPE_WRITE, CMDTYPE_READ); }
        inline void TrackResourceRTVUsage(Resource& resource) { TrackResourceUsage(resource, D3D12_RESOURCE_STATE_RENDER_TARGET, CMDTYPE_ANY, CMDTYPE_WRITE); }
        ...
        void BindVertexBufferView(const ResourceView&view, INT offset, const _range_t<UINT>&bindRange, UINT bindStride, D3D12_VERTEX_BUFFER_VIEW (&heap)[D3D11_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT]);
        inline void BindVertexBufferView(const ResourceView& view, INT offset, const _range_t<UINT>& bindRange, UINT bindStride)
        {
    ...
    private:
        ...
        static const u32 RootParameterCountMax = 16;
        ...
        static const u32 TimerCountMax = 2048;
#endif
    };
}
