...
CCryDX12DeviceContext* CCryDX12DeviceContext::Create(CCryDX12Device* pDeviced, bool isDeferred)
{
    return DX12::PassAddRef(new CCryDX12DeviceContext(pDeviced, isDeferred));
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

CCryDX12DeviceContext::CCryDX12DeviceContext(CCryDX12Device* pDevice, bool isDeferred)
    : Super()
    , m_bDeferred(isDeferred)
    , m_pDevice(pDevice)
    ...
    , m_bInCopyRegion{}
{
    ...
    CD3DX12_RESOURCE_DESC bufferTimeStamp =
        ...
        IID_PPV_ARGS(&m_TimestampDownloadBuffer)))
    {
     ...
     CD3DX12_RESOURCE_DESC bufferOcclusion =
...
bool CCryDX12DeviceContext::PreparePSO(DX12::CommandMode commandMode)
{
    ...
    if (state.m_StateFlags & EPSPB_PipelineState)
    {
        ...
        if (DX12_SUBMISSION_MODE == DX12_SUBMISSION_PERPSO && m_OutstandingQueries == 0)
        {
            bForceFlush = true;
        }
    }
...
void CCryDX12DeviceContext::PrepareGraphicsFF()
{
    ...
    {
        R_ASSERT2(state.InputAssembler.IndexBuffer, "IndexBuffer is required by the Draw but has not been set!");
    ...
    if (stateFlags & EPSPB_VertexBuffers)
    {
        ...
        for (UINT i = 0, S = state.InputAssembler.NumVertexBuffers.Get(); i < S; ++i)
        {
            ...
            if (buffer)
            {
                _range_t<UINT> bindRange = _range_t<UINT> (state.InputAssembler.Offsets.Get(i), state.InputAssembler.Offsets.Get(i));
                UINT bindStride = state.InputAssembler.Strides.Get(i);
                R_ASSERT2(buffer, "VertexBuffer is required by the PSO but has not been set!");
...
void CCryDX12DeviceContext::BindResources(DX12::CommandMode commandMode)
{
    ...
    u32 resourceDescriptorOffset = 0;
    u32 samplerDescriptorOffset = 0;
...
void CCryDX12DeviceContext::ResumeDirectCommandQueue()
{
    ...
    R_ASSERT2(m_DirectCommandList == nullptr, "CommandList hasn't been submitted!");
    m_DirectListPool.AcquireCommandList(m_DirectCommandList);
...
{
    u64 fenceValuesMasked[CMDQUEUE_NUM];
...
void CCryDX12DeviceContext::SubmitAllCommands(bool wait, const std::atomic<u64>(&fenceValues)[CMDQUEUE_NUM])
{
    SubmitDirectCommands(wait, fenceValues[CMDQUEUE_GRAPHICS]);
    SubmitCopyCommands(wait, fenceValues[CMDQUEUE_COPY]);
}

void CCryDX12DeviceContext::SubmitAllCommands(bool wait, const std::atomic<u64>(&fenceValues)[CMDQUEUE_NUM], int fenceId)
{
    u64 fenceValuesMasked[CMDQUEUE_NUM];
    fenceValuesMasked[CMDQUEUE_GRAPHICS] = fenceValues[CMDQUEUE_GRAPHICS];
    fenceValuesMasked[CMDQUEUE_COPY] = fenceValues[CMDQUEUE_COPY];
...
UINT CCryDX12DeviceContext::TimestampIndex(DX12::CommandList* pCmdList)
{
...
#endif
        if (DX12_SUBMISSION_MODE <= DX12_SUBMISSION_PERDRAW && m_OutstandingQueries == 0)
        {
    ...
    _Out_  D3D11_MAPPED_SUBRESOURCE* pMappedResource)
{
    ...
    if (!resource.IsOffCard())
    {
        ...
        if (textureDesc.Dimension == D3D12_RESOURCE_DIMENSION::D3D12_RESOURCE_DIMENSION_TEXTURE2D)
        {
            ...
            if (S_OK != hr)
            {
                R_ASSERT2(0, "Could not map resource!");
                return hr;
            }
    ...
    R_ASSERT2((MapType - 1) < DX12_ARRAY_SIZE(pRanges), "Invalid map type");

    R_ASSERT2(!D3D12IsLayoutOpaque(resource.GetDesc().Layout), "Opaque layouts are unmappable until 12.2!");
    ...
    _In_  ID3D11Asynchronous* pAsync)
{
    ...
    else if (desc.Query == D3D11_QUERY_OCCLUSION)
    {
        ...
        R_ASSERT2(m_OutstandingQueries != 0, "End without a Start");
        --m_OutstandingQueries;
    }
    ...
    _In_  UINT GetDataFlags)
{
    ...
    if (desc.Query == D3D11_QUERY_EVENT)
    {
        R_ASSERT2(DataSize >= sizeof(BOOL), "Invalid data size");
    ...
    {
        R_ASSERT2(DataSize >= sizeof(D3D11_QUERY_DATA_TIMESTAMP_DISJOINT), "Invalid data size");
    ...
    {
        R_ASSERT2(DataSize >= sizeof(UINT64), "Invalid data size");
...
static void ValidateSwapChain(CCryDX12RenderTargetView& rtv)
{
#if defined(DEBUG)
    if (DX12::SwapChain* swapChain = rtv.GetDX12Resource().GetDX12SwapChain())
    {
        R_ASSERT2(swapChain->GetCurrentBackBuffer().GetD3D12Resource() == rtv.GetDX12Resource().GetD3D12Resource(), "invalid swap chain buffer");
    }
#endif
}
    ...
    _In_  size_t numInitialData)
{
    ...
    R_ASSERT(numInitialData <= maxSubResource);
    ...
    _In_  UINT CopyFlags)
{
    ...
    R_ASSERT2(dstResource.GetDX12SwapChain() == nullptr, "Can't copy to swap chain buffer");
    ...
    _In_  UINT CopyFlags)
{
    ...
    R_ASSERT2(resource.GetDX12SwapChain() == nullptr, "Can't copy to swap chain buffer");
    ...
    _Out_ uint32_t* pRowPitch)
{
    ...
    R_ASSERT4(SubResource < MAX_SUBRESOURCES, "Too many sub resources: (sub ID %d requested, %d allowed)",  std::to_string(SubResource).c_str(),  std::to_string(MAX_SUBRESOURCES).c_str());
    ...
    *ppStagingMemory = *reinterpret_cast<uint8_t**>(ppStagingMemory) + layouts[SubResource].Offset;
