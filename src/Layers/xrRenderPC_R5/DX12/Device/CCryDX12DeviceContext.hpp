...
class CCryDX12DeviceContext
    : public CCryDX12Object<ID3D11DeviceContext1>
{
...
public:
    ...
    static CCryDX12DeviceContext* Create(CCryDX12Device* pDevice, bool isDeferred);

    CCryDX12DeviceContext(CCryDX12Device* pDevice, bool isDeferred);

    virtual ~CCryDX12DeviceContext();

    IC bool IsDeferred() const { return m_bDeferred; }

    IC void CeaseCoreCommandList(uint32_t nPoolId)
    {
    ...
    IC void ResumeCoreCommandList(uint32_t nPoolId)
    {
        if (nPoolId == CMDQUEUE_GRAPHICS)
        {
    ...
    IC DX12::CommandListPool& GetCoreCommandListPool(int nPoolId)
    {
        if (nPoolId == CMDQUEUE_GRAPHICS)
        {
    ...
    IC DX12::CommandListPool& GetCoreGraphicsCommandListPool()
    {
        return m_DirectListPool;
    }

    IC DX12::CommandList* GetCoreGraphicsCommandList() const
    {
    ...
    IC ID3D12Device* GetD3D12Device() const
    {
...
private:
    ...
    void SubmitAllCommands(bool wait, const std::atomic<u64>(&fenceValues)[CMDQUEUE_NUM]);
    void SubmitAllCommands(bool wait, const std::atomic<u64>(&fenceValues)[CMDQUEUE_NUM], int fenceId);
    ...
    _smart_ptr<DX12::CommandList> m_DirectCommandList;
    _smart_ptr<DX12::CommandList> m_CopyCommandList;
