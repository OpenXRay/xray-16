...
void CRenderTarget::accum_omnip_geom_create()
{
    // vertices
    {
        ...
        BYTE* pData = static_cast<BYTE*>(g_accum_omnip_vb.GetHostPointer());
        CopyMemory(pData, du_sphere_part_vertices, vCount * vSize);
        g_accum_omnip_vb.Flush();
    }
    ...
    // Indices
    {
        ...
        BYTE* pData = static_cast<BYTE*>(g_accum_omnip_ib.GetHostPointer());
        CopyMemory(pData, du_sphere_part_faces, iCount * 2);
        g_accum_omnip_ib.Flush();
    }
}
...
#endif // DEBUG
    g_accum_omnip_ib.Destroy();
#ifdef DEBUG
    _SHOW_REF("g_accum_omnip_vb", &g_accum_omnip_vb);
#endif // DEBUG
    g_accum_omnip_vb.Destroy();
}
