...
#endif // CONFIG_PROFILE_LOCKS
{
    ...
    m_pGSBrowser = ServerBrowserNewA(GAMESPY_GAMENAME, GAMESPY_GAMENAME, secretKey,
        0, GAMESPY_BROWSER_MAX_UPDATES, QVERSION_QR2, SBFalse, SBCallback, this);
    if (!m_pGSBrowser)
    {
...
void			CGameSpy_Browser::RefreshListInternet (const char* FilterStr)
{
	...
	SBError error = ServerBrowserUpdateA(m_pGSBrowser, onUpdate ? SBTrue : SBFalse,
        SBFalse, targetFields, fieldCount, FilterStr);
...
void	CGameSpy_Browser::ReadServerInfo	(ServerInfo* pServerInfo, void* gsServer)
{
	...
	{
		PlayerInfo PInfo;
		_snprintf_s(PInfo.Name, sizeof(PInfo.Name)-1, "%s", SBServerGetPlayerStringValueA(pServer, i,	"player", "Unknown"));
