...
private:
    IC static bool has_same_edge(const type_face* F1, const type_face* F2, u16& F1_edge_index, u16& F2_edge_index)
    {
        ...
        for (int e = 0; e < 3; e++)
        {
            ...
            if (v1_a > v1_b) swap(v1_a, v1_b);

            for (int r = 0; r < 3; ++r)
            {
                type_vertex *v2_a, *v2_b;
                F2->EdgeVerts(r, &v2_a, &v2_b);
                if (v2_a > v2_b) swap(v2_a, v2_b);
    ...
    IC static bool do_connect_faces(const type_face& start, const type_face& test, u16 start_common_edge_idx,
...
public:
    ...
    IC bool add_adjacents(u32 start_face_idx, u32 test_face_idx)
    {
        ...
        if (is_processed(*test_face)) return false;
        ...
        if (has_same_edge(start_face, test_face, StartFace_common_edge_index, TestFace_common_edge_index)) {
            if ((start_face == test_face) || do_connect_faces(*start_face, *test_face, StartFace_common_edge_index,
