...
lzo1x_999_compress_internal(const lzo_byte* in, lzo_uint in_len, lzo_byte* out, lzo_uintp out_len, lzo_voidp wrkmem,
    const lzo_byte* dict, lzo_uint dict_len, lzo_progress_callback_t cb, int try_lazy, lzo_uint good_length,
    lzo_uint max_lazy, lzo_uint nice_length, lzo_uint max_chain, lzo_uint32 flags);
...
static int len_of_coded_match(lzo_uint m_len, lzo_uint m_off, lzo_uint lit)
{
    ...
    if (m_len < 2) return -1;
    if (m_len == 2) return (m_off <= M1_MAX_OFFSET && lit > 0 && lit < 4) ? 2 : -1;
    if (m_len <= M2_MAX_LEN && m_off <= M2_MAX_OFFSET) return 2;
    if (m_len == M2_MIN_LEN && m_off <= MX_MAX_OFFSET && lit >= 4) return 2;
...
static lzo_int min_gain(lzo_uint ahead, lzo_uint lit1, lzo_uint lit2, int l1, int l2, int l3)
{
    ...
    if (l3 > 0) lazy_match_min_gain -= (ahead - l3) * 2;

    if (lazy_match_min_gain < 0) lazy_match_min_gain = 0;
...
#if defined(LZO1Z)
    if (*m_off == c->last_m_off && *m_len <= M2_MAX_LEN) return;
#if 1
    ...
    lzo_uint max_lazy, lzo_uint nice_length, lzo_uint max_chain, lzo_uint32 flags)
{
    ...
    if (r != 0) return r;
    if (max_chain > 0) swd->max_chain = max_chain;
    if (nice_length > 0) swd->nice_length = nice_length;
    ...
    while (c->look > 0)
    {
        ...
        else if (m_len == M2_MIN_LEN)
        {
            ...
            if (m_off > MX_MAX_OFFSET && lit >= 4) m_len = 0;
        }
...
/* a match */
#if defined(SWD_BEST_OFF)
        if (swd->use_best_off) better_match(swd, &m_len, &m_off);
#endif
        ...
        while (ahead < max_ahead && c->look > m_len)
        {
            ...
            if (m_off == c->last_m_off && c->m_off != c->last_m_off)
                if (m_len >= M2_MIN_LEN && m_len <= M2_MAX_LEN) c->m_len = 0;
#endif
            if (c->m_len < m_len) continue;
#if 1
            if (c->m_len == m_len && c->m_off >= m_off) continue;
#endif
#if defined(SWD_BEST_OFF)
            if (swd->use_best_off) better_match(swd, &c->m_len, &c->m_off);
#endif
    ...
    /* store final run */
    if (lit > 0) op = STORE_RUN(c, op, ii, lit);
    ...
    *out_len = op - out;
    ...
    if (c->cb) (*c->cb)(c->textsize, c->codesize);
    ...
    const lzo_byte* dict, lzo_uint dict_len, lzo_progress_callback_t cb, int compression_level)
{
    ...
    if (compression_level < 1 || compression_level > 9) return LZO_E_ERROR;
