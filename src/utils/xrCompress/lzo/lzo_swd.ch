...
#if !defined(HEAD3)
#if 1
#define HEAD3(b, p) (((0x9f5f * (((((lzo_uint32)b[p] << 5) ^ b[p + 1]) << 5) ^ b[p + 2])) >> 5) & (SWD_HSIZE - 1))
#else
#define HEAD3(b, p) (((0x9f5f * (((((lzo_uint32)b[p + 2] << 5) ^ b[p + 1]) << 5) ^ b[p])) >> 5) & (SWD_HSIZE - 1))
#endif
#endif
 ...
 */
#if defined(__LZO_CHECKER)
#define s_head3(s, key) ((s->llen3[key] == 0) ? SWD_UINT_MAX : s->head3[key])
#else
...
static void swd_initdict(lzo_swd_t* s, const lzo_byte* dict, lzo_uint dict_len)
{
    s->dict = s->dict_end = NULL;
    s->dict_len = 0;

    if (!dict || dict_len <= 0) return;
    if (dict_len > s->n) {
        dict += dict_len - s->n;
        dict_len = s->n;
    }
...
static void swd_insertdict(lzo_swd_t* s, lzo_uint node, lzo_uint len)
{
    lzo_uint key;
...
static int swd_init(lzo_swd_t* s, const lzo_byte* dict, lzo_uint dict_len)
{
    lzo_uint i = 0;
    int c = 0;
    ...
    s->b_size = s->n + s->f;
    if (2 * s->f >= s->n || s->b_size + s->f >= SWD_UINT_MAX) return LZO_E_ERROR;
    ...
    if (s->look < 3) memset(&s->b[s->bp + s->look], 0, 3);
#endif
...
static void swd_exit(lzo_swd_t* s)
{
#if defined(__LZO_CHECKER)
...
static __inline__ void swd_getbyte(lzo_swd_t* s)
{
    int c;
    ...
    if ((c = getbyte(*(s->c))) < 0) {
        ...
        s->b[s->ip] = 0;
        if (s->ip < s->f) s->b_wrap[s->ip] = 0;
#endif
    }
    else
    {
        s->b[s->ip] = LZO_BYTE(c);
        if (s->ip < s->f) s->b_wrap[s->ip] = LZO_BYTE(c);
    }
    if (++s->ip == s->b_size) s->ip = 0;
...
static __inline__ void swd_remove_node(lzo_swd_t* s, lzo_uint node)
{
    if (s->node_count == 0) {
        lzo_uint key;
        ...
        if (s->first_rp != LZO_UINT_MAX) {
            if (node != s->first_rp)
                printf("Remove %5d: %5d %5d %5d %5d  %6d %6d\n", node, s->rp, s->ip, s->bp, s->first_rp, s->ip - node,
                    s->ip - s->bp);
        ...
        if ((lzo_uint)s->head2[key] == node) s->head2[key] = NIL2;
#endif
    }
    else
        --s->node_count;
...
static void swd_accept(lzo_swd_t* s, lzo_uint n)
{
    assert(n <= s->look);
...
static void swd_search(lzo_swd_t* s, lzo_uint node, lzo_uint cnt)
{
    ...
    for (; cnt-- > 0; node = s->succ3[node])
    {
        ...
        if (
#if 1
            p2[m_len - 1] == scan_end1 && p2[m_len] == p1[m_len] &&
#endif
            p2[0] == p1[0] && p2[1] == p1[1])
        {
            lzo_uint i;
            ...
            if (memcmp(bp, &b[node], i) != 0)
                printf("%5ld %5ld %02x%02x %02x%02x\n", (long)s->bp, (long)node, bp[0], bp[1], b[node], b[node + 1]);
#endif
            assert(memcmp(bp, &b[node], i) == 0);

#if defined(SWD_BEST_OFF)
            if (i < SWD_BEST_OFF) {
                if (s->best_pos[i] == 0) s->best_pos[i] = node + 1;
            }
#endif
            if (i > m_len) {
                s->m_len = m_len = i;
                ...
                if (m_len > (lzo_uint)s->best3[node]) return;
                scan_end1 = bp[m_len - 1];
            }
        }
    }
...
static lzo_bool swd_search2(lzo_swd_t* s)
{
    lzo_uint key;
    ...
    if (memcmp(&s->b[s->bp], &s->b[key], 2) != 0)
        printf("%5ld %5ld %02x%02x %02x%02x\n", (long)s->bp, (long)key, s->b[s->bp], s->b[s->bp + 1], s->b[key],
            s->b[key + 1]);
#endif
    assert(memcmp(&s->b[s->bp], &s->b[key], 2) == 0);
#if defined(SWD_BEST_OFF)
    if (s->best_pos[2] == 0) s->best_pos[2] = key + 1;
#endif
...
static void swd_findbest(lzo_swd_t* s)
{
    lzo_uint key;
    lzo_uint cnt, node;
    lzo_uint len;
    ...
    else
    {
        ...
        if (swd_search2(s))
#endif
            if (s->look >= 3) swd_search(s, node, cnt);
        if (s->m_len > len) s->m_off = swd_pos2off(s, s->m_pos);
