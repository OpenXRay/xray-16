...
{
    if (rbits) *rbits = RBITS;
    if (clevel) *clevel = CLEVEL;
...
/* init dictionary */
    ...
    do
    {
        ...
        if (LZO_CHECK_MPOS_NON_DET(m_pos, m_off, in, ip, MAX_OFFSET)) goto literal;
        if (m_pos[0] == ip[0] && m_pos[1] == ip[1] && m_pos[2] == ip[2]) goto match;
        DINDEX2(dindex, ip);
        GINDEX(m_pos, m_off, dict, dindex, in);
        if (LZO_CHECK_MPOS_NON_DET(m_pos, m_off, in, ip, MAX_OFFSET)) goto literal;
        if (m_pos[0] == ip[0] && m_pos[1] == ip[1] && m_pos[2] == ip[2]) goto match;
...
#error "MBITS not yet implemented"
#endif
            {
                ...
                *op++ = LZO_BYTE(m_off >> OBITS);
                ...
                if (m_off < OSIZE) lzo_stats->short_match_offset_osize[m_len]++;
                if (m_off < 256) lzo_stats->short_match_offset_256[m_len]++;
                if (m_off < 1024) lzo_stats->short_match_offset_1024[m_len]++;
#endif
...
/* 2c) Insert phrases (beginning with ii+1) into the dictionary. */
    ...
    if (pd(in_end + DVAL_LEN, ii) > 0) op = store_run(op, ii, pd(in_end + DVAL_LEN, ii));
