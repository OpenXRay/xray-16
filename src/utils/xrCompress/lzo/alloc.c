...
lzo_alloc_internal(lzo_uint nelems, lzo_uint size)
{
    ...
    if (nelems <= 0 || size <= 0 || s < nelems || s < size) return NULL;

#if defined(__palmos__)
    p = (lzo_voidp)MemPtrNew(s);
#elif (LZO_UINT_MAX <= SIZE_T_MAX)
    if (s < SIZE_T_MAX) p = (lzo_voidp)malloc((size_t)s);
#elif defined(HAVE_HALLOC) && defined(__DMC__)
    if (size < SIZE_T_MAX) p = (lzo_voidp)_halloc(nelems, (size_t)size);
#elif defined(HAVE_HALLOC)
    if (size < SIZE_T_MAX) p = (lzo_voidp)halloc(nelems, (size_t)size);
#else
    if (s < SIZE_T_MAX) p = (lzo_voidp)malloc((size_t)s);
#endif
