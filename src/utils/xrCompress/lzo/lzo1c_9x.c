    ...
    lzo_progress_callback_t cb, lzo_uint max_chain)
{
    ...
    /* sanity check */
    if (!lzo_assert(LZO1C_999_MEM_COMPRESS >= lzo_sizeof(lzo_swd_t))) return LZO_E_ERROR;
    ...
    while (c->look > 0)
    {
        ...
        /* try a lazy match */
        if (m_len == 0) lazy_match_min_gain = -1;
        if (lazy_match_min_gain >= 0 && c->look > m_len) {
            ...
            if (m_len <= M2_MAX_LEN && m_off <= M2_MAX_OFFSET && c->m_off > M2_MAX_OFFSET) lazy_match_min_gain += 1;
    ...
    /* store final run */
    if (lit > 0) op = STORE_RUN(op, ii, lit);
