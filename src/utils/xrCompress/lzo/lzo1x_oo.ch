...
LZO_PUBLIC(int)
DO_OPTIMIZE(lzo_byte* in, lzo_uint in_len, lzo_byte* out, lzo_uintp out_len, lzo_voidp wrkmem)
{
    lzo_byte* op;
    lzo_byte* ip;
    lzo_uint t;
        ...
        *op++ = *m_pos++;
        ...
        while (TEST_IP && TEST_OP)
        {
            if (t < 16) /* a M1 match */
            {
                ...
                else if (nl == 0 && ip[0] < 16 && ip[0] != 0 && (lit + 2 + ip[0] < 16))
                {
                    t = *ip++;
                    /* remove short run */
                    *litp &= ~3;
                    ...
                    litp += 2;
                    if (lit > 0) lzo_memmove(litp + 1, litp, lit);
            ...
            match:
                if (t >= 64) /* a M2 match */
                {
                    ...
                    t = (t >> 4) - 3;
#endif
                    if (litp == NULL) goto copy_m;

                    nl = ip[-2] & 3;
                    /* test if in beetween two long literal runs */
                    if (t == 1 && lit > 3 && nl == 0 && ip[0] < 16 && ip[0] != 0 && (lit + 3 + ip[0] < 16)) {
                        assert(*litp == lit - 3);
                        t = *ip++;
                ...
                else
                {
                    ...
                    /* test if a literal run follows */
                    else if (t == 1 && lit <= 3 && nl == 0 && ip[0] < 16 && ip[0] != 0 && (lit + 3 + ip[0] < 16))
                    {
                        assert(litp == ip - 3 - lit - 2);
                        assert((lzo_uint)(*litp & 3) == lit);
                        t = *ip++;
