...
#include LZO_CODE_MATCH_INCLUDE_FILE
    ...
    ************************************************************************/
...
#if defined(LZO_COLLECT_STATS)
    {
        ...
        {
            p = dict[i];
            if (BOUNDS_CHECKING_OFF_IN_EXPR(p == NULL || p < in || p > in_end)) lzo_stats->unused_dict_entries++;
        }
        lzo_stats->unused_dict_entries_percent = 100.0 * lzo_stats->unused_dict_entries / D_SIZE;
    }
#endif
