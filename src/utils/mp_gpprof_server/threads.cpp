...
mutex::mutex()
{
    if (pthread_mutex_init(&m_mutex, NULL) != 0) throw std::runtime_error("failed to initialize mutex");
}
mutex::~mutex()
{
...
condition::condition()
{
    if (pthread_cond_init(&m_cond, NULL) != 0) throw std::runtime_error("failed to initialize condition variable");
}
