...
PTW32_INTERLOCKED_LONG WINAPI ptw32_InterlockedCompareExchange(
    PTW32_INTERLOCKED_LPLONG location, PTW32_INTERLOCKED_LONG value, PTW32_INTERLOCKED_LONG comparand)
{
...
#pragma disable_message(200)
     ...
     */
    if (ptw32_smp_system) /* *INDENT-OFF* */
...
#define HAVE_INLINABLE_INTERLOCKED_XCHG
    ...
    {
        __asm__ __volatile__("xchgl          %2,%1" : "=r"(result) : "m"(*location), "0"(value));
    }
    else
    {
        /*
