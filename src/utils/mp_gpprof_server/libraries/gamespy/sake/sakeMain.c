...
SAKEStartupResult SAKE_CALL sakeStartup(SAKE* sakePtr)
{
    ...
    if (__GSIACResult != GSIACAvailable) return SAKEStartupResult_NOT_AVAILABLE;

    // check that the core is initialized
    if (gsCoreIsShutdown()) return SAKEStartupResult_CORE_SHUTDOWN;

    // allocate the sake object
    sake = (SAKE)gsimalloc(sizeof(SAKEInternal));
    if (sake == NULL) return SAKEStartupResult_OUT_OF_MEMORY;
...
SAKEField* SAKE_CALL sakeGetFieldByName(const char* name, SAKEField* fields, int numFields)
{
    ...
    {
        if (strcmp(fields[i].mName, name) == 0) return &fields[i];
    }
...
gsi_bool SAKE_CALL sakeGetFileDownloadURL(SAKE sake, int fileId, gsi_char url[SAKE_MAX_URL_LENGTH])
{
    ...
    if (!sake || !url || !sake->mIsGameAuthenticated || !sake->mIsProfileAuthenticated) return gsi_false;
...
{
    if (resultCode >= SAKEFileResult_UNKNOWN_ERROR) return SAKEFileResult_UNKNOWN_ERROR;
...
gsi_bool SAKE_CALL sakeGetFileResultFromHeaders(const char* headers, SAKEFileResult* result)
{
    ...
    if (gsi_is_false(foundResultCode)) return gsi_false;
