    ...
    GHTTPResult httpResult, GSXmlStreamWriter requestData, GSXmlStreamReader responseData, void* userData)
{
    ...
    if (!request || !request->mSake || !request->mInfo) return;
            ...
            if (result == SAKERequestResult_SUCCESS) {
                if (request->mInfo->mSakeOutputSize != 0) {
                    ...
                    if (request->mOutput) {
                        ...
                        if (result == SAKERequestResult_SUCCESS) output = request->mOutput;
                    }
                    else
                    {
    ...
    // free data
    if (request->mInfo->mFreeDataFunc) request->mInfo->mFreeDataFunc(request);
...
static SAKEStartRequestResult SAKE_CALL sakeiSetupRequest(SAKERequest request)
{
    ...
    if (result != SAKEStartRequestResult_SUCCESS) return result;
    ...
    if (request->mSoapRequest == NULL) return SAKEStartRequestResult_OUT_OF_MEMORY;
    ...
    if (result != SAKEStartRequestResult_SUCCESS) {
        ...
        if (info->mFreeDataFunc) info->mFreeDataFunc(request);
        return result;
    }
