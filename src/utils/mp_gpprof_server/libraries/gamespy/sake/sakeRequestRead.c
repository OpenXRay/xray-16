...
static SAKEStartRequestResult SAKE_CALL sakeiValidateRequestFieldNames(char** fields, int numFields)
{
    ...
    {
        if (!fields[i] || !fields[i][0]) return SAKEStartRequestResult_BAD_FIELD_NAME;
    }
...

{
    ...
    if (gsi_is_false(gsXmlMoveToChild(request->mSoapResponse, "values"))) return SAKERequestResult_MALFORMED_RESPONSE;
    ...
    *numRecords = gsXmlCountChildren(request->mSoapResponse, "ArrayOfRecordValue");
    ...
    if (!outputRecords) return SAKERequestResult_OUT_OF_MEMORY;
    ...
    *outputRecordsPtr = outputRecords;
    ...
    for (recordIndex = 0; recordIndex < *numRecords; recordIndex++)
    {
        ...
        if (!outputRecord) return SAKERequestResult_OUT_OF_MEMORY;
        ...
        for (fieldIndex = 0; fieldIndex < numFields; fieldIndex++)
        {
            ...
            else if (gsXmlMoveToChild(request->mSoapResponse, "unicodeStringValue"))
            {
                ...
                if (!valueUnicode) return SAKERequestResult_OUT_OF_MEMORY;
            ...
            else if (gsXmlMoveToChild(request->mSoapResponse, "binaryDataValue"))
            {
                ...
                if (len > 0) {
                    ...
                    if (!value) return SAKERequestResult_OUT_OF_MEMORY;
...
static void SAKE_CALL sakeiFreeOutputRecord(int numFields, SAKEField* record)
{
    ...
    for (i = 0; i < numFields; i++)
    {
        ...
        if (record[i].mType == SAKEFieldType_UNICODE_STRING) gsifree(record[i].mValue.mUnicodeString);
    }
    gsifree(record);
}
...
static void SAKE_CALL sakeiFreeOutputRecords(int numFields, int numRecords, SAKEField** records)
{
    ...
    for (i = 0; i < numRecords; i++)
    {
        ...
        for (j = 0; j < numFields; j++)
        {
            ...
            if (records[i][j].mType == SAKEFieldType_UNICODE_STRING) gsifree(records[i][j].mValue.mUnicodeString);
        }
...
static SAKEStartRequestResult SAKE_CALL sakeiSearchForRecordsValidateInput(SAKERequest request)
{
    ...
    // check the tableid
    if (!input->mTableId) return SAKEStartRequestResult_BAD_TABLEID;

    // check the num fields
    if (input->mNumFields <= 0) return SAKEStartRequestResult_BAD_NUM_FIELDS;

    // check for NULL field names
    if (!input->mFieldNames) return SAKEStartRequestResult_BAD_FIELDS;

    // check the offset
    if (input->mOffset < 0) return SAKEStartRequestResult_BAD_OFFSET;

    // check the max
    if (input->mMaxRecords <= 0) return SAKEStartRequestResult_BAD_MAX;
...
static void sakeiSearchForRecordsFreeData(SAKERequest request)
{
    ...
    if (output) sakeiFreeOutputRecords(input->mNumFields, output->mNumRecords, output->mRecords);
}
...
static SAKEStartRequestResult SAKE_CALL sakeiGetSpecificRecordsValidateInput(SAKERequest request)
{
    ...
    if (input->mNumRecordIds <= 0) return SAKEStartRequestResult_BAD_NUM_RECORDIDS;

    // check the recordids
    if (!input->mRecordIds) return SAKEStartRequestResult_BAD_RECORDIDS;
...
static void sakeiGetRandomRecordFreeData(SAKERequest request)
{
    ...
    if (output) sakeiFreeOutputRecord(input->mNumFields, output->mRecord);
}
