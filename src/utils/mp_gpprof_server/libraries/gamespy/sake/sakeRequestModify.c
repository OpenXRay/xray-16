...
static SAKEStartRequestResult SAKE_CALL sakeiValidateRequestFields(SAKEField* fields, int numFields)
{
    ...
    {
        if (!fields[i].mName || !fields[i].mName[0]) return SAKEStartRequestResult_BAD_FIELD_NAME;
        if ((fields[i].mType >= SAKEFieldType_NUM_FIELD_TYPES)) return SAKEStartRequestResult_BAD_FIELD_TYPE;
        if (fields[i].mType == SAKEFieldType_ASCII_STRING) {
            if (!fields[i].mValue.mAsciiString) return SAKEStartRequestResult_BAD_FIELD_VALUE;
        }
        if (fields[i].mType == SAKEFieldType_UNICODE_STRING) {
            if (!fields[i].mValue.mUnicodeString) return SAKEStartRequestResult_BAD_FIELD_VALUE;
        }
        if (fields[i].mType == SAKEFieldType_BINARY_DATA) {
            if (fields[i].mValue.mBinaryData.mLength < 0) return SAKEStartRequestResult_BAD_FIELD_VALUE;
...
static SAKEStartRequestResult SAKE_CALL sakeiCreateRecordValidateInput(SAKERequest request)
{
    ...
    if (!input->mFields && (input->mNumFields > 0)) return SAKEStartRequestResult_BAD_FIELDS;
...
static SAKEStartRequestResult SAKE_CALL sakeiUpdateRecordValidateInput(SAKERequest request)
{
    ...
    // check for NULL fields
    if (!input->mFields) return SAKEStartRequestResult_BAD_FIELDS;
