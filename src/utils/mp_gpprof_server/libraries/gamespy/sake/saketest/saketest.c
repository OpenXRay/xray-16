...
static GSIACResult CheckServices(void)
{
    ...
    switch (aResult)
    {
    case GSIACAvailable: printf("Online Services are available\r\n"); break;
...
#define RESULT(x)                                                                                                      \
    case x: resultString = _T(#x); break;
    switch (arg->result)
    {
        RESULT(GP_NO_ERROR)
    ...
    default: resultString = _T("Unknown result!\n");
    }

#define ERRORCODE(x)                                                                                                   \
    case x: errorCodeString = _T(#x); break;
    ...
    default: errorCodeString = _T("Unknown error code!\n");
    }
...
static const char* FieldTypeToString(SAKEField* field)
{
    ...
    if (type == SAKEFieldType_BYTE) return "byte";
    if (type == SAKEFieldType_SHORT) return "short";
    if (type == SAKEFieldType_INT) return "int";
    if (type == SAKEFieldType_FLOAT) return "float";
    if (type == SAKEFieldType_ASCII_STRING) return "asciiString";
    if (type == SAKEFieldType_UNICODE_STRING) return "unicodeString";
    if (type == SAKEFieldType_BOOLEAN) return "boolean";
    if (type == SAKEFieldType_DATE_AND_TIME) return "dateAndTime";
...
static const char* FieldValueToString(SAKEField* field)
{
    ...
    else if (type == SAKEFieldType_UNICODE_STRING)
    {
        ...
        if (UnicodeStringLen(value->mUnicodeString) > 20) value->mUnicodeString[20] = 0;
...
static const char* RequestResultToString(SAKERequestResult requestResult)
{
    ...
    {
    case SAKERequestResult_SUCCESS: return "SUCCESS";
    case SAKERequestResult_CONNECTION_TIMEOUT: return "CONNECTION_TIMEOUT";
    case SAKERequestResult_CONNECTION_ERROR: return "CONNECTION_ERROR";
    case SAKERequestResult_MALFORMED_RESPONSE: return "MALFORMED_RESPONSE";
    case SAKERequestResult_OUT_OF_MEMORY: return "OUT_OF_MEMORY";
    case SAKERequestResult_DATABASE_UNAVAILABLE: return "DATABASE_UNAVAILABLE";
    case SAKERequestResult_LOGIN_TICKET_INVALID: return "LOGIN_TICKET_INVALID";
    case SAKERequestResult_LOGIN_TICKET_EXPIRED: return "LOGIN_TICKET_EXPIRED";
    case SAKERequestResult_TABLE_NOT_FOUND: return "TABLE_NOT_FOUND";
    case SAKERequestResult_RECORD_NOT_FOUND: return "RECORD_NOT_FOUND";
    case SAKERequestResult_FIELD_NOT_FOUND: return "FIELD_NOT_FOUND";
    case SAKERequestResult_FIELD_TYPE_INVALID: return "FIELD_TYPE_INVALID";
    case SAKERequestResult_NO_PERMISSION: return "NO_PERMISSION";
    case SAKERequestResult_RECORD_LIMIT_REACHED: return "RECORD_LIMIT_REACHED";
    case SAKERequestResult_ALREADY_RATED: return "ALREADY_RATED";
    case SAKERequestResult_NOT_RATEABLE: return "NOT_RATEABLE";
    case SAKERequestResult_NOT_OWNED: return "NOT_OWNED";
    case SAKERequestResult_FILTER_INVALID: return "FILTER_INVALID";
    case SAKERequestResult_SORT_INVALID: return "SORT_INVALID";
    case SAKERequestResult_UNKNOWN_ERROR: return "UNKNOWN_ERROR";
    default: break;
    }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
static void CreateRecordCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "CreateRecord") == gsi_false) return;
...
static void UpdateRecordCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    NumOperations--;
    ...
    if (HandleRequestResult(result, "UpdateRecord") == gsi_false) return;
}

static void DeleteRecordCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    NumOperations--;
    ...
    if (HandleRequestResult(result, "DeleteRecord") == gsi_false) return;
}

static void SearchForRecordsCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "SearchForRecords") == gsi_false) return;

    DisplayReadResults(output->mRecords, output->mNumRecords, input->mNumFields);
}

static void GetMyRecordsCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "GetMyRecords") == gsi_false) return;

    DisplayReadResults(output->mRecords, output->mNumRecords, input->mNumFields);
}

static void GetSpecificRecordsCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "GetSpecificRecords") == gsi_false) return;
...
static void GetRandomRecordCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "GetRandomRecord") == gsi_false) return;
...
static void RateRecordCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "RateRecord") == gsi_false) return;

    printf("NumRatings: %d\n", output->mNumRatings);
    printf("AverageRating: %0.3f\n", output->mAverageRating);
}

static void GetRecordLimitCallback(
    SAKE sake, SAKERequest request, SAKERequestResult result, void* inputData, void* outputData, void* userData)
{
    ...
    if (HandleRequestResult(result, "GetRecordLimit") == gsi_false) return;
...
static GHTTPBool UploadCompletedCallback(
...
static GHTTPBool DownloadCompletedCallback(
    GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
    SAKEFileResult fileResult;
...
static void postCallback(
    GHTTPRequest request, int bytesPosted, int totalBytes, int objectsPosted, int totalObjects, void* param)
{
    printf("==============================\n");
...
static void RunTests(SAKE sake)
{
...
#if 1
    {
        ...
        static SAKERequest request;
        static char* fieldNames[] = {"recordid", "ownerid", "MyByte", "MyShort", "MyInt", "MyFloat", "MyAsciiString",
            "MyUnicodeString", "MyBoolean", "MyDateAndTime", "MyBinaryData", "MyFileID", "num_ratings",
            "average_rating"};
        SAKEStartRequestResult startRequestResult;
...
static void LoginAndAuthenticate(SAKE sake)
{
    ...
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_RECV_BUDDY_REQUEST, (GPCallback)RecvBuddyRequest, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_RECV_BUDDY_STATUS, (GPCallback)RecvBuddyStatus, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_RECV_BUDDY_MESSAGE, (GPCallback)RecvBuddyMessage, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_RECV_GAME_INVITE, (GPCallback)RecvGameInvite, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_TRANSFER_CALLBACK, (GPCallback)TransferCallback, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(gpSetCallback(pconn, GP_RECV_BUDDY_AUTH, (GPCallback)RecvBuddyAuth, NULL), "gpSetCallback failed");
    CHECK_GP_RESULT(
        gpSetCallback(pconn, GP_RECV_BUDDY_REVOKE, (GPCallback)RecvBuddyRevoke, NULL), "gpSetCallback failed");
...
// enable Win32 C Runtime debugging
    ...
    if (CheckServices() != GSIACAvailable) return 0;
