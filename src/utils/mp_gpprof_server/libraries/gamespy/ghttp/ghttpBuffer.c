...
GHTTPBool ghiInitBuffer(struct GHIConnection* connection, GHIBuffer* buffer, int initialSize, int sizeIncrement)
{
    ...
    if (!buffer) return GHTTPFalse;
    if (initialSize <= 0) return GHTTPFalse;
    if (sizeIncrement <= 0) return GHTTPFalse;
...
GHTTPBool ghiInitFixedBuffer(struct GHIConnection* connection, GHIBuffer* buffer, char* userBuffer, int size)
{
    ...
    if (!connection) return GHTTPFalse;
    if (!buffer) return GHTTPFalse;
    if (!userBuffer) return GHTTPFalse;
    if (size <= 0) return GHTTPFalse;
...
void ghiFreeBuffer(GHIBuffer* buffer)
{
    ...
    // Cleanup the struct.
    //////////////////////
    if (!buffer->dontFree) gsifree(buffer->data);
...
GHTTPBool ghiAppendDataToBuffer(GHIBuffer* buffer, const char* data, int dataLen)
{
    ...
    //////////////
    if (!buffer) return GHTTPFalse;
    if (!data) return GHTTPFalse;
    if (dataLen < 0) return GHTTPFalse;
    if (buffer->readOnly) return GHTTPFalse;

    // Get the string length if needed.
    ///////////////////////////////////
    if (dataLen == 0) dataLen = (int)strlen(data);
...
GHTTPBool ghiEncryptDataToBuffer(GHIBuffer* buffer, const char* data, int dataLen)
{
    ...
    ///////////////////////////////////
    if (dataLen == 0) dataLen = (int)strlen(data);
    if (dataLen == 0) return GHTTPTrue; // no data and strlen == 0
    ...
    do
    {
        ...
        if (result == GHIEncryptionResult_BufferTooSmall) {
            if (ghiResizeBuffer(buffer, buffer->sizeIncrement) == GHTTPFalse) return GHTTPFalse;
            bufSpace = buffer->size - buffer->len;
        }
...
{
    if (!ghiAppendDataToBuffer(buffer, name, 0)) return GHTTPFalse;
    if (!ghiAppendDataToBuffer(buffer, ": ", 2)) return GHTTPFalse;
    if (!ghiAppendDataToBuffer(buffer, value, 0)) return GHTTPFalse;
    if (!ghiAppendDataToBuffer(buffer, CRLF, 2)) return GHTTPFalse;
...
void ghiResetBuffer(GHIBuffer* buffer)
{
    ...
    // Start with an empty string.
    //////////////////////////////
    if (!buffer->readOnly) *buffer->data = '\0';
}
...
GHTTPBool ghiSendBufferedData(struct GHIConnection* connection)
{
    ...
    do
    {
        ...
        if (gsiSocketIsError(rcode)) return GHTTPFalse;
    ...
    )
{
    // Verify parameters
    assert(bufferIn != NULL);
    if (bytesToCopy == 0) return GHTTPTrue;

    // Make sure the bufferIn isn't too small
    if (bufferIn->len < bytesToCopy) return GHTTPFalse;
