...
CGhttpmfcDlg::CGhttpmfcDlg(CWnd* pParent /*=NULL*/) : CDialog(CGhttpmfcDlg::IDD, pParent)
{
    char buffer[512];
    int rcode;
    FILE* fp;
    ...
    GHTTPRequest request, GHTTPResult result, char* buffer, GHTTPByteCount bufferLen, void* param)
{
    ...
    static char* resultStrings[] = {"GHTTPSuccess", "GHTTPOutOfMemory", "GHTTPBufferOverflow", "GHTTPParseURLFailed",
        "GHTTPHostLookupFailed", "GHTTPSocketFailed", "GHTTPConnectFailed", "GHTTPBadResponse", "GHTTPRequestRejected",
        "GHTTPUnauthorized", "GHTTPForbidden", "GHTTPFileNotFound", "GHTTPServerError", "GHTTPFileWriteFailed",
        "GHTTPFileReadFailed", "GHTTPFileIncomplete", "GHTTPFileToBig", "GHTTPEncryptionError",
...
void ProgressCallback(GHTTPRequest request, GHTTPState state, const char* buffer, GHTTPByteCount bufferLen,
    GHTTPByteCount bytesReceived, GHTTPByteCount totalSize, void* param)
{
    ...
    if (state == GHTTPReceivingFile) {
        ...
        if ((dlg->m_type == 0) || (dlg->m_type == 4)) dlg->m_file = buffer;
    }
...
void CGhttpmfcDlg::OnStart()
{
    ...
    if (m_type == 0) {
        m_request = ghttpGetEx(m_url, m_headers, NULL, 0, NULL, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
            ProgressCallback, CompletedCallback, this);
    }
    else if (m_type == 1)
    {
        m_request = ghttpSaveEx(m_url, m_saveAs, m_headers, NULL, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
            ProgressCallback, CompletedCallback, this);
    }
    else if (m_type == 2)
    {
        m_request = ghttpStreamEx(m_url, m_headers, NULL, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
            ProgressCallback, CompletedCallback, this);
    }
    else if (m_type == 3)
    {
        m_request = ghttpHeadEx(
            m_url, m_headers, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking, ProgressCallback, CompletedCallback, this);
    }
    else if (m_type == 4)
    {
        GHTTPPost post;
        ...
        if (m_postFile) {
            ...
            if (!m_memFile) m_memFile = (char*)malloc(memFileSize);
...
#if 1
        m_request = ghttpGetEx(m_url, m_headers, NULL, 0, post, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking,
            ProgressCallback, CompletedCallback, this);
#else
        m_request = ghttpPostEx(m_url, m_headers, post, (GHTTPBool)m_throttle, (GHTTPBool)m_blocking, ProgressCallback,
...
{
    if (m_request >= 0) ghttpCancelRequest(m_request);
}
...
void CGhttpmfcDlg::OnTimer(UINT nIDEvent)
{
    if (nIDEvent == 50) {
        ...
        if (m_request >= 0) {
            ...
            // added new states for asynch DNS lookup - set all to HOST_LOOKUP for radio buttons
            if (m_state == GHTTPSocketInit || m_state == GHTTPHostLookup || m_state == GHTTPLookupPending) m_state = 0;
            UpdateData(FALSE);
        }
    }
...
void CGhttpmfcDlg::OnThrottle()
{
    ...
    if (m_request >= 0) ghttpSetThrottle(m_request, (GHTTPBool)m_throttle);
}

void CGhttpmfcDlg::OnThink()
{
    if (m_request >= 0) ghttpThink();
}
    ...
    if (SUCCEEDED(result)) {
        ...
        if (SUCCEEDED(result) && data) {
            ...
            if (SUCCEEDED(result) && (len > 0)) {
                ...
                // Apparently, ports are optional - I would presume that you should revert to the default port when it
                // is missing
                //
