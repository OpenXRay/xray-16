...
void ghiCallCompletedCallback(GHIConnection* connection)
{
    ...
    /////////////////////////
    if (!connection->completedCallback) return;
    ...
    // Check for gsifree.
    //////////////////
    if (buffer && !freeBuffer) connection->getFileBuffer.dontFree = GHTTPTrue;
}
...
void ghiCallProgressCallback(GHIConnection* connection, const char* buffer, GHTTPByteCount bufferLen)
{
    ...
    // Check for no callback.
    /////////////////////////
    if (!connection->progressCallback) return;
...
void ghiCallPostCallback(GHIConnection* connection)
{
    ...
    // Check for no callback.
    /////////////////////////
    if (!connection->postingState.callback) return;
