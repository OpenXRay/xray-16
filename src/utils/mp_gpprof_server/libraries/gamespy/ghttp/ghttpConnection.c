...
static int ghiFindFreeSlot(void)
{
    ...
    {
        if (!ghiConnections[i]->inUse) return i;
    }
...
GHTTPBool ghiFreeConnection(GHIConnection* connection)
{
    ...
    if (!connection->inUse) return GHTTPFalse;
    if (connection->request < 0) return GHTTPFalse;
    if (connection->request >= ghiConnectionsLen) return GHTTPFalse;
...
#ifndef NOFILE
    if (connection->saveFile) fclose(connection->saveFile);
#endif
    ...
    if (connection->postingState.states) ghiPostCleanupState(connection);
...
GHIConnection* ghiRequestToConnection(GHTTPRequest request)
{
    ...
    ////////////////////////
    if (!connection->inUse) connection = NULL;
...
void ghiEnumConnections(GHTTPBool (*callback)(GHIConnection*))
{
    ...
    ////////////////////////////
    if (ghiNumConnections <= 0) return;

    ghiLock();
    for (i = 0; i < ghiConnectionsLen; i++)
        if (ghiConnections[i]->inUse) callback(ghiConnections[i]);
    ghiUnlock();
}
