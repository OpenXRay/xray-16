...
GHTTPBool ghttpSetRequestEncryptionEngine(GHTTPRequest request, GHTTPEncryptionEngine engine)
{
    ...
    if (!connection) return GHTTPFalse;
    ...
    if (connection->encryptor.mEngine == engine) return GHTTPTrue;
    ...
    if ((engine == GHTTPEncryptionEngine_None) && (strncmp(connection->URL, "https://", 8) == 0)) return GHTTPFalse;
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    if (matrixSslOpen() < 0) return GHIEncryptionResult_Error;

    if (matrixSslReadKeys(&keys, NULL, NULL, NULL, NULL) < 0) return GHIEncryptionResult_Error;

    if (matrixSslNewSession((ssl_t**)&theEncryptor->mInterface, keys, id, 0) < 0) return GHIEncryptionResult_Error;
...
GHIEncryptionResult ghiEncryptorSslInitFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    if (ecodeResult != 0) return GHIEncryptionResult_Error;                                        // error!
    ...
    const char* theEncryptedBuffer, int* theEncryptedLength, char* theDecryptedBuffer, int* theDecryptedLength)
{
    ...
    while (decryptMore != GHTTPFalse && ((inBuf.end - inBuf.start) > 0))
    {
        ...
        case SSL_SUCCESS:
            ...
            if (matrixSslHandshakeIsComplete(theEncryptor->mInterface)) theEncryptor->mSessionEstablished = GHTTPTrue;
            break;
        ...
        case SSL_ALERT:
            ...
            if (alertdescription == SSL_ALERT_CLOSE_NOTIFY) decryptMore = GHTTPFalse;
            break;
...
GHIEncryptionResult ghiEncryptorSslStartFunc(struct GHIConnection* connection, struct GHIEncryptor* theEncryptor)
{
    ...
    if (result != SSL_ENONE) {
        ...
        if (result == SSL_EWANT_READ || result == SSL_EWANT_WRITE) return GHIEncryptionResult_Success;
...
static void ghiEncryptorWriteNBOLength(unsigned char* buf, int value, int size)
{
    ...
    if (size > sizeof(NBO)) return;  // can't write more than 4 bytes!
...
static GHTTPBool ghiEncryptorReadNBOLength(GHIBuffer* data, int* value, int size)
{
    ...
    if (size > sizeof(*value)) return GHTTPFalse;
    if (GHTTPFalse == ghiReadDataFromBufferFixed(data, ((char*)value) + (sizeof(int) - size), size)) return GHTTPFalse;
...
static GHTTPBool ghiEncryptorParseASN1Sequence(GHIBuffer* data, int* lenOut)
{
    ...
    if (GHTTPFalse == ghiReadDataFromBufferFixed(data, &tempChar, 1)) return GHTTPFalse;
    ...
    if ((tempChar & 0x80) == 0x80) {
        ...
        tempChar ^= 0x80;
        if (GHTTPFalse == ghiEncryptorReadNBOLength(data, &tempInt, tempChar)) return GHTTPFalse;
        if (tempInt > (data->len - data->pos)) return GHTTPFalse;
    ...
    {
        if ((int)tempChar > (data->len - data->pos)) return GHTTPFalse;
    ...
    const char* thePlainTextBuffer, int thePlainTextLength, char* theEncryptedBuffer, int* theEncryptedLength)
{
    if (theEncryptor != NULL) {
        ...
        if (sslInterface == NULL || theEncryptor->mSessionEstablished == GHTTPFalse) {
            ...
            if (thePlainTextLength > *theEncryptedLength) return GHIEncryptionResult_BufferTooSmall;
    ...
    const char* theEncryptedBuffer, int* theEncryptedLength, char* theDecryptedBuffer, int* theDecryptedLength)
{
    ...
    sslInterface = (gsSSL*)theEncryptor->mInterface;
    if (sslInterface == NULL) return GHIEncryptionResult_Error;
    ...
    while (readPos < *theEncryptedLength)
    {
        ...
        if ((*theEncryptedLength - readPos) < sizeof(gsSSLRecordHeaderMsg)) break;
        ...
        case GS_SSL_CONTENT_HANDSHAKE:
        {
            ...
            if (result != GHIEncryptionResult_Success) return result;  // error!
...
#define CHECK(a)                                                                                                       \
    {                                                                                                                  \
        if (GHTTPFalse == a) return GHIEncryptionResult_Error;                                                         \
    }
    ...
    struct GHIConnection* connection, struct GHIEncryptor* encryptor, GHIBuffer* data)
{
    ...
    while (data->pos < data->len)
    {
        ...
        if (messageType == GS_SSL_HANDSHAKE_SERVERHELLO) {
            ...
            if (sslInterface->sessionLen != 0) return GHIEncryptionResult_Error;  // abort connection

            CHECK(ghiEncryptorReadNBOLength(data, &msgDataLen, 3));

            // check reported size against the actual bytes remaining
            if (msgDataLen > (data->len - data->pos)) return GHIEncryptionResult_Error;  // abort connection
            ...
            CHECK(ghiReadDataFromBufferFixed(data, &tempChar, 1));
            if (tempChar != 0x00) return GHIEncryptionResult_Error;
        ...
        else if (messageType == GS_SSL_HANDSHAKE_CERTIFICATE)
        {
            ...
            if (msgLength != certListLen + 3) return GHIEncryptionResult_Error;
            ...
            if (certListLen > (data->len - data->pos)) return GHIEncryptionResult_Error;
            ...
            while (data->pos < certListEndPos)
            {
                ...
                if (certLength > (data->len - data->pos)) return GHIEncryptionResult_Error;  // certificate too big

                // 0xFFFF is max message size in SSL v3.0, we don't currently support
                // split messages
                if (certLength > 0xFFFF) return GHIEncryptionResult_Error;
                ...
                if (certCount == 1) {
                    ...
                    if ((data->data[data->pos] & 0x80) == 0x80)  // ASN1 variable length field
                    {
                        ...
                        if (lensize > 4) return GHIEncryptionResult_Error;
                        temp = 0;
                        while (lensize-- > 0)
            ...
            if (data->pos != certListEndPos) return GHIEncryptionResult_Error;  // bytes hanging off the end!

            // todo: verify certificate chain
            //       first certificate is the server's, the rest likely belong to CA
            if (GHTTPFalse == ghiCertificateChainIsValid(sslInterface)) return GHIEncryptionResult_Error;
