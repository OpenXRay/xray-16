...
static void ghiPostDataFree(void* elem)
{
    ...
    // Free the name.
    /////////////////
    if (data->type != GHIXmlData) gsifree(data->name);
...
GHTTPBool ghiPostAddString(GHTTPPost _post, const char* name, const char* string)
{
    ...
    if (rcode != len) {
        ...
        //////////////////////////////////////////
        for (i = 0; string[i]; i++)
            if (!strchr(GHI_LEGAL_URLENCODED_CHARS, string[i]) && (string[i] != ' ')) count++;
    ...
    GHTTPPost _post, const char* name, const char* filename, const char* reportFilename, const char* contentType)
{
    ...
    if (post->hasSoap == GHTTPTrue) post->useDIME = GHTTPTrue;
...
GHTTPBool ghiPostAddXml(GHTTPPost _post, GSXmlStreamWriter xml)
{
    ...
    // if we have both soap and a file we MUST use DIME
    if (post->hasFiles == GHTTPTrue) post->useDIME = GHTTPTrue;
...
static int ghiPostGetHasFilesContentLength(struct GHIConnection* connection)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        else if (data->type == GHIFileDisk)
        {
            ...
            if (!post->useDIME) total += (int)strlen(data->data.fileDisk.reportFilename);
        ...
        else if (data->type == GHIFileMemory)
        {
            ...
            if (!post->useDIME) total += (int)strlen(data->data.fileMemory.reportFilename);
...
static int ghiPostGetContentLength(struct GHIConnection* connection)
{
    ...
    if (post->hasFiles) return ghiPostGetHasFilesContentLength(connection);
...
#endif
        if (!state->state.fileDisk.file) return GHTTPFalse;

        // Get the file length.
        ///////////////////////
        if (fseek(state->state.fileDisk.file, 0, SEEK_END) != 0) return GHTTPFalse;
        state->state.fileDisk.len = ftell(state->state.fileDisk.file);
        if (state->state.fileDisk.len == EOF) return GHTTPFalse;
...
static void ghiPostStateCleanup(GHIPostState* state)
{
    ...
    {
        if (state->state.fileDisk.file) fclose(state->state.fileDisk.file);
...
GHTTPBool ghiPostInitState(struct GHIConnection* connection)
{
    ...
    if (!connection->post) return GHTTPFalse;
    ...
    if (!connection->postingState.states) return GHTTPFalse;
...
static GHIPostingResult ghiPostStringStateDoPosting(GHIPostState* state, GHIConnection* connection)
{
    ...
    ///////////////////////////
    if (state->data->data.string.len == 0) return GHIPostingDone;
    ...
    if (connection->encryptor.mEngine == GHTTPEncryptionEngine_None) {
        ...
        if (!ghiSendBufferedData(connection)) return GHIPostingError;

        if (connection->sendBuffer.pos == connection->sendBuffer.len) ghiResetBuffer(&connection->sendBuffer);
        return GHIPostingDone;
    }
    else
    {
...
static GHIPostingResult ghiPostXmlStateDoPosting(GHIPostState* state, GHIConnection* connection)
{
    ...
    else
    {
        ...
        if (result == GHITrySendError) return GHIPostingError;
    ...
    do
    {
        ...
        if (state->pos == state->state.fileDisk.len) {
            ...
            if (connection->post->useDIME) {
                ...
                if (padlen != 4 && padlen > 0) {
                    if (GHITrySendError == ghiTrySendThenBuffer(connection, pad, padlen)) return GHIPostingError;
                }
            }
            return GHIPostingDone;
        }
...
static GHIPostingResult ghiPostFileMemoryStateDoPosting(GHIPostState* state, GHIConnection* connection)
{
    ...
    /////////////////////////
    if (state->data->data.fileMemory.len == 0) return GHIPostingDone;
    ...
    if (connection->encryptor.mEngine == GHTTPEncryptionEngine_None) {
        ...
        do
        {
            ...
            if (gsiSocketIsError(rcode)) return GHIPostingError;
    ...
    GHIPostState* state, GHIConnection* connection, GHTTPBool first, GHTTPBool last)
{
    ...
    if (state->pos == -1) {
        ...
        else
        {
            ...
            else if (state->data->type == GHIXmlData)
            {
                if (connection->post->useDIME) {
                    ...
                    if (first) header.mVersionAndFlags |= GHI_DIMEFLAG_FIRSTRECORD;
                    if (last) header.mVersionAndFlags |= GHI_DIMEFLAG_LASTRECORD;
            ...
            connection->encryptor.mEncryptOnBuffer == GHTTPTrue)
        {
            ...
            if (GHTTPFalse == ghiEncryptDataToBuffer(&connection->sendBuffer, buffer, len)) return GHIPostingError;
            if (GHTTPFalse == ghiSendBufferedData(connection)) return GHIPostingError;

            // any data remaining?
            if (connection->sendBuffer.pos < connection->sendBuffer.len) return GHIPostingPosting;
        ...
        else
        {
            ...
            /////////////////////////////////////////
            if (result == GHITrySendBuffered) return GHIPostingPosting;
    ...
    // Post based on type.
    //////////////////////
    if (state->data->type == GHIString) return ghiPostStringStateDoPosting(state, connection);

    if (state->data->type == GHIXmlData) return ghiPostXmlStateDoPosting(state, connection);

    if (state->data->type == GHIFileDisk) return ghiPostFileDiskStateDoPosting(state, connection);
...
GHIPostingResult ghiPostDoPosting(struct GHIConnection* connection)
{
    ...
    if (connection->sendBuffer.pos < connection->sendBuffer.len) {
        ...
        //////////////////////////////////////////////////////////
        if (connection->postingState.waitPostContinue) return GHIPostingWaitForContinue;

        // Was that all that's left?
        ////////////////////////////
        if (connection->postingState.index == len) return GHIPostingDone;
    }
    ...
    if (connection->postingState.waitPostContinue) {
        if (connection->post->hasFiles || connection->post->hasSoap) {
            ...
            {
                if (connection->postingState.waitPostContinue == GHTTPTrue) return GHIPostingWaitForContinue;
                // else
                //	fall through
            }
        }
    ...
    while (postingState->index < len)
    {
        ...
        ///////////////////////////
        if (postingResult == GHIPostingPosting) return GHIPostingPosting;
    ...
    if (connection->post->hasFiles && !connection->post->useDIME) {
        ...
        if (trySendResult == GHITrySendError) return GHIPostingError;
    }
