...
GHIRecvResult ghiDoReceive(GHIConnection* connection, char buffer[], int* bufferLen)
{
    ...
    if (connection->throttle) {
        ...
        ///////////////////////////
        now = current_time();
        if (now < (connection->lastThrottleRecv + ghiThrottleTimeDelay)) return GHINoData;
...
int ghiDoSend(struct GHIConnection* connection, const char* buffer, int len)
{
    ...
    if (gsiSocketIsError(rcode)) {
        ...
        if ((error == WSAEWOULDBLOCK) || (error == WSAEINPROGRESS) || (error == WSAETIMEDOUT)) return 0;
...
GHITrySendResult ghiTrySendThenBuffer(GHIConnection* connection, const char* buffer, int len)
{
    ...
    {
        if (!ghiEncryptDataToBuffer(&connection->sendBuffer, buffer + rcode, len - rcode)) return GHITrySendError;
    ...
    if (connection->sendBuffer.pos >= connection->sendBuffer.len) {
        ...
        if (gsiSocketIsError(rcode)) return GHITrySendError;

        // Was it all sent?
        ///////////////////
        if (rcode == len) return GHITrySendSent;
    }

    // Buffer whatever wasn't sent.
    ///////////////////////////////
    if (!ghiAppendDataToBuffer(&connection->sendBuffer, buffer + rcode, len - rcode)) return GHITrySendError;
    return GHITrySendBuffered;
}
    ...
    )
{
    ...
    *proxyAddress = goastrdup(server);
    if (!*proxyAddress) return GHTTPFalse;
...
GHTTPBool ghiSetProxy(const char* server)
{
    ...
    if (server && *server) return ghiParseProxyServer(server, &ghiProxyAddress, &ghiProxyPort);
...
GHTTPBool ghiSetRequestProxy(GHTTPRequest request, const char* server)
{
    ...
    if (connection == NULL) return GHTTPFalse;
