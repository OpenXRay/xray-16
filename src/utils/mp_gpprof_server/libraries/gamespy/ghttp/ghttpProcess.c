...
static GHTTPBool ghiParseURL(GHIConnection* connection)
{
    ...
    if (!connection->URL) return GHTTPFalse;
    ...
    if (!connection->serverAddress) return GHTTPFalse;
    URL[nIndex] = tempChar;
    URL += nIndex;
    ...
    if (*URL == ':') {
        ...
        if (!connection->serverPort) return GHTTPFalse;
        do
        {
            URL++;
        } while (*URL && (*URL != '/'));
    ...
    while ((str = strchr(connection->requestPath, ' ')) != NULL)
        *str = '+';
    if (!connection->requestPath) return GHTTPFalse;
...
void ghiDoConnecting(GHIConnection* connection)
{
    ...
    if (connection->socket == INVALID_SOCKET) {
        ...
        /////////////////////////////////////////////
        if (connection->throttle) SetReceiveBufferSize(connection->socket, ghiThrottleBufferSize);
...
void ghiDoSecuringSession(GHIConnection* connection)
{
    ...
    else
    {
        ...
        if (connection->sendBuffer.pos < connection->sendBuffer.len) {
            ...
            /////////////////////////////////
            if (connection->sendBuffer.pos < connection->sendBuffer.len) return;
        ...
        if (result == GHIRecvData) {
            ...
            if (!ghiAppendDataToBuffer(&connection->decodeBuffer, buffer, bufferLen)) return;
...
void ghiDoSendingRequest(GHIConnection* connection)
{
    ...
    if (!connection->sendBuffer.len) {
        ...
        // Add user-headers.
        ////////////////////
        if (connection->sendHeaders) ghiAppendDataToBuffer(writeBuffer, connection->sendHeaders, 0);
    ...
    // Send what we can.
    ////////////////////
    if (!ghiSendBufferedData(connection)) return;
...
void ghiDoPosting(GHIConnection* connection)
{
    ...
    /////////////////////////////////////////
    if (oldBytesPosted != connection->postingState.bytesPosted) ghiCallPostCallback(connection);
...
void ghiDoReceivingStatus(GHIConnection* connection)
{
    ...
    ///////////////////////////
    if (result == GHIError) return;
    if (result == GHINoData) return;
    ...
    if (result == GHIRecvData) {
        ...
        else
        {
            ...
            ///////////////////////////////////////
            if (!ghiAppendDataToBuffer(&connection->recvBuffer, buffer, bufferLen)) return;
        }
    }
    ...
    if (endOfStatus) {
        ...
        *endOfStatus = '\0';
        ...
        // Parse the status line.
        /////////////////////////
        if (!ghiParseStatus(connection)) return;
...
static GHTTPBool ghiDeliverIncomingFileData(GHIConnection* connection, char* data, int len)
{
    ...
    if (connection->type == GHIGET) {
        ...
        //////////////////////////
        if (!ghiAppendDataToBuffer(&connection->getFileBuffer, data, len)) return GHTTPFalse;
    ...
    if (connection->chunkedTransfer) {
        ...
        while (len > 0)
        {
            ...
            else if (connection->chunkReadingState == CRChunk)
            {
                ...
                // Deliver the bytes.
                /////////////////////
                if (!ghiDeliverIncomingFileData(connection, data, numBytes)) return GHTTPFalse;
                ...
                ///////////////////////////
                if (connection->chunkBytesLeft == 0) connection->chunkReadingState = CRCRLF;
            }
            ...
            else if (connection->chunkReadingState == CRCRLF)
            {
                ...
                ///////////////////////////////
                if (!endOfFooter) return GHTTPTrue;
...
void ghiDoReceivingHeaders(GHIConnection* connection)
{
    ...
    if (endOfHeaders) {
        ...
        ////////////////////////////
        if (GHTTPTrue == hasHeaders) endOfHeaders += 2;
        *endOfHeaders = '\0';
...
////////////////////////////////////////////////
        ...
        /////////////////////////////////////////////
        if (fileLength > 0) ghiProcessIncomingFileData(connection, fileStart, fileLength);
    ...
    while (!connection->completed && (running_time < connection->maxRecvTime))
    {
            ...
            connection->encryptor.mEncryptOnBuffer == GHTTPTrue)
        {
            ...
            if (decryptedLen) {
                ...
                if (!ghiProcessIncomingFileData(connection, decryptedData, decryptedLen)) return;
            }
        }
        else
        {
            // Process the data.
            ////////////////////
            if (!ghiProcessIncomingFileData(connection, buffer, bufferLen)) return;
        }
