...
static GHTTPBool ghiProcessConnection(GHIConnection* connection)
{
    ...
    /////////////////////////////////////////////////////////////////
    if (connection->processing) return GHTTPFalse;
    ...
    if (connection->state == GHTTPSocketInit) ghiDoSocketInit(connection);
    if (connection->state == GHTTPHostLookup) ghiDoHostLookup(connection);
    if (connection->state == GHTTPLookupPending) ghiDoLookupPending(connection);
    if (connection->state == GHTTPConnecting) ghiDoConnecting(connection);
    if (connection->state == GHTTPSecuringSession) ghiDoSecuringSession(connection);
    if (connection->state == GHTTPSendingRequest) ghiDoSendingRequest(connection);
    if (connection->state == GHTTPPosting) ghiDoPosting(connection);
    if (connection->state == GHTTPWaiting) ghiDoWaiting(connection);
    if (connection->state == GHTTPReceivingStatus) ghiDoReceivingStatus(connection);
    if (connection->state == GHTTPReceivingHeaders) ghiDoReceivingHeaders(connection);
    if (connection->state == GHTTPReceivingFile) ghiDoReceivingFile(connection);

    // Check for a redirect.
    ////////////////////////
    if (connection->redirectURL) ghiRedirectConnection(connection);
    ...
    ghttpCompletedCallback completedCallback, void* param)
{
    ...
    // Check args.
    //////////////
    if (!URL || !URL[0]) return GHTTPInvalidURL;
    if (bufferSize < 0) return GHTTPInvalidBufferSize;
    if (buffer && !bufferSize) return GHTTPInvalidBufferSize;

    // Startup if it hasn't been done.
    //////////////////////////////////
    if (!ghiReferenceCount) ghttpStartup();

    // Get a new connection object.
    ///////////////////////////////
    connection = ghiNewConnection();
    if (!connection) return GHTTPInsufficientMemory;
    ...
    ghttpCompletedCallback completedCallback, void* param)
{
    ...
    assert(URL != NULL);
    UCS2ToAsciiString(URL, (char*)URL_A);
    if (headers != NULL) UCS2ToAsciiString(headers, headers_A);
    ...
    ghttpCompletedCallback completedCallback, void* param)
{
    ...
    //////////////
    if (!URL || !URL[0]) return GHTTPInvalidURL;
    if (!filename || !filename[0]) return GHTTPInvalidFileName;
    ...
    GHTTPBool blocking, ghttpProgressCallback progressCallback, ghttpCompletedCallback completedCallback, void* param)
{
    char URL_A[1024] = {'\0'};
    char headers_A[1024] = {'\0'};
    if (URL != NULL) UCS2ToAsciiString(URL, URL_A);
    if (headers != NULL) UCS2ToAsciiString(headers, headers_A);
    ...
    ghttpProgressCallback progressCallback, ghttpCompletedCallback completedCallback, void* param)
{
    ...
    //////////////
    if (!URL || !URL[0]) return GHTTPInvalidURL;
    if (!post) return GHTTPInvalidPost;
...
GHTTPState ghttpGetState(GHTTPRequest request)
{
    ...
    //////////////////////////////////////////////
    connection = ghiRequestToConnection(request);
    if (!connection) return (GHTTPState)0;
...
const char* ghttpGetResponseStatus(GHTTPRequest request, int* statusCode)
{
    ...
    /////////////////////////////////////////
    if (connection->state <= GHTTPReceivingStatus) return NULL;

    // Set the status code.
    ///////////////////////
    if (statusCode) *statusCode = connection->statusCode;
...
const char* ghttpGetHeaders(GHTTPRequest request)
{
    ...
    //////////////////////////////////////////
    if (connection->state < GHTTPReceivingHeaders) return NULL;

    // Verify we have headers.
    //////////////////////////
    if (connection->headerStringIndex >= connection->recvBuffer.len) return NULL;
...
GHTTPBool ghttpReuseSocket(GHTTPRequest request, SOCKET socket)
{
    ...
    connection = ghiRequestToConnection(request);
    if (!connection) return GHTTPFalse;
    if (connection->state != GHTTPSocketInit) return GHTTPFalse;
    if (connection->socket != INVALID_SOCKET) return GHTTPFalse;
...
GHTTPBool ghttpPostAddStringA(GHTTPPost post, const char* name, const char* string)
{
    ...
    if (!post) return GHTTPFalse;
    if (!name || !name[0]) return GHTTPFalse;
    if (!string) string = "";
...
GHTTPBool ghttpPostAddStringW(GHTTPPost post, const unsigned short* name, const unsigned short* string)
{
    ...
    if (string != NULL) UCS2ToAsciiString(string, string_A);
    ...
    GHTTPPost post, const char* name, const char* filename, const char* reportFilename, const char* contentType)
{
    ...
    if (!post) return GHTTPFalse;
    if (!name || !name[0]) return GHTTPFalse;
    if (!filename || !filename[0]) return GHTTPFalse;
    if (!reportFilename || !reportFilename[0]) reportFilename = filename;
    ...
    const char* reportFilename, const char* contentType)
{
    ...
    assert(bufferLen >= 0);
#ifdef _DEBUG
    if (bufferLen > 0) assert(buffer);
#endif
    assert(reportFilename && reportFilename[0]);

    if (!post) return GHTTPFalse;
    if (!name || !name[0]) return GHTTPFalse;
    if (bufferLen < 0) return GHTTPFalse;
    if (!bufferLen && !buffer) return GHTTPFalse;
    if (!contentType) contentType = "application/octet-stream";
