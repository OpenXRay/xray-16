...
{
    if (array->elemfreefn != NULL) array->elemfreefn(ArrayNth(array, n));
}
...
void* ArrayNth(DArray array, int n)
{
    ...
    if (!((n >= 0) && (n < array->count))) return NULL;
...
{
    GS_ASSERT(array);
    if (array) ArrayInsertAt(array, newElem, array->count);
}
...
static void* mylsearch(const void* key, void* base, int count, int size, ArrayCompareFn comparator)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (comparator(key, (char*)base + size * i) == 0) return (char*)base + size * i;
    }
    return NULL;
}
