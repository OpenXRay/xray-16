...
CGt2testDlg::CGt2testDlg(CWnd* pParent /*=NULL*/) : CDialog(CGt2testDlg::IDD, pParent)
{
    m_delayedSends = NULL;
...
static void DelayCheck(DArray delayedMessages, unsigned long now, BOOL send)
{
    ...
    for (i = (num - 1); i >= 0; i--)
    {
        ...
        if ((now - message->startTime) > message->delayTime) {
            if (send)
                gt2FilteredSend(
                    message->connection, message->filterID, message->message, message->len, message->reliable);
            else
                gt2FilteredReceive(
                    message->connection, message->filterID, message->message, message->len, message->reliable);
...
static void RemoveDelayedMessages(DArray delayedMessages, GT2Connection connection)
{
    ...
    for (i = (num - 1); i >= 0; i--)
    {
        ...
        if (message->connection == connection) ArrayDeleteAt(delayedMessages, i);
    }
}
...
void CGt2testDlg::OnTimer(UINT nIDEvent)
{
    ...
    if (nIDEvent == 100) {
        ...
        if (newState != m_connectionState) {
            ...
            if (newState != -1) ((CButton*)GetDlgItem(IDC_CONNECTING + newState))->SetCheck(BST_CHECKED);
...
static const char* ResultToString(GT2Result result)
{
    switch (result)
    {
    case GT2Success: return "GT2Success";
    case GT2OutOfMemory: return "GT2OutOfMemory";
    case GT2Rejected: return "GT2Rejected";
    case GT2NetworkError: return "GT2NetworkError";
    case GT2AddressError: return "GT2AddressError";
    case GT2DuplicateAddress: return "GT2DuplicateAddress";
    case GT2TimedOut: return "GT2TimedOut";
    case GT2NegotiationError: return "GT2NegotiationError";
    }
...
int CGt2testDlg::GetConnectionIndex(GT2Connection connection)
{
    ...
    {
        if (((ConnectionInfo*)m_connections.GetItemDataPtr(nIndex))->connection == connection) return nIndex;
    }
...
{
    if (!m_connections.IsWindowEnabled()) return NULL;
...
static void SendDumpCallback(GT2Socket socket, GT2Connection connection, unsigned int ip, unsigned short port,
...
static void ReceiveDumpCallback(GT2Socket socket, GT2Connection connection, unsigned int ip, unsigned short port,
    GT2Bool reset, const GT2Byte* message, int len)
{
    CString str;
    if (reset)
...
void CGt2testDlg::OnCreateSocket()
{
    ...
    GT2Result result =
        gt2CreateSocket(&m_socket, m_localAddress, atoi(m_outBufferSize), atoi(m_inBufferSize), SocketErrorCallback);
...
static void ConnectAttemptCallback(GT2Socket socket, GT2Connection connection, unsigned int ip, unsigned short port,
    int latency, GT2Byte* message, int len)
{
    ...
    if (accept) {
        ...
        else
        {
            ...
            if (dlg->m_acceptMode == 2) dlg->MessageBox("Connection already closed");
        }
    }
    else
    {
...
void CGt2testDlg::OnConnect()
{
    ...
    GT2Result result = gt2Connect(m_socket, &connection, m_remoteAddress, (GT2Byte*)(LPCSTR)m_connectMessage, -1,
...
static void SendFilterCallbackROT13(
...
static void ReceiveFilterCallbackROT13(
    GT2Connection connection, int filterID, GT2Byte* message, int len, GT2Bool reliable)
{
    // ROT13 the string
...
static void SendFilterCallbackDrop(
    GT2Connection connection, int filterID, const GT2Byte* message, int len, GT2Bool reliable)
{
    // if it's unreliable, check if it should be dropped
    if (!reliable && CheckDrop(atoi(dlg->m_sendDropValue))) return;

    // pass it back to GT2
    gt2FilteredSend(connection, filterID, message, len, reliable);
}

static void ReceiveFilterCallbackDrop(
    GT2Connection connection, int filterID, GT2Byte* message, int len, GT2Bool reliable)
{
    // if it's unreliable, check if it should be dropped
    if (!reliable && CheckDrop(atoi(dlg->m_receiveDropValue))) return;

    // pass it back to GT2
    gt2FilteredReceive(connection, filterID, message, len, reliable);
}

static void AddDelayedMessage(DArray array, int delayValue, GT2Connection connection, int filterID,
...
static void SendFilterCallbackDelay(
    GT2Connection connection, int filterID, const GT2Byte* message, int len, GT2Bool reliable)
{
...
static void ReceiveFilterCallbackDelay(
    GT2Connection connection, int filterID, GT2Byte* message, int len, GT2Bool reliable)
{
    // add it to the delayed receives list
    AddDelayedMessage(
        dlg->m_delayedReceives, atoi(dlg->m_receiveDelayValue), connection, filterID, message, len, reliable);
}
