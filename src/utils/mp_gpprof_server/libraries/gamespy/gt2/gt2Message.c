...
static GT2Bool gti2ConnectionError(GT2Connection connection, GT2Result result, GT2CloseReason reason)
{
    ...
    if (connection->state < GTI2Connected) {
        ...
        if (connection->initiated) {
            ...
            // call the callback
            if (!gti2ConnectedCallback(connection, result, NULL, 0)) return GT2False;
        }
        else
        {
            // are we waiting for accept/reject?
            if (connection->state == GTI2AwaitingAcceptReject) connection->freeAtAcceptReject = GT2True;
    ...
    else if (connection->state != GTI2Closed)
    {
        ...
        // call the callback
        if (!gti2ClosedCallback(connection, reason)) return GT2False;
    }
...
static GT2Bool gti2ConnectionMemoryError(GT2Connection connection)
{
    ...
    if (!gti2SendClosed(connection)) return GT2False;
...
static GT2Bool gti2HandleESN(GT2Connection connection, unsigned short ESN)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (gti2SNDiff(message->serialNumber, ESN) >= 0) break;
    }

    // check for not removing any
    if (i == 0) return GT2True;
...
{
    // check the state
    if ((connection->state != GTI2Connected) && (connection->state != GTI2Closing)) return GT2True;

    // do we need to filter it?
    if (ArrayLength(connection->receiveFilters)) {
        if (!gti2ReceiveFilterCallback(connection, 0, message, len, GT2False)) return GT2False;
        return GT2True;
    }

    // we received data, call the callback
    if (!gti2ReceivedCallback(connection, message, len, GT2False)) return GT2False;
...
static GT2Bool gti2HandleAppReliable(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    else
    {
        // do we need to filter it?
        if (ArrayLength(connection->receiveFilters)) {
            if (!gti2ReceiveFilterCallback(connection, 0, message, len, GT2True)) return GT2False;
            return GT2True;
        }

        // we received data, call the callback
        if (!gti2ReceivedCallback(connection, message, len, GT2True)) return GT2False;
    }
...
static GT2Bool gti2HandleClientChallenge(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    if (!gti2SendServerChallenge(connection, response, challenge)) return GT2False;
...
static GT2Bool gti2HandleAccept(GT2Connection connection)
{
    ...
    // call the callback
    if (!gti2ConnectedCallback(connection, GT2Success, NULL, 0)) return GT2False;
...
static GT2Bool gti2HandleReject(GT2Connection connection, GT2Byte* message, int len)
{
    ...
    // call the callback
    if (!gti2ConnectedCallback(connection, GT2Rejected, message, len)) return GT2False;
...
static GT2Bool gti2HandleClose(GT2Connection connection)
{
    ...
    if (!gti2ConnectionError(connection, GT2Rejected, localClose ? GT2LocalClose : GT2RemoteClose)) return GT2False;
...
static GT2Bool gti2DeliverReliableMessage(GT2Connection connection, GTI2MessageType type, GT2Byte* message, int len)
{
    ...
    if (type == GTI2MsgAppReliable) {
        if (!gti2HandleAppReliable(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgClientChallenge)
    {
        if (!gti2HandleClientChallenge(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgServerChallenge)
    {
        if (!gti2HandleServerChallenge(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgClientResponse)
    {
        if (!gti2HandleClientResponse(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgAccept)
    {
        if (!gti2HandleAccept(connection)) return GT2False;
    }
    else if (type == GTI2MsgReject)
    {
        if (!gti2HandleReject(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgClose)
    {
        if (!gti2HandleClose(connection)) return GT2False;
    }
    ...
    GT2Connection connection, GTI2MessageType type, unsigned short SN, GT2Byte* message, int len, GT2Bool* overflow)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (gti2SNDiff(bufferedMessage->serialNumber, SN) > 0) break;
    }
    ...
    if (num == 0) {
        ...
        if (!gti2SendNack(connection, connection->expectedSerialNumber, (unsigned short)(SN - 1))) return GT2False;
    }
    else
    {
...
static GT2Bool gti2HandleReliableMessage(GT2Connection connection, GTI2MessageType type, GT2Byte* message, int len)
{
    ...
    // handle the ESN
    if (!gti2HandleESN(connection, ESN)) return GT2False;
    ...
    if (SN == connection->expectedSerialNumber) {
        ...
        // deliver the message
        if (!gti2DeliverReliableMessage(connection, type, message, len)) return GT2False;

        // check if there are any messages in the hold that can now be delivered
        if (!gti2DeliverHoldMessages(connection)) return GT2False;
    ...
    if (!gti2BufferIncomingMessage(connection, type, SN, message, len, &overflow)) return GT2False;

    // check for a buffer overflow
    if (overflow) {
        if (!gti2ConnectionMemoryError(connection)) return GT2False;
    }
...
static GT2Bool gti2HandleNack(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            if (!gti2ResendMessage(connection, messageInfo)) return GT2False;
        }
    }
...
static GT2Bool gti2HandlePong(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    if (!connection->callbacks.ping) return GT2True;

    // is this a pong we're interested in?
    // "time" + ping-sent-time
    if (len != (4 + sizeof(gsi_time))) return GT2True;
    if (memcmp(message, "time", 4) != 0) return GT2True;

    // get the start time
    memcpy(&startTime, message + 4, sizeof(gsi_time));

    // call the callback
    if (!gti2PingCallback(connection, (int)(current_time() - startTime))) return GT2False;
...
static GT2Bool gti2HandleClosed(GT2Connection connection)
{
    ...
    if (connection->state == GTI2Closed) return GT2True;
...
static GT2Bool gti2HandleUnreliableMessage(GT2Connection connection, GTI2MessageType type, GT2Byte* message, int len)
{
    ...
    if (type == GTI2MsgAck) {
        if (!gti2HandleAck(connection, dataStart, dataLen)) return GT2False;
    }
    else if (type == GTI2MsgNack)
    {
        if (!gti2HandleNack(connection, dataStart, dataLen)) return GT2False;
    }
    else if (type == GTI2MsgPing)
    {
        if (!gti2HandlePing(connection, message, len)) return GT2False;
    }
    else if (type == GTI2MsgPong)
    {
        if (!gti2HandlePong(connection, dataStart, dataLen)) return GT2False;
    }
    else if (type == GTI2MsgClosed)
    {
        if (!gti2HandleClosed(connection)) return GT2False;
    }
...
static GT2Bool gti2HandleMessage(GT2Socket socket, GT2Byte* message, int len, unsigned int ip, unsigned short port)
{
    ...
    if (socket->receiveDumpCallback) {
        if (!gti2DumpCallback(socket, connection, ip, port, GT2False, message, len, GT2False)) return GT2False;
    }
    ...
    if (!connection) {
        ...
        if (!gti2UnrecognizedMessageCallback(socket, ip, port, message, len, &handled)) return GT2False;

        // if they handled it, we don't care about it.
        if (handled) return GT2True;
        ...
        if (!magicString || (actualMessage[GTI2_MAGIC_STRING_LEN] != GTI2MsgClientChallenge)) {
            ...
            if (!magicString || (actualMessage[GTI2_MAGIC_STRING_LEN] != GTI2MsgClosed)) {
                if (!gti2SendClosedOnSocket(socket, ip, port)) return GT2False;
            }
            return GT2True;
        }

        // if we're not listening, we just ignore this
        if (!socket->connectAttemptCallback) return GT2True;
    ...
    if (!magicString) {
        ...
        {
            if (!gti2HandleAppUnreliable(connection, message, len)) return GT2False;
        }
    ...
    if (type < GTI2NumReliableMessages) {
        // handle it
        if (!gti2HandleReliableMessage(connection, type, message, len)) return GT2False;
        return GT2True;
    }

    // handle unreliable messages
    if (!gti2HandleUnreliableMessage(connection, type, message, len)) return GT2False;
...
GT2Bool gti2HandleConnectionReset(GT2Socket socket, unsigned int ip, unsigned short port)
{
    ...
    if (socket->receiveDumpCallback) {
        if (!gti2DumpCallback(socket, connection, ip, port, GT2True, NULL, 0, GT2False)) return GT2False;
    }

    // there's no connection, so ignore it
    if (!connection) return GT2True;

    // are we waiting for a response from the server?
    if (connection->state == GTI2AwaitingServerChallenge) {
        // are we still within the timeout time?
        if (!connection->timeout || ((current_time() - connection->startTime) < connection->timeout)) return GT2True;

        // report this as a timeout
        if (!gti2ConnectionError(connection, GT2TimedOut, GT2RemoteClose)) return GT2False;
    }
    else
    {
        // report the error
        if (!gti2ConnectionError(connection, GT2Rejected, GT2RemoteClose)) return GT2False;
    }
...
GT2Bool gti2HandleHostUnreachable(GT2Socket socket, unsigned int ip, unsigned short port, GT2Bool send)
{
    ...
    // let the dump know about this
    if (socket->receiveDumpCallback) {
        if (!gti2DumpCallback(socket, connection, ip, port, GT2True, NULL, 0, send)) return GT2False;
    }
...
static GT2Bool gti2StoreOutgoingReliableMessageInfo(GT2Connection connection, unsigned short SN, int len)
{
    ...
    // make sure the length is one more
    if (ArrayLength(connection->outgoingBufferMessages) != (num + 1)) return GT2False;
...
GT2Bool gti2SendAppReliable(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    // begin the message
    if (!gti2BeginReliableMessage(connection, GTI2MsgAppReliable, totalLen, &overflow)) return GT2False;
    if (overflow) return GT2True;

    // write the message
    gti2BufferWriteData(&connection->outgoingBuffer, message, len);

    // end the message
    if (!gti2EndReliableMessage(connection)) return GT2False;
...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    {
        if (!gti2ConnectionSendData(connection, message, len)) return GT2False;
    ...
    // do the send
    if (!gti2ConnectionSendData(connection, start, totalLen)) return GT2False;
...
GT2Bool gti2SendAck(GT2Connection connection)
{
    ...
    // send it
    if (!gti2ConnectionSendData(connection, (const GT2Byte*)buffer, pos)) return GT2False;
...
GT2Bool gti2SendClosedOnSocket(GT2Socket socket, unsigned int ip, unsigned short port)
{
    ...
    // send it
    if (!gti2SocketSend(socket, ip, port, (const GT2Byte*)buffer, pos)) return GT2False;
...
GT2Bool gti2ResendMessage(GT2Connection connection, GTI2OutgoingBufferMessage* message)
{
    ...
    if (type == GTI2MsgServerChallenge) connection->challengeTime = connection->lastSend;
...
{
    if (reliable) return gti2SendAppReliable(connection, message, len);
...
GT2Bool gti2WasMessageIDConfirmed(const GT2Connection connection, GT2MessageID messageID)
{
    ...
    if (gti2SNDiff(messageID, message->serialNumber) < 0) return GT2True;
