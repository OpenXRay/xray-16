...
static int gtiDecodeSingle(char elemType, char* inBuffer, int inLength, va_list* args)
{
    ...
    case GT_FLOAT: {
        ...
        // no endian convert
        GT_FLOAT_TYPE* v;
        if (inLength < sizeof(GT_FLOAT_TYPE)) return -1;
    ...
    case GT_DOUBLE:
        ...
        {
            GT_DOUBLE_TYPE* v;
            if (inLength < sizeof(GT_DOUBLE_TYPE)) return -1;
...
static int gtiEncodeSingle(char elemType, char* outBuffer, int outLength, va_list* args)
{
    ...
    case GT_FLOAT:  // floats are promoted to double in varargs, need to demote
    {
        ...
        f = (float)temp;
        if (outLength < sizeof(float)) return -1;
    ...
    {
        double v;
        if (outLength < sizeof(double)) return -1;
    ...
    case GT_RAW_PTR:
    {
        ...
        GT_RAW_TYPE data = va_arg(*args, GT_RAW_TYPE);
        len = va_arg(*args, int);
        if (outLength < len + (int)sizeof(len)) return -1;
    ...
    int usetype, GTMessageType msgType, const char* fmtString, char* outBuffer, int outLength, va_list* args)
{
    ...
    if (usetype) {
        ...
        if (outLength < elemSize) return -1;
