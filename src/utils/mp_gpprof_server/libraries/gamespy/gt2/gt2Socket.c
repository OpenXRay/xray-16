...
static int GS_STATIC_CALLBACK gti2ConnectionCompare(const void* elem1, const void* elem2)
{
    ...
    if (connection1->ip != connection2->ip) return (int)(connection1->ip - connection2->ip);
...
GT2Connection gti2SocketFindConnection(GT2Socket socket, unsigned int ip, unsigned short port)
{
    ...
    if (connectionPtrPtr) return *connectionPtrPtr;
    ...
    gt2SocketErrorCallback callback, GTI2ProtocolType type)
{
    ...
    if (!incomingBufferSize) incomingBufferSize = GTI2_DEFAULT_INCOMING_BUFFER_SIZE;
    if (!outgoingBufferSize) outgoingBufferSize = GTI2_DEFAULT_OUTGOING_BUFFER_SIZE;

    // convert the address to an IP and port
    if (!gt2StringToAddress(localAddress, &ip, &port)) return GT2AddressError;

    // allocate the object
    socketTemp = (GT2Socket)gsimalloc(sizeof(GTI2Socket));
    if (!socketTemp) return GT2OutOfMemory;
...
GT2Result gti2NewSocketConnection(GT2Socket socket, GT2Connection* connection, unsigned int ip, unsigned short port)
{
    ...
    if (gti2SocketFindConnection(socket, ip, port)) return GT2DuplicateAddress;

    // allocate a connection object
    connectionPtr = gti2CreateConnectionObject();
    if (!connectionPtr) goto out_of_memory;
    ...
    if (!gti2AllocateBuffer(&connectionPtr->incomingBuffer, socket->incomingBufferSize)) goto out_of_memory;
    if (!gti2AllocateBuffer(&connectionPtr->outgoingBuffer, socket->outgoingBufferSize)) goto out_of_memory;

    // allocate the message arrays
    connectionPtr->incomingBufferMessages = ArrayNew(sizeof(GTI2IncomingBufferMessage), 64, NULL);
    if (!connectionPtr->incomingBufferMessages) goto out_of_memory;
    connectionPtr->outgoingBufferMessages = ArrayNew(sizeof(GTI2OutgoingBufferMessage), 64, NULL);
    if (!connectionPtr->outgoingBufferMessages) goto out_of_memory;

    // allocate the filter arrays
    connectionPtr->sendFilters = ArrayNew(sizeof(gt2SendFilterCallback), 2, NULL);
    if (!connectionPtr->sendFilters) goto out_of_memory;
    connectionPtr->receiveFilters = ArrayNew(sizeof(gt2ReceiveFilterCallback), 2, NULL);
    if (!connectionPtr->receiveFilters) goto out_of_memory;
    ...
    *connection = gti2SocketFindConnection(socket, ip, port);
    if (!*connection) goto out_of_memory;
...
out_of_memory:
    ...
    if (connectionPtr) {
        ...
        if (connectionPtr->incomingBufferMessages) ArrayFree(connectionPtr->incomingBufferMessages);
        if (connectionPtr->outgoingBufferMessages) ArrayFree(connectionPtr->outgoingBufferMessages);
        if (connectionPtr->sendFilters) ArrayFree(connectionPtr->sendFilters);
        if (connectionPtr->receiveFilters) ArrayFree(connectionPtr->receiveFilters);
...
void gti2FreeSocketConnection(GT2Connection connection)
{
    ...
    if (connection->freeAtAcceptReject || connection->callbackLevel) return;
...
GT2Bool gti2SocketSend(GT2Socket socket, unsigned int ip, unsigned short port, const GT2Byte* message, int len)
{
    ...
    if ((rand() % 100) < GTI2_DROP_SEND_RATE) return GT2True;
#endif
...
// do the send
    ...
    if (gsiSocketIsError(rcode)) {
        ...
        if (rcode == WSAECONNRESET) {
            // handle the reset
            if (!gti2HandleConnectionReset(socket, ip, port)) return GT2False;
        }
#ifndef SN_SYSTEMS
        else if (rcode == WSAEHOSTUNREACH)
        {
            if (!gti2HandleHostUnreachable(socket, ip, port, GT2True)) return GT2False;
        }
#endif
...
static int gti2SocketConnectionsThinkMap(void* elem, void* clientData)
{
    ...
    if (connection->state != GTI2Closed) {
        // think
        if (!gti2ConnectionThink(connection, now)) return 0;
    }
...
GT2Bool gti2SocketConnectionsThink(GT2Socket socket)
{
    ...
    if (TableMapSafe2(socket->connections, gti2SocketConnectionsThinkMap, &now)) return GT2False;
...
void gti2SocketError(GT2Socket socket)
{
    ...
    if (!gti2SocketErrorCallback(socket)) return;
