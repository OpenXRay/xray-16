...
#if !defined(_NITRO)
    {GT2True, FILL_IN, 2047, GT2True}, {GT2True, FILL_IN, (3 * 1024) - 1, GT2True},
...
static void EncodeTest()
{
    ...
    printf("\nretlen=%d,retlen2=%d\n", retlen, retlen2);
    retlen = gtEncode(0, bitstr, outBuff, 1024, bits[0], bits[1], bits[2], bits[3], bits[4], bits[5], bits[6], bits[7],
        bits[8], bits[9]);
    retlen2 = gtDecode(bitstr, outBuff, retlen, bits2, bits2 + 1, bits2 + 2, bits2 + 3, bits2 + 4, bits2 + 5, bits2 + 6,
        bits2 + 7, bits2 + 8, bits2 + 9);
...
int test_main(int argc, char** argv)
{
    ...
    if (argc > 1) {
        ...
        if (strncmp(argv[argc], "-l", 2) == 0) {
            ...
            if (argv[argc]) strcpy(localAddress, argv[argc++]);
        }
        // Check for connect.
        else if (strncmp(argv[argc], "-c", 2) == 0)
        {
            ...
            if (argv[argc]) strcpy(remoteAddress, argv[argc++]);
            if (argv[argc]) strcpy(localAddress, argv[argc++]);
        }
    }
    ...
    do
    {
        ...
        result = gt2Connect(socket, &Connection, remoteAddress, (GT2Byte*)initialMessage.data, initialMessage.len,
