    ...
    else
    {
        ...
        else
        {
            ...
            for (check = (colon + 1); *check; check++)
                if (!isdigit(*check)) return GT2False;

            // Get the port.
            temp = atoi(colon + 1);
            if ((temp < 0) || (temp > 0xFFFF)) return GT2False;
        ...
        if (host) {
            ...
            if (srcIP == INADDR_NONE) {
                ...
                if (hostent == NULL) return GT2False;
...
unsigned int gt2XnAddrToIP(XNADDR theAddr, XNKID theKeyId)
{
    ...
    IN_ADDR anAddr;
    if (XNetXnAddrToInAddr(&theAddr, &theKeyId, &anAddr) == 0) return anAddr.s_addr;
    return 0;
}
...
GT2Bool gt2IPToXnAddr(int ip, XNADDR* theAddr, XNKID* theKeyId)
{
    IN_ADDR anAddr;
    anAddr.s_addr = ip;
    if (XNetInAddrToXnAddr(anAddr, theAddr, theKeyId) == 0) return GT2True;
    return GT2False;
}
...
static const char* gti2HandleHostInfo(HOSTENT* host, char*** aliases, unsigned int*** ips)
{
    if (!host || (host->h_addrtype != AF_INET) || (host->h_length != 4)) return NULL;

    if (aliases) *aliases = host->h_aliases;
    if (ips) *ips = (unsigned int**)host->h_addr_list;
...
const char* gt2StringToHostInfo(const char* string, char*** aliases, unsigned int*** ips)
{
    ...
    ip = inet_addr(string);
    if (ip != INADDR_NONE) return gt2IPToHostInfo(ip, aliases, ips);
...
char** gt2IPToAliases(unsigned int ip)
{
    ...
    if (!gt2IPToHostInfo(ip, &aliases, NULL)) return NULL;
...
char** gt2StringToAliases(const char* string)
{
    ...
    if (!gt2StringToHostInfo(string, &aliases, NULL)) return NULL;
...
unsigned int** gt2IPToIPs(unsigned int ip)
{
    ...
    if (!gt2IPToHostInfo(ip, NULL, &ips)) return NULL;
...
unsigned int** gt2StringToIPs(const char* string)
{
    ...
    if (!gt2StringToHostInfo(string, NULL, &ips)) return NULL;
