...
static int GS_STATIC_CALLBACK gti2SendFiltersCompare(const void* elem1, const void* elem2)
{
    ...
    if (*callback1 == *callback2) return 0;
...
GT2Bool gti2AddSendFilter(GT2Connection connection, gt2SendFilterCallback callback)
{
    ...
    if (!connection->sendFilters) return GT2False;
...
GT2Bool gti2AddReceiveFilter(GT2Connection connection, gt2ReceiveFilterCallback callback)
{
    ...
    if (!connection->receiveFilters) return GT2False;
...
void gti2RemoveSendFilter(GT2Connection connection, gt2SendFilterCallback callback)
{
    ...
    if (!connection->sendFilters) return;
...
void gti2RemoveReceiveFilter(GT2Connection connection, gt2ReceiveFilterCallback callback)
{
    ...
    // Check for no filters.
    if (!connection->receiveFilters) return;
...
GT2Bool gti2FilteredSend(GT2Connection connection, int filterID, const GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (connection->state != GTI2Connected) return GT2True;
    ...
    if (filterID < 0) return GT2True;
    if (filterID >= num) return GT2True;

    // Is it the last one?
    if (filterID == (num - 1)) {
        // Do the actual send.
        if (!gti2Send(connection, message, len, reliable)) return GT2False;
    }
    else
    {
        // Filter it.
        if (!gti2SendFilterCallback(connection, ++filterID, message, len, reliable)) return GT2False;
    }
...
GT2Bool gti2FilteredReceive(GT2Connection connection, int filterID, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (filterID == (num - 1)) {
        // call the callback
        if (!gti2ReceivedCallback(connection, message, len, reliable)) return GT2False;
    }
    else
    {
        // Filter it.
        if (!gti2ReceiveFilterCallback(connection, ++filterID, message, len, reliable)) return GT2False;
    }
