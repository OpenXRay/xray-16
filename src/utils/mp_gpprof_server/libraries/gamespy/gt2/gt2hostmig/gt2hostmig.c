...
GT2Bool ParseMessage(const char* message, int len)
{
    ...
    for (i = 0; i < (len - 1); i++)
    {
        if (!message[i]) return GT2False;
    }
    if (message[i]) return GT2False;
    if (message[0] != '\\') return GT2False;

    // Find the key/value starts.
    keyStart = &message[1];
    valueStart = strchr(keyStart, '\\');
    if (!valueStart) return GT2False;
    valueStart++;

    // Copy in the key.
    copyLen = (valueStart - keyStart - 1);
    if (copyLen >= sizeof(messageKey)) return GT2False;
    memcpy(messageKey, keyStart, copyLen);
    messageKey[copyLen] = '\0';

    // Copy in the value.
    if (strlen(valueStart) >= sizeof(messageValue)) return GT2False;
...
void NewBackupHost(int index)
{
    ...
    {
        if (clients[i] && (i != index)) TellBackupHost(i);
    }
}
...
void server_key_callback(int keyid, qr2_buffer_t outbuf, void* userdata)
{
    switch (keyid)
    {
    case HOSTNAME_KEY: qr2_buffer_add(outbuf, QR2_HOSTNAME); break;
    case GAMEVER_KEY: qr2_buffer_add(outbuf, QR2_GAME_VERSION); break;
    case HOSTPORT_KEY: qr2_buffer_add_int(outbuf, PORT); break;
    case NUMPLAYERS_KEY: qr2_buffer_add_int(outbuf, numClients); break;
    case MAXPLAYERS_KEY: qr2_buffer_add_int(outbuf, QR2_MAX_PLAYERS); break;
...
void playerkey_callback(int keyid, int index, qr2_buffer_t outbuf, void* userdata)
{
    ...
    switch (keyid)
    {
    case PLAYER__KEY: qr2_buffer_add(outbuf, GetConnectionName(clients[index])); break;
    ...
    int latency, GT2Byte* message, int len)
{
    ...
    // We have a spot.
    if (!gt2Accept(connection, &connectionCallbacks)) return;
...
void ReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    else
    {
        ...
        if (strcasecmp(messageKey, "echo") == 0) return;
...
GT2Bool StartHosting(void)
{
    ...
    if (rcode != e_qrnoerror) return GT2False;
#endif
...
int main(int argc, char** argv)
{
    ...
    // Host if no args.
    if (argc < 2) {
        if (!StartHosting()) printf("Failed to start hosting\n");
    }
    else
    {
