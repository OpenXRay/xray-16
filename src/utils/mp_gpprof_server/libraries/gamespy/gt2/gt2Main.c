...
void gt2Think(GT2Socket socket)
{
    ...
    if (!gti2ReceiveMessages(socket)) return;

    // let the connections think
    if (!gti2SocketConnectionsThink(socket)) return;
...
GT2Result gt2SendRawUDP(GT2Socket socket, const char* remoteAddress, const GT2Byte* message, int len)
{
    ...
    // get the ip and port
    if (!gt2StringToAddress(remoteAddress, &ip, &port) || !port) return GT2AddressError;

    // check for invalid IP ranges
    // class D (224-239.*, multicast) and class E (240-255.*, experimental)
    if ((ntohl(ip) & GTI2_INVALID_IP_MASK) == GTI2_INVALID_IP_MASK) return GT2AddressError;

    // check if this is for broadcast
    if (!ip) {
        // check if broadcast is enable
        if (!socket->broadcastEnabled) {
            if (!SetSockBroadcast(socket->socket)) return GT2NetworkError;
    ...
    int len, int timeout, GT2ConnectionCallbacks* callbacks, GT2Bool blocking)
{
    ...
    {
        // get the ip and port
        if (!gt2StringToAddress(remoteAddress, &ip, &port) || !ip || !port) return GT2AddressError;
    }
    ...
    if (!blocking) {
        if (connection) *connection = connectionTemp;
        return GT2Success;
    }
    ...
    // is it success?
    if (connectionTemp->state == GTI2Connected) *connection = connectionTemp;
...
GT2Result gt2Send(GT2Connection connection, const GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (connection->state != GTI2Connected) return GT2InvalidConnection;
    ...
    if (reliable && connection->socket->protocolType == GTI2VdpProtocol) {
        ...
        if (vdpDataLength + connection->socket->protocolOffset != len) return GT2InvalidMessage;
    }
    ...
    if (gti2Send(connection, message, len, reliable)) return GT2Success;
...
{
    if (connection->state < GTI2Connected) return GT2Connecting;
    if (connection->state == GTI2Connected) return GT2Connected;
    if (connection->state == GTI2Closing) return GT2Closing;
    return GT2Closed;
}
