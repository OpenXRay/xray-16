...
static HRESULT ReadMMIO(HMMIO hmmioIn, MMCKINFO* pckInRIFF, WAVEFORMATEX** ppwfxInfo)
{
    ...
    *ppwfxInfo = NULL;
    ...
    if (pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM) {
        if (NULL == (*ppwfxInfo = (WAVEFORMATEX*)malloc(sizeof(WAVEFORMATEX)))) return E_FAIL;
...
static GT2Bool LoadSound(int sound, const char* filename, LPDIRECTSOUNDBUFFER restoreBuffer)
{
    ...
    /////////////////////////////////////////
    if (!restoreBuffer) ArrayAppend(SoundList, &soundEffect);
...
GT2Bool InitializeSound(void)
{
    ...
    if (!SoundList) return GT2False;

    // Init direct sound.
    /////////////////////
    result = DirectSoundCreate(NULL, &DirectSound, NULL);
    if (FAILED(result)) return GT2False;
    ...
    {
        assert(soundFiles[i]);
        if (!soundFiles[i]) return GT2False;
...
void PlaySoundEffect(int sound)
{
    ...
    for (i = 0; i < soundEffect->numDuplicates; i++)
    {
        ...
        if (status & DSBSTATUS_BUFFERLOST) {
            ...
            ///////////////////////////////
            if (!LoadSound(sound, soundFiles[sound], soundEffect->SoundBuffers[i])) continue;
        }
    ...
    if (result == DSERR_BUFFERLOST) {
        ...
        // Fill the sound data back in.
        ///////////////////////////////
        if (!LoadSound(sound, soundFiles[sound], soundEffect->SoundBuffers[i])) return;
