...
float* ComputeNewPosition(
    V2f dest, const V2f position, int motion, float rotation, unsigned long diff, int unitsPerSec, GT2Bool clamp)
{
    V2f forward;
    float scale;
    ...
    if (motion == BACKWARD) scale = -scale;
    ...
    // Clamp it.
    ////////////
    if (clamp) ClampV2f(dest, dest, 0, MAP_MAX);
...
unsigned short RotationToUnsignedShort(float rotation)
{
    ...
    if (rotation > 0xFFFF) rotation = 0xFFFF;
