...
static void SendUpdates(void)
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        if (clients[i].used) {
            ...
            rcode =
                gtEncodeNoType(MSG_S_UPDATE_CLIENT_STR, buffer, len, i, PositionToUnsignedShort(client->position[0]),
                    PositionToUnsignedShort(client->position[1]), RotationToUnsignedShort(client->rotation),
                    client->score, client->motion == FORWARD ? 1 : 0, client->motion == BACKWARD ? 1 : 0,
                    client->turning == RIGHT ? 1 : 0, client->turning == LEFT ? 1 : 0, client->dead);
            if (rcode == -1) return;
            buffer += rcode;
            len -= rcode;
        }
    ...
    for (i = 0; i < MAX_OBJECTS; i++)
    {
        if (sObjects[i].used) {
            ...
            rcode = gtEncodeNoType(MSG_S_UPDATE_OBJECT_STR, buffer, len, object->type,
                PositionToUnsignedShort(object->position[0]), PositionToUnsignedShort(object->position[1]),
                RotationToUnsignedShort(object->rotation), (now - object->startTime));
            if (rcode == -1) return;
            buffer += rcode;
            len -= rcode;
        }
...
static void ServerReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (!clients[clientIndex].used) return;
    ...
    else if (type == MSG_C_PRESS)
    {
        ...
        // Is this too soon?
        ////////////////////
        if ((now - client->lastPress) < PRESS_TIME) return;
...
static void ServerClosedCallback(GT2Connection connection, GT2CloseReason reason)
{
    ...
    //////////////////////////////////
    for (i = 0; i < MAX_CLIENTS; i++)
        if (clients[i].used) SendDelClient(i, clientIndex);
    ...
    {
        object = &sObjects[i];
        if (object->used && (object->owner == clientIndex)) object->owner = -1;
    }
}

static void ServerConnectAttemptCallback(GT2Socket socket, GT2Connection connection, unsigned int ip,
    unsigned short port, int latency, GT2Byte* message, int len)
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        if (clients[i].used) {
            ...
            if (i != newClientIndex) SendAddClient(newClientIndex, i);
        }
    }
...
GT2Bool InitializeServer(void)
{
    ...
    if (result != GT2Success) return GT2False;
...
void ServerThink(unsigned long now)
{
    ...
    // Check for an update.
    ///////////////////////
    if (diff < SERVER_THINK_TIME) return;
