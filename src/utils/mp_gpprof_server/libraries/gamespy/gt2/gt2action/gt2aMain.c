...
static void Timer(int value)
{
    ...
    ////////////////////////////
    if (host) ServerThink(now);
    if (!dedicated) ClientThink(now);
    DisplayThink(now);
}
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (strncasecmp(argv[i], "-c", 2) == 0) {
            if (++i < argc) {
                ...
                if (!strchr(serverAddress, ':')) strcat(serverAddress, PORT_STRING);
                host = GT2False;
            }
        }
        // Fullscreen.
        ...
        {
            if (++i < argc) screenWidth = atoi(argv[i]);
        }
        // Height.
        //////////
        else if (strcasecmp(argv[i], "-height") == 0)
        {
            if (++i < argc) screenHeight = atoi(argv[i]);
        }
    }
