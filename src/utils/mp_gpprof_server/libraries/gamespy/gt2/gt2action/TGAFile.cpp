...
TGAFile::LoadTGA(const char* fileName, UINT32* width, UINT32* height)
{
    ...
    *height = 0;
    ...
    if (tgahd.ColorMapType != 0) {
        ...
        if ((tgahd.ImageType != TGA_RAWRGBTYPE) && (tgahd.ImageType != TGA_RAWMONOTYPE) &&
            (tgahd.ImageType != TGA_RAWENCODETYPE))
            mode = mode | MAPPED;
    }

    // Check Run Length Encoding
    if ((tgahd.ImageType == TGA_MAPENCODETYPE) || (tgahd.ImageType == TGA_RAWENCODETYPE) ||
...
//		Return 0 on Success
BOOL TGAFile::TGA_GetMapEntry(BYTE* Red, BYTE* Green, BYTE* Blue, BYTE* Alpha, FILE* fp, UINT32 Depth)
{
    UINT32 j, k, l;
    BYTE i, r, g, b, a = 0;
    long lResult;
...
BOOL TGAFile::TGA_GetPixelValue(
    BYTE* rRed, BYTE* rGrn, BYTE* rBlu, BYTE* rAlp, BYTE** ppTGAData, UINT32 PixelDepth, RGBQUAD* CColMap)
{
    //
...
//		Retruns - TRUE on success
BOOL TGAFile::TGA_GetPixelValue(
    BYTE* rRed, BYTE* rGrn, BYTE* rBlu, BYTE* rAlp, FILE* fp, UINT32 PixelDepth, RGBQUAD* CColMap)
{
    BYTE i, j, k;
    long lResult;
...
extern "C" unsigned char* LoadTGAFile(const char* filename, int* width, int* height)
{
    TGAFile tgaFile;
