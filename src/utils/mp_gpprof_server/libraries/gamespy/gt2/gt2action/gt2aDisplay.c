...
static void DrawChat(void)
{
    ...
    ///////////////////////////////
    str = GetChatBuffer();
    if (str) DrawString(str, x, WINDOW_MAX - 500, White, scale);
}
...
static void DrawPlayer(Player* player, const V2f position, float rotation)
{
    ...
    textPosition[0] = -(width / 2);
		...
		textPosition[1] = 700;
#else
    if ((position[0] + textPosition[0]) < 20) textPosition[0] = (20 - position[0]);
    diff = (MAP_MAX - (position[0] + textPosition[0] + width));
    if (diff < 0) textPosition[0] += diff;
...
static void Predict(V2f newPosition, float* rotation, Player* player)
{
    ...
    if (DrawWithPredictionOption && !player->dead && player->motion) {
        ...
        /////////////////////////////////
        if (!local) *rotation = ComputeNewRotation(player->rotation, player->turning, Diff, PLAYER_TURN_SPEED);
    }
    else
    {
        ...
        //////////////////////////////////////
        if (!local) *rotation = player->rotation;
    }

    // If local, use our rotation.
    //////////////////////////////
    if (local) *rotation = localRotation;
}
    ...
    for (i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (player->used && (i != localIndex)) {
            ...
            if (ViewClippingOption) {
                ...
                if (distance >= FULL_VIEW) continue;
            }
...
static void DrawConnecting(void)
{
    ...
    ///////////////////////////////////////////////////////////
    if (width == -1) width = GetStringWidth(text);
...
{
    if (!DrawBackgroundOption) return;
...
static void DrawGraphs(void)
{
    ...
    if (updateHistoryStart == -1) return;
...
static void DrawRadar(void)
{
    ...
    // Scale points.
    ////////////////
    if (RadarPointScaleOption != 1) glPointSize(RadarPointScaleOption);
    ...
    // Unscale points.
    //////////////////
    if (RadarPointScaleOption != 1) glPointSize(1);
    glPopMatrix();
