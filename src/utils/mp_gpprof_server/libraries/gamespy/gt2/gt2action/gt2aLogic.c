...
static SObject* AddObject(ObjectType type)
{
    ...
    for (i = 0; i < MAX_OBJECTS; i++)
        if (!sObjects[i].used) break;

    // Nothing open?
    ////////////////
    if (i == MAX_OBJECTS) return NULL;
...
static void RocketTouchObject(SObject* rocket, SObject* object)
{
    ...
    ///////////////////////////////////////////////////////////
    if ((object->type == ObjectRocket) && (object->owner == rocket->owner)) return;

    // We pass through explosions.
    //////////////////////////////
    if (object->type == ObjectExplosion) return;
...
static void RocketTouchClient(SObject* rocket, Client* client)
{
    ...
    // Check for self.
    //////////////////
    if (rocket->owner == client->index) return;
...
static void MineThink(SObject* mine)
{
    ...
    else
    {
        ...
        if ((int)((Now - mine->startTime) / 1000) > mine->count) {
            ...
            //////////////////////////////////////////
            if (mine->count >= ((MINE_TIME / 1000) - 3)) SendSound(SOUND_MINE, -1);
        }
    }
}
...
static void MineTouchClient(SObject* mine, Client* client)
{
    ...
    /////////////////////////////
    if ((Now - mine->startTime) < MINE_ARM_TIME) return;
...
static void SpawnAsteroids(void)
{
    ...
    for (i = 0; i < NUM_ASTEROIDS; i++)
    {
        ...
        if (asteroid) InitAsteroid(asteroid);
    }
}
...
static void AsteroidDie(SObject* asteroid)
{
    ...
    //////////////////////////////////////
    if (!NumAsteroids) SpawnAsteroids();
}
...
static void ExplosionTouchClient(SObject* explosion, Client* client)
{
    ...
    //////////////////////////////////
    if ((Now - explosion->startTime) > EXPLOSION_DANGER_TIME) return;
...
static void ClientThink(Client* client)
{
    ...
    if (client->dead) {
        // Time to spawn?
        /////////////////
        if (Now > client->spawnTime) ClientSpawn(client);
    }
    ...
    for (i = 0; i < MAX_OBJECTS; i++)
    {
        ...
        if (object->used) {
            ...
            if (object->remove) {
                ...
                /////////////////////////
                if (object->die) object->die(object);
