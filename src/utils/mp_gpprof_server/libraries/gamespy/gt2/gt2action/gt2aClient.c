...
static void ClientReceivedCallback(GT2Connection connection, GT2Byte* message, int len, GT2Bool reliable)
{
    ...
    if (type == MSG_S_ADDCLIENT) {
        ...
        if ((newPlayerIndex < 0) || (newPlayerIndex >= MAX_PLAYERS)) return;
    ...
    else if (type == MSG_S_DELCLIENT)
    {
        ...
        if ((delPlayerIndex < 0) || (delPlayerIndex >= MAX_PLAYERS)) return;
    ...
    else if (type == MSG_S_UPDATE)
    {
        ...
        else
        {
            ...
            //////////////////////////
            if (updateID < nextServerUpdateID) return;
        ...
        for (i = 0; i < updatedClients; i++)
        {
            // Decode the update.
            /////////////////////
            rcode = gtDecodeNoType(MSG_S_UPDATE_CLIENT_STR, (char*)message, len, &index, &packedPosition[0],
                &packedPosition[1], &packedRotation, &score, &forward, &backward, &right, &left, &dead);
            if (rcode == -1) return;
            message += rcode;
            len -= rcode;
            ...
            else if (left)
            {
                if (player->roll > -1) {
                    ...
                    if (player->roll < -1) player->roll = -1;
                }
            }
            else if (right)
            {
        ...
        for (i = 0; i < updatedObjects; i++)
        {
            // Decode the object.
            /////////////////////
            rcode = gtDecodeNoType(MSG_S_UPDATE_OBJECT_STR, (char*)message, len, &type, &packedPosition[0],
                &packedPosition[1], &packedRotation, &time);
            if (rcode == -1) return;
            message += rcode;
            len -= rcode;
...
void SendUpdate(void)
{
    if (connected) {
        ...
        // Encode the messsage.
        ///////////////////////
        rcode = gtEncode(MSG_C_UPDATE, MSG_C_UPDATE_STR, buffer, sizeof(buffer), nextClientUpdateID++,
            RotationToUnsignedShort(localRotation), localMotion == FORWARD ? 1 : 0, localMotion == BACKWARD ? 1 : 0,
...
GT2Bool InitializeClient(void)
{
    ...
    /////////////////////////////////////////////////////////
    if (!serverAddress[0]) strcpy(serverAddress, "127.0.0.1" PORT_STRING);
