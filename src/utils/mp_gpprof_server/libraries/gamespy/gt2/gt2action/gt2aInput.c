...
static void KeyboardPress(unsigned char key, int x, int y)
{
    ...
    case 't': SetKeyboardChat(); break;

    case 'p': TOGGLE(DrawWithPredictionOption); break;

    case 'c': TOGGLE(ViewClippingOption); break;
    ...
    case 'j': TOGGLE(UseJoystick); break;

    case 'g': TOGGLE(DrawGraphsOption); break;

    case 'r': TOGGLE(DrawBoundingCirclesOption); break;
    }
}
...
static void KeyboardRelease(unsigned char key, int x, int y)
{
    ...
    switch (key)
    {
    case 'b': SendPress("mine"); break;

    case ' ': SendPress("rocket"); break;
    }
}
...
{
    if (chatting) return (const char*)chatBuffer;
...
{
    if (nChatChars) chatBuffer[--nChatChars] = '\0';
}
...
static void ChatSend(void)
{
    ...
    // Send it.
    ///////////
    if (nChatChars > 0) SendChat(chatBuffer);
}
...
static void MouseMotionPassive(int x, int y)
{
    ...
    if ((x <= 0) || (x >= 639)) glutWarpPointer(320, 240);
...
static void MouseButton(int button, int state, int x, int y)
{
    ...
    case GLUT_LEFT_BUTTON:
        if (state == GLUT_DOWN) SendPress("rocket");
        break;
...
static void MainMenuCallback(int item)
{
    switch (item)
    {
    case MainToggleSound: ToggleSound(); break;
    case MainToggleViewClipping: TOGGLE(ViewClippingOption); break;
    }
}
...
static void GraphMenuCallback(int item)
{
    switch (item)
    {
    case GraphToggleGraphs: TOGGLE(DrawGraphsOption); break;
    case GraphToggleUpdateTime: TOGGLE(DrawUpdateTimeOption); break;
    case GraphToggleUpdateLength: TOGGLE(DrawUpdateLengthOption); break;
    case GraphToggleFrameTime: TOGGLE(DrawFrameTimeOption); break;
    case GraphToggleMarks: TOGGLE(DrawMarksOption); break;
    }
}
...
static void DrawMenuCallback(int item)
{
    switch (item)
    {
    case DrawToggleFPS: TOGGLE(DrawFPSOption); break;
    case DrawTogglePrediction: TOGGLE(DrawWithPredictionOption); break;
    case DrawToggleBackground: TOGGLE(DrawBackgroundOption); break;
    case DrawToggleStars: TOGGLE(DrawStarsOption); break;
    case DrawToggleBoundingCircles: TOGGLE(DrawBoundingCirclesOption); break;
    }
}
...
static void RadarMenuCallback(int item)
{
    switch (item)
    {
    case RadarDrawRadar: TOGGLE(DrawRadarOption); break;
    case RadarSmallRadar: RadarScaleOption = 0.5; break;
    case RadarMediumRadar: RadarScaleOption = 1; break;
    case RadarLargeRadar: RadarScaleOption = 2; break;
    case RadarSmallPoints: RadarPointScaleOption = 1; break;
    case RadarMediumPoints: RadarPointScaleOption = 2; break;
    case RadarLargePoints: RadarPointScaleOption = 3; break;
    }
}
