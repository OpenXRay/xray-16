...
GT2Result gti2NewOutgoingConnection(GT2Socket socket, GT2Connection* connection, unsigned int ip, unsigned short port)
{
    ...
    if (result != GT2Success) return result;
    ...
    GT2Connection connection, const GT2Byte* message, int len, GT2ConnectionCallbacks* callbacks)
{
    ...
    if (len > 0) {
        ...
        if (!connection->initialMessage) return GT2OutOfMemory;
    ...
    // copy the callbacks
    if (callbacks) connection->callbacks = *callbacks;
...
GT2Bool gti2AcceptConnection(GT2Connection connection, GT2ConnectionCallbacks* callbacks)
{
    ...
    if (connection->state != GTI2AwaitingAcceptReject) return GT2False;
...
void gti2RejectConnection(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    // check that we're still awaiting this
    if (connection->state != GTI2AwaitingAcceptReject) return;
...
GT2Bool gti2ConnectionSendData(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    if (!gti2SocketSend(connection->socket, connection->ip, connection->port, message, len)) return GT2False;
    ...
    if (connection->state < GTI2Connected) {
        ...
        if (connection->initiated) {
            ...
            if (connection->timeout) {
                ...
                if ((now - connection->startTime) > connection->timeout) timedOut = GT2True;
            }
        }
        else
        {
            ...
            if (connection->state < GTI2AwaitingAcceptReject) {
                // check the time taken against the timeout
                if ((now - connection->startTime) > GTI2_SERVER_TIMEOUT) timedOut = GT2True;
            }
        }
        ...
        if (timedOut) {
            ...
            // call the callback
            if (!gti2ConnectedCallback(connection, GT2TimedOut, NULL, 0)) return GT2False;
        }
    }
...
static GT2Bool gti2SendRetries(GT2Connection connection, gsi_time now)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if ((now - message->lastSend) > GTI2_RESEND_TIME) {
            if (!gti2ResendMessage(connection, message)) return GT2False;
        }
    }
...
static GT2Bool gti2CheckKeepAlive(GT2Connection connection, gsi_time now)
{
    if ((now - connection->lastSend) > GTI2_KEEP_ALIVE_TIME) {
        if (!gti2SendKeepAlive(connection)) return GT2False;
    }
...
{
    // check timeout
    if (!gti2CheckTimeout(connection, now)) return GT2False;

    // check keep alives
    if (!gti2CheckKeepAlive(connection, now)) return GT2False;

    // send retries
    if (!gti2SendRetries(connection, now)) return GT2False;

    // check the pending ack
    if (!gti2CheckPendingAck(connection, now)) return GT2False;
...
void gti2CloseConnection(GT2Connection connection, GT2Bool hard)
{
    ...
    if (hard) {
        ...
        if (connection->state >= GTI2Closed) return;
...
{
    if (connection->initialMessage) gsifree(connection->initialMessage);

    if (connection->incomingBuffer.buffer) gsifree(connection->incomingBuffer.buffer);
    if (connection->outgoingBuffer.buffer) gsifree(connection->outgoingBuffer.buffer);

    if (connection->incomingBufferMessages) ArrayFree(connection->incomingBufferMessages);
    if (connection->outgoingBufferMessages) ArrayFree(connection->outgoingBufferMessages);

    if (connection->sendFilters) ArrayFree(connection->sendFilters);
    if (connection->receiveFilters) ArrayFree(connection->receiveFilters);
