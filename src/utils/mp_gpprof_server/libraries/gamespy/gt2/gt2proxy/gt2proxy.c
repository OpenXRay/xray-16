...
void RemoveServerSocket(GT2Socket socket)
{
    ...
    if (index != NOT_FOUND) ArrayDeleteAt(ServerSockets, index);
}
...
void ClientClosedCallback(GT2Connection connection, GT2CloseReason reason)
{
    ...
    // Update stats.
    ////////////////
    if (reason == GT2LocalClose) clientCloses++;
#endif
}
...
void ServerConnectedCallback(GT2Connection connection, GT2Result result, GT2Byte* message, int len)
{
    ...
    if (result == GT2Success) {
        // Accept it.
        /////////////
        if (!gt2Accept(clientConnection, &ClientConnectionCallbacks)) gt2CloseConnection(connection);
    }
    else
    {
...
void ServerClosedCallback(GT2Connection connection, GT2CloseReason reason)
{
    ...
    // Update stats.
    ////////////////
    if (reason == GT2LocalClose) serverCloses++;
#endif
}
...
void DisplayStats(void)
{
    ...
    printf("%d connect attempts, %d (%d%%) accepted, %d (%d%%) rejected\n", numConnectAttempts, numAccepted,
        (numConnectAttempts) ? (numAccepted * 100) / numConnectAttempts : 0, numRejected,
    ...
    printf("client: %d messages, %d (%d%%) reliable, %d (%d%%) unreliable\n", total, clientReliableMessagesSent,
        (total) ? (clientReliableMessagesSent * 100) / total : 0, clientUnreliableMessagesSent,
...
int main(int argc, char** argv)
{
    ...
    ///////////////////////////////
    if (argc >= 3) strcpy(LocalAddress, argv[2]);
