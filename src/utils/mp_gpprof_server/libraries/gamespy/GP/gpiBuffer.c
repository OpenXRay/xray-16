...
GPResult gpiAppendCharToBuffer(GPConnection* connection, GPIBuffer* outputBuffer, char c)
{
    ...
    if (size == len) {
        ...
        if (output == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    }
    ...
    GPConnection* connection, SOCKET sock, const char* buffer, int bufferLen, GPIBool* closed, int* sent, char id[3])
{
    ...
    if (gsiSocketIsError(rcode)) {
        ...
        if ((rcode != WSAEWOULDBLOCK) && (rcode != WSAEINPROGRESS) && (rcode != WSAETIMEDOUT)) {
            ...
            if ((id[0] == 'P') && (id[1] == 'R')) return GP_NETWORK_ERROR;
...
GPResult gpiSendOrBufferStringLenToPeer(GPConnection* connection, GPIPeer_st peer, const char* string, int stringLen)
{
    ...
    /////////////////////////////
    if (stringLen == 0) return GP_NO_ERROR;
    ...
    GPConnection* connection, SOCKET sock, GPIBuffer* inputBuffer, int* bytesRead, GPIBool* connClosed, char id[3])
{
    ...
    do
    {
        ...
        if ((len + GPI_READ_SIZE) > size) {
            ...
            if (buffer == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
        }
    ...
    GPIBool clipSentData, char id[3])
{
    ...
    // Check for nothing to send.
    /////////////////////////////
    if (remaining == 0) return GP_NO_ERROR;
    ...
    if (connClosed) *connClosed = closed;
    ...
    GPIBool* closed, GPIBool clipSentData)
{
    ...
    else
    {
        ...
        do
        {
            ...
            if (sendAmount <= (GS_UDP_MSG_HEADER_LEN + GS_UDP_RELIABLE_MSG_HEADER)) break;
    ...
    GPConnection* connection, GPIBuffer* inputBuffer, char** message, int* type, int* plen)
{
    ...
    *message = NULL;
    ...
    /////////////////////////////
    if (inputBuffer->len < 5) return GP_NO_ERROR;
    ...
    if (str != NULL) {
        ...
        //////////////////////////////
        if (strncmp(str - 5, "\\msg\\", 5) != 0) return GP_NETWORK_ERROR;
        ...
        *str = '\0';
        ...
        // Read the header.
        ///////////////////
        if (!gpiValueForKey(inputBuffer->buffer, "\\m\\", intValue, sizeof(intValue))) return GP_NETWORK_ERROR;
        *type = atoi(intValue);
        ...
        if (inputBuffer->len > ((str - inputBuffer->buffer) + len)) {
            ...
            //////////////////////////////
            if (str[len] != '\0') return GP_NETWORK_ERROR;
...
{
    if (!buffer || !buffer->buffer || !buffer->pos) return GP_NO_ERROR;

    buffer->len -= buffer->pos;
    if (buffer->len) memmove(buffer->buffer, buffer->buffer + buffer->pos, (unsigned int)buffer->len);
