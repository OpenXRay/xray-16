...
static GPResult gpiProcessPeerInitiatingConnection(GPConnection* connection, GPIPeer* peer)
{
    ...
    GS_ASSERT(peer);
    if (!peer) return GP_NETWORK_ERROR;

    GS_ASSERT(peer->state != GPI_PEER_DISCONNECTED && peer->state != GPI_PEER_NOT_CONNECTED);
    if (peer->state == GPI_PEER_DISCONNECTED || peer->state == GPI_PEER_NOT_CONNECTED) return GP_NETWORK_ERROR;
    // Check the state.
    ///////////////////
    switch (peer->state)
    {
        ...
        if (aPeerState == GS_UDP_PEER_CONNECTED) {
            ...
            for (pcurr = iconnection->peerList; pcurr != NULL; pcurr = pcurr->pnext)
                if ((pcurr->profile == peer->profile) && (pcurr != peer))
                    if (pcurr->state <= GPI_PEER_CONNECTING) freePeerSig = GPIFalse;
    ...
    case GPI_PEER_WAITING:
    {
        ...
        // Check for a final.
        /////////////////////
        if (peer->inputBuffer.buffer) str = strstr(peer->inputBuffer.buffer, "\\final\\");
        if (str != NULL) {
            str[0] = '\0';
            str += 7;
    ...
    if (peer->outputBuffer.len > 0) {
        ...
        if (connClosed || (result != GP_NO_ERROR)) peer->state = GPI_PEER_DISCONNECTED;
    }
...
static GPResult gpiProcessPeerAcceptingConnection(GPConnection* connection, GPIPeer* peer)
{
    ...
    if (peer->state != GPI_PEER_WAITING) return GP_NETWORK_ERROR;
...
GPResult gpiPeerSendMessages(GPConnection* connection, GPIPeer* peer)
{
    ...
    //////////////////////////////////////////////////////////////////////
    if (peer->outputBuffer.len) return GP_NO_ERROR;
    ...
    while (ArrayLength(peer->messages))
    {
        ...
        //////////////////////////
        if (message->buffer.pos != message->buffer.len) break;
...
static GPResult gpiProcessPeerConnected(GPConnection* connection, GPIPeer* peer)
{
    ...
    if (!peer->outputBuffer.len) {
        ...
        if (peer->state == GPI_PEER_DISCONNECTED) return GP_NO_ERROR;
    }
    ...
    */
    ...
    gsUdpEngineGetPeerState(peer->ip, peer->port, &aPeerState);
    // if(connClosed)
    if (aPeerState == GS_UDP_PEER_CLOSED) peer->state = GPI_PEER_DISCONNECTED;
...
static GPResult gpiProcessPeer(GPConnection* connection, GPIPeer* peer)
{
    ...
    if (peer->state == GPI_PEER_NOT_CONNECTED) return GP_NETWORK_ERROR;
...
void gpiRemovePeer(GPConnection* connection, GPIPeer* peer)
{
    ...
    if (iconnection->peerList == NULL) return;
...
{
    GS_ASSERT(peer);
    if (!peer) return gsi_false;

    if (peer && peer->state != GPI_PEER_CONNECTED) return gsi_false;
...
GPResult gpiPeerAddMessage(GPConnection* connection, GPIPeer* peer, int type, const char* message)
{
    ...
    if (peer == NULL) return GP_NETWORK_ERROR;
    ...
    GPConnection* connection, GPIPeer* peer, int type, const struct GPITransferID_s* transferID)
{
    ...
    if (!transferID) return GP_NETWORK_ERROR;
    // Start the message.
    /////////////////////
...
GPResult gpiPeerFinishTransferMessage(GPConnection* connection, GPIPeer* peer, const char* message, int len)
{
    ...
    if (len == -1) len = (int)strlen(message);
