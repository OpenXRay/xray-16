...
static GPIBool gpiIsValidDate(int day, int month, int year)
{
    // Check for a blank.
    /////////////////////
    if ((day == 0) && (month == 0) && (year == 0)) return GPITrue;

    // Check for negatives.
    ///////////////////////
    if ((day < 0) || (month < 0) || (year < 0)) return GPIFalse;
    ...
    case 2:
        ...
        if ((((year % 4) == 0) && ((year % 100) != 0)) || ((year % 400) == 0)) {
            if (day > 29) return GPIFalse;
        }
        else
        {
            if (day > 28) return GPIFalse;
        }
        break;
    ...
    // PANTS|02.14.2000
    /////////////////////////////////////////////
    if (year < 1900) return GPIFalse;
    if (year > 2079) return GPIFalse;
    if (year == 2079) {
        if (month > 6) return GPIFalse;
        if ((month == 6) && (day > 6)) return GPIFalse;
    }
...
static GPResult gpiDateToInt(GPConnection* connection, int* date, int day, int month, int year)
{
    ...
    if (!gpiIsValidDate(day, month, year)) Error(connection, GP_PARAMETER_ERROR, "Invalid date.");
...
static GPResult gpiIntToDate(GPConnection* connection, int date, int* day, int* month, int* year)
{
    ...
    if (!gpiIsValidDate(d, m, y)) Error(connection, GP_PARAMETER_ERROR, "Invalid date.");
...
GPResult gpiProcessGetInfo(GPConnection* connection, GPIOperation* operation, const char* input)
{
    ...
    // Check for an error.
    //////////////////////
    if (gpiCheckForError(connection, input, GPITrue)) return GP_SERVER_ERROR;
    ...
    // Parse the info.
    //////////////////
    if (!gpiValueForKey(input, "\\nick\\", infoCache.nick, GP_NICK_LEN)) infoCache.nick[0] = '\0';
    if (!gpiValueForKey(input, "\\uniquenick\\", infoCache.uniquenick, GP_UNIQUENICK_LEN))
        infoCache.uniquenick[0] = '\0';
    if (!gpiValueForKey(input, "\\email\\", infoCache.email, GP_EMAIL_LEN)) infoCache.email[0] = '\0';
    if (!gpiValueForKey(input, "\\firstname\\", infoCache.firstname, GP_FIRSTNAME_LEN)) infoCache.firstname[0] = '\0';
    if (!gpiValueForKey(input, "\\lastname\\", infoCache.lastname, GP_LASTNAME_LEN)) infoCache.lastname[0] = '\0';
    if (!gpiValueForKey(input, "\\icquin\\", buffer, sizeof(buffer)))
        infoCache.icquin = -1;
    else
        infoCache.icquin = atoi(buffer);
    if (!gpiValueForKey(input, "\\homepage\\", infoCache.homepage, GP_HOMEPAGE_LEN)) infoCache.homepage[0] = '\0';
    ...
    if (!gpiValueForKey(input, "\\loc\\", infoCache.place, GP_PLACE_LEN)) infoCache.place[0] = '\0';
    ...
    if (!gpiValueForKey(input, "\\aim\\", infoCache.aimname, GP_AIMNAME_LEN)) infoCache.aimname[0] = '\0';
    ...
    for (peer = iconnection->peerList; peer != NULL; peer = peer->pnext)
    {
        ...
        if (peer->profile == profileid) {
            ...
            if (peer->state == GPI_PEER_GETTING_SIG) {
                ...
                /////////////////////////////////////////////////////////
                if (!profile) profile = gpiProfileListAdd(connection, profileid);
    ...
    // Cache info?
    //////////////
    if (!profile && iconnection->infoCaching) profile = gpiProfileListAdd(connection, profileid);
    ...
    // Caching info?
    ////////////////
    if (iconnection->infoCaching) gpiSetInfoCache(connection, profile, &infoCache);
    ...
    if (callback.callback != NULL) {
        ...
        if (arg == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiSetInfoi(GPConnection* connection, GPEnum info, int value)
{
    ...
    case GP_ZIPCODE:
        // Error check.
        ///////////////
        if (value < 0) Error(connection, GP_PARAMETER_ERROR, "Invalid zipcode.");
...
GPResult gpiSetInfos(GPConnection* connection, GPEnum info, const char* value)
{
    ...
    // Error check.
    ///////////////
    if (value == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid value.");
    ...
    {
    case GP_NICK:
        if (!value[0]) Error(connection, GP_PARAMETER_ERROR, "Invalid value.");
    ...
    case GP_COUNTRYCODE:
        // Error check.
        ///////////////
        if (strlen(value) != 2) Error(connection, GP_PARAMETER_ERROR, "Invalid countrycode.");
...
GPResult gpiSetInfod(GPConnection* connection, GPEnum info, int day, int month, int year)
{
    ...
    ///////////////////////////////////////
    if (info != GP_BIRTHDAY) Error(connection, GP_PARAMETER_ERROR, "Invalid info.");
    ...
    GPConnection* connection, GPProfile profile, GPEnum checkCache, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    //////////////////////////////
    if (!iconnection->infoCaching) useCache = GPIFalse;
...
GPResult gpiGetInfoNoWait(GPConnection* connection, GPProfile profile, GPGetInfoResponseArg* arg)
{
    ...
    // Check the info cache state.
    //////////////////////////////
    if (!iconnection->infoCaching) return GP_NETWORK_ERROR;

    // Check to see if we have the info cached.
    ///////////////////////////////////////////
    if (!gpiGetProfile(connection, profile, &pProfile) || !pProfile->cache) return GP_NETWORK_ERROR;
...
GPIBool gpiSetInfoCache(GPConnection* connection, pGPIProfile profile, const GPIInfoCache* cache)
{
    ...
    ///////////////////////////////
    if (!iconnection->infoCaching) return GPITrue;
