...
static void gpiTransferFree(void* elem)
{
    ...
    if (transfer->message) freeclear(transfer->message);

    if (transfer->baseDirectory) gsifree(transfer->baseDirectory);
...
GPResult gpiInitTransfers(GPConnection* connection)
{
    ...
    if (!iconnection->transfers) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
static GPResult gpiNewTransfer(GPConnection* connection, GPITransfer** transfer, GPProfile profile, GPIBool sender)
{
    ...
    if (!transferTemp.files) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
#ifdef WIN32
    if (GetTempPathA(sizeof(buffer), buffer) != 0) pTransfer->baseDirectory = goastrdup(buffer);
#endif
...
GPIFile* gpiAddFileToTransfer(GPITransfer* transfer, const char* path, const char* name)
{
    ...
    /////////////////////////
    if (name[strlen(name) - 1] == '/') file.flags = GPI_FILE_DIRECTORY;
...
static GPITransfer* gpiFindTransferByTransferID(GPConnection* connection, GPITransferID* transferID)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (memcmp(&transfer->transferID, transferID, sizeof(GPITransferID)) == 0) return transfer;
    }
    ...
    const char* headers, const char* buffer, int bufferLen)
{
    ...
    /////////////////////////////////////////////////////////////
    if (!iconnection->callbacks[GPI_TRANSFER_CALLBACK].callback) return GPIFalse;

    // Check the version.
    /////////////////////
    if (!gpiValueForKey(headers, "\\version\\", intValue, sizeof(intValue))) return GPIFalse;
    version = atoi(intValue);
    if (version < 1) return GPIFalse;

    // Get the number of files.
    ///////////////////////////
    if (!gpiValueForKey(headers, "\\num\\", intValue, sizeof(intValue))) return GPIFalse;
    numFiles = atoi(intValue);
    if (numFiles < 1) return GPIFalse;

    // Create the transfer object.
    //////////////////////////////
    if (gpiNewReceiverTransfer(connection, &transfer, peer->profile, transferID) != GP_NO_ERROR) return GPIFalse;
...
#endif
    {
        ...
        if (aResult != GP_NO_ERROR) return GPIFalse;
    }
    ...
    GPConnection* connection, GPITransfer* transfer, const char* headers, const char* buffer, int bufferLen)
{
    ...
    if (!transfer->sender) return GPIFalse;
    ...
    // Get the result.
    //////////////////
    if (!gpiValueForKey(headers, "\\result\\", intValue, sizeof(intValue))) return GPIFalse;
...
static GPIBool gpiHandleBegin(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    // Get the file.
    ////////////////
    if (!gpiValueForKey(headers, "\\file\\", intValue, sizeof(intValue))) return GPIFalse;
    fileIndex = atoi(intValue);
    if ((fileIndex < 0) || (fileIndex >= ArrayLength(transfer->files))) return GPIFalse;
    if (fileIndex != transfer->currentFile) return GPIFalse;
    file = (GPIFile*)ArrayNth(transfer->files, fileIndex);

    // Is this a directory?
    ///////////////////////
    if (file->flags & GPI_FILE_DIRECTORY) return GPIFalse;

    // Get the size.
    ////////////////
    if (!gpiValueForKey(headers, "\\size\\", intValue, sizeof(intValue))) return GPIFalse;
    size = atoi(intValue);
    if (size < 0) return GPIFalse;
    ...
    // Get the mod time.
    ////////////////////
    if (!gpiValueForKey(headers, "\\mtime\\", intValue, sizeof(intValue))) return GPIFalse;
    mtime = (unsigned int)strtoul(intValue, NULL, 10);
...
static GPIBool gpiHandleEnd(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    if (transfer->currentFile == -1) return GPIFalse;
    ...
    fileIndex = atoi(intValue);
    if ((fileIndex < 0) || (fileIndex >= ArrayLength(transfer->files))) return GPIFalse;
    if (fileIndex != transfer->currentFile) return GPITrue;
    ...
    else
    {
        ...
        //////////////////
        assert(file->file);
        if (!file->file) return GPIFalse;

        // Get the remote md5.
        //////////////////////
        if (!gpiValueForKey(headers, "\\md5\\", remoteMD5, sizeof(remoteMD5))) return GPIFalse;
    ...
    GPConnection* connection, GPITransfer* transfer, const char* headers, const char* buffer, int bufferLen)
{
    ...
    if (transfer->sender) {
        ...
        // Get the progress.
        ////////////////////
        if (!gpiValueForKey(headers, "\\pro\\", intValue, sizeof(intValue))) return GPIFalse;
...
static GPIBool gpiHandleSkip(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    // Get the reason.
    //////////////////
    if (!gpiValueForKey(headers, "\\reason\\", intValue, sizeof(intValue))) return GPIFalse;
...
static GPIBool gpiHandleTransferThrottle(GPConnection* connection, GPITransfer* transfer, const char* headers)
{
    ...
    // Get the throttle.
    ////////////////////
    if (!gpiValueForKey(headers, "\\rate\\", intValue, sizeof(intValue))) return GPIFalse;
...
GPResult gpiProcessCurrentFile(GPConnection* connection, GPITransfer* transfer)
{
    ...
    //////////////////////////////////////
    if (file->flags & GPI_FILE_CONFIRMING) return GP_NO_ERROR;
#endif
    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; (file->progress < file->size) && !transfer->peer->outputBuffer.len /*&& (i < 20)*/; i++)
            {
                ...
                ///////////////////////////
                if ((file->acknowledged + GPI_ACKNOWLEDGED_WINDOW) < file->progress) break;
#endif
...
GPResult gpiProcessTransfer(GPConnection* connection, GPITransfer* transfer)
{
    ...
    /////////////////////////////////////
    if (!transfer->sender) return GP_NO_ERROR;

    // Is the transfer finished?
    ////////////////////////////
    if (transfer->state >= GPITransferComplete) return GP_NO_ERROR;
    ...
    ///////////////////////////////////////////////
    if (transfer->throttle == 0) return GP_NO_ERROR;

    // Don't send files if we're not transfering yet.
    //////////////////////////////////////////////////
    if (transfer->state < GPITransferTransferring) return GP_NO_ERROR;

    // Don't send files if we have regular messages pending.
    ////////////////////////////////////////////////////////
    if (ArrayLength(transfer->peer->messages)) return GP_NO_ERROR;
    ...
    while (transfer->currentFile < len)
    {
        ...
        if (currentFile == transfer->currentFile) break;
    }
...
GPIBool gpiGetTransferFileInfo(FILE* file, int* size, gsi_time* modTime)
{
    ...
    if (_fstat(_fileno(file), &stats) != 0) return GPIFalse;

    *size = (int)stats.st_size;
    *modTime = (gsi_time)stats.st_mtime;
#else
    if (fseek(file, 0, SEEK_END) != 0) return GPIFalse;

    *size = (int)ftell(file);
    if (*size == -1) return GPIFalse;
    ...
    GPConnection* connection, GPIPeer* peer, int type, const char* headers, const char* buffer, int len)
{
    ...
    // Get the transfer ID.
    ///////////////////////
    if (!gpiValueForKey(headers, "\\xfer\\", value, sizeof(value))) return;
    if (sscanf(value, "%d %u %u", &transferID.profileid, &transferID.count, &transferID.time) != 3) return;
    ...
    ///////////////////////////////////////
    if (!success) gpiTransferError(connection, transfer);
#endif
