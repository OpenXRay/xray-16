...
void gpiCallErrorCallback(GPConnection* connection, GPResult result, GPEnum fatal)
{
    ...
    if (fatal == GP_FATAL) iconnection->fatalError = GPITrue;
    ...
    GPConnection* connection, GPICallback callback, void* arg, const struct GPIOperation_s* operation, int type)
{
    ...
    // Update the list.
    ///////////////////
    if (iconnection->callbackList == NULL) iconnection->callbackList = data;
    if (iconnection->lastCallback != NULL) iconnection->lastCallback->pnext = data;
...
static void gpiCallCallback(GPConnection* connection, GPICallbackData* data)
{
    ...
    else if (data->type == GPI_ADD_REVERSE_BUDDIES_LIST)
    {
        ...
        if (arg->matches) freeclear(arg->matches);
    }
