...
GPResult gpiProcessRecvBuddyMessage(GPConnection* connection, const char* input)
{
    ...
    case GPI_BM_MESSAGE:
        ...
        if (callback.callback != NULL) {
            ...
            if (arg->message == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    case GPI_BM_REQUEST:
        ...
        if (!profile) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    case GPI_BM_STATUS:
        ...
        if (!profile->blocked) {
            ...
            if (!profile->buddyStatus) {
                ...
                if (!profile->buddyStatus) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
            ...
            if (!gpiValueForKey(buffer, "|ss|", strTemp, GP_STATUS_STRING_LEN)) strTemp[0] = '\0';
            buddyStatus->statusString = goastrdup(strTemp);
            if (!buddyStatus->statusString) Error(connection, GP_MEMORY_ERROR, "Out of memory.");

            // Get the location string.
            ///////////////////////////
            freeclear(buddyStatus->locationString);
            if (!gpiValueForKey(buffer, "|ls|", strTemp, GP_LOCATION_STRING_LEN)) strTemp[0] = '\0';
            buddyStatus->locationString = goastrdup(strTemp);
            if (!buddyStatus->locationString) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiProcessRecvBuddyStatusInfo(GPConnection* connection, const char* input)
{
    ...
    if (!profile->blocked) {
        ...
        if (!profile->buddyStatusInfo) {
            ...
            if (!profile->buddyStatusInfo) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
            ...
            if (!profile->buddyStatusInfo->extendedInfoKeys) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
        }
        ...
        if (callback.callback != NULL) {
            ...
            if (anArg == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    GPConnection* connection, int profileid, int type, const char* message, int sendOption, GPIPeerOp* peerOp)
{
    ...
    if (!peer) {
        ...
        {
            if (sendOption == GP_DONT_ROUTE) return GP_NETWORK_ERROR;
        ...
        if (!peer) return GP_MEMORY_ERROR;
    ...
    else if (peer->state == GPI_PEER_DISCONNECTED)
    {
        if (gpiGetProfile(connection, profileid, &profile)) {
            ...
            if (profile->buddyStatusInfo) profile->buddyStatusInfo->buddyPort = 0;

            // send the message through the server
            if (sendOption == GP_DONT_ROUTE) return GP_NETWORK_ERROR;
            if (type < 100) return gpiSendServerBuddyMessage(connection, profileid, type, message);
        }
    }
...
GPResult gpiBuddyHandleKeyRequest(GPConnection* connection, GPIPeer* peer)
{
    ...
    if (strcmp(message, "") != 0) freeclear(message);
    return GP_NO_ERROR;
}
...
GPResult gpiBuddyHandleKeyReply(GPConnection* connection, GPIPeer* peer, char* buffer)
{
    ...
    if (!gpiGetProfile(connection, peer->profile, &pProfile)) Error(connection, GP_PARAMETER_ERROR, "Invalid profile.");
    ...
    if (strcmp(buffer, "") == 0) {
        ...
        for (anIterator = peer->peerOpQueue.first; anIterator != NULL; anIterator = anIterator->next)
            if (anIterator->type == GPI_BM_KEYS_REQUEST) break;
...
GPResult gpiAuthBuddyRequest(GPConnection* connection, GPProfile profile)
{
    ...
    /////////////////////////
    if (!pProfile->authSig) Error(connection, GP_PARAMETER_ERROR, "Invalid profile.");
