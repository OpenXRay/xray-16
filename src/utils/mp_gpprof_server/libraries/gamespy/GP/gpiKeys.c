...
GPResult gpiStatusInfoKeysInit(GPConnection* connection)
{
    ...
    if (!iconnection->extendedInfoKeys) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiStatusInfoAddKey(GPConnection* connection, DArray keys, const char* theKeyName, const char* theKeyValue)
{
    ...
    if (!theKeyName) Error(connection, GP_PARAMETER_ERROR, "Invalid key name");
    if (!theKeyValue) Error(connection, GP_PARAMETER_ERROR, "Invalid key value");
...
GPResult gpiStatusInfoDelKey(GPConnection* connection, DArray keys, const char* keyName)
{
    ...
    if (!keyName) Error(connection, GP_PARAMETER_ERROR, "Invalid key name");
