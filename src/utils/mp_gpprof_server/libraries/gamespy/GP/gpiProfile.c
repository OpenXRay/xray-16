...
GPIBool gpiInitProfiles(GPConnection* connection)
{
    ...
    if (!iconnection->profileList.profileTable) return GPIFalse;
...
static GPResult gpiReadDiskProfile(GPConnection* connection, GPIBool* failedOut)
{
    ...
    if (pProfile) {
        ...
        *pProfile = profile;
        ...
        //////////////////////////
        if (valid) gpiSetInfoCache(connection, pProfile, &infoCache);
    }
    *failedOut = GPIFalse;
    return GP_NO_ERROR;
...
static GPIBool gpiSaveDiskProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    ...
    /////////////////////////////
    if (profile->userId != 0) fprintf(fp, "userid=%d\n", profile->userId);
    ...
    if (profile->cache) {
        ...
        if (profile->cache->sex == GP_MALE) fprintf(fp, "sex=Male\n");
...
GPIProfile* gpiProfileListAdd(GPConnection* connection, int id)
{
    ...
    ///////////////////////////////////////////
    if (gpiGetProfile(connection, (GPProfile)id, &pProfile)) return pProfile;
    ...
    GPConnection* connection, const char nick[31], GPEnum replace, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    // Error check.
    ///////////////
    if (nick == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid nick.");
...
GPResult gpiAddToBlockedList(GPConnection* connection, int profileid)
{
    ...
    if (!profile->blocked) {
        ...
        ///////////////////////////////////////////////////////////////////////////////////////
        if (!iconnection->npSyncLock) gpiAddToNpBlockList(connection, profileid);
#endif
    }
...
static GPIBool gpiFixBlockIndices(GPConnection* connection, GPIProfile* profile, void* data)
{
    ...
    if (profile->blocked && (profile->blockIndex > baseIndex)) profile->blockIndex--;
    return GPITrue;
}
