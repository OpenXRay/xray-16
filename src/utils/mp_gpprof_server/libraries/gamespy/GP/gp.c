...
GPResult gpInitialize(GPConnection* connection, int productID, int namespaceID, int partnerID)
{
    ...
    /////////////////////////////////////
    if (__GSIACResult != GSIACAvailable) return GP_PARAMETER_ERROR;

    // Error check.
    ///////////////
    if (connection == NULL) return GP_PARAMETER_ERROR;
...
void gpDestroy(GPConnection* connection)
{
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return;
...
GPResult gpEnable(GPConnection* connection, GPEnum state)
{
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
...
GPResult gpProcess(GPConnection* connection)
{
    ...
    /////////////////////////////
    if (iconnection->simulation) return GP_NO_ERROR;
...
GPResult gpSetCallback(GPConnection* connection, GPEnum func, GPCallback callback, void* param)
{
    ...
    ///////////////////////
    index = func;
    if ((index < 0) || (index >= GPI_NUM_CALLBACKS)) Error(connection, GP_PARAMETER_ERROR, "Invalid func.");
    ...
    const char password[GP_PASSWORD_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
    if ((nick == NULL) || (nick[0] == '\0')) return GP_PARAMETER_ERROR;
    if ((email == NULL) || (email[0] == '\0')) return GP_PARAMETER_ERROR;
    if ((password == NULL) || (password[0] == '\0')) return GP_PARAMETER_ERROR;

    // Check for no callback.
    /////////////////////////
    if (callback == NULL) Error(connection, GP_PARAMETER_ERROR, "No callback.");
    ...
    const char cdkey[GP_CDKEY_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
    if ((nick == NULL) || (nick[0] == '\0')) return GP_PARAMETER_ERROR;
    if (uniquenick == NULL) uniquenick = "";
    if ((email == NULL) || (email[0] == '\0')) return GP_PARAMETER_ERROR;
    if ((password == NULL) || (password[0] == '\0')) return GP_PARAMETER_ERROR;
    if (cdkey && (cdkey[0] == '\0')) cdkey = NULL;
    ...
    ////////////////////////////////
    if (strlen(nick) >= GP_NICK_LEN) Error(connection, GP_PARAMETER_ERROR, "Nick too long.");

    // Check the length of the uniquenick.
    //////////////////////////////////////
    if (strlen(uniquenick) >= GP_UNIQUENICK_LEN) Error(connection, GP_PARAMETER_ERROR, "Uniquenick too long.");

    // Check the length of the email.
    /////////////////////////////////
    if (strlen(email) >= GP_EMAIL_LEN) Error(connection, GP_PARAMETER_ERROR, "Email too long.");

    // Check the length of the password.
    ////////////////////////////////////
    if (strlen(password) >= GP_PASSWORD_LEN) Error(connection, GP_PARAMETER_ERROR, "Password too long.");
    ...
    const char password[GP_PASSWORD_LEN], GPEnum firewall, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
    if ((uniquenick == NULL) || (uniquenick[0] == '\0')) return GP_PARAMETER_ERROR;
    if ((password == NULL) || (password[0] == '\0')) return GP_PARAMETER_ERROR;
    ...
    void* param)
{
    ...
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
    if ((authtoken == NULL) || (authtoken[0] == '\0')) return GP_PARAMETER_ERROR;
    if ((partnerchallenge == NULL) || (partnerchallenge[0] == '\0')) return GP_PARAMETER_ERROR;
...
void gpDisconnect(GPConnection* connection)
{
    ...
    /////////////////////////////
    iconnection = (GPIConnection*)*connection;
    if (iconnection->simulation) return;
    ...
    //////////////////////////////////////////////////////////////////////////
    if (oldState == GPI_CONNECTED) iconnection->connectState = GPI_DISCONNECTED;
}
    ...
    const char password[GP_PASSWORD_LEN], GPEnum blocking, GPCallback callback, void* param)
{
    ...
    // Check the length of the password.
    ////////////////////////////////////
    if (password && (strlen(password) >= GP_PASSWORD_LEN)) Error(connection, GP_PARAMETER_ERROR, "Password too long.");
    ...
    GPConnection* connection, const char desirednick[GP_NICK_LEN], GPEnum blocking, GPCallback callback, void* param)
{
    ...
    ///////////////////////////////////////
    if (strlen(desirednick) >= GP_UNIQUENICK_LEN) Error(connection, GP_PARAMETER_ERROR, "Desirednick too long.");
    ...
    GPConnection* connection, const char cdkey[GP_CDKEY_LEN], GPEnum blocking, GPCallback callback, void* param)
{
    ...
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return GP_PARAMETER_ERROR;
    if ((cdkey == NULL) || (cdkey[0] == '\0')) return GP_PARAMETER_ERROR;
...
GPResult gpGetErrorCode(GPConnection* connection, GPErrorCode* errorCode)
{
    ...
    // Error check.
    ///////////////
    if (errorCode == NULL) return GP_PARAMETER_ERROR;
...
GPResult gpGetErrorStringA(GPConnection* connection, char errorString[GP_ERROR_STRING_LEN])
{
    ...
    // Error check.
    ///////////////
    if (errorString == NULL) return GP_PARAMETER_ERROR;
    ...
    GPCallback callback, void* param)
{
    ...
    // PANTS|05.18.00
    /////////////////////
    if ((nick == NULL) || (nick[0] == '\0')) Error(connection, GP_PARAMETER_ERROR, "Invalid nick.");
    ...
    GPEnum blocking, GPCallback callback, void* param)
{
    ...
    {
        if ((nick[i] & 0xFF00) != 0) return GP_PARAMETER_ERROR;
    }
...
GPResult gpUserIDFromProfile(GPConnection* connection, GPProfile profile, int* userid)
{
    ...
    //////////////////////////
    if (!gpiGetProfile(connection, profile, &pProfile)) Error(connection, GP_PARAMETER_ERROR, "Invalid profile.");
    ...
    GPConnection* connection, GPProfile profile, GPEnum checkCache, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL) || (profile == 0)) return GP_PARAMETER_ERROR;
...
GPResult gpGetInfoNoWait(GPConnection* connection, GPProfile profile, GPGetInfoResponseArg* arg)
{
    ...
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL) || (profile == 0) || (arg == NULL)) return GP_PARAMETER_ERROR;
...
GPResult gpSendBuddyRequestA(GPConnection* connection, GPProfile profile, const char reason[GP_REASON_LEN])
{
    ...
    // Error check.
    ///////////////
    if (reason == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid reason.");

    // Replace backslashes in reason.
    /////////////////////////////////
    strzcpy(reasonFixed, reason, GP_REASON_LEN);
    for (i = 0; reasonFixed[i]; i++)
        if (reasonFixed[i] == '\\') reasonFixed[i] = '/';
...
GPResult gpDenyBuddyRequest(GPConnection* connection, GPProfile profile)
{
    ...
    // Get the profile.
    ///////////////////
    if (!gpiGetProfile(connection, profile, &pProfile)) return GP_NO_ERROR;
    ...
    if (!iconnection->infoCaching && (pProfile->requestCount <= 0)) {
        ...
        if (gpiCanFreeProfile(pProfile)) gpiRemoveProfile(connection, pProfile);
    }
...
GPResult gpGetBuddyStatus(GPConnection* connection, int index, GPBuddyStatus* status)
{
    ...
    ///////////////////////////
    if (status == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid status.");

    // Check the buddy index.
    /////////////////////////
    num = iconnection->profileList.numBuddies;
    if ((index < 0) || (index >= num)) Error(connection, GP_PARAMETER_ERROR, "Invalid index.");

    // Find the buddy with this index.
    //////////////////////////////////
    profile = gpiFindBuddy(connection, index);
    if (!profile) Error(connection, GP_PARAMETER_ERROR, "Invalid index.");
...
GPResult gpGetBuddyStatusInfo(GPConnection* connection, int index, GPBuddyStatusInfo* statusInfo)
{
    ...
    // Check for a NULL status.
    ///////////////////////////
    if (statusInfo == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid status.");
...
GPResult gpSetBuddyAddr(GPConnection* connection, int index, unsigned int buddyIp, unsigned short buddyPort)
{
    ...
    if (buddyIp == 0 || buddyPort == 0) Error(connection, GP_PARAMETER_ERROR, "Invalid IP and port");
...
int gpIsBuddy(GPConnection* connection, GPProfile profile)
{
    ...
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return 0;
    ...
    // Check for simulation mode.
    /////////////////////////////
    if (iconnection->simulation) return 0;
...
GPResult gpGetBlockedProfile(GPConnection* connection, int index, GPProfile* profile)
{
    ...
    ////////////////////////////
    if (profile == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid profile container");
    ...
    if (!pProfile) Error(connection, GP_PARAMETER_ERROR, "Invalid index.");
...
gsi_bool gpIsBlocked(GPConnection* connection, GPProfile profile)
{
    ...
    // Error check.
    ///////////////
    if ((connection == NULL) || (*connection == NULL)) return gsi_false;
    ...
    // Check for simulation mode.
    /////////////////////////////
    if (iconnection->simulation) return gsi_false;

    // Get the index.
    /////////////////
    if (gpiGetProfile(connection, profile, &pProfile) && pProfile->blocked) return gsi_true;
    ...
    const char locationString[GP_LOCATION_STRING_LEN])
{
    ...
    // Error check.
    ///////////////
    if (statusString == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid statusString.");
    if (locationString == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid locationString.");

    // Replace backslashes with slashes.
    ////////////////////////////////////
    strzcpy(statusStringFixed, statusString, GP_STATUS_STRING_LEN);
    for (i = 0; statusStringFixed[i]; i++)
        if (statusStringFixed[i] == '\\') statusStringFixed[i] = '/';
    strzcpy(locationStringFixed, locationString, GP_LOCATION_STRING_LEN);
    for (i = 0; locationStringFixed[i]; i++)
        if (locationStringFixed[i] == '\\') locationStringFixed[i] = '/';
    ...
    const char* gameMapName, int gameMapNameLen)
{
    ...
    if (richStatus == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid richStatus.");
...
GPResult gpSendBuddyMessageA(GPConnection* connection, GPProfile profile, const char* message)
{
    ...
    // Error check.
    ///////////////
    if (message == NULL) Error(connection, GP_PARAMETER_ERROR, "Invalid message.");
...
GPResult gpSetInvitableGames(GPConnection* connection, int numProductIDs, const int* productIDs)
{
    ...
    // Error check.
    ///////////////
    if (numProductIDs < 0) Error(connection, GP_PARAMETER_ERROR, "Invalid numProductIDs.");
    if ((numProductIDs > 0) && (productIDs == NULL)) Error(connection, GP_PARAMETER_ERROR, "Invalid productIDs.");
    ...
    for (i = 0; i < numProductIDs; i++)
    {
        ...
        if (i < (numProductIDs - 1)) gpiAppendStringToBuffer(connection, &iconnection->outputBuffer, ",");
    }
    gpiAppendStringToBuffer(connection, &iconnection->outputBuffer, "\\final\\");
...
static GPResult gpiAddSendingFileA(GPConnection* connection, GPITransfer* transfer, const char* path, const char* name)
{
    ...
    ////////////////////////////////
    if (!path && !name) Error(connection, GP_PARAMETER_ERROR, "File missing path and name.");
    if (path && !path[0]) Error(connection, GP_PARAMETER_ERROR, "Empty path.");
    if (name && !name[0]) Error(connection, GP_PARAMETER_ERROR, "Empty name.");
    ...
    if (path) {
        ...
        if (!fileVerify) Error(connection, GP_PARAMETER_ERROR, "Can't find file.");
    ...
    if (name) {
        ...
        if (strcspn(name, ":*?\"<>|\n") != len) Error(connection, GP_PARAMETER_ERROR, "Invalid character in filename.");
    }
    ...
    if (!file) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    gpSendFilesCallback callback, void* param)
{
    ...
    // Check for simulation mode.
    /////////////////////////////
    if (iconnection->simulation) Error(connection, GP_PARAMETER_ERROR, "Cannot send files in simulation mode.");
    ...
    // Check other stuff.
    /////////////////////
    if (!callback) Error(connection, GP_PARAMETER_ERROR, "No callback.");
    if (!iconnection->callbacks[GPI_TRANSFER_CALLBACK].callback) Error(connection, GP_PARAMETER_ERROR, "No callback.");
    ...
    // Successful so far.
    /////////////////////
    if (transfer) *transfer = pTransfer->localID;
    ...
    gpSendFilesCallback callback, void* param)
{
    ...
    if (message == NULL) return gpSendFilesA(connection, transfer, profile, NULL, callback, param);
...
GPResult gpAcceptTransferA(GPConnection* connection, GPTransfer transfer, const char* message)
{
    ...
    if (!pTransfer) Error(connection, GP_PARAMETER_ERROR, "Invalid transfer.");

    // Check that we have a directory set.
    //////////////////////////////////////
    if (!pTransfer->baseDirectory) Error(connection, GP_PARAMETER_ERROR, "No transfer directory set.");

    // Check if this transfer has been cancelled.
    /////////////////////////////////////////////
    if (pTransfer->state & GPITransferCancelled) Error(connection, GP_PARAMETER_ERROR, "Transfer already cancelled.");
...
GPResult gpAcceptTransferW(GPConnection* connection, GPTransfer transfer, const unsigned short* message)
{
    ...
    if (message == NULL) return gpAcceptTransferA(connection, transfer, NULL);
...
GPResult gpRejectTransferA(GPConnection* connection, GPTransfer transfer, const char* message)
{
    ...
    ////////////////////
    pTransfer = gpiFindTransferByLocalID(connection, transfer);
    if (!pTransfer) return GP_NO_ERROR;

    // Check if this transfer has been cancelled.
    /////////////////////////////////////////////
    if (pTransfer->state & GPITransferCancelled) return GP_NO_ERROR;
...
GPResult gpRejectTransferW(GPConnection* connection, GPTransfer transfer, const unsigned short* message)
{
    ...
    if (message == NULL) return gpRejectTransferA(connection, transfer, NULL);
...
GPResult gpFreeTransfer(GPConnection* connection, GPTransfer transfer)
{
    ...
    ////////////////////////
    if (pTransfer->state < GPITransferComplete) gpiCancelTransfer(connection, pTransfer);
...
GPResult gpSetTransferDirectoryA(GPConnection* connection, GPTransfer transfer, const char* directory)
{
    ...
    if (!directory || !directory[0]) Error(connection, GP_PARAMETER_ERROR, "Invalid directory.");
    lastChar = directory[strlen(directory) - 1];
    if ((lastChar != '\\') && (lastChar != '/')) Error(connection, GP_PARAMETER_ERROR, "Invalid directory.");
    ...
    //////////////////////////////////////////
    if (pTransfer->sender) Error(connection, GP_PARAMETER_ERROR, "Sender has no transfer directory.");
    ...
    ///////////////////////////////
    if (pTransfer->baseDirectory) gsifree(pTransfer->baseDirectory);
    pTransfer->baseDirectory = NULL;

    // Set the directory.
    /////////////////////
    pTransfer->baseDirectory = goastrdup(directory);
    if (!pTransfer->baseDirectory) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpSkipFile(GPConnection* connection, GPTransfer transfer, int index)
{
    ...
    /////////////////////////////////
    if (index < pTransfer->currentFile) return GP_NO_ERROR;
    ...
    if (pTransfer->currentFile != index) {
        ...
        //////////////////////////////////////////////////////////////
        if (!pTransfer->sender) gpiSkipFile(connection, pTransfer, index, GPI_SKIP_USER_SKIP);
...
GPResult gpGetNumTransfers(GPConnection* connection, int* num)
{
    ...
    // Check for NULL.
    //////////////////
    if (num == NULL) Error(connection, GP_PARAMETER_ERROR, "NULL pointer.");
...
GPResult gpGetTransfer(GPConnection* connection, int index, GPTransfer* transfer)
{
    ...
    // Check for NULL.
    //////////////////
    if (transfer == NULL) Error(connection, GP_PARAMETER_ERROR, "NULL pointer.");
    ...
    ///////////////////////////////
    if (localID == -1) Error(connection, GP_PARAMETER_ERROR, "Index out of range.");
