...
void RecvBuddyStatus(GPConnection* connection, void* arg_, void* param)
{
    ...
    if (arg->profile == other) {
        ...
        if (status.status == 1 && _tcscmp(status.statusString, "Ready") && _tcscmp(status.statusString, "BlockTime")) {
            if (appState < 1) {
                ...
                CHECK_GP_RESULT(
                    gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("Ready"), _T("gptestc")), "gpSetStatus failed");
    ...
    printf("  Status: %s, Status String: %s, Location String: %s, IP: %d, Port: %d\n", statusToString[status.status],
        status.statusString, status.locationString, status.ip, status.port);
...
// gsSetDebugLevel(GSIDebugCat_App, GSIDebugType_All, GSIDebugLevel_Hardcore);  // Show All app comment
    ...
    CHECK_GP_RESULT(gpProfileSearch(pconn, _T(""), _T(""), _T("dan@gamespy.com"), _T(""), _T(""), 0, GP_BLOCKING,
                        (GPCallback)ProfileSearchResponse, NULL),
        "gpProfileSearch failed");
    printf(" Search on nick ('crt')\n");
    CHECK_GP_RESULT(gpProfileSearch(pconn, _T("crt"), _T(""), _T(""), _T(""), _T(""), 0, GP_BLOCKING,
                        (GPCallback)ProfileSearchResponse, NULL),
        "gpProfileSearch failed");
    printf(" Search on nick and email ('gptestc1', gptestc@gptestc.com)\n");
    CHECK_GP_RESULT(gpProfileSearch(pconn, GPTC_NICK1, _T(""), GPTC_EMAIL1, _T(""), _T(""), 0, GP_BLOCKING,
                        (GPCallback)ProfileSearchResponse, NULL),
        "gpProfileSearch failed");
    printf(" Search on unique nick for different namespaces...\n");
    CHECK_GP_RESULT(gpProfileSearchUniquenick(pconn, GPTC_NICK1, namespaceIds, GP_MAX_NAMESPACEIDS, GP_BLOCKING,
                        (GPCallback)ProfileSearchResponse, NULL),
    ...
    if (noComLineArgs) {
        ...
        CHECK_GP_RESULT(
            gpSetStatus(pconn, (GPEnum)GP_ONLINE, _T("DoneBlockTesting"), _T("gptestc")), "gpSetStatus failed");
