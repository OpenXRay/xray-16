...
bool ReadProfile(FILE* fp, Profile* profile)
{
    ...
    // Read the user id.
    ////////////////////
    if (!ReadField(fp, intBuffer)) return false;
...
void Log(const char* buffer)
{
    ...
    sprintf(timeBuffer, "%-2.2d/%-2.2d/%-2.2d %-2.2d:%-2.2d:%-2.2d: ", ltime->tm_mon + 1, ltime->tm_mday,
        ltime->tm_year % 100, ltime->tm_hour, ltime->tm_min, ltime->tm_sec);
...
void ErrorCallback(GPConnection* connection, GPErrorArg* arg, Profile* profile)
{
    ...
    case GP_BM_NOT_BUDDY: return;
    }

// Get a string for the result.
///////////////////////////////
#define RESULT(x)                                                                                                      \
    case x: resultString = #x; break;
    switch (arg->result)
    {
        RESULT(GP_NO_ERROR)
    ...
    default: resultString = "Unknown result!\n";
    }

// Get a string for the error code.
///////////////////////////////////
#define ERRORCODE(x)                                                                                                   \
    case x: errorCodeString = #x; break;
    switch (arg->errorCode)
    {
    ...
    default: errorCodeString = "Unknown error code!\n";
    }
...
void RecvBuddyStatusCallback(GPConnection* connection, GPRecvBuddyStatusArg* arg, Profile* profile)
{
    ...
    ////////////////////////////
    if (RandomInt(0, 9) == 0) MessageOther(profile, arg->profile);
...
void RecvBuddyRequestCallback(GPConnection* connection, GPRecvBuddyRequestArg* arg, Profile* profile)
{
    ...
    ///////////////////////////
    if (RandomBool()) gpAuthBuddyRequest(&profile->gp, arg->profile);
    ...
    if (arg->result == GP_NO_ERROR) {
        ...
        if (arg->profile != RemoteAuthProfiles[profile->index % 10]) {
            ...
            sprintf(buffer, "%d: XXX Connection ProfileID Error (%d != %d) [%d]: %d.%03ds\n", totalConnected,
                arg->profile, RemoteAuthProfiles[profile->index % 10], (profile->index % 10), ms / 1000, ms % 1000);
            Log(buffer);
        }
...
void AddOther(Profile* profile, GPProfile gpProfile)
{
    ...
    // Check for full.
    //////////////////
    if (profile->numOthers == MAX_OTHERS) return;

    // Check if we already have this one.
    /////////////////////////////////////
    for (i = 0; i < profile->numOthers; i++)
        if (profile->others[i] == gpProfile) return;
...
/////////////////////
...
#else
        {
            ...
            const char* partnerchallenges[] = {"WI[A)IuA", "RNd$)q)0", "LQTi;4#X", "AOA)(57=", "ob=0XiWJ", "mRnTENJ@",
                "9%@A+NKA", "L=0O)Lg2", "YR6[Zz%B", "$kWZ%]~@"};
            int tokenIndex = (profile->index % 10);
            gpConnectPreAuthenticated(&profile->gp, authtokens[tokenIndex], partnerchallenges[tokenIndex], GP_FIREWALL,
    ...
    case OpProfileSearch:
        ...
        num = RandomInt(0, 3);
        if (num == 0)
            gpProfileSearch(&profile->gp, "pants", "dan@gamespy.com", NULL, NULL, 0, GP_NON_BLOCKING,
    ...
    case OpBuddyRequest: PrintOp(profile, "OpBuddyRequest");
        ...
        ////////////////////////////////
        if (profile->numOthers > 0) gpSendBuddyRequest(&profile->gp, RandomOther(profile), "STRESS-TEST");
#endif
        break;
    ...
    case OpSendBuddyMessage:
        ...
        /////////////////////////
        if (profile->numOthers > 0) MessageOther(profile, RandomOther(profile));
        break;
...
bool Validate(void)
{
    ...
    */
    ...
    for (i = 0; i < numProfiles; i++)
    {
        // Check for shutdown.
        //////////////////////
        if (current_time() > startShutdownTime) return false;
        ...
        if (validationData.result == GP_NO_ERROR) {
            ...
            for (n = 0; (n < validationData.numNicks) && !match; n++)
            {
                ...
                ////////////////////////////////////////////////
                if (strcasecmp(profile->nick, validationData.nicks[n]) == 0) match = true;
            }
...
////////////////////////
    ...
    if (CONNECTION_MANAGER[0]) strcpy(GPConnectionManagerHostname, CONNECTION_MANAGER);
#endif
    ...
    //////////////////////////////////////
    if (maxConnections > numProfiles) maxConnections = numProfiles;
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (strcasecmp(argv[i], "-m") == 0) {
            if (++i < argc) {
                ...
                if (maxConnections <= 0) maxConnections = DEFAULT_MAX_CONNECTED;
            }
        }
        ...
        else if (strcasecmp(argv[i], "-t") == 0)
        {
            if (++i < argc) {
                ...
                if (runTime <= 0) runTime = DEFAULT_RUN_TIME;
            }
        }
        ...
        else if (strcasecmp(argv[i], "-s") == 0)
        {
            if (++i < argc) {
                ...
                if (connectsPerSecond <= 0) connectsPerSecond = DEFAULT_CONNECTS_PER_SEC;
            }
        }
    }
