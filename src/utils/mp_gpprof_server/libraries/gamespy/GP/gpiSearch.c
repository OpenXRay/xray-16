...
static GPResult gpiStartProfileSearch(GPConnection* connection, GPIOperation* operation)
{
    ...
    if (data->inputBuffer.buffer == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
static GPResult gpiInitSearchData(GPConnection* connection, GPISearchData** searchData, int type)
{
    ...
    if (data == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    if (data->outputBuffer.buffer == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    GPConnection* connection, GPISearchData* data, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    //////////////////////////
    if (operation->blocking) CHECK_RESULT(gpiProcess(connection, operation->id));
    ...
    const int namespaceIDs[], int numNamespaces, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    // Error check.
    ///////////////
    if ((uniquenick == NULL) || (*uniquenick == '\0')) Error(connection, GP_PARAMETER_ERROR, "No search criteria.");
    ...
    const char password[GP_PASSWORD_LEN], GPEnum blocking, GPCallback callback, void* param)
{
    ...
    _strlwr(data->email);
    strzcpy(data->nick, nick, GP_NICK_LEN);
    if (password) strzcpy(data->password, password, GP_PASSWORD_LEN);
    ...
    GPEnum blocking, GPCallback callback, void* param)
{
    ...
    strzcpy(data->password, password, GP_PASSWORD_LEN);
    strzcpy(data->uniquenick, uniquenick, GP_UNIQUENICK_LEN);
    if (cdkey) strzcpy(data->cdkey, cdkey, GP_CDKEY_LEN);
            ...
            if (state == GPI_CONNECTED) {
                ...
                else if (data->type == GPI_SEARCH_PROFILE_UNIQUENICK)
                {
                    ...
                    if (data->numNamespaces > 0) {
                        ...
                        {
                            if (i > 0) gpiAppendCharToBuffer(connection, &data->outputBuffer, ',');
                    ...
                    do
                    {
                        ...
                        if (strcmp(key, "bsrdone") == 0) {
                            ...
                            if (strcmp(key, "more") == 0) {
                                ...
                                /////////////////////////////////////
                                if (strcmp(value, "0") != 0) arg.more = GP_MORE;
                            }
                        ...
                        else if (strcmp(key, "bsr") == 0)
                        {
                            ...
                            if (arg.matches == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
//////////////////////////////////
                    ...
                    // Call the callback.
                    /////////////////////
                    if (callback.callback != NULL) callback.callback(connection, &arg, callback.param);
...
//////////
                ...
                else if (data->type == GPI_SEARCH_NEWUSER)
                {
                    ...
                    if (callback.callback != NULL) {
                        ...
                        rcode = atoi(value);
                        if (rcode) iconnection->errorCode = (GPErrorCode)rcode;
                        if (!gpiValueForKey(data->inputBuffer.buffer, "\\pid\\", value, sizeof(value))) {
                            if (rcode == 0)
                                CallbackFatalError(
                                    connection, GP_NETWORK_ERROR, GP_PARSE, "Error reading from the search server.");
                ...
                else if (data->type == GPI_SEARCH_SUGGEST_UNIQUE)
                {
                    ...
                    if (callback.callback != NULL) {
                        ...
                        if (!arg->suggestedNicks) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiProcessSearches(GPConnection* connection)
{
    ...
    if (iconnection->numSearches > 0) {
        ...
        if (searchList == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
        ...
        for (i = 0; i < num; i++)
        {
            ...
            if (result != GP_NO_ERROR) searchList[i]->result = result;
        }
        ...
        for (i = 0; i < num; i++)
        {
            ...
            data->processing = GPIFalse;
            if (data->remove) gpiRemoveOperation(connection, searchList[i]);
        }
