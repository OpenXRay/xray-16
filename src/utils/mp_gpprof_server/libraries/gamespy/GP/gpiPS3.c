...
GPResult gpiCheckNpStatus(GPConnection* connection)
{
    ...
    if (status == SCE_NP_MANAGER_STATUS_ONLINE) {
        ...
        if (!iconnection->npTransactionList) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    }
...
GPResult gpiDestroyNpBasic(GPConnection* connection)
{
    ...
    //////////////////////////////////////////////////////
    if (iconnection->npLookupTitleCtxId >= 0) sceNpLookupDestroyTitleCtx(iconnection->npLookupTitleCtxId);

    // Do not destroy NpLookup or NpBasic if Game is using it
    /////////////////////////////////////////////////////////
    if (!iconnection->npLookupGameInitialized) sceNpLookupTerm();
    ...
    ///////////////////////////////////////////////
    if (iconnection->npTransactionList) ArrayFree(iconnection->npTransactionList);
...
GPResult gpiProcessNp(GPConnection* connection)
{
    ...
    //////////////////////////////////////////////
    if (!iconnection->npTransactionList) return GP_NO_ERROR;

    // Need to process Sysutil for the Async lookups
    /////////////////////////////////////////////////
    if (ArrayLength(iconnection->npTransactionList) > 0) cellSysutilCheckCallback();
    ...
    for (i = 0; i < ArrayLength(iconnection->npTransactionList); i++)
    {
        ...
        if (!transaction->npLookupDone) {
            if (sceNpLookupPollAsync(transaction->npTransId, &ret) == 0) transaction->npLookupDone = gsi_true;
        }
        else
        {
            if (ret < 0) {
