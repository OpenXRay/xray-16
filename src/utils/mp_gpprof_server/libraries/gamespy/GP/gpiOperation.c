    ...
    GPCallback callback, void* param)
{
    ...
    if (operation == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
    ...
    else
    {
        ...
        if (iconnection->nextOperationID < 2) iconnection->nextOperationID = 2;
    }
...
GPIBool gpiOperationsAreBlocking(const GPConnection* connection)
{
    ...
    for (operation = iconnection->operationList; operation != NULL; operation = operation->pnext)
    {
        ...
        //////////////////////////
        if ((operation->blocking) && (operation->type != GPI_PROFILE_SEARCH)) return GPITrue;
    }
...
GPResult gpiProcessOperation(GPConnection* connection, GPIOperation* operation, const char* input)
{
    ...
    if (result != GP_NO_ERROR) operation->result = result;
