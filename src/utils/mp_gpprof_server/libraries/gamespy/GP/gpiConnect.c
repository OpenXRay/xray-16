    ...
    GPIBool newuser, GPEnum blocking, GPCallback callback, void* param)
{
    ...
    /////////////////////////////////////////////
    if (iconnection->connectState == GPI_DISCONNECTED) CHECK_RESULT(gpiReset(connection));

    // Error check.
    ///////////////
    if (iconnection->connectState != GPI_NOT_CONNECTED) Error(connection, GP_PARAMETER_ERROR, "Invalid connection.");
...
GPResult gpiProcessConnect(GPConnection* connection, GPIOperation* operation, const char* input)
{
    ...
    if (gpiCheckForError(connection, input, GPIFalse)) {
        ...
        else if (iconnection->errorCode == GP_NEWUSER_BAD_NICK)
        {
            // Store the pid.
            /////////////////
            if (gpiValueForKey(input, "\\pid\\", buffer, sizeof(buffer))) iconnection->profileid = atoi(buffer);
        }
    ...
    case GPI_LOGIN:
        ...
        // Get the uniquenick.
        //////////////////////
        if (!gpiValueForKey(input, "\\uniquenick\\", uniquenick, sizeof(uniquenick))) uniquenick[0] = '\0';
...
////////////////////////////////
        ...
        if (!iconnection->npInitialized) gpiInitializeNpBasic(connection);
#endif
...
GPResult gpiCheckConnect(GPConnection* connection)
{
    ...
    ////////////////////////////////////
    if (state == GPI_NOT_CONNECTED) return GP_NO_ERROR;
...
static GPIBool gpiDisconnectCleanupProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    ...
    if (profile->blocked) profile->buddyOrBlockCache = gsi_true;
...
void gpiDisconnect(GPConnection* connection, GPIBool tellServer)
{
    ...
    // PANTS|05.15.00
    ///////////////////////////////////////
    if (iconnection->connectState == GPI_DISCONNECTED) return;
    ...
    if (iconnection->connectState != GPI_NOT_CONNECTED) {
        ...
        if (/*iconnection->peerSocket != INVALID_SOCKET*/ gsUdpEngineIsInitialized()) {
            ...
            if (gsUdpEngineNoMoreMsgHandlers() && gsUdpEngineNoApp()) gsUdpEngineShutdown();
        }
