...
GPResult gpiInitialize(GPConnection* connection, int productID, int namespaceID, int partnerID)
{
    ...
    *connection = NULL;
    ...
    if (iconnection == NULL) return GP_MEMORY_ERROR;
...
void gpiDestroy(GPConnection* connection)
{
    ...
    // Destroy NP
    /////////////
    if (iconnection->npInitialized) gpiDestroyNpBasic(connection);
#endif
    ...
    do
    {
        ...
        while ((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
        {
            ...
            if (len > iconnection->inputBufferSize) {
                ...
                if (tempPtr == NULL) Error(connection, GP_MEMORY_ERROR, "Out of memory.");
...
GPResult gpiProcess(GPConnection* connection, int blockingOperationID)
{
    ...
    */
        ...
        (iconnection->connectState == GPI_PROFILE_DELETING))
    {
        ...
        ////////////////////////////////////////////////////////////////////////////////
        if (iconnection->npInitialized && !iconnection->npStatusRetrieved) gpiCheckNpStatus(connection);
        ...
        if (iconnection->npInitialized && iconnection->npStatusRetrieved) {
            ...
            if ((current_time() - iconnection->loginTime) > GPI_NP_SYNC_DELAY) {
                if (iconnection->npPerformBuddySync) gpiSyncNpBuddies(connection);
                if (iconnection->npPerformBlockSync) gpiSyncNpBlockList(connection);
            }
        ...
        //////////////////////////
        if (result == GP_NO_ERROR) result = gpiProcessConnectionManager(connection);

        // Process peer messaging stuff.
        ////////////////////////////////
        if (result == GP_NO_ERROR) result = gpiProcessPeers(connection);

#ifndef NOFILE
        // Process transfers.
        /////////////////////
        if (result == GP_NO_ERROR) result = gpiProcessTransfers(connection);
#endif
    }

    // Process searches.
    ////////////////////
    if (result == GP_NO_ERROR) result = gpiProcessSearches(connection);
...
static GPIBool gpiReportProfile(GPConnection* connection, GPIProfile* profile, void* data)
{
    ...
    if (profile->buddyStatus) {
        ...
        if (profile->buddyStatus->statusString) temp += (int)(strlen(profile->buddyStatus->statusString) + 1);
        if (profile->buddyStatus->locationString) temp += (int)(strlen(profile->buddyStatus->locationString) + 1);
#ifdef GSI_UNICODE
...
//			temp += (wcslen(profile->buddyStatus->locationString_W) + 2);
    ...
    if (profile->cache) {
        ...
        if (profile->cache->nick) temp += (int)(strlen(profile->cache->nick) + 1);
        if (profile->cache->uniquenick) temp += (int)(strlen(profile->cache->uniquenick) + 1);
        if (profile->cache->email) temp += (int)(strlen(profile->cache->email) + 1);
        if (profile->cache->firstname) temp += (int)(strlen(profile->cache->firstname) + 1);
        if (profile->cache->lastname) temp += (int)(strlen(profile->cache->lastname) + 1);
        if (profile->cache->homepage) temp += (int)(strlen(profile->cache->homepage) + 1);
        nInfoMemory += temp;
        nTotalMemory += temp;
    }
