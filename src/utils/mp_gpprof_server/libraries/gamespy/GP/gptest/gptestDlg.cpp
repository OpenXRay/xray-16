    ...
    const char* theTokenStr, va_list theParamList)
{
    ...
    vsDebugOut(NULL, "[%s][%s] ", gGSIDebugCatStrings[theCat], gGSIDebugTypeStrings[theType]);
...
CGptestDlg::CGptestDlg(CWnd* pParent /*=NULL*/)
    : CDialog(CGptestDlg::IDD, pParent), m_StatusState(0), m_QueryPort(0), m_HostPort(0), m_RichStatus(_T("")),
      m_GameType(_T("")), m_GameVariant(_T("")), m_GameMapname(_T("")), m_KeyName(_T("")), m_KeyValue(_T("")),
      m_HostIp(_T("")), m_HostPrivateIp(_T("")), m_cdkey(_T(""))
{
...
void ConnectResponse(GPConnection* connection, void* arg_, void* param)
{
    ...
    if (arg->result == GP_NO_ERROR) {
        ...
        // gpSetStatusInfo(connection, (GPEnum)dlg->m_StatusState, 0, 0, 0, 0, 0, dlg->m_RichStatus,
        // dlg->m_RichStatus.GetLength(), "", 0, "", 0, "", 0);
        CString text;
...
void RecvBuddyStatus(GPConnection* connection, void* arg_, void* param)
{
    ...
    for (i = 0; i < count; i++)
        if (dlg->m_buddies.GetItemData(i) == (DWORD)arg->profile) dlg->m_buddies.DeleteString(i);
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (text.Left(3).Compare("ON:") != 0) break;
    }
...
void Whois(GPConnection* connection, void* arg_, void* param)
{
    ...
    if (arg->result == GP_NO_ERROR) {
        ...
        // Email.
        /////////
        case 2: strcpy(whois, arg->email); break;
        }
    }
    else
    {
...
void GetReverseBuddiesResponse(GPConnection* connection, void* arg_, void* param)
{
    ...
    if (arg->result == GP_NO_ERROR) {
        ...
        for (i = 0; i < arg->numProfiles; i++)
        {
            ...
            str.Format("Revoke %s [%s] (%s %s, %s, %d)?", match->nick, match->uniquenick, match->firstname,
                match->lastname, match->email, match->profile);
...
LPCSTR TypeToString(GPEnum type)
{
    switch (type)
    {
    case GP_TRANSFER_SEND_REQUEST: return "GP_TRANSFER_SEND_REQUEST";
    case GP_TRANSFER_ACCEPTED: return "GP_TRANSFER_ACCEPTED";
    case GP_TRANSFER_REJECTED: return "GP_TRANSFER_REJECTED";
    case GP_TRANSFER_NOT_ACCEPTING: return "GP_TRANSFER_NOT_ACCEPTING";
    case GP_TRANSFER_NO_CONNECTION: return "GP_TRANSFER_NO_CONNECTION";
    case GP_TRANSFER_DONE: return "GP_TRANSFER_DONE";
    case GP_TRANSFER_CANCELLED: return "GP_TRANSFER_CANCELLED";
    case GP_TRANSFER_LOST_CONNECTION: return "GP_TRANSFER_LOST_CONNECTION";
    case GP_TRANSFER_ERROR: return "GP_TRANSFER_ERROR";
    case GP_TRANSFER_THROTTLE: return "GP_TRANSFER_THROTTLE";
    case GP_FILE_BEGIN: return "GP_FILE_BEGIN";
    case GP_FILE_PROGRESS: return "GP_FILE_PROGRESS";
    case GP_FILE_END: return "GP_FILE_END";
    case GP_FILE_DIRECTORY: return "GP_FILE_DIRECTORY";
    case GP_FILE_SKIP: return "GP_FILE_SKIP";
    case GP_FILE_FAILED: return "GP_FILE_FAILED";
    }
...
void TransferCallback(GPConnection* connection, void* arg_, void* param)
{
    ...
    case GP_TRANSFER_ERROR: gpFreeTransfer(connection, arg->transfer); break;
    }
    GSI_UNUSED(connection);
    GSI_UNUSED(param);
}
...
void CGptestDlg::CodeToString(CString& string)
{
    ...
    switch (code)
    {
        CtoS(GP_GENERAL) CtoS(GP_PARSE) CtoS(GP_NOT_LOGGED_IN) CtoS(GP_BAD_SESSKEY) CtoS(GP_DATABASE) CtoS(GP_NETWORK)
            CtoS(GP_FORCED_DISCONNECT) CtoS(GP_CONNECTION_CLOSED) CtoS(GP_LOGIN) CtoS(GP_LOGIN_TIMEOUT) CtoS(
                GP_LOGIN_BAD_NICK) CtoS(GP_LOGIN_BAD_EMAIL) CtoS(GP_LOGIN_BAD_PASSWORD) CtoS(GP_LOGIN_BAD_PROFILE)
                CtoS(GP_LOGIN_PROFILE_DELETED) CtoS(GP_LOGIN_CONNECTION_FAILED) CtoS(GP_LOGIN_SERVER_AUTH_FAILED) CtoS(
                    GP_LOGIN_BAD_UNIQUENICK) CtoS(GP_LOGIN_BAD_PREAUTH) CtoS(GP_NEWUSER) CtoS(GP_NEWUSER_BAD_NICK)
                    CtoS(GP_NEWUSER_BAD_PASSWORD) CtoS(GP_NEWUSER_UNIQUENICK_INVALID) CtoS(GP_NEWUSER_UNIQUENICK_INUSE)
                        CtoS(GP_UPDATEUI) CtoS(GP_UPDATEUI_BAD_EMAIL) CtoS(GP_NEWPROFILE) CtoS(GP_NEWPROFILE_BAD_NICK)
                            CtoS(GP_NEWPROFILE_BAD_OLD_NICK) CtoS(GP_UPDATEPRO) CtoS(GP_UPDATEPRO_BAD_NICK)
                                CtoS(GP_ADDBUDDY) CtoS(GP_ADDBUDDY_BAD_FROM) CtoS(GP_ADDBUDDY_BAD_NEW)
                                    CtoS(GP_ADDBUDDY_ALREADY_BUDDY) CtoS(GP_AUTHADD) CtoS(GP_AUTHADD_BAD_FROM)
                                        CtoS(GP_AUTHADD_BAD_SIG) CtoS(GP_STATUS) CtoS(GP_BM) CtoS(GP_BM_NOT_BUDDY)
                                            CtoS(GP_GETPROFILE) CtoS(GP_GETPROFILE_BAD_PROFILE) CtoS(GP_DELBUDDY)
                                                CtoS(GP_DELBUDDY_NOT_BUDDY) CtoS(GP_DELPROFILE)
                                                    CtoS(GP_DELPROFILE_LAST_PROFILE) CtoS(GP_SEARCH)
                                                        CtoS(GP_SEARCH_CONNECTION_FAILED) CtoS(GP_CHECK)
                                                            CtoS(GP_CHECK_BAD_EMAIL) CtoS(GP_CHECK_BAD_NICK)
                                                                CtoS(GP_CHECK_BAD_PASSWORD) CtoS(GP_REGISTERUNIQUENICK)
                                                                    CtoS(GP_REGISTERUNIQUENICK_TAKEN)
                                                                        CtoS(GP_REGISTERUNIQUENICK_RESERVED)
                                                                            CtoS(GP_REGISTERUNIQUENICK_BAD_NAMESPACE)
                                                                                CtoS(GP_REGISTERCDKEY_BAD_KEY) CtoS(
...
void CGptestDlg::OnConnect()
{
    ...
    SetHost();
    CHECK(gpConnect(
        &m_connection, m_nick, m_email, m_password, (GPEnum)m_firewall, (GPEnum)m_blocking, ConnectResponse, NULL));
}
...
void CGptestDlg::OnConnectunique()
{
    ...
    CHECK(gpConnectUniqueNick(
        &m_connection, m_uniquenick, m_password, (GPEnum)m_firewall, (GPEnum)m_blocking, ConnectResponse, NULL));
}
...
void CGptestDlg::OnConnectpreauth()
{
    ...
    CHECK(gpConnectPreAuthenticated(
        &m_connection, m_authtoken, m_partnerchallenge, (GPEnum)m_firewall, (GPEnum)m_blocking, ConnectResponse, NULL));
}
...
void CGptestDlg::OnNewuser()
{
    ...
    SetHost();
    CHECK(gpNewUser(
        &m_connection, m_nick, m_uniquenick, m_email, m_password, NULL, (GPEnum)m_blocking, NewUserResponse, NULL));
    //	CHECK(gpConnectNewUser(&m_connection, m_nick, m_uniquenick, m_email, m_password, NULL, (GPEnum)m_firewall,
...
void CGptestDlg::OnSearch()
{
    ...
    CHECK(gpProfileSearch(&m_connection, m_snick, m_suniquenick, m_semail, m_sfirstname, m_slastname, m_sicquin,
...
void CGptestDlg::OnSetinfo()
{
    ...
    if (!m_icountrycode.IsEmpty()) gpSetInfos(&m_connection, GP_COUNTRYCODE, m_icountrycode);
    gpSetInfod(&m_connection, GP_BIRTHDAY, m_ibirthday, m_ibirthmonth, m_ibirthyear);
    gpSetInfos(&m_connection, GP_SEX, m_isex);
    mask = 0;
    if (m_ipmhomepage) mask |= GP_MASK_HOMEPAGE;
    if (m_ipmzipcode) mask |= GP_MASK_ZIPCODE;
    if (m_ipmcountrycode) mask |= GP_MASK_COUNTRYCODE;
    if (m_ipmbirthday) mask |= GP_MASK_BIRTHDAY;
    if (m_ipmsex) mask |= GP_MASK_SEX;
    if (m_ipmemail) mask |= GP_MASK_EMAIL;
...
void CGptestDlg::OnNewpro()
{
    ...
    UpdateData();
    CHECK(gpNewProfile(&m_connection, m_newnick, m_replace ? GP_REPLACE : GP_DONT_REPLACE, (GPEnum)m_blocking,
        NewProfileResponse, NULL));
}
...
void CGptestDlg::OnSetstatusinfo()
{
    ...
    gpSetStatusInfo(&m_connection, aStatusState, aHostIp, aHostPrivateIp, aQueryPort, aHostPort, sessFlags,
    (LPCSTR)m_RichStatus,
        m_RichStatus.GetLength(), (LPCSTR)m_GameType, m_GameType.GetLength(), (LPCSTR)m_GameVariant,
...
void CGptestDlg::OnRegisterCdKey()
{
    ...
    if (m_cdkey.GetLength() > GP_CDKEY_LEN) return;
...
void CGptestDlg::OnAddBlock()
{
    ...
    if (index != LB_ERR) {
        ...
        int i;
        for (i = 0; i < count; i++)
            if (m_buddies.GetItemData(i) == (DWORD)profile) m_buddies.DeleteString(i);
