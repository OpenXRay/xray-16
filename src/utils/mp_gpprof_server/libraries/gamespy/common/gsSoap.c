...
void gsiCancelSoap(GSSoapTask* theTask)
{
    ...
    if (gsi_is_false(theTask->mCompleted)) gsiCoreCancelTask(theTask->mCoreTask);
}
...
static void gsiSoapTaskExecute(void* theTask)
{
    ...
    if (aSoapTask->mCustomFunc != NULL) (aSoapTask->mCustomFunc)(aSoapTask->mPostData, aSoapTask->mUserData);
...
static void gsiSoapTaskCancel(void* theTask)
{
    ...
    if (gsi_is_false(soapTask->mCompleted)) {
        if (soapTask->mRequestId >= 0) ghttpCancelRequest(soapTask->mRequestId);
