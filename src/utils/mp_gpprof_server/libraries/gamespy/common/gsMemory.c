...
{
    if (!_this->next) return PTR_ALIGNMENT; /*Nub*/
    ;
...
// return NULL if no next free chunk.
{
    ...
    if (header == header_in) return NULL;
...
// update some stats
    ...
    if (ReAlloc) {
        ...
        if (_this->MemUsed > _this->HWMemUsed) _this->HWMemUsed = _this->MemUsed;
...
// returns true if this is a valid heap ptr
{
    ...
    {
        header = header->next;
        if (headertofind == header) return gsi_true;
    }
...
//--------------------------------------------------------------------------
{
    ...
    if (destptr < (gsi_u32)prev) ChunkMove(prev, (MEM_CHUNK*)destptr);
#endif
}
...
void MEM_CHUNK_POOLMemStatsGet(MEM_CHUNK_POOL* _this, MEM_STATS* pS)
{
    ...
    // Update stats
    if (pS->MemUsed_At_HighWater < pS->MemUsed) pS->MemUsed_At_HighWater = pS->MemUsed;
