...
#define GS_XML_CHECK(a)                                                                                                \
    {                                                                                                                  \
        if (gsi_is_false(a)) return gsi_false;                                                                         \
    }
...
gsi_bool gsXmlParseBuffer(GSXmlStreamReader stream, char* data, int len)
{
    ...
    while (readPos < len)
    {
        if (gsi_is_false(gsiXmlUtilParseElement(reader, data, len, &readPos, -1))) return gsi_false;
    }
...
gsi_bool gsXmlCloseWriter(GSXmlStreamWriter stream)
{
    ...
    if (gsi_is_false(gsiXmlUtilWriteString(writer, GS_XML_SOAP_FOOTER))) return gsi_false;
...
static gsi_bool gsiXmlUtilSkipWhiteSpace(GSIXmlStreamReader* stream, const char* buffer, int len, int* pos)
{
    ...
    while (*pos < len)
    {
        if (NULL == strchr(GS_XML_WHITESPACE, buffer[*pos])) return gsi_true;
...
static gsi_bool gsiXmlUtilParseElement(GSIXmlStreamReader* stream, char* buffer, int len, int* pos, int parentIndex)
{
    ...
    if (*pos >= len) return gsi_true;  // legal EOF

    memset(&newElem, 0, sizeof(newElem));

    // elements must begin with '<'
    if (buffer[*pos] != '<') return gsi_false;
    (*pos)++;
    if (*pos >= len) return gsi_false;
    ...
    if ((!isalnum(buffer[startPos + 1]) && buffer[*pos] == buffer[startPos + 1]) || buffer[*pos] == '/') {
        (*pos)++;
        GS_XML_CHECK(gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos));
        if (buffer[*pos] != '>') return gsi_false;  // only legal character here is closing brace
        (*pos)++;
    ...
    if (buffer[*pos] != '<') {
        ...
        if (storeElement) ArrayReplaceAt(stream->mElementArray, &newElem, newElem.mIndex);
        GS_XML_CHECK(gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos));
    }
    ...
    while (*pos < len)
    {
        ...
        if (buffer[*pos] == '/') {
            ...
            (*pos)++;
            GS_XML_CHECK(gsiXmlUtilSkipWhiteSpace(stream, buffer, len, pos));
            if ((*pos) + newElem.mName.mLen >= len) return gsi_false;  // EOF before tag close
        ...
        {
            if (newElem.mValue.mData != NULL) return gsi_false;  // elements with value cannot have children
    ...
    GSIXmlStreamReader* stream, const char* buffer, int len, int* pos, GSIXmlString* strOut)
{
    ...
    if (!isalnum(buffer[*pos]) || isdigit(buffer[*pos])) return gsi_false;
    ...
    while (*pos < len && NULL == strchr(GS_XML_WHITESPACE, buffer[*pos]))
    {
        ...
        if (buffer[*pos] == ':') {
            if (gsi_is_true(haveNamespace)) return gsi_false;  // already have a namespace!
...
static gsi_bool gsiXmlUtilParseString(GSIXmlStreamReader* stream, char* buffer, int len, int* pos, GSIXmlString* strOut)
{
    ...
    startCh = buffer[*pos];
    if (startCh != '\"' && startCh != '\'') return gsi_false;

    (*pos)++;
    strStart = &buffer[*pos];  // remember this for easier processing below
    strOut->mLen = 0;

    if (*pos >= len) return gsi_false;  // EOF when looking for string terminator
    ...
    if (gsi_is_false(gsiXmlUtilDecodeString(strStart, &strOut->mLen))) return gsi_false;
...
static gsi_bool gsiXmlUtilDecodeString(char* buffer, int* len)
{
    ...
    while (readPos < *len)
    {
        ...
        else if (strncmp(&buffer[readPos], "&#x", 3) == 0)
        {
            ...
            if (digitEnd == NULL) return gsi_false;                     // missing ';'
            if (digitEnd - &buffer[readPos + 3] > 8) return gsi_false;  // too many digits before end
    ...
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const gsi_u8* data, int len)
{
    ...
    if (gsi_is_false(gsXmlWriteOpenTag(stream, namespaceName, tag))) return gsi_false;
    ...
    while (pos < len)
    {
        ...
        pos++;
        if (gsi_is_false(gsiXmlUtilWriteChar(writer, hex[0]))) return gsi_false;
        if (gsi_is_false(gsiXmlUtilWriteChar(writer, hex[1]))) return gsi_false;
    }

    if (gsi_is_false(gsXmlWriteCloseTag(stream, namespaceName, tag))) return gsi_false;
    ...
    GSXmlStreamWriter stream, const char* namespaceName, const char* tag, const gsi_u8* data, int len)
{
    ...
    {
        b64[4] = '\0';
        if (gsi_is_false(gsiXmlUtilWriteString(writer, b64))) return gsi_false;
    }
...
static gsi_bool gsiXmlUtilWriteChar(GSIXmlStreamWriter* stream, char ch)
{
    ...
    if (stream->mLen >= stream->mCapacity) {
        if (gsi_is_false(gsiXmlUtilGrowBuffer(stream))) return gsi_false;  // OOM
    }
...
static gsi_bool gsiXmlUtilWriteUnicodeString(GSIXmlStreamWriter* stream, const unsigned short* str)
{
    ...
    for (pos = 0; pos < strLen; pos++)
    {
        ...
        if (gsi_is_false(gsiXmlUtilWriteXmlSafeString(stream, utf8String))) return gsi_false;
    }
    return gsi_true;
}
...
static gsi_bool gsiXmlUtilWriteXmlSafeString(GSIXmlStreamWriter* stream, const char* str)
{
    ...
    for (pos = 0; pos < strLen; pos++)
    {
        ...
        if (gsi_is_false(result)) return gsi_false;
    }
    return gsi_true;
}
...
static gsi_bool gsiXmlUtilGrowBuffer(GSIXmlStreamWriter* stream)
{
    ...
    if (newBuf == NULL) return gsi_false;
    if (newBuf != stream->mBuffer) stream->mBuffer = (char*)newBuf;
...
gsi_bool gsXmlMoveToParent(GSXmlStreamReader stream)
{
    ...
    else
    {
        ...
        if (elem->mParentIndex == -1) return gsi_false;  // current elem is at highest level
        if (elem->mParentIndex >= ArrayLength(reader->mElementArray)) return gsi_false;  // parent is invalid!
...
gsi_bool gsXmlMoveToSibling(GSXmlStreamReader stream, const char* matchtag)
{
    ...
    for (i = (reader->mElemReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchElem->mParentIndex < reader->mElemReadIndex) return gsi_false;
    }
...
gsi_bool gsXmlReadChildAsString(GSXmlStreamReader stream, const char* matchtag, const char** valueOut, int* lenOut)
{
    ...
    if (reader->mValueReadIndex == -1) reader->mValueReadIndex = reader->mElemReadIndex;  // start at current element
    ...
    for (i = (reader->mValueReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex < reader->mElemReadIndex) return gsi_false;
    }
    return gsi_false;
}
    ...
    for (i = (reader->mValueReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex == reader->mElemReadIndex) {
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName))) {
                ...
                if (lenOut != NULL) *lenOut = writepos;
...
gsi_bool gsXmlReadChildAsInt(GSXmlStreamReader stream, const char* matchtag, int* valueOut)
{
    ...
    for (i = (reader->mValueReadIndex + 1); i < ArrayLength(reader->mElementArray); i++)
    {
        ...
        if (searchValueElem->mParentIndex == reader->mElemReadIndex) {
            ...
            if (gsi_is_true(gsiXmlUtilTagMatches(matchtag, &searchValueElem->mName))) {
                reader->mValueReadIndex = i;
                if (searchValueElem->mValue.mData == NULL) return gsi_false;  // invalid type!
...
static gsi_bool gsiXmlUtilTagMatches(const char* matchtag, GSIXmlString* xmlstr)
{
    ...
    if (matchtag == NULL) return gsi_true;
    if (matchtag[strlen(matchtag) - 1] == ':') return gsi_false;  // illegal to end with ':'

    // find post-namespace positions
    matchNoNamespace = strchr(matchtag, ':');
    if (matchNoNamespace == NULL) matchNoNamespace = matchtag;
...
gsi_bool gsXmlReadChildAsLargeInt(GSXmlStreamReader stream, const char* matchtag, struct gsLargeInt_s* valueOut)
{
    ...
    if (len % GS_LARGEINT_DIGIT_SIZE_BYTES != 0) valueOut->mLength++;
