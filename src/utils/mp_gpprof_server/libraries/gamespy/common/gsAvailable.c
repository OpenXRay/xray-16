...
void GSIStartAvailableCheckA(const char* gamename)
{
    ...
    if (!override) sprintf(hostname, "%s.available." GSI_DOMAIN_NAME, gamename);
    ...
    if (AC.sock == INVALID_SOCKET) return;
...
static int HandlePacket(char* packet, int len, SOCKADDR_IN* address, int* disabledservices)
{
    ...
    // check the IP
    if (memcmp(&address->sin_addr, &AC.address.sin_addr, sizeof(IN_ADDR)) != 0) return 1;

    // check the port
    if (address->sin_port != AC.address.sin_port) return 1;

    // check the header
    if (memcmp(packet, "\xFE\xFD\x09", 3) != 0) return 1;
