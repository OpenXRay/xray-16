...
static void ShowMenuScreen(void)
{
    ...
    if (keyboard && list) OS_Panic("Can't have a keyboard and a list on a menu screen\n");

    // call the init func
    if (configuration->initFunc) {
        configuration->initFunc();
        if (NextMenuScreenConfiguration) return;
    }

    // if there is a keyboard, get the initial position
    if (keyboard) keyboardPos = (int)strlen(Screen.keyboardText);
    ...
    while (!done)
    {
        ...
        if (list) {
            ...
            {
                if (Screen.list[numListItems][0] == '\0') break;
            }
        }
        ...
        else if (wasTouching)
        {
            ...
            {
                if (listScroll > 0) listScroll--;
            }
            // check for touching down on the list
            else if (wasTouchingDown)
            {
                if (listScroll < (numListItems - numListRows)) listScroll++;
            }
        ...
        if (list) {
            ...
            for (i = 0; i < numListRows; i++)
            {
                ...
                if (listItem >= numListItems) break;

                // is this item being touched?
                if (touching && (touchingLine == (startListLine + 2 + i))) Screen.listSelection = listItem;
        ...
        if (keyboard) {
            ...
            for (i = 0; i < numKeyboardRows; i++)
            {
                ...
                if (touching && (touchingLine == keyboardLine)) {
                    ...
                    {
                        if (touchingChar != ' ') wasTouchingChar = touchingChar;
