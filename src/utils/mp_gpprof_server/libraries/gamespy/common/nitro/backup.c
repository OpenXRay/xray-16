...
static const CARDBackupType BackupTypes[] = {
    CARD_BACKUP_TYPE_EEPROM_4KBITS, CARD_BACKUP_TYPE_EEPROM_64KBITS, CARD_BACKUP_TYPE_EEPROM_512KBITS,
...
static const char* BackupTypeDescriptions[] = {"EEPROM 4 kb", "EEPROM 64 kb", "EEPROM 512 kb",
#if defined(SUPPORT_FLASH)
    "FLASH 2 mb",
#endif
#if defined(SUPPORT_FRAM)
...
void BackupInit(void)
{
    ...
    if (lockID == OS_LOCK_ID_ERROR) OS_Panic("OS_GetLockID() failed\n");
    LockID = (u16)lockID;
