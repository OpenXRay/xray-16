...
static MenuScreenConfiguration msInitializingWireless = {"Initializing Wireless", {{mscCancel}},
    InitializingWirelessInit, InitializingWirelessChose, InitializingWirelessThink, SCREEN_OPTION_ANIMATED};

static void FailedToInitializeWirelessInit(void);
static void FailedToInitializeWirelessChose(const char* choice);

static MenuScreenConfiguration msFailedToInitializeWireless = {"Failed to Initialize Wireless", {{mscOK}},
    FailedToInitializeWirelessInit, FailedToInitializeWirelessChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};

static void SelectNetworkConfigurationInit(void);
static void SelectNetworkConfigurationChose(const char* choice);

static MenuScreenConfiguration msSelectNetworkConfiguration = {"Select Network Configuration",
    {{mscUseSelectedConfiguration, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscCreateNewConfiguration},
        {mscRenameSelectedConfiguration, CHOICE_OPTION_NEEDS_LIST_SELECTION},
        {mscDeleteSelectedConfiguration, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscExit}},
    SelectNetworkConfigurationInit, SelectNetworkConfigurationChose, NULL, SCREEN_OPTION_LIST};

static void RenameConfigurationInit(void);
static void RenameConfigurationChose(const char* choice);

static MenuScreenConfiguration msRenameConfiguration = {"Rename Configuration", {{mscRename}, {mscCancel}},
    RenameConfigurationInit, RenameConfigurationChose, NULL, SCREEN_OPTION_KEYBOARD | SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msSearchingForNetworks = {"Searching for Networks",
    {{mscConnectToSelectedNetwork, CHOICE_OPTION_NEEDS_LIST_SELECTION},
        {mscViewNetworkInformation, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscCancel}},
    SearchingForNetworksInit, SearchingForNetworksChose, SearchingForNetworksThink,
...
static MenuScreenConfiguration msSearchingForSavedNetwork = {"Searching for Saved Network", {{mscCancel}},
    SearchingForSavedNetworkInit, SearchingForSavedNetworkChose, SearchingForSavedNetworkThink, SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msNetworkInformation = {
    "Network Information", {{mscBack}}, NetworkInformationInit, NetworkInformationChose, NULL};
...
static MenuScreenConfiguration msSelectSecurityType = {"Select Security Type",
    {{mscUseKeyType, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscBack}}, SelectSecurityTypeInit, SelectSecurityTypeChose,
    NULL, SCREEN_OPTION_LIST};

static void EnterSecurityKeyInit(void);
static void EnterSecurityKeyChose(const char* choice);

static MenuScreenConfiguration msEnterSecurityKey = {"Enter Security (WEP) Key", {{mscEnter}, {mscBack}},
    EnterSecurityKeyInit, EnterSecurityKeyChose, NULL, SCREEN_OPTION_KEYBOARD};

static void BadSecurityKeyEnteredInit(void);
static void BadSecurityKeyEnteredChose(const char* choice);

static MenuScreenConfiguration msBadSecurityKeyEntered = {"Bad Security Key Entered", {{mscBack}},
    BadSecurityKeyEnteredInit, BadSecurityKeyEnteredChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConnectingToNetwork = {"Connecting to Network", {{mscCancel}}, ConnectingToNetworkInit,
    ConnectingToNetworkChose, ConnectingToNetworkThink, SCREEN_OPTION_ANIMATED};

static void FailedToConnectToNetworkInit(void);
static void FailedToConnectToNetworkChose(const char* choice);

static MenuScreenConfiguration msFailedToConnectToNetwork = {"Failed to Connect to Network", {{mscBack}},
    FailedToConnectToNetworkInit, FailedToConnectToNetworkChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConnectingToInternet = {"Connecting to Internet", {{mscCancel}},
    ConnectingToInternetInit, ConnectingToInternetChose, ConnectingToInternetThink, SCREEN_OPTION_ANIMATED};

static void FailedToConnectToInternetInit(void);
static void FailedToConnectToInternetChose(const char* choice);

static MenuScreenConfiguration msFailedToConnectToInternet = {"Failed to Connect to Internet", {{mscBack}},
    FailedToConnectToInternetInit, FailedToConnectToInternetChose, NULL, SCREEN_OPTION_EXTRAS_CENTERED};
...
static MenuScreenConfiguration msConnectedToInternet = {
    "Connected to Internet", {{mscContinue}}, ConnectedToInternetInit, ConnectedToInternetChose};

static void SaveNetworkConfigurationInit(void);
static void SaveNetworkConfigurationChose(const char* choice);

static MenuScreenConfiguration msSaveNetworkConfiguration = {"Save Network Configuration", {{mscSave}, {mscDontSave}},
    SaveNetworkConfigurationInit, SaveNetworkConfigurationChose, NULL, SCREEN_OPTION_KEYBOARD};
...
{
    if (result == IW_RESULT_SUCCESS) return "success";
    if (result == IW_RESULT_FAILURE) return "failure";
    if (result == IW_RESULT_PROGRESS) return "progress";
    if (result == IW_RESULT_ACCEPT) return "accept";
    if (result == IW_RESULT_REJECT) return "reject";
    if (result == IW_RESULT_WMDISABLE) return "wmdisable";
    if (result == IW_RESULT_MEMORYERROR) return "memoryerror";
    if (result == IW_RESULT_FATALERROR) return "fatalerror";
    return "unknown result";
}

static const char* IWNotifyToString(int notify)
{
    if (notify == IW_NOTIFY_COMMON) return "common";
    if (notify == IW_NOTIFY_STARTUP) return "startup";
    if (notify == IW_NOTIFY_CLEANUP) return "cleanup";
    if (notify == IW_NOTIFY_SEARCH_START) return "search_start";
    if (notify == IW_NOTIFY_SEARCH_END) return "search_end";
    if (notify == IW_NOTIFY_CONNECT) return "connect";
    if (notify == IW_NOTIFY_DISCONNECT) return "disconnect";
    if (notify == IW_NOTIFY_FOUND) return "found";
    if (notify == IW_NOTIFY_BEACON_LOST) return "beacon_lost";
    if (notify == IW_NOTIFY_FATALERROR) return "fatalerror";
    return "unknown notify";
}

static const char* IWPhaseToString(int phase)
{
    if (phase == IW_PHASE_NULL) return "null";
    if (phase == IW_PHASE_WAIT) return "wait";
    if (phase == IW_PHASE_WAIT_IDLE) return "wait_idle";
    if (phase == IW_PHASE_IDLE) return "idle";
    if (phase == IW_PHASE_IDLE_WAIT) return "idle_wait";
    if (phase == IW_PHASE_IDLE_SCAN) return "idle_scan";
    if (phase == IW_PHASE_SCAN) return "scan";
    if (phase == IW_PHASE_SCAN_IDLE) return "scan_idle";
    if (phase == IW_PHASE_IDLE_LINK) return "idle_link";
    if (phase == IW_PHASE_LINK) return "link";
    if (phase == IW_PHASE_LINK_IDLE) return "link_idle";
    if (phase == IW_PHASE_FATALERROR) return "fatalerror";
...
static gsi_bool IW_CheckResult(int result, const char* funcName)
{
    ...
    case IW_RESULT_ACCEPT: return gsi_true;
    case IW_RESULT_FAILURE:
    case IW_RESULT_REJECT: OS_Printf("  IW_Phase = %s\n", IWPhaseToString(IW_GetPhase()));
...
static gsi_bool IW_Poll(int polling, int success, const char* funcName, gsi_bool* keepPolling)
{
    ...
    phase = IW_GetPhase();
    if (phase == polling) {
        if (keepPolling) *keepPolling = gsi_true;
        return gsi_true;
    }

    if (keepPolling) *keepPolling = gsi_false;

    if (phase == success) return gsi_true;
...
static void LoadSettings(void)
{
    ...
    if (memcmp(SavedSettings.magic, MagicString, MagicStringLen) != 0) SavedSettings.numConfigs = 0;
...
static void InitializingWirelessChose(const char* choice)
{
    if (choice == mscCancel) SetNextMenuScreen(&msCancelled);
}
...
static void FailedToInitializeWirelessChose(const char* choice)
{
    if (choice == mscOK) SetNextMenuScreen(&msFailed);
}
...
static void NetworkInformationInit(void)
{
    ...
    // BSSID / MAC
    snprintf(screen->extraText[0], MAX_EXTRA_TEXT_STRING_LEN, "  MAC: %02X:%02X:%02X:%02X:%02X:%02X",
        APList[APIndex].bssid[0], APList[APIndex].bssid[1], APList[APIndex].bssid[2], APList[APIndex].bssid[3],
...
static void NetworkInformationChose(const char* choice)
{
    if (choice == mscBack) SetNextMenuScreen(&msSearchingForNetworks);
}
...
static void BadSecurityKeyEnteredChose(const char* choice)
{
    if (choice == mscBack) SetNextMenuScreen(&msEnterSecurityKey);
}
...
static void ConnectedToInternetInit(void)
{
    ...
    IP_GetMacAddr(NULL, mac);
    snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  MAC:       %02X-%02X-%02X-%02X-%02X-%02X", mac[0],
...
static void ConfigurationSavedChose(const char* choice)
{
    if (choice == mscContinue) ExitMenu();
}
