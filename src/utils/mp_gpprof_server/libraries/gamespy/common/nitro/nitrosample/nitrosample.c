...
static MenuScreenConfiguration msCheckingBackendAvailability = {"Checking Backend Availability", {{mscCancel}},
    CheckingBackendAvailabilityInit, CheckingBackendAvailabilityChose, CheckingBackendAvailabilityThink,
...
static MenuScreenConfiguration msFailedToIntializePeerSDK = {
    "Failed to Initialize Peer SDK", {{mscExit}}, NULL, FailedToIntializePeerSDKChose};

static void HostOrListChose(const char* choice);

static MenuScreenConfiguration msHostOrList = {
    "Host or List", {{mscHostServer}, {mscListServers}, {mscExit}}, NULL, HostOrListChose};

static void ChooseHostingNameInit(void);
static void ChooseHostingNameChose(const char* choice);

static MenuScreenConfiguration msChooseHostingName = {"Choose Hosting Name", {{mscStartHosting}, {mscCancel}},
    ChooseHostingNameInit, ChooseHostingNameChose, NULL, SCREEN_OPTION_KEYBOARD};

static void HostingServerInit(void);
static void HostingServerChose(const char* choice);
static void HostingServerThink(void);

static MenuScreenConfiguration msHostingServer = {"Hosting Server", {{mscStopHosting}}, HostingServerInit,
...
static MenuScreenConfiguration msErrorStartingHost = {
    "Error Starting Host", {{mscMainMenu}}, ErrorStartingHostInit, ErrorStartingHostChose};
...
static MenuScreenConfiguration msListingHosts = {"Listing Hosts",
    {{mscViewHostInfo, CHOICE_OPTION_NEEDS_LIST_SELECTION}, {mscMainMenu}}, ListingHostsInit, ListingHostsChose,
    ListingHostsThink, SCREEN_OPTION_LIST | SCREEN_OPTION_ANIMATED};
...
static MenuScreenConfiguration msErrorListingHosts = {
    "Error Listing Hosts", {{mscMainMenu}}, ErrorListingHostsInit, ErrorListingHostsChose};
...
{
    // Output line prefix
    Printf("[%s][%s][%s] ", gGSIDebugCatStrings[category], gGSIDebugTypeStrings[type],
...
{
    if (result == e_qrnoerror) return "no error";
    if (result == e_qrwsockerror) return "socket error";
    if (result == e_qrbinderror) return "bind error";
    if (result == e_qrdnserror) return "dns lookup error";
    if (result == e_qrconnerror) return "nat error";
    if (result == e_qrnochallengeerror) return "no challenge received";
...
static void ServerKeyCallback(PEER peer, int key, qr2_buffer_t buffer, void* param)
{
    switch (key)
    {
    case HOSTNAME_KEY: qr2_buffer_add(buffer, Hostname); break;
    case MAPNAME_KEY: qr2_buffer_add(buffer, Mapnames[Mapname]); break;
    case GAMETYPE_KEY: qr2_buffer_add(buffer, Gametypes[Gametype]); break;
    default: qr2_buffer_add(buffer, _T(""));
    }
...
static void ListingGamesCallback(PEER peer, PEERBool success, const gsi_char* name, SBServer server, PEERBool staging,
...
static void ErrorStartingHostChose(const char* choice)
{
    if (choice == mscMainMenu) SetNextMenuScreen(&msHostOrList);
}
...
static void ViewHostInfoInit(void)
{
    ...
    snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  %s:%d", SBServerGetPublicAddress(SelectedServer),
        SBServerGetPublicQueryPort(SelectedServer));

    // private address
    strcpy(screen->extraText[line++], "[private address]");
    snprintf(screen->extraText[line++], MAX_EXTRA_TEXT_STRING_LEN, "  %s:%d", SBServerGetPrivateAddress(SelectedServer),
...
static void ViewHostInfoChose(const char* choice)
{
    if (choice == mscBack) SetNextMenuScreen(&msListingHosts);
}
