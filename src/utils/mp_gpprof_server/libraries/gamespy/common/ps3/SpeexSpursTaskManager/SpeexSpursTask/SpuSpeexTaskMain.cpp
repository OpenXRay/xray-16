...
void gviSpursSpeexEncoderInitialize(SpursSpeexTaskOutput* spuOutput)
{
    ...
    if (bitsPerFrame % 8) spuOutput->mSpeexEncodedFrameSize++;
...
void gviSpursSpeexEncode(SpursSpeexTaskOutput* spuTaskOut)
{
    ...
    cellDmaGet(inBuffer, (uint64_t)gviSpursSpeexTaskDesc.mInputBuffer,
        gviSpursSpeexTaskDesc.mInputBufferSize * sizeof(short), DMA_TAG(1), 0, 0);
    ...
    spuTaskOut->mSpeexOutBufferSize =
        speex_bits_write(&gviSpursSpeexBits, (char*)outBuffer, gviSpursSpeexTaskDesc.mEncodedFrameSize);
    ...
    cellDmaPut(outBuffer, (uint64_t)gviSpursSpeexTaskDesc.mOutputBuffer, gviSpursSpeexTaskDesc.mOutputBufferSize,
        DMA_TAG(1), 0, 0);
    cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void gviSpursSpeexDecodeAdd(SpursSpeexTaskOutput* spuTaskOut)
{
    ...
    cellDmaGet(inBuffer, (uint64_t)gviSpursSpeexTaskDesc.mInputBuffer, gviSpursSpeexTaskDesc.mInputBufferSize,
        DMA_TAG(1), 0, 0);
    cellDmaWaitTagStatusAll(DMA_MASK(1));
    ...
    cellDmaPut(outBuffer, (uint64_t)gviSpursSpeexTaskDesc.mOutputBuffer,
        gviSpursSpeexTaskDesc.mOutputBufferSize * sizeof(short), DMA_TAG(1), 0, 0);
    cellDmaWaitTagStatusAll(DMA_MASK(1));
    // spuDebugPrintf("[Speex][SPU] done transferring data back\n");
    free(speexBuffer);
    free(inBuffer);
...
void procesEncodeInit(unsigned int uiPtr)
{
    ...
    cellDmaPut(
        &spuOutput, (uint64_t)gviSpursSpeexTaskDesc.mSpeexTaskOutput, sizeof(SpursSpeexTaskOutput), DMA_TAG(1), 0, 0);
    cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void processEncode(unsigned int uiPtr)
{
    ...
    cellDmaLargeGet(gviSpursSpeexStateBuffer, (uint64_t)gviSpursSpeexTaskDesc.mSpeexStateBuffer,
        SPEEX_ENCODER_STATE_BUFFER_SIZE, DMA_TAG(1), 0, 0);
    cellDmaWaitTagStatusAll(DMA_MASK(1));
...
void cellSpursMain(qword argTask, uint64_t argTaskset)
{
    ...
    default:
    {
        // spuDebugPrintf("SPURS Sample:unknown case in switch uiCommand: %x uiArg1 %x uiArg2
