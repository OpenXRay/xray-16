...
SpursSpeexTaskManager::SpursSpeexTaskManager(
    spursThreadSupportInterface* threadInterface, unsigned int maxNumOutstandingTasks)
    : m_threadInterface(threadInterface), m_maxNumOutstandingTasks(maxNumOutstandingTasks)
{
...
int SpursSpeexTaskManager::issueEncodeInitTask(int theQuality, int theGviSpeexSamplesPerSecond,
    SpursSpeexTaskOutput* taskOutput, char* userAllocatedSpeexBuffer, int userAllocatedSpeexBufferSize)
{
    ...
    if (issueTask(taskDesc, SPEEX_TASK_ENCODE_INIT_COMMAND) != 0) return -1;
    return 0;
}
    ...
    int userAllocatedSpeexBufferSize)
{
    ...
    if (issueTask(taskDesc, SPEEX_TASK_ENCODE_COMMAND) != 0) return -1;
    return 0;
}

int SpursSpeexTaskManager::issueDecodeAddTask(char* decoderStateBuffer, int decoderStateBufferSize, char* inBuffer,
    ...
    struct SpursSpeexTaskOutput* taskOutput)
{
    ...
    taskDesc.mOutputBufferSize = outBufferSize;
    taskDesc.mSpeexTaskOutput = taskOutput;
    if (issueTask(taskDesc, SPEEX_TASK_DECODEADD_COMMAND) != 0) return -1;
    return 0;
}

int SpursSpeexTaskManager::issueDecodeSetTask(char* decoderStateBuffer, int decoderStateBufferSize, char* inBuffer,
    int inBufferSize, int encodedFrameSize, short* outBuffer, int outBufferSize,
    struct SpursSpeexTaskOutput* taskOutput)
{
    m_taskBusy[m_currentTask] = true;
    ...
    taskDesc.mOutputBufferSize = outBufferSize;
    taskDesc.mSpeexTaskOutput = taskOutput;
    if (issueTask(taskDesc, SPEEX_TASK_DECODESET_COMMAND) != 0) return -1;
    return 0;
}
    ...
    char* decoderStateBuffer, int decoderStateBufferSize, int sampleRate, struct SpursSpeexTaskOutput* taskOutput)
{
    ...
    taskDesc.mSpeexTaskOutput = taskOutput;
    if (issueTask(taskDesc, SPEEX_TASK_DECODE_INIT_COMMAND) != 0) return -1;
    return 0;
}
...
int SpursSpeexTaskManager::flush()
{
    ...
    while (m_numBusyTasks > 0)
    {
        ...
        if (m_threadInterface->waitForResponse(&taskId, &outputSize) != 0) return -1;
