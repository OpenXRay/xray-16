...
void _SpursPrintfThreadMain(uint64_t arg)
{
    ...
    while (1)
    {
        iReturn = sys_event_queue_receive(_g_SpuPrintfEventQueue, &event, SYS_NO_TIMEOUT);
...
int initializeSpursTaskSet(CellSpurs* pSpurs, int iSPUCount, uint8_t* puiPriorities)
{
    ...
    else
    {
        ...
        iReturn = sys_ppu_thread_get_priority(idCurrentThread, &iCurrentThreadPriority);
        if (iReturn != CELL_OK) {
        ...
        // E be done.
        iReturn = cellSpursInitialize(
            g_spursInstance, iNumSPUs, SPURS_SPU_THREAD_PRIORITY, iCurrentThreadPriority - 1, false);
        if (iReturn != CELL_OK) {
    ...
    iReturn = cellSpursCreateTaskset(g_spursInstance, &g_spursTaskSet, 0, pPriorities, iNumSPUs);
...
int SpursSupportInterface::startSPU()
{
    ...
    if (!m_bQueueInitialized) {
        ...
        iReturn = cellSpursQueueInitialize(&g_spursTaskSet, &m_responseQueue, m_aResponseBuffer,
            sizeof(CellSPURSArgument), CELL_SPURS_RESPONSE_QUEUE_SIZE, CELL_SPURS_QUEUE_SPU2PPU);
        if (iReturn != CELL_OK) {
            return -2;
        }
...
int SpursSupportInterface::waitForResponse(unsigned int* puiArgument0, unsigned int* puiArgument1)
{
    ...
    if (puiArgument0) *puiArgument0 = response.uiArgument0;

    if (puiArgument1) *puiArgument1 = response.uiArgument1;
    return 0;
}
