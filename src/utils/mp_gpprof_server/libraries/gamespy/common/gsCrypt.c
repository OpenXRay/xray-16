...
gsi_bool gsiCryptRSAMaskData(unsigned char* data, gsi_u32 dataLen, const unsigned char* seed, gsi_u32 seedLen)
{
    ...
    if (seedLen > GS_CRYPT_RSA_DATABLOCKSIZE) return gsi_false;
...
// Perform the hash
        ...
        {
            if ((gsi_u32)(i + k) >= dataLen) return gsi_true;
    ...
    unsigned char buffer[GS_CRYPT_RSA_BYTE_SIZE])
{
    ...
    // 1. check length
    if (len > maxPlainTextLen) return -1;
    ...
    const unsigned char cipherText[GS_CRYPT_RSA_BYTE_SIZE], unsigned char* plainText, gsi_u32* lenout)
{
    ...
    if (lintRSAPacket.mLength < (GS_CRYPT_RSA_BYTE_SIZE / GS_LARGEINT_DIGIT_SIZE_BYTES)) return -1;
    ...
    if (temp[0] != 0x00) return -1;
    if (temp[1] != 0x02) return -2;
    ...
    if (i == GS_CRYPT_RSA_BYTE_SIZE) return -4; // no payload
    ...
    const unsigned char cipherText[GS_CRYPT_RSA_BYTE_SIZE], unsigned char* plainText, gsi_u32* lenout)
{
    ...
    // check post exponentiation length
    if (lintRSAPacket.mLength < (GS_CRYPT_RSA_BYTE_SIZE / 4)) return -1;
    ...
    if (i == GS_CRYPT_RSA_BYTE_SIZE || packet->maskedData[i] != 0x01) return -3; // must be a 0x01 following the pad
    i++;
    if (i == GS_CRYPT_RSA_BYTE_SIZE) return -4; // founnd the separator, but no message!
    ...
    unsigned char* signedDataOut, gsi_u32* lenOut)
{
    ...
    if (hashLen + anOidLen + aReservedLength > aKeyByteLength) return -2; // key is too small or hash is too large
    ...
    const unsigned char* sig, gsi_u32 sigLen)
{
    ...
    if (hash == NULL || sig == NULL) return -1;
    if (sigLen != publicKey->modulus.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES) return -1;
    if (hashLen != GS_CRYPT_MD5_HASHSIZE && hashLen != GS_CRYPT_SHA1_HASHSIZE) return -1; // invalid hashsize
    ...
    if (packet[0] != 0x00 || packet[1] != 0x01) return -2;

    // Loop through the 0xFF's
    for (i = 2; i < GS_CRYPT_RSA_BYTE_SIZE; i++)
    {
        if (packet[i] == 0x00) break;
        if (packet[i] != 0xFF) return -3;
    }
    ...
    if ((i + sizeof(md5Header) + GS_CRYPT_MD5_HASHSIZE) == (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES)) {
        // MD5 Hash
        //   1. verify header
        if (0 != memcmp(md5Header, &packet[i], sizeof(md5Header))) return -4;
        i += sizeof(md5Header);
        //   2. compare hashes
        if (hashLen != GS_CRYPT_MD5_HASHSIZE) return -5;
        if (0 != memcmp(&packet[i], hash, GS_CRYPT_MD5_HASHSIZE)) return -5;
    }
             ...
             (lintRSASignature.mLength * GS_LARGEINT_DIGIT_SIZE_BYTES))
    {
        // SHA1 Hash
        //   1. verify header
        if (0 != memcmp(sha1Header, &packet[i], sizeof(sha1Header))) return -4;
        i += sizeof(sha1Header);
        //   2. compare hashes
        if (hashLen != GS_CRYPT_SHA1_HASHSIZE) return -5;
        if (0 != memcmp(&packet[i], hash, GS_CRYPT_SHA1_HASHSIZE)) return -5;
    }
    else
