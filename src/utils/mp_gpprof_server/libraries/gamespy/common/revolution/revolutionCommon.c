...
BOOL StartNetwork()
{
    ...
    OSReport("SOInit() ...");
    {
        ...
        if (rc != SO_SUCCESS) {
            ...
            case SO_EINVAL: OSReport("failed (SO_EINVAL)\n"); break;
            case SO_ENOMEM: OSReport("failed (SO_ENOMEM)\n"); break;
            default: OSReport("failed (%d)\n", rc);
            };
            return FALSE;
        }
    }
...
void main(int argc, char** argv)
{
    ...
    if (heapHandleSocket == MEM_HEAP_INVALID_HANDLE) OSHalt("MEMCreateExpHeapEx failed.\n");
    OSSetArenaLo((u8*)arenaLo + SOCKET_HEAPSIZE_DEFAULT);

    // Start the network
    if (!StartNetwork()) OSHalt("Network was not started\n");
    OSReport("=======================================================================\n");
