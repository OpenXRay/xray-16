...
int bind(SOCKET sock, const SOCKADDR* addr, int len)
{
    ...
    if (((const SOCKADDR_IN*)addr)->port == 0) return 0;
...
unsigned long inet_addr(const char* name)
{
    ...
    if (rcode == FALSE) return INADDR_NONE;
    return addr.addr;
}
...
int GSISocketSelect(SOCKET theSocket, int* theReadFlag, int* theWriteFlag, int* theExceptFlag)
{
    ...
    pollFD.fd = theSocket;
    pollFD.events = 0;
    if (theReadFlag != NULL) pollFD.events |= SO_POLLRDNORM;
    if (theWriteFlag != NULL) pollFD.events |= SO_POLLWRNORM;
    pollFD.revents = 0;

    rcode = SOPoll(&pollFD, 1, 0);
    if (rcode < 0) return REVOlUTION_SOCKET_ERROR;
