...
static unsigned short PadReadData(int events[NumPadEvents])
{
    ...
    if (scePadRead(0, 0, rdata) == 0) return 0;
...
int PadInit(void)
{
    ...
    if (0 >= sceSifLoadModule(ROOT_DIR "sio2man.irx", 0, NULL)) return 0;
...
void PadReadInput(int events[NumPadEvents])
{
    ...
    case 0:
    {
        ...
        if (mPadState != scePadStateStable && mPadState != scePadStateFindCTP1) break;
    ...
    case 40:
        ...
        // Set main mode
        if (scePadSetMainMode(0, 0, 1, 0) == 1) mPadPhase++;
        break;
    ...
    case 70:
        ...
        for (i = 2; i < 6; i++)
            mActAlign[i] = 0xff;
        if (scePadSetActAlign(0, 0, mActAlign)) mPadPhase++;
        break;
