...
static sn_int32 do_initialisation(void)
{
    ...
    while (device_attached == SN_DEV_TYPE_NONE)
    {
        ...
        if (device_attached == SN_DEV_TYPE_NONE) sn_delay(10);
    }
    ...
    do
    {
        ...
        // don't hog the CPU
        if (got_ip == SN_FALSE) sn_delay(100);
...
static void event_handler(const char* ifname, int af, int type)
{
    ...
    case sceEENETCTL_IEV_Attach:
        if (strcmp(ifname, EENET_IFNAME)) break;
...
int ent_cnf_load_config(const char* fname, const char* usr_name)
{
    ...
    ret = sceSifCallRpc(&cd, ENT_CNF_SIFRPC_LOAD_CONFIG, 0, (void*)rpc_buffer, ee_rpc_size(len), (void*)rpc_buffer,
...
// stop and unload modules
    ...
    // free the EENet pool
    if (eenet_pool != NULL) free(eenet_pool);
}
