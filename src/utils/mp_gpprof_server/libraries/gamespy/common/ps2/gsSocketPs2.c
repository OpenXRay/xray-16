...
int GSISocketSelect(SOCKET theSocket, int* theReadFlag, int* theWriteFlag, int* theExceptFlag)
{
    ...
    // Init the flags to 0
    if ((theReadFlag != NULL)) *theReadFlag = 0;
    if ((theWriteFlag != NULL)) *theWriteFlag = 0;
    ...
    if (result > 0) {
        ...
        if ((theReadFlag != NULL)) *theReadFlag = (aPollFdSet.revents & sceINET_POLLIN) ? 1 : 0;
        if ((theWriteFlag != NULL)) *theWriteFlag = (aPollFdSet.revents & sceINET_POLLOUT) ? 1 : 0;
        if ((theExceptFlag != NULL)) *theExceptFlag = (aPollFdSet.revents & sceINET_POLLERR) ? 1 : 0;
    }
    return result;
}
