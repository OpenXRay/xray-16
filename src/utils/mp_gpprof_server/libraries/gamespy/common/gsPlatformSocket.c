...
int SetSockBroadcast(SOCKET sock)
{
    ...
    int optval = 1;
    if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, (char*)&optval, sizeof(optval)) != 0) return 0;
#else
    GSI_UNUSED(sock);
#endif
...
int GetReceiveBufferSize(SOCKET sock)
{
    ...
    if (gsiSocketIsError(rcode)) return -1;
...
#endif
    if (gsiSocketIsError(aResult)) return -1;
...
{
    int aReadFlag = 0;
    if (1 == GSISocketSelect(sock, &aReadFlag, NULL, NULL)) return aReadFlag;
...
{
    int aWriteFlag = 0;
    if (1 == GSISocketSelect(sock, NULL, &aWriteFlag, NULL)) return aWriteFlag;
...
#define MAX_IPS 5
    ...
    for (i = 0; i < num; i++)
    {
        ...
        // get the flags
        len = sizeof(flags);
        if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_IFFLAGS, &flags, &len) != 0) continue;
        // printf("eenet%d flags: 0x%X\n", i, flags);

        // check for up, running, and non-loopback
        if (!(flags & (IFF_UP | IFF_RUNNING)) || (flags & IFF_LOOPBACK)) continue;
        // printf("eenet%d: up and running, non-loopback\n", i);

        // get the address
        len = sizeof(address);
        if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_ADDR, &address, &len) != 0) continue;
        // printf("eenet%d: %s\n", i, inet_ntoa(address));
...
#define MAX_IPS sceLIBNET_MAX_INTERFACE
    ...
    if (aNumInterfaces < 1) return NULL;
...
#define MAX_IPS 5
    ...
    if ((rcode == XNET_GET_XNADDR_NONE) || (rcode == XNET_GET_XNADDR_TROUBLESHOOT)) return NULL;
...
gsi_u32 gsiGetBroadcastIP(void)
{
    ...
    */
    ...
    if (addrtbl == NULL) return 0xFFFFFFFF;
