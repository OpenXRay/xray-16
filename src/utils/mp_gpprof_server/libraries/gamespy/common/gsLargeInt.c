...
{
    if (length > GS_LARGEINT_MAX_DIGITS) return gsi_false;
...
gsi_bool gsiLargeIntSizePower2(const gsLargeInt_t* src1, const gsLargeInt_t* src2, l_word* lenout)
{
    ...
    *lenout = (l_word)i;
    ...
    if (*lenout > GS_LARGEINT_MAX_DIGITS) return gsi_false;
    return gsi_true;
}
...
gsi_bool gsLargeIntAdd(const gsLargeInt_t* src1, const gsLargeInt_t* src2, gsLargeInt_t* dest)
{
    ...
    if (gsi_is_false(result)) memset(dest, 0, sizeof(gsLargeInt_t));  // overflow
    return result;
}
    ...
    l_word* lenout, l_word maxlen)
{
    ...
    if (maxlen < length1 || maxlen < length2) return gsi_false;  // dest not large enough, OVERFLOW
    ...
    {
        if (maxlen <= i) return gsi_false;  // OVERFLOW, no room for extra digit
        dest[i++] = (l_word)carry;
        carry = 0;
    }
    ...
    if (i < longerLen) {
        // check overlap
        if (&dest[i] != &longerSrc[i]) memcpy(&dest[i], &longerSrc[i], (longerLen - i) * sizeof(l_word));
        i = longerLen;
    }
    *lenout = (l_word)i;
...
gsi_bool gsLargeIntSub(const gsLargeInt_t* src1, const gsLargeInt_t* src2, gsLargeInt_t* dest)
{
    ...
    if (gsi_is_true(result)) gsiLargeIntStripLeadingZeroes(dest);
    return result;
}
    ...
    for (i = 0; i < length2; i++)
    {
        ...
        if (data2[i] != 0) {
            ...
            for (k = 0; k < length1; k++)
            {
                ...
                if (digit > (gsi_i32)temp.mLength) temp.mLength = (l_word)digit;
            }
        }
    }
    ...
    const l_word* src, l_word len, const gsLargeInt_t* div, gsLargeInt_t* dest, gsLargeInt_t* remainder)
{
    ...
    if (scopyLen > GS_LARGEINT_MAX_DIGITS && dest != NULL) return gsi_false;
    ...
    do
    {
        ...
        else if (result == 0)
        {
            ...
            if (readIndex < 0) endLoop = gsi_true;
            ;  // no more digits
        }
        else
        {
...
gsi_bool gsLargeIntKMult(const gsLargeInt_t* src1, const gsLargeInt_t* src2, gsLargeInt_t* dest)
{
    ...
    if (max(src1->mLength, src2->mLength) < GS_LARGEINT_KARATSUBA_CUTOFF) return gsLargeIntMult(src1, src2, dest);
    ...
    const l_word* data1, const l_word* data2, l_word length, l_word* dest, l_word* lenout, l_word maxlen)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (gsi_is_false(result)) return gsi_false;  // could not resize
...
gsi_bool gsLargeIntPowerMod(const gsLargeInt_t* b, const gsLargeInt_t* p, const gsLargeInt_t* m, gsLargeInt_t* dest)
{
    ...
    if (R.mLength > GS_LARGEINT_MAX_DIGITS) return gsi_false;  // you need to increase the large int capacity
    ...
    */
    ...
    // if (k >= 3)
    {
        ...
        if (lut == NULL) return gsi_false;  // out of memory
    ...
    for (i = 0; i < kdigits; i++)
    {
        ...
        if ((bitReadIndex % k) != 0) bitReadIndex += k - (bitReadIndex % k);  // round up to next k
        if (i != 0) {
            if (bitReadIndex - (l_index * GS_LARGEINT_DIGIT_SIZE_BITS) > GS_LARGEINT_DIGIT_SIZE_BITS) l_index++;
        }
    ...
    if (gsi_is_false(gsiLargeIntMultM(dest, &one, &mod, modPrime, dest))) return gsi_false;
...
gsi_bool gsLargeIntPowerMod(const gsLargeInt_t* b, const gsLargeInt_t* p, const gsLargeInt_t* m, gsLargeInt_t* dest)
{
    ...
    for (i = (int)(expHighBit - 1); i >= 0; i--)
    {
        ...
        // THE REPEATED 1<<digitbit caused the optimizer to 'splode
        {
            ...
            if (mask == masked) gsiLargeIntMultM(dest, &xwiggle, &mod, modPrime, dest);
        }
    }
...
gsi_bool gsLargeIntSetFromHexString(gsLargeInt_t* lint, const char* hexstream)
{
    ...
    if ((len / 2) > (GS_LARGEINT_MAX_DIGITS * GS_LARGEINT_DIGIT_SIZE_BYTES)) return gsi_false;
...
gsi_bool gsLargeIntReverseBytes(gsLargeInt_t* lint)
{
    ...
    if (lint->mLength == 0) return gsi_true;
...
void gsLargeIntAddToMD5(const gsLargeInt_t* _lint, MD5_CTX* md5)
{
    ...
    if (byteLength == 0) return;  // no data
