...
unsigned int gsiGetResolvedIP(GSIResolveHostnameHandle handle)
{
    ...
    if (!handle->finishedResolving) return GSI_STILL_RESOLVING_HOSTNAME;
...
unsigned short* goawstrdup(const unsigned short* src)
{
    ...
    if (res != NULL) wcscpy((wchar_t*)res, (const wchar_t*)src);
    return res;
}
...
time_t gsiDateToSeconds(struct tm* tb)
{
     ...
     */
    if (((tmptm1 = tb->tm_year) < BASE_YEAR - 1) || (tmptm1 > MAX_YEAR + 1)) return MKTIME_ERROR;
    ...
    if ((tb->tm_mon < 0) || (tb->tm_mon > 11)) {
         ...
         */
        if ((tmptm1 < BASE_YEAR - 1) || (tmptm1 > MAX_YEAR + 1)) return MKTIME_ERROR;
    }
    ...
    if (ChkAdd(tmptm1, tmptm3, tmptm2)) return MKTIME_ERROR;
     ...
     */
    tmptm2 = tmptm1 * 24L;
    if (ChkMul(tmptm2, tmptm1, 24L)) return MKTIME_ERROR;

    tmptm1 = tmptm2 + (tmptm3 = (long)tb->tm_hour);
    if (ChkAdd(tmptm1, tmptm2, tmptm3)) return MKTIME_ERROR;
    ...
    tmptm2 = tmptm1 * 60L;
    if (ChkMul(tmptm2, tmptm1, 60L)) return MKTIME_ERROR;
    ...
    if ((tbtemp = gsiSecondsToDate(&tmptm1)) == NULL) return MKTIME_ERROR;
...
{
    if (inlen >= 2) trip[0] = (char)(quart[0] << 2 | quart[1] >> 4);
    if (inlen >= 3) trip[1] = (char)((quart[1] & 0x0F) << 4 | quart[2] >> 2);
    if (inlen >= 4) trip[2] = (char)((quart[2] & 0x3) << 6 | quart[3]);
}
...
int B64DecodeLen(const char* input, int encodingType)
{
    ...
    while (*input)
    {
        if (*input == encoding[2]) return (input - holdin) / 4 * 3 + (input - holdin - 1) % 4;
        input++;
    }
...
gsi_bool B64EncodeStream(B64StreamData* data, char output[4])
{
    ...
    if (data->len <= 0) return gsi_false;
    ...
    if (data->len < 0) {
        output[3] = encoding[2];
        if (data->len == -2) output[2] = encoding[2];
    }
...
char* gsiXxteaAlg(const char* sIn, int nIn, char key[XXTEA_KEY_SIZE], int bEnc, int* nOut)
{
    ...
    /////////////////////////////////
    // ERROR CHECK!
    if (!sIn || !key[0] || nIn == 0) return NULL;
...
static const char* GetMAC(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        // get the type
        len = sizeof(type);
        if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_IFTYPE, &type, &len) != 0) continue;
        // printf("eenet%d type: %d\n", i, type);

        // check for ethernet
        if (type != sceEENET_IFTYPE_ETHER) continue;
        // printf("eenet%d: ethernet\n", i);

        // get the address
        len = sizeof(linkAddress);
        if (sceEENetGetIfinfo(interface->ifn_name, sceEENET_IFINFO_MACADDR, &linkAddress, &len) != 0) continue;
...
static const char* GetMAC(void)
{
    ...
    if (result == sceINETE_OK) return MAC;
