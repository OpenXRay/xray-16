...
int _NetworkAdHocSocketCreate(gsi_u16 port)
{
    ...
    printf("Create Adhoc Socket: MAC:%x,%x,%x,%x,%x,%x\n", (gsi_u32)tmp_local.addr[0], (gsi_u32)tmp_local.addr[1],
        (gsi_u32)tmp_local.addr[2], (gsi_u32)tmp_local.addr[3], (gsi_u32)tmp_local.addr[4], (gsi_u32)tmp_local.addr[5]);
#endif
...
#endif
#ifdef _PSPNET_LOG
    printf("AdHoc SendTo: MAC:%x,%x,%x,%x,%x,%x port=%d Len=%d\n", (gsi_u32)dest_addr[0], (gsi_u32)dest_addr[1],
        (gsi_u32)dest_addr[2], (gsi_u32)dest_addr[3], (gsi_u32)dest_addr[4], (gsi_u32)dest_addr[5], dest_port, len);
#endif
    // todo: translate return value to GSI specific
    return ret;
}
    ...
    u_int16* sport)
{
    ...
    if (ret >= 0)
        printf("AdHoc Recv From: MAC:%x,%x,%x,%x,%x,%x: port=%d len:%d\n", saddr[0], saddr[1], saddr[2], saddr[3],
            saddr[4], saddr[5], *sport, bufferlen);
#endif
