...
static void gsiResolveHostnameThread(void* arg)
{
    ...
    else
    {
        ...
        if (result < 0) info->ip = GSI_ERROR_RESOLVING_HOSTNAME;
...
gsi_i64 gsiStringToInt64(const char* theNumberStr)
{
    ...
    else if ((flags & FL_OVERFLOW) || (!(flags & FL_UNSIGNED) && (((flags & FL_NEG) && (number > GSI_MIN_I64)) ||
    ...
    if (flags & FL_NEG) /* negate result if there was a neg sign */
        number = -number;
