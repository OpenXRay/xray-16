...
static SceUID gsiPspLoadModule(const char* path)
{
    ...
    if (ret < 0) {
        ...
        if (ret < 0) printf("sceKernelUnloadModule() failed. ret = 0x%x\n", ret);
        return ret;
    }
    ret = modid;
...
void gsiPspApctlHandler(int prev_state, int new_state, int event, int error_code, void* arg)
{
    ...
    if (new_state == SCE_NET_APCTL_STATE_Disconnected) {
        if (event == SCE_NET_APCTL_EVENT_DISCONNECT_REQ) gDisconnected = 1;
...
{
    if (shutDownServices & PSPNET_CONNECTED) gsiPspnetDisconnect();

    if (shutDownServices & PSPNET_APDUMDLG_STARTED) sceNetApDialogDummyTerm();

    if (shutDownServices & PSPNET_APCTLHDLR)
        if (gPspnetApctlHandlerId >= 0) sceNetApctlDelHandler(gPspnetApctlHandlerId);
...
int main(int argc, char** argp)
{
    ...
    if (ret < 0) return SCE_KERNEL_EXIT_SUCCESS;
