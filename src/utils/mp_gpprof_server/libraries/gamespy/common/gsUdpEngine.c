...
int gsUdpRemotePeerCompare(const void* theFirstPeer, const void* theSecondPeer)
{
    ...
    if (aPeer1->mAddr != aPeer2->mAddr) return 1;
    if (aPeer1->mPort != aPeer2->mPort) return 1;
...
{
    GSUdpRemotePeer *aPeer1 = (GSUdpRemotePeer *)theFirstPeer, *aPeer2 = (GSUdpRemotePeer *)theSecondPeer;
    if (aPeer1->mConnection != aPeer2->mConnection) return 1;
    return 0;
}
...
void gsUdpSocketError(GT2Socket theSocket)
{
    ...
    if (aUdp->mAppNetworkError) aUdp->mAppNetworkError(GS_UDP_NETWORK_ERROR, aUdp->mAppUserData);
    len = ArrayLength(aUdp->mMsgHandlers);
    for (i = 0; i < len; i++)
    {
        GSUdpMsgHandler* aMsgHandler = (GSUdpMsgHandler*)ArrayNth(aUdp->mMsgHandlers, i);
        if (aMsgHandler->mNetworkError) aMsgHandler->mNetworkError(GS_UDP_NETWORK_ERROR, aMsgHandler->mUserData);
    }
    ...
    unsigned char* theMsg, unsigned int theMsgLen, gsi_bool theReliable)
{
    ...
    if (aResult != GT2Success) return GS_UDP_SEND_FAILED;
    return GS_UDP_NO_ERROR;
}
