...
static void gsiCoreTaskDispatchCallback(GSTask* theTask, GSTaskResult theResult)
{
    if (theTask->mIsCallbackPending) {
        ...
        if (theTask->mCallbackFunc) (theTask->mCallbackFunc)(theTask->mTaskData, theResult);
    }
}
...
GSTaskResult gsCoreTaskThink(GSTask* theTask)
{
    ...
    if (theTask == NULL) return GSTaskResult_Finished;

    // If the task is running let it think (it may be cancelled and still running)
    if (theTask->mIsRunning && theTask->mThinkFunc) aResult = (theTask->mThinkFunc)(theTask->mTaskData);
    ...
    else if (aResult != GSTaskResult_InProgress)
    {
        ...
        if (theTask->mCleanupFunc) removeTask = (theTask->mCleanupFunc)(theTask->mTaskData);
...
void gsCoreThink(gsi_time theMS)
{
    ...
    if (gsi_is_false(aCore->mIsInitialized)) return;
...
// cancelled tasks continue processing until the cancel is acknowledge by the task
    ...
    {
        int len = ArrayLength(aCore->mTaskArray);
        if (len > 0) allTasksAreDead = 0;
        for (i = (len - 1); i >= 0; i--)
        {
            GSTask* task = *(GSTask**)ArrayNth(aCore->mTaskArray, i);
            if (gsi_is_true(task->mAutoThink)) gsCoreTaskThink(task);
            if (theMS != 0 && (current_time() - aStartTime > theMS)) break;
        }
    }
#else
    for (i = 0; i < GSICORE_MAXTASKS; i++)
    {
        if (aCore->mTaskArray[i] != NULL) {
            ...
            if (aCore->mTaskArray[i]->mAutoThink == gsi_true) gsCoreTaskThink(aCore->mTaskArray[i]);
        }
...
GSCoreValue gsCoreIsShutdown()
{
    ...
    if (gsi_is_true(aCore->mIsShuttingDown)) return GSCore_SHUTDOWN_PENDING;
    if (aCore->mRefCount == 0) return GSCore_SHUTDOWN_COMPLETE;
...
void gsiCoreExecuteTask(GSTask* theTask, gsi_time theTimeoutMs)
{
    ...
    // Execute the task
    if (theTask->mExecuteFunc) (theTask->mExecuteFunc)(theTask->mTaskData);
...
void gsiCoreCancelTask(GSTask* theTask)
{
    ...
    if (theTask->mIsRunning && !theTask->mIsCanceled) {
        ...
        if (theTask->mCancelFunc) (theTask->mCancelFunc)(theTask->mTaskData);
    }
    gsiLeaveCriticalSection(&aCore->mQueueCrit);
    GSI_UNUSED(aCore);
}
