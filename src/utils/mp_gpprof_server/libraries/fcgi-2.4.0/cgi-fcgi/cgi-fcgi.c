        ...
        else
        {
            ...
            case FCGI_END_REQUEST:
                ...
                if (contentLen == 0) {
                    ...
                    exitStatus = (erBody.appStatusB3 << 24) + (erBody.appStatusB2 << 16) + (erBody.appStatusB1 << 8) +
                                 (erBody.appStatusB0);
                    exitStatusSet = TRUE;
            ...
            /* coming soon */
            default: exit(FCGX_PROTOCOL_ERROR);
            }
            if (contentLen == 0) {
                if (paddingLen > 0) {
...
static void WebServerReadHandler(ClientData dc, int bytesRead)
{
    ...
    *((FCGI_Header*)&fromWS.buff[0]) = MakeHeader(FCGI_STDIN, requestId, bytesRead, 0);
    bytesToRead -= bytesRead;
    fromWS.stop = &fromWS.buff[sizeof(FCGI_Header) + bytesRead];
    webServerReadHandlerEOF = (bytesRead == 0);

    if (bytesToRead <= 0) WriteStdinEof();
...
static void ScheduleIo(void)
{
     ...
     */
    if (!fcgiWritePending && appServerSock != -1 && ((length = fromWS.stop - fromWS.next) != 0)) {
        if (OS_AsyncWrite(appServerSock, 0, fromWS.next, length, AppServerWriteHandler, (ClientData)appServerSock) ==
            -1)
        {
            FCGIexit(OS_Errno);
        }
    ...
    if (!fcgiReadPending && appServerSock != -1) {
        ...
        if (OS_AsyncRead(appServerSock, 0, fromAS.next, BUFFLEN, AppServerReadHandler, (ClientData)appServerSock) == -1)
        {
            FCGIexit(OS_Errno);
        }
        else
     ...
     */
    if ((bytesToRead > 0) && !webServerReadHandlerEOF && !wsReadPending && !fcgiWritePending &&
        fromWS.next == &fromWS.buff[0])
    {
        if (OS_AsyncReadStdin(fromWS.next + sizeof(FCGI_Header), BUFFLEN - sizeof(FCGI_Header), WebServerReadHandler,
                STDIN_FILENO) == -1)
        {
            FCGIexit(OS_Errno);
        }
 ...
 *----------------------------------------------------------------------
 */
static void FCGIUtil_BuildNameValueHeader(int nameLen, int valueLen, unsigned char* headerBuffPtr, int* headerLenPtr)
{
...
static int ParseArgs(
    int argc, char* argv[], int* doBindPtr, int* doStartPtr, char* connectPathPtr, char* appPathPtr, int* nServersPtr)
{
    int i, x, err = 0, ac;
    ...
    for (i = 1; i < argc; i++)
    {
        if (argv[i][0] == '-') {
            if (!strcmp(argv[i], "-f")) {
                ...
                err = ParseArgs(ac, av, doBindPtr, doStartPtr, connectPathPtr, appPathPtr, nServersPtr);
                for (x = 1; x < ac; x++)
                {
                    ASSERT(av[x] != NULL);
                    free(av[x]);
...
int main(int argc, char** argv)
{
    ...
    if (ParseArgs(argc, argv, &doBind, &doStart, (char*)&bindPath, (char*)&appPath, &nServers)) {
    ...
    for (; *envp != NULL; envp++)
    {
        ...
        FCGIUtil_BuildNameValueHeader(equalPtr - *envp, valueLen, &headerBuff[0], &headerLen);
