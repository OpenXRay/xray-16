...
inline std::basic_ostream<char, Traits>& operator<<(
    std::basic_ostream<char, Traits>& ostr, gamespy_profile::profile_data const& profile_data)
{
    using namespace gamespy_profile;
    for (int i = 0; i < at_awards_count; ++i)
    {
        char const* tmp_award_name = get_award_name(enum_awards_t(i));
        ostr << tmp_award_name << "=" << profile_data.m_awards[i].m_count << crlf << tmp_award_name
