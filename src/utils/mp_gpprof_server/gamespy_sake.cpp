...
{
    if (check_gamespy_services() != GSIACAvailable) throw std::runtime_error("GameSpy services are unavailable");
...
bool sake_processor::gp_processor::login()
{
    ...
    if (tmp_res != GP_NO_ERROR) return false;
...
void sake_processor::fetch()
{
    ...
    if (!create_request_string()) return;
