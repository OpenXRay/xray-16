...
void requests_poll::add_new_request(request_ptr_t fcgx_request, user_name_t const profile_name)
{
    m_new_request_sync.lock();
    m_new_requests.push_back(new fetch_profile_request(fcgx_request, profile_name));
...
bool requests_poll::request_processor(sake_processor* sproc)
{
    ...
    m_new_request_sync.lock();
    sproc->begin_fetch();
    for (requests_t::iterator i = m_new_requests.begin(), ie = m_new_requests.end(); i != ie; ++i)
    {
...
void requests_poll::process_result(sake_processor* sproc)
{
    ...
    m_new_request_sync.lock();
    for (requests_t::iterator i = m_active_requests.begin(), ie = m_active_requests.end(); i != ie; ++i)
    {
