...
bool profiles_cache::search(user_name_t const& profname, gamespy_profile::profile_data& dest_data)
{
    cache_item search_item;
    strcpy(search_item.m_user_name, profname);
    cache_t::iterator tmp_iter = std::lower_bound(m_cache.begin(), m_cache.end(), search_item, cache_item_less_p());
    if (tmp_iter != m_cache.end()) {
        if (strcmp(tmp_iter->m_user_name, profname)) return false;
...
bool const profiles_cache::cache_item_less_p::operator()(cache_item const& left, cache_item const& right) const
{
    return strcmp(left.m_user_name, right.m_user_name) < 0;
};

bool profiles_cache::add(char const* profname, gamespy_profile::profile_data const& src_data)
{
    if (!get_free_space()) {
        return false;
    }
...
bool const profiles_cache::cache_item_expired_p::operator()(cache_item const& left) const
{
    if (!left.m_hits_count) return true;
    if ((m_current_time - left.m_creation_time) >= m_expire_time) return true;
    return false;
}
...
void profiles_cache::clear_expired()
{
    ...
    m_cache.erase(std::remove_if(m_cache.begin(), m_cache.end(), tmp_predicate), m_cache.end());
}