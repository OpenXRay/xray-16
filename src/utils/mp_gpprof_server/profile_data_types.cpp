...
enum_awards_t get_award_by_stat_id_name(char const* stat_name)
{
    ...
    {
        if (!strcmp(stat_name, ATLAS_GET_STAT_NAME(award_atlas_stats_map[i][ap_award_id]))) {
...
enum_awards_t get_award_by_stat_rdate_name(char const* stat_name)
{
    for (int i = 0; i < at_awards_count; ++i)
    {
        if (!strcmp(stat_name, ATLAS_GET_STAT_NAME(award_atlas_stats_map[i][ap_award_rdate]))) {
            return static_cast<enum_awards_t>(i);
        }
    }
    return static_cast<enum_awards_t>(at_awards_count);
...
static unsigned short best_scores_atlas_stats_map[bst_score_types_count] = {STAT_BestScore_KillsInRow,
...
enum_best_score_type get_best_score_type_by_sname(char const* stat_name)
{
    ...
    {
        if (!strcmp(stat_name, ATLAS_GET_STAT_NAME(best_scores_atlas_stats_map[i]))) {
