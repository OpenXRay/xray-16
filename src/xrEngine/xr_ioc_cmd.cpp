...
void CCC_LoadCFG::Execute(LPCSTR args)
{
    ...
    if (NULL == FS.exist(cfg_full_name)) xr_strcpy(cfg_full_name, cfg_name);
...
public:
    ...
    virtual void Execute(LPCSTR args)
    {
        ...
        if (!op_client[0] && strstr(op_server, "single")) xr_strcpy(op_client, "localhost");
