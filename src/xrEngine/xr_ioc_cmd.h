...
public:
    IConsole_Command(const char* N BENCH_SEC_SIGN) : cName(N), bEnabled(TRUE), bLowerCaseArgs(FALSE), bEmptyArgsHandled(FALSE)
    {
...
public:
    CCC_Mask(const char* N, Flags32* V, u32 M) : IConsole_Command(N), value(V), mask(M){};
...
public:
    CCC_ToggleMask(const char* N, Flags32* V, u32 M) : IConsole_Command(N), value(V), mask(M) { bEmptyArgsHandled = TRUE; }
    const bool GetValue() const { return value->test(mask); }
    virtual void Execute(const char* /*args*/)
    {
...
public:
    CCC_Token(const char* N, u32* V, const xr_token* T) : IConsole_Command(N), value(V), tokens(T){}
...
public:
    CCC_Vector3(const char* N, Fvector* V, const Fvector _min, const Fvector _max) : IConsole_Command(N), value(V)
    {
        min.set(_min);
        max.set(_max);
    };
...
public:
    ...
    CCC_Integer(const char* N, int* V, int _min = 0, int _max = 999) : IConsole_Command(N), value(V), min(_min), max(_max){}
...
public:
    CCC_String(const char* N, char* V, int _size = 2) : IConsole_Command(N), value(V), size(_size)
    {
    ...
    virtual void Execute(const char* args) { strncpy_s(value, size, args, size - 1); }
    ...
    virtual void fill_tips(vecTips& tips, u32 mode)
    {
        tips.push_back((const char*)value);
        IConsole_Command::fill_tips(tips, mode);
    }
};
