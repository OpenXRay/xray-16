...
IC
    ...
    void
    blit(u32* dest, u32 ds_x, u32 ds_y, u32* src, u32 ss_x, u32 ss_y, u32 px, u32 py)
{
...
IC
    ...
    void
    blit_r(u32* dest, u32 ds_x, u32 ds_y, u32* src, u32 ss_x, u32 ss_y, u32 px, u32 py)
{
...
IC
    ...
    void
    _rect_register(U8Vec& mask, int dest_width, int dest_height, Irect& R)
{
    u32 s_x = R.width() + 1;
    u32 s_y = R.height() + 1;
...
IC
    ...
    bool
    _rect_test(U8Vec& mask, int dest_width, int dest_height, Irect& R)
{
    u32 s_x = R.width() + 1;
    u32 s_y = R.height() + 1;
...
IC
    ...
    bool
    _rect_place(U8Vec& mask, int dest_width, int dest_height, Irect& r, BOOL& bRotated)
{
    Irect R;
...
int CImageManager::CreateMergedTexture(u32 layer_cnt, SSimpleImageVec& src_images, SSimpleImage& dst_image,
    ...
    U32Vec& dest_remap)
{
    ...
    for (s_it = src_images.begin(); s_it != src_images.end(); s_it++)
    {
        ...
        {
            if (!bRotated)
                blit(dest_layers[k].begin(), dest_width, dest_height, s_it->layers[k].begin(), s_it->w, s_it->h, R.lt.x,
                    R.lt.y);
            else
                blit_r(dest_layers[k].begin(), dest_width, dest_height, s_it->layers[k].begin(), s_it->w, s_it->h,
                    R.lt.x, R.lt.y);
        }
    }
...
int CImageManager::CreateMergedTexture(const RStringVec& _names, LPCSTR dest_name, STextureParams::ETFormat fmt,
    int dest_width, int dest_height, Fvector2Vec& dest_offset, Fvector2Vec& dest_scale, boolVec& dest_rotate,
    U32Vec& dest_remap)
{
    if (_names.empty())
    ...
    for (RStringVecIt n_it = src_names.begin(); n_it != src_names.end(); n_it++, s_it++)
    {
        ...
        if (!Surface_Load(
                EFS.ChangeFileExt(t_name, ".dds" /*".tga"*/).c_str(), s_it->layers.back(), s_it->w, s_it->h, s_it->a))
        {
    ...
    for (s_it = src_items.begin(); s_it != src_items.end(); s_it++)
    {
        ...
        // Perform BLIT
        if (!bRotated)
            blit(dest_pixels.begin(), dest_width, dest_height, s_it->layers.back().begin(), s_it->w, s_it->h, R.lt.x,
                R.lt.y);
        else
            blit_r(dest_pixels.begin(), dest_width, dest_height, s_it->layers.back().begin(), s_it->w, s_it->h, R.lt.x,
                R.lt.y);
    }
...
int CImageManager::CreateMergedTexture(const RStringVec& src_names, LPCSTR dest_name, STextureParams::ETFormat fmt,
    int dest_width_min, int dest_width_max, int dest_height_min, int dest_height_max, Fvector2Vec& dest_offset,
