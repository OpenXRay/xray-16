...
xr_token ECORE_API eo_type_token[] = {{"Static", 0}, {"Dynamic", CEditableObject::eoDynamic},
    {"Progressive Dynamic", CEditableObject::eoDynamic | CEditableObject::eoProgressive},
    {"HOM", CEditableObject::eoHOM}, {"Multiple Usage", CEditableObject::eoMultipleUsage | CEditableObject::eoUsingLOD},
    {"Sound Occluder", CEditableObject::eoSoundOccluder}, {0, 0}};
...
void CEditableObject::FillBasicProps(LPCSTR pref, PropItemVec& items)
{
    ...
    PHelper().CreateCaption(
        items, PrepareKey(pref, "Version\\Creation Time"), Trim(AnsiString(ctime(&m_CreateTime))).c_str());
    PHelper().CreateCaption(
        items, PrepareKey(pref, "Version\\Modified Time"), Trim(AnsiString(ctime(&m_ModifTime))).c_str());
    ...
    V->OnChangeEvent.bind(this, &CEditableObject::OnChangeTransform);
    V = PHelper().CreateCaption(items, PrepareKey(pref, "Transform\\BBox Min"),
        shared_str().printf("{%3.2f, %3.2f, %3.2f}", VPUSH(GetBox().vMin)));
    V = PHelper().CreateCaption(items, PrepareKey(pref, "Transform\\BBox Max"),
        shared_str().printf("{%3.2f, %3.2f, %3.2f}", VPUSH(GetBox().vMax)));
...
void CEditableObject::FillSummaryProps(LPCSTR pref, PropItemVec& items)
{
    ...
    for (EditMeshIt m_it = FirstMesh(); m_it != LastMesh(); m_it++)
    {
        ...
        PHelper().CreateCaption(items,
            PrepareKey(pref, AnsiString(AnsiString("Geometry\\Meshes\\") + MESH->Name().c_str()).c_str()), t.c_str());
    }
