...
void TUI::OnReceiveMail(LPCSTR msg)
{
    ...
    if (cnt)
    {
        ...
        {
            ELog.DlgMsg(
                mtInformation, "'%s EDITOR': Critical update!", AnsiString(UI->EditorName()).UpperCase().c_str());
            while (1)
            {
        ...
        {
            ELog.Msg(
                mtInformation, "'%s EDITOR': Super critical update!", AnsiString(UI->EditorName()).UpperCase().c_str());
        ...
        else if (p[0] == "info")
        {
            if (cnt > 1)
                ELog.DlgMsg(
                    mtInformation, "'%s EDITOR': %s", AnsiString(UI->EditorName()).UpperCase().c_str(), p[1].c_str());
        }
        else if (p[0] == "error")
        {
            if (cnt > 1)
                ELog.DlgMsg(mtError, "'%s EDITOR': %s", AnsiString(UI->EditorName()).UpperCase().c_str(), p[1].c_str());
        }
    }
}
