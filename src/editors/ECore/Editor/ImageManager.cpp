...
bool CImageManager::MakeGameTexture(LPCSTR game_name, u32 *data, const STextureParams &tp)
{
    ...
    if (1!=res)
    {
        ...
        switch (res)
        {
            case 0: ELog.DlgMsg(mtError, "Can't make game texture '%s'.", game_name);
                break;
            case -1000: ELog.Msg(mtError, "Invalid gloss mask '%s'.", game_name);
                return true;
        }
        return false;
    }
...
bool CImageManager::MakeGameTexture(ETextureThumbnail *THM, LPCSTR game_name, u32 *load_data)
{
    ...
    if (1!=res)
    {
        ...
        switch (res)
        {
            case 0: ELog.DlgMsg(mtError, "Can't make game texture '%s'.", THM->m_SrcName.c_str());
                break;
            case -1000: ELog.Msg(mtError, "Invalid gloss mask '%s'.", THM->m_SrcName.c_str());
                return true;
        }
        return false;
    }
...
void GET(U32Vec &pixels, u32 w, u32 h, u32 x, u32 y, u32 ref, u32 &count, u32 &r, u32 &g, u32 &b)
{
    // wrap pixels
    if (x>=w)
        return;
