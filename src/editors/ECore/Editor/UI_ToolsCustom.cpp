...
bool __fastcall CToolCustom::MouseStart(TShiftState Shift)
{
    ...
    if (m_Action == etaMove && m_pAxisMoveObject)
    {
        ...
        if (pinfo.e_mesh)
        {
            ...
            if (0 == xr_stricmp(mn, "axis_x"))
                SetAxis(etAxisX);
            else if (0 == xr_stricmp(mn, "axis_y"))
                SetAxis(etAxisY);
            else if (0 == xr_stricmp(mn, "axis_z"))
                SetAxis(etAxisZ);
            else if (0 == xr_stricmp(mn, "center"))
                SetAxis(etAxisZX);
            else
             ...
             ++s_it)
        {
            ...
            for (int idx = 0; idx < m_pAxisMoveObject->Meshes().size(); ++idx)
            {
                ...
                if ((m_Axis == idx) || (idx == etAxisZX) ||
                    (m_Axis == etAxisZX && (idx == etAxisX || idx == etAxisZ)) || (m_Axis == etAxisUndefined))
