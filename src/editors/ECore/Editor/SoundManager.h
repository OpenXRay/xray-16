...
public:
    ...
    void SynchronizeSounds(bool sync_thm, bool sync_game, bool bForceGame, FS_FileSet* source_map,
        AStringVec* sync_list_without_extention, FS_FileSet* modif_map = 0);
    //	void 		ChangeFileAgeTo		(FS_FileSet* tgt_map, int age);
    void CreateSoundThumbnail(
        ESoundThumbnail* THM, const AnsiString& src_name, LPCSTR path = 0, bool bSetDefParam = true);
    void CleanupSounds();
