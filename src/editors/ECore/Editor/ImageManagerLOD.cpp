...
void CreateLODSamples(const Fbox& bbox, U32Vec& tgt_data, u32 tgt_w, u32 tgt_h)
{
    ...
    for (int frame = 0; frame < LOD_SAMPLE_COUNT; frame++)
    {
        ...
        imf_Process(
            d_pixels.begin(), tgt_w, tgt_h, s_pixels.begin(), tgt_w * lod_ss_quality, tgt_h * lod_ss_quality, imf_box);
        // copy LOD to final
        for (u32 y = 0; y < tgt_h; y++)
...
void CImageManager::CreateLODTexture(
    CEditableObject* OBJECT, U32Vec& lod_pixels, U32Vec& nm_pixels, u32 tgt_w, u32 tgt_h, int _samples, int quality)
{
    // build hemi light
    BLVec simple_hemi;
    // fill simple hemi
