...
bool ESceneWallmarkTool::LoadStream(IReader& F)
{
    ...
    if (version == 0x0003) m_ShName = "effects\\wallmarkmult";
    ...
    for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++)
    {
        ...
        for (WMVecIt w_it = slot->items.begin(); w_it != slot->items.end(); w_it++)
        {
            ...
            if (W->verts.size() > MAX_WALLMARK_VERTEX_COUNT) {
                ELog.Msg(mtError, "ERROR: Invalid wallmark (Contain more than %d vertices). Removed.",
...
BOOL ESceneWallmarkTool::AddWallmark_internal(
    const Fvector& start, const Fvector& dir, shared_str sh, shared_str tx, float width, float height, float rotate)
{
...
void ESceneWallmarkTool::GetStaticDesc(int& v_cnt, int& f_cnt, bool b_selected_only, bool b_cform)
{
    ...
    for (WMSVecIt slot_it = marks.begin(); slot_it != marks.end(); slot_it++)
    {
        ...
        for (WMVecIt w_it = slot->items.begin(); w_it != slot->items.end(); w_it++)
        {
            ...
            if (b_selected_only && !W->flags.test(wallmark::flSelected)) continue;
