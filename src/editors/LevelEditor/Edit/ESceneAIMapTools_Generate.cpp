...
void SnapXZ(Fvector &V, float ps)
{
    V.x = snapto(V.x, ps);
    V.z = snapto(V.z, ps);
}
...
bool ESceneAIMapTool::RealUpdateSnapList()
{
    ...
    if (!GetSnapList()->empty())
    {
        ...
        if (nodes_bb.is_valid())
            bb.merge(snap_bb, nodes_bb); else
            bb.set(snap_bb);
...
SAINode *ESceneAIMapTool::FindNeighbor(SAINode *N, int side, bool bIgnoreConstraints)
{
    ...
    switch (side)
    {
        case 0: Pos.x -= m_Params.fPatchSize;
            break;
        case 1: Pos.z += m_Params.fPatchSize;
            break;
        case 2: Pos.x += m_Params.fPatchSize;
            break;
        case 3: Pos.z -= m_Params.fPatchSize;
            break;
    }
