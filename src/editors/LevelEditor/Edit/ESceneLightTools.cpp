...
void ESceneLightTool::FillProp(LPCSTR pref, PropItemVec& items)
{
    ...
    for (; _I != _E; _I++)
    {
        ...
        {
            B = PHelper().CreateButton(items, PrepareKey(pref, "Common\\Controls\\User", *_I->name), "Rename,Remove",
                ButtonValue::flFirstOnly);
...
CEditFlare::flSource);
    PHelper().CreateFloat	(items, PHelper().PrepareKey(pref,"Sun\\Source\\Radius"),			&F.m_Source.fRadius,
0.f,10.f);
    prop 					= PHelper().CreateTexture(items, PHelper().PrepareKey(pref,"Sun\\Source\\Texture"),
F.m_Source.texture,		sizeof(F.m_Source.texture));
    prop->OnChangeEvent		= OnNeedUpdate;

    PHelper().CreateFlag32(items, PHelper().PrepareKey(pref,"Sun\\Gradient\\Enabled"),		&F.m_Flags,
CEditFlare::flGradient);
    PHelper().CreateFloat	(items, PHelper().PrepareKey(pref,"Sun\\Gradient\\Radius"),		&F.m_Gradient.fRadius,
0.f,100.f);
    PHelper().CreateFloat	(items, PHelper().PrepareKey(pref,"Sun\\Gradient\\Opacity"),		&F.m_Gradient.fOpacity,
0.f,1.f);
    prop					= PHelper().CreateTexture(items, PHelper().PrepareKey(pref,"Sun\\Gradient\\Texture"),
F.m_Gradient.texture,	sizeof(F.m_Gradient.texture));
    prop->OnChangeEvent		= OnNeedUpdate;
...
l->m_LensFlare.m_Flags.is(CLensFlare::flGradient)?"on":"off");
                   pIni->w_string(l->Name, "source",
l->m_LensFlare.m_Flags.is(CLensFlare::flSource)?"on":"off");
