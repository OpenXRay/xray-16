...
BOOL SceneBuilder::BuildLTX()
{
    ...
    if (FS.exist(ltx_filename.c_str())) EFS.MarkFile(ltx_filename.c_str(), true);
    ...
    if (F) {
        ...
        F->w_string(buff);
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDDeathmatch)) F->w_string("deathmatch");
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDTeamDeathmatch)) F->w_string("teamdeathmatch");
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDArtefactHunt)) F->w_string("artefacthunt");
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDCaptureTheArtefact)) F->w_string("capturetheartefact");
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDDominationZone)) F->w_string("dominationzone");
        if (Scene->m_LevelOp.m_mapUsage.MatchType(eGameIDTeamDominationZone)) F->w_string("teamdominationzone");
        ...
        F->w_string(";");
        if (Scene->m_LevelOp.m_BOPText.size()) F->w_stringZ(Scene->m_LevelOp.m_BOPText);
