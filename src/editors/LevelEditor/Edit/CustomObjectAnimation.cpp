...
void CCustomObject::OnMotionControlClick(ButtonValue* value, bool& bModif, bool& bSafe)
{
    ...
    break;
    case 3: { m_MotionParams->bPlay = TRUE;
    }
    break;
    case 4: { m_MotionParams->bPlay = FALSE;
    }
    break;
    case 5:
    {
...
void CCustomObject::OnMotionCommandsClick(ButtonValue* value, bool& bModif, bool& bSafe)
{
    ...
    switch (B->btn_num)
    {
    case 0: AnimationCreateKey(m_MotionParams->t_current); break;
    case 1: AnimationDeleteKey(m_MotionParams->t_current); break;
    case 2:
    {
...
void CCustomObject::AnimationFillProp(LPCSTR pref, PropItemVec& items)
{
    ...
    if (Motionable())
    {
        ...
        V = PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\Start Frame (sec)"), &m_MotionParams->min_t,
            -10000.f, m_MotionParams->max_t, 1.f / 30.f, 3);
        V->OnChangeEvent.bind(this, &CCustomObject::OnMotionFrameChange);
        V = PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\End Frame (sec)"), &m_MotionParams->max_t,
            m_MotionParams->min_t, 10000.f, 1.f / 30.f, 3);
        V->OnChangeEvent.bind(this, &CCustomObject::OnMotionFrameChange);
        V = PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\Current Frame (sec)"), &m_MotionParams->t_current,
            -10000.f, 10000.f, 1.f / 30.f, 3);
        V->OnChangeEvent.bind(this, &CCustomObject::OnMotionCurrentFrameChange);

        V = PHelper().CreateFloat(items, PrepareKey(pref, "Motion\\ChangeKeyTime(sec)"), &m_MotionParams->tmp, -10000.f,
            10000.f, 1.f / 30.f, 3);
