...
void CSectorItem::GetTransform(Fmatrix& parent) { object->GetFullTransformToWorld(parent); }
bool CSectorItem::IsItem(const char* O, const char* M)
{
    return (0 == xr_stricmp(O, object->Name)) && (0 == _stricmp(M, mesh->Name().c_str()));
}

//------------------------------------------------------------------------------

CSector::CSector(LPVOID data, LPCSTR name) : CCustomObject(data, name) { Construct(data); }
