...
//---------------------------------------------------------------------------
xr_token function_token[] = {{"Constant", WaveForm::fCONSTANT}, {"Sin", WaveForm::fSIN},
    {"Triangle", WaveForm::fTRIANGLE}, {"Square", WaveForm::fSQUARE}, {"Saw-Tooth", WaveForm::fSAWTOOTH},
    {"Inv Saw-Tooth", WaveForm::fINVSAWTOOTH}, {0, 0}};
...
{
    for (int i = 0; token_list[i].name; i++)
        if (!xr_stricmp(val, token_list[i].name))
            return token_list[i].id;
    return 0;
}

//---------------------------------------------------------------------------
__fastcall TfrmShaderFunction::TfrmShaderFunction(TComponent* Owner) : TForm(Owner)
{
...
void __fastcall TfrmShaderFunction::stFunctionMouseDown(
...
void __fastcall TfrmShaderFunction::FormClose(TObject* Sender, TCloseAction& Action)
{
    Action = caFree;
    form = 0;
}
