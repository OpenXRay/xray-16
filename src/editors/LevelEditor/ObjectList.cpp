            ...
            if (OBJCLASS_GROUP == it->first)
            {
                for (ObjectIt _F = lst.begin(); _F != lst.end(); ++_F)
                {
                    ...
                    for (ObjectIt _G = grp_lst.begin(); _G != grp_lst.end(); ++_G)
                    {
                        ...
                        TElTreeItem* obj = AddObject(grp_node, str_name, (void*)(*_G),
                            (*_G)->m_CO_Flags.test(CCustomObject::flObjectInGroupUnique) ? clBlack :
                                                                                           TColor(0x00a9a6a0));
                    }
                }
...
void __fastcall TfrmObjectList::ElEdit1KeyDown(TObject* Sender, WORD& Key, TShiftState Shift)
{
    if (Key == VK_F3)
    {
        ...
        ProcessFindItemInList(
            (find_node && find_node->GetNext()) ? find_node->GetNext() : tvItems->Items->GetFirstNode(), str);
    }
}
