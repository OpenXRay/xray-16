...
__fastcall TClipMaker::TClipMaker(TComponent* Owner) : TForm(Owner)
{
    ...
    m_ObjectItems = TItemList::CreateForm(
        "", paAnimSelect, alClient, TItemList::ilDragCustom | TItemList::ilMultiSelect | TItemList::ilSuppressStatus);
}
...
//.
//---------------------------------------------------------------------------
void __fastcall TClipMaker::ClipDragOver(TObject* Sender, TObject* Source, int X, int Y, TDragState State, bool& Accept)
{
    // TEMP
    /*
        Accept = false;
...
void __fastcall TClipMaker::ClipDragDrop(TObject* Sender, TObject* Source, int X, int Y)
{
...
void __fastcall TClipMaker::ClipStartDrag(TObject* Sender, TDragObject*& DragObject)
{
...
void __fastcall TClipMaker::ClipEndDrag(TObject* Sender, TObject* Target, int X, int Y)
{
    drag_obj = 0xFFFF;
    RepaintClips();
}
...
void __fastcall TClipMaker::ClipMouseMove(TObject* Sender, TShiftState Shift, int X, int Y)
{
    VERIFY(sel_clip);
    ...
    {
        if (!g_resizing) P->Cursor = crDefault;
    }
    if (g_resizing) {
        ...
        if (!fis_zero(dx)) {
            ...
            UI->ShowHint(AnsiString().sprintf(
                "Length: %s", FloatTimeToStrTime(sel_clip->Length(), false, true, true, true).c_str()));
        }
        UpdateClips();
    }
}
...
void __fastcall TClipMaker::ClipMouseUp(TObject* Sender, TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if (Button == mbLeft) {
        UI->HideHint();
        g_resizing = FALSE;
...
void __fastcall TClipMaker::BPDragDrop(TObject* Sender, TObject* Source, int X, int Y)
{
...
void __fastcall TClipMaker::BPMouseDown(TObject* Sender, TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TMxPanel* P = dynamic_cast<TMxPanel*>(Sender);
    if (P) {
        if (Button == mbRight) {
...
void __fastcall TClipMaker::BPMouseUp(TObject* Sender, TMouseButton Button, TShiftState Shift, int X, int Y)
{
    drag_state = Shift;
}
//---------------------------------------------------------------------------
    ...
    if (sel_clip) {
        ...
        /*
           ...
           (k<(u16)m_CurrentObject->BoneParts().size())?&m_CurrentObject->BoneParts()[k]:0;
                    ...
                    shared_str tmp;
                    if (MI)
                        tmp.sprintf("%s [%3.2fs, %s]",mname.c_str(),MI->GetLength()/MD->Speed(),MD->bone_or_part?"stop
           at end":"looped");
                ...
                if (sel_clip->fx.valid())
                    PHelper().CreateFloat	(p_items,PrepareKey("Current Clip\\FXs",*sel_clip->fx.name),
...
void __fastcall TClipMaker::ClipPaint(TObject* Sender)
{
    ...
    for (AnimClipIt it = clips.begin(); it != clips.end(); ++it)
    {
        ...
        canvas->Pen->Width = 1;
        canvas->Brush->Color =
            (*it == sel_clip) ? (drag_obj == P->Tag ? CLIP_ACTIVE_DRAG_COLOR : CLIP_ACTIVE_COLOR) : CLIP_INACTIVE_COLOR;
        canvas->Rectangle(R);
        R.Top += 1;
        R.Bottom -= 1;
        R.Left += 1;
...
void __fastcall TClipMaker::BPOnPaint(TObject* Sender)
{
    ...
    else if ((bp->Tag >= 0) && (bp->Tag < MAX_PARTS))
    {
        ...
        for (AnimClipIt it = clips.begin(); it != clips.end(); ++it)
        {
            ...
            if (!mn.IsEmpty()) {
                canvas->Brush->Color = (*it == sel_clip) ?
...
void TClipMaker::RealUpdateClips()
{
    ...
    // select default clip
    if (!clips.empty() && (sel_clip == 0)) SelectClip(clips[0]);
}
//---------------------------------------------------------------------------
...
void TClipMaker::PlayAnimation(CAnimationClip* clip)
{
    u8 pc = m_RenderObject->partitions().count();
    for (u8 k = 0; k < pc; ++k)
        if (clip->animItems[k].valid()) m_RenderObject->LL_PlayCycle(k, clip->animItems[k].mid, FALSE, 0, 0, 0);
}
//---------------------------------------------------------------------------

void TClipMaker::OnFrame()
{
    if (m_RTFlags.is(flRT_UpdateClips)) RealUpdateClips();
    if (m_RTFlags.is(flRT_RepaintClips)) RealRepaintClips();
...
void __fastcall TClipMaker::ebTrashDragOver(
...
void __fastcall TClipMaker::ebTrashDragDrop(TObject* Sender, TObject* Source, int X, int Y)
{
    TMxPanel* P = dynamic_cast<TMxPanel*>(Source);
    if (P == paClips) {
        RemoveClip(sel_clip);
...
{
    if (mrYes == ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbCancel, "Remove all clips?")) Clear();
}
//---------------------------------------------------------------------------
