...
IC
    ...
    bool
    clip_pred(TClipMaker::CUIClip* x, TClipMaker::CUIClip* y)
{
...
IC
    ...
    bool
    clip_pred_float(float x, TClipMaker::CUIClip* y)
{
    return x < y->run_time;
};
...
void TClipMaker::RealUpdateProperties()
{
    ...
    if (sel_clip) {
        ...
        for (u16 k = 0; k < 4; k++)
        {
            ...
            shared_str tmp;
            if (MI)
                tmp.sprintf("%s [%3.2fs, %s]", mname.c_str(), MI->GetLength() / MD->Speed(),
        ...
        if (sel_clip->fx.valid())
            PHelper().CreateFloat(
                p_items, PrepareKey("Current Clip\\FXs", *sel_clip->fx.name), &sel_clip->fx_power, 0.f, 1000.f);
    }
...
void TClipMaker::PlayAnimation(CUIClip* clip)
{
    for (u32 k = 0; k < m_CurrentObject->BoneParts().size(); k++)
        if (clip->cycles[k].valid()) ATools->m_RenderObject.PlayCycle(*clip->cycles[k].name, k, clip->cycles[k].slot);
