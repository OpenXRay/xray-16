...
TClipMaker* TClipMaker::CreateForm() { return new TClipMaker((TComponent*)0); }
...
TClipMaker::CUIClip* TClipMaker::FindClip(int x) { return FindClip(float(x) / m_Zoom); }
//---------------------------------------------------------------------------
...
void TClipMaker::PlayAnimation(CUIClip* clip)
{
    ...
    if (clip->fx.valid())
        ATools->m_RenderObject.PlayFX(*clip->fx.name, clip->fx_power, clip->fx.slot);
}
