    ...
    switch (V->btn_num)
    {
        ...
        { // export
            ...
            if (EFS.GetSaveName(_import_, fname))
            {
                ...
                xr_vector<shared_str>::iterator
                it = m_pEditObject->m_SMotionRefs.begin();
                xr_vector<shared_str>::iterator
                it_e = m_pEditObject->m_SMotionRefs.end();
                string64 buff;
        ...
        { // save
            ...
            if (mr!=mrCancel)
            {
                if (EFS.GetSaveName(_smotion_, fn, 0, 1))
                {
                    switch (mr)
                    {
                        case mrYes: SaveMotions(fn.c_str(), true);
                            break;
                        case mrNo: SaveMotions(fn.c_str(), false);
                            break;
                    }
                }
            }
...
void CActorTools::OnMarksControlClick12(ButtonValue *V, bool &bModif, bool &bSafe)
{
    switch (V->btn_num)
    {
        case 0: AddMarksChannel(true);
            break;
        case 1: RemoveMarksChannel(true);
            break;
    }
    bModif = true;
}
...
void CActorTools::OnMarksControlClick34(ButtonValue *V, bool &bModif, bool &bSafe)
{
    switch (V->btn_num)
    {
        case 0: AddMarksChannel(false);
            break;
        case 1: RemoveMarksChannel(false);
            break;
    }
    bModif = true;
}
...
void CActorTools::OnBoxAxisClick(ButtonValue *V, bool &bModif, bool &bSafe)
{
    ...
    switch (V->btn_num)
    {
        case 0: BONE->shape.box.m_rotate.k.set(1, 0, 0);
            break;
        case 1: BONE->shape.box.m_rotate.k.set(0, 1, 0);
            break;
        case 2: BONE->shape.box.m_rotate.k.set(0, 0, 1);
            break;
    }
...
void CActorTools::OnCylinderAxisClick(ButtonValue *V, bool &bModif, bool &bSafe)
{
    ...
    switch (V->btn_num)
    {
        case 0: BONE->shape.cylinder.m_direction.set(1, 0, 0);
            break;
        case 1: BONE->shape.cylinder.m_direction.set(0, 1, 0);
            break;
        case 2: BONE->shape.cylinder.m_direction.set(0, 0, 1);
            break;
    }
    ExecCommand(COMMAND_UPDATE_PROPERTIES);
}
...
void CActorTools::OnBoneShapeClick(ButtonValue *V, bool &bModif, bool &bSafe)
{
    ...
    switch (V->btn_num)
    {
        case 0: m_pEditObject->GenerateBoneShape(false);
            break;
        case 1: m_pEditObject->GenerateBoneShape(true);
            break;
    }
}
