...
xr_token sa_token[] = {{"+Z", PreviewModel::saZp}, {"-Z", PreviewModel::saZn}, {"+X", PreviewModel::saXp},
    {"-X", PreviewModel::saXn}, {0, 0}};
...
bool CActorTools::OnCreate()
{
    ...
    m_ObjectItems = TItemList::CreateForm("", fraLeftBar->paObjectProps, alClient,
        TItemList::ilDragCustom | TItemList::ilMultiSelect | TItemList::ilSuppressStatus);
    m_ObjectItems->SetOnItemsFocusedEvent(
        fastdelegate::bind<TOnILItemsFocused>(this, &CActorTools::OnObjectItemFocused));
    m_Props = TProperties::CreateForm("", fraLeftBar->paItemProps, alClient,
...
{
    if (physics_world()) destroy_physics_world();
            ...
            if (EPrefs->object_flags.is(epoDrawJoints)) {
                ...
                if (K) {
                    ...
                    for (u16 bidx = 0; bidx < bcnt; ++bidx)
                    {
                        ...
                        Fvector p1 = M.c;
                        u32 c_joint =
                            /*(*b_it)->flags.is(CBone::flSelected)?color_bone_sel_color:*/ color_bone_norm_color;
...
void CActorTools::OnFrame()
{
    ...
    if (m_pEditObject) {
        ...
        if (!fraLeftBar->ebRenderEngineStyle->Down) m_pEditObject->OnFrame();
...
void CActorTools::ZoomObject(BOOL bSelOnly)
{
    ...
    if (m_pEditObject) {
            ...
            if (m_pEditObject->GetSelectedBones(lst)) {
                ...
                for (BoneIt b_it = lst.begin(); b_it != lst.end(); b_it++)
                {
                    ...
                    case SBoneShape::stBox:
                        r = _max(_max((*b_it)->shape.box.m_halfsize.x, (*b_it)->shape.box.m_halfsize.y),
        ...
        break;
        default: BB = m_pEditObject->GetBox();
        }
...
void CActorTools::Clear()
{
    ...
    m_CurrentSlot = 0;
    // delete visuals
    if (m_pEditObject) m_pEditObject->DeletePhysicsShell();
    xr_delete(m_pEditObject);
    m_RenderObject.Clear();
    //	m_PreviewObject.Clear();
...
bool CActorTools::ExportCPP(LPCSTR name)
{
    if (m_pEditObject) {
        ...
        for (EditMeshIt m_it = meshes.begin(); m_it != meshes.end(); m_it++)
        {
            ...
            {
                sprintf(tmp, "\t\t%-d,\t\t%-d,\t\t%-d,", faces[f_id].pv[0].pindex, faces[f_id].pv[1].pindex,
...
bool __fastcall CActorTools::MouseEnd(TShiftState Shift)
{
    ...
    case etaMove:
    {
        ...
        {
        case emObject:
            if (Shift.Contains(ssCtrl)) OnMotionKeysModified();
            break;

        case emBone:
            if (Shift.Contains(ssCtrl)) OnBoneModified();

            if (Shift.Contains(ssAlt)) OnBoneModified();
            break;
        }
    }
    break;
...
void __fastcall CActorTools::MouseMove(TShiftState Shift)
{
    ...
    case etaMove:
    {
        ...
        {
        case emObject:
            if (true || Shift.Contains(ssCtrl)) m_pEditObject->a_vPosition.add(m_MovedAmount);
            break;
    ...
    case etaRotate:
    {
        ...
        {
        case emObject:
            if (Shift.Contains(ssCtrl)) m_pEditObject->a_vRotate.mad(m_RotateVector, m_RotateAmount);
            break;
