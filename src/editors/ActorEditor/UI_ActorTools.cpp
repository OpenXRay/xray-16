...
void PreviewModel::Render()
{
    if (m_pObject)
    {
        ...
        switch (m_ScrollAxis)
        {
            ...
            case saZn: angle = PI;
                break;
            case saXp: angle = PI_DIV_2;
                break;
            case saXn: angle = -PI_DIV_2;
                break;
            default: THROW;
        }
        Fmatrix R, T;
    ...
    if (IsModified())
    {
        ...
        switch (mr)
        {
            case mrYes: if (!ExecCommand(COMMAND_SAVE))
                ...
                break;
            case mrNo: m_bObjectModified = false;
                break;
...
void CActorTools::OnItemModified()
{
    switch (m_EditMode)
    {
        case emObject: OnObjectModified();
            break;
        case emMotion: OnMotionDefsModified();
            break;
        case emBone: OnBoneModified();
            break;
        case emSurface: OnObjectModified();
            break;
        case emMesh: break;
    }
}
