...
void CheckEffect(const xr_string& group_path, const shared_str& eff_full_name, xr_string& res_name, bool bRenameOnly)
{
    ...
    if (0 != xr_stricmp(res_name.c_str(), eff_full_name.c_str()))
    {
        ...
        VERIFY(0 == xr_stricmp(new_ped->m_Name.c_str(), res_name.c_str()));
    }
}
...
PS::CPEDef* CParticleTool::FindPE(LPCSTR name) { return ::Render->PSLibrary.FindPED(name); }
PS::CPGDef* CParticleTool::FindPG(LPCSTR name) { return ::Render->PSLibrary.FindPGD(name); }
