...
bool CParticleTool::OnCreate()
{
    ...
    m_ItemProps = TProperties::CreateForm("Item Props", fmItemProp, alClient,
        fastdelegate::bind<TOnModifiedEvent>(this, &CParticleTool::OnItemModified), 0, 0,
        TProperties::plFolderStore | TProperties::plFullExpand /*| TProperties::plItemFolders*/ | TProperties::plIFTop);
    m_ItemProps->Visible = true;
    // item list
    m_PList = TItemList::CreateForm(
        "Items", fraLeftBar->paPSList, alClient, TItemList::ilEditMenu | TItemList::ilDragAllowed);
...
void CParticleTool::Render()
{
    ...
    case emEffect:
    {
        if (m_EditPE && m_EditPE->GetDefinition()) m_EditPE->GetDefinition()->Render(m_Transform);
    }
    break;
    case emGroup:
    {
    ...
    if (m_Flags.is(flAnimatedPath)) m_ParentAnimator->DrawPath();
...
void CParticleTool::OnFrame()
{
    ...
    if (m_Flags.is(flApplyParent)) RealApplyParent();
...
void CParticleTool::SetCurrentPE(PS::CPEDef* P)
{
    ...
    if (m_LibPED != P) {
        ...
        if (m_LibPED) m_EditMode = emEffect;
    }
    FillReferenceList();
}
...
void CParticleTool::SetCurrentPG(PS::CPGDef* P)
{
    ...
    if (m_LibPGD != P) {
        ...
        if (m_LibPGD) m_EditMode = emGroup;
    }
    FillReferenceList();
}
...
void CParticleTool::MouseMove(TShiftState Shift)
{
    ...
    case etaSelect: break;
    case etaAdd: break;
    case etaMove: m_Transform.c.add(m_MovedAmount); break;
    case etaRotate:
    {
        Fmatrix mR;
        mR.identity();
...
PS::CPEDef* CParticleTool::AppendPE(PS::CPEDef* src)
{
    ...
    AnsiString new_name = FHelper.GenerateName(
        pref, 2, fastdelegate::bind<TFindObjectByName>(&(::Render->PSLibrary), &CPSLibrary::FindByName), false, true);
...
void CParticleTool::OnChangeMotion(PropValue* sender)
{
    ...
    if (V) {
        ...
        if (V->value->size()) m_ParentAnimator->Load(V->value->c_str());
    }
    if (m_Flags.is(flAnimatedParent)) m_ParentAnimator->Play(true);
}
