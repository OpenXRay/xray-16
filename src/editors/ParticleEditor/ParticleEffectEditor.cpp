...
void __fastcall PS::CPEDef::FindActionByName(LPCSTR new_name, bool& res)
{
    ...
    for (EPAVecIt s_it = m_EActionList.begin(); s_it != m_EActionList.end(); s_it++)
        if (0 == xr_stricmp(new_name, *(*s_it)->actionName))
        {
            res = true;
            break;
        };
...
void __fastcall PS::CPEDef::OnActionsClick(ButtonValue* B, bool& bDataModified, bool& bSafe)
{
    ...
    case 0:
    {
        ...
        if (TfrmChoseItem::SelectItem(
                smCustom, nm, 1, 0, fastdelegate::bind<TOnChooseFillItems>(this, &PS::CPEDef::FillActionList)) &&
            nm)
        {
            for (int i = 0; actions_token[i].name; i++)
            {
                    ...
                    A->actionName =
                        FHelper
                            .GenerateName(pref.c_str(), 2,
                                fastdelegate::bind<TFindObjectByName>(this, &PS::CPEDef::FindActionByName), true, true)
                            .LowerCase()
                            .c_str();
...
void PS::CPEDef::FillProp(LPCSTR pref, ::PropItemVec& items, ::ListItem* owner)
{
    ...
    for (EPAVecIt s_it = m_EActionList.begin(); s_it != m_EActionList.end(); s_it++)
    {
        ...
        shared_str a_pref = PrepareKey(
            pref, "Actions", AnsiString().sprintf("%s (%s)", *(*s_it)->actionType, *(*s_it)->actionName).c_str());
