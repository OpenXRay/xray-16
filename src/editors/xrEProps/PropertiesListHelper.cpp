...
PropValue* CPropHelper::AppendValue(
    PropItemVec& items, const shared_str& key, PropValue* val, EPropType type, u32 item_flags)
{
...
ButtonValue* CPropHelper::CreateButton(
    PropItemVec& items, shared_str key, shared_str val, u32 flags, ButtonValue::TOnBtnClick onClick)
{
...
FloatValue* CPropHelper::CreateFloat(
    PropItemVec& items, shared_str key, float* val, float mn, float mx, float inc, int decim)
{
...
VectorValue* CPropHelper::CreateVector(
    PropItemVec& items, shared_str key, Fvector* val, float mn, float mx, float inc, int decim)
{
    return (VectorValue*)AppendValue(items, key, new VectorValue(val, mn, mx, inc, decim), PROP_VECTOR);
}
Flag8Value* CPropHelper::CreateFlag8(
    PropItemVec& items, shared_str key, Flags8* val, u8 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
...
Flag16Value* CPropHelper::CreateFlag16(
    PropItemVec& items, shared_str key, Flags16* val, u16 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
...
Flag32Value* CPropHelper::CreateFlag32(
    PropItemVec& items, shared_str key, Flags32* val, u32 mask, LPCSTR c0, LPCSTR c1, u32 flags)
{
...
TokenValueSH* CPropHelper::CreateTokenSH(
...
//---------------------------------------------------------------------------
ChooseValue* CPropHelper::CreateChoose(PropItemVec& items, shared_str key, shared_str* val, u32 mode, LPCSTR path,
    void* fill_param, u32 sub_item_count, u32 choose_flags)
{
...
FloatValue* CPropHelper::CreateAngle(
    PropItemVec& items, shared_str key, float* val, float mn, float mx, float inc, int decim)
{
...
RTextValue* CPropHelper::CreateNameCB(PropItemVec& items, shared_str key, shared_str* val, TOnDrawTextEvent draw,
    RTextValue::TOnBeforeEditEvent before, RTextValue::TOnAfterEditEvent after)
{
    RTextValue* V = (RTextValue*)CreateRText(items, key, val);
