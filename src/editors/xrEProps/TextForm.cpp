...
TfrmText* __fastcall TfrmText::CreateForm(AnsiString& txt, LPCSTR caption, u32 flags, int lim, LPCSTR apply_name,
    TOnApplyClick on_apply, TOnCloseClick on_close, TOnCodeInsight on_insight)
{
    TfrmText* form = new TfrmText((TComponent*)0);
...
bool __fastcall TfrmText::RunEditor(AnsiString& txt, LPCSTR caption, u32 flags, int lim, LPCSTR apply_name,
    TOnApplyClick on_apply, TOnCloseClick on_close, TOnCodeInsight on_insight)
{
    TfrmText* form = new TfrmText((TComponent*)0);
    form = new TfrmText((TComponent*)0);
    form->Caption = caption;
    form->m_Text = &txt;
    form->mmText->ReadOnly = flags & flReadOnly;
    form->mmText->Text = txt;
...
{
    sbStatusPanel->Panels->Items[0]->Text =
        AnsiString().sprintf("%3d:%3d", mmText->CaretPos.y + 1, mmText->CaretPos.x + 1);
}
//---------------------------------------------------------------------------
