...
void TfrmChoseItem::AppendEvents(u32 choose_ID, LPCSTR caption, TOnChooseFillItems on_fill, TOnChooseSelectItem on_sel,
    TOnDrawThumbnail on_thm, TOnChooseClose on_close, u32 flags)
{
    EventsMapIt it = m_Events.find(choose_ID);
...
int __fastcall TfrmChoseItem::SelectItem(u32 choose_ID, LPCSTR& dest, int sel_cnt, LPCSTR init_name,
    TOnChooseFillItems item_fill, void* fill_param, TOnChooseSelectItem item_select, ChooseItemVec* items, u32 mask)
{
    VERIFY(!form);
...
void __fastcall TfrmChoseItem::tvMultiDragOver(
...
void __fastcall TfrmChoseItem::tvMultiStartDrag(TObject* Sender, TDragObject*& DragObject)
{
