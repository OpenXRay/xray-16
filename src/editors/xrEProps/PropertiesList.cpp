...
TProperties* TProperties::CreateForm(const AnsiString& title, TWinControl* parent, TAlign align, TOnModifiedEvent modif,
    TOnItemFocused focused, TOnCloseEvent on_close, u32 flags)
{
...
TProperties* TProperties::CreateModalForm(const AnsiString& title, bool bShowButtonsBar, TOnModifiedEvent modif,
    TOnItemFocused focused, TOnCloseEvent on_close, u32 flags)
{
...
void __fastcall TProperties::AssignItems(PropItemVec& items)
{
    ...
    // create EL items
    if (m_Flags.is(plItemFolders))
        m_Folders->AssignItems(folder_items, m_Flags.is(plFullExpand), m_Flags.is(plFullSort));
    // fill
    ...
    TObject* Sender, TElTreeItem* Item, TCanvas* Surface, TRect& R, int SectionIndex)
{
    ...
    else if (SectionIndex == 1)
    {
        ...
        else
        {
            ...
            case PROP_FLAG:
            {
                FlagValueCustom* V = dynamic_cast<FlagValueCustom*>(prop->GetFrontValue());
                R_ASSERT(V);
                if (V->HaveCaption())
                    OutText(prop->GetDrawText().c_str(), Surface, R, prop->Enabled(), m_BMEllipsis);
                else
                    OutBOOL(V->m_Flags.is(FlagValueCustom::flInvertedDraw) ? !V->GetValueEx() : V->GetValueEx(),
            ...
            case PROP_CHOOSE:
            {
                ...
                if (miDrawThumbnails->Checked && prop->m_Flags.is(PropItem::flDrawThumbnail))
                {
                    ...
                    if (!V->OnDrawThumbnailEvent.empty())
                        V->OnDrawThumbnailEvent(prop->GetDrawText().c_str(), Surface->Handle,
            ...
            break;
            case PROP_WAVE: OutText(prop->GetDrawText().c_str(), Surface, R, prop->Enabled(), m_BMEllipsis); break;
            case PROP_TOKEN:
            case PROP_RTOKEN:
            case PROP_SH_TOKEN:
            case PROP_RLIST:
            case PROP_CLIST: OutText(prop->GetDrawText().c_str(), Surface, R, prop->Enabled(), m_BMEllipsis); break;
            case PROP_TIME:
            {
        ...
        if ((HC == 1) && (Button == mbLeft))
        {
            ...
            if (m_FirstClickItem == item)
            {
                ...
                case PROP_BUTTON:
                {
                    ...
                    R_ASSERT(FV);
                    int btn_num =
                        iFloor((X - prop->draw_rect.x1) * (float(FV->value.size()) / float(prop->draw_rect.width())));
                ...
                {
                    pmEnum->Items->Clear();
                    TokenValueCustom* T = dynamic_cast<TokenValueCustom*>(prop->GetFrontValue());
                    R_ASSERT(T);
                ...
                {
                    pmEnum->Items->Clear();
                    RTokenValueCustom* T = dynamic_cast<RTokenValueCustom*>(prop->GetFrontValue());
                    R_ASSERT(T);
                    TMenuItem* mi = new TMenuItem((TComponent*)0);
                    mi->Caption = "-";
                    pmEnum->Items->Add(mi);
                ...
                case PROP_STEXT:
                case PROP_RTEXT:
                    if ((X - prop->draw_rect.x1) < (prop->draw_rect.width() - (m_BMEllipsis->Width + 2)))
                        PrepareLWText(item);
                    else
                        ExecTextEditor(prop);
                    break;
                case PROP_SHORTCUT: PrepareSCText(item); break;
                case PROP_TIME: PrepareLWText(item); break;
                case PROP_TEXTURE2:
                {
                    pmEnum->Items->Clear();
                    TMenuItem* mi = new TMenuItem((TComponent*)0);
        ...
        case PROP_BUTTON:
        {
            if (Shift.Contains(ssLeft))
            {
                ...
                if (((Y > prop->draw_rect.y1) && (Y < prop->draw_rect.y2)) &&
                    ...
                    btn_num =
                        iFloor((X - prop->draw_rect.x1) * (float(V->value.size()) / float(prop->draw_rect.width())));
                for (PropItem::PropValueIt it = prop->values.begin(); it != prop->values.end(); it++)
                {
                    ButtonValue* V = dynamic_cast<ButtonValue*>(*it);
                    R_ASSERT(V);
...
void __fastcall TProperties::PMItemClick(TObject* Sender)
{
    ...
    if (mi)
    {
        ...
        case PROP_TOKEN:
        {
            TokenValueCustom* V = dynamic_cast<TokenValueCustom*>(prop->GetFrontValue());
            R_ASSERT(V);
        ...
        case PROP_RTOKEN:
        {
            RTokenValueCustom* V = dynamic_cast<RTokenValueCustom*>(prop->GetFrontValue());
            R_ASSERT(V);
            BOOL bRes = FALSE;
...
void __fastcall TProperties::ChooseClick(TElTreeItem* item)
{
    ...
    //
    LPCSTR new_val = 0;
    if (TfrmChoseItem::SelectItem(V->m_ChooseID, new_val, V->subitem, edit_val.c_str(), 0, V->m_FillParam, 0,
...
void __fastcall TProperties::hkShortcut_KeyDown(TObject* Sender, WORD& Key, TShiftState Shift)
{
    /*
        if (VK_RETURN==Key){
            ApplySCText();
...
void __fastcall TProperties::tvPropertiesCompareItems(TObject* Sender, TElTreeItem* Item1, TElTreeItem* Item2, int& res)
{
