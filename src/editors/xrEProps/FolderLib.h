...
public:
    ...
    TElTreeItem* FindObject(
        TElTree* tv, AnsiString full_name, TElTreeItem** last_valid_node = 0, int* last_valid_idx = 0);
    TElTreeItem* FindFolder(
        TElTree* tv, AnsiString full_name, TElTreeItem** last_valid_node = 0, int* last_valid_idx = 0);
    TElTreeItem* FindItem(
        TElTree* tv, AnsiString full_name, TElTreeItem** last_valid_node = 0, int* last_valid_idx = 0);
    void GenerateFolderName(
        TElTree* tv, TElTreeItem* node, AnsiString& name, AnsiString pref = "folder", bool num_first = false);
    void GenerateObjectName(
        TElTree* tv, TElTreeItem* node, AnsiString& name, AnsiString pref = "object", bool num_first = false);
    ...
    BOOL RemoveItem(
        TElTree* tv, TElTreeItem* pNode, TOnItemRemove OnRemoveItem, TOnItemAfterRemove OnAfterRemoveItem = 0);
    // drag'n'drop
