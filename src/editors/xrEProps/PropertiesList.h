...
class XR_EPROPS_API TProperties : public TForm
{
    ...
    void __fastcall tvPropertiesItemDraw(
    ...
    void __fastcall tvPropertiesShowLineHint(TObject* Sender, TElTreeItem* Item, TElHeaderSection* Section,
        TElFString& Text, THintWindow* HintWindow, TPoint& MousePos, bool& DoShowHint);
    void __fastcall tvPropertiesCompareItems(TObject* Sender, TElTreeItem* Item1, TElTreeItem* Item2, int& res);
...
public: // User declarations
    ...
    static TProperties* CreateModalForm(const AnsiString& title, bool bShowButtonsBar = true,
        TOnModifiedEvent modif = 0, TOnItemFocused focused = 0, TOnCloseEvent close = 0,
        u32 flags = plFolderStore | plFullExpand);
    static void DestroyForm(TProperties*& props);
    static int EditPropertiesModal(PropItemVec& values, LPCSTR title, bool bShowButtonsBar = true,
        TOnModifiedEvent modif = 0, TOnItemFocused focused = 0, TOnCloseEvent close = 0,
        u32 flags = plFolderStore | plFullExpand);
    ...
    {
        fs->WriteInteger(
            AnsiString().sprintf("%s_column0_width", Caption.c_str()), tvProperties->HeaderSections->Item[0]->Width);
        fs->WriteInteger(
            AnsiString().sprintf("%s_column1_width", Caption.c_str()), tvProperties->HeaderSections->Item[1]->Width);
