...
//---------------------------------------------------------------------------
xr_token mode_token[] = {{"Programmable", CMatrix::modeProgrammable}, {"TCM", CMatrix::modeTCM},
    {"Spherical Reflection", CMatrix::modeS_refl}, {"Cube Reflection", CMatrix::modeC_refl}, {0, 0}};
...
void CSHEngineTools::RealUpdateProperties()
{
    ...
    { // fill Tree
        ...
        while (!data.eof())
        {
            ...
            switch (type)
            {
            case xrPID_MARKER: marker_text = key; break;
            case xrPID_TOKEN:
            {
                xrP_TOKEN* V = (xrP_TOKEN*)data.pointer();
                sz = sizeof(xrP_TOKEN) + sizeof(xrP_TOKEN::Item) * V->Count;
                PHelper().CreateTokenSH(items, PrepareKey(marker_text.c_str(), key), &V->IDselected,
