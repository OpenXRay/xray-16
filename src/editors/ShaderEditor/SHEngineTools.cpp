...
xr_token preview_obj_token[] = {{"None", pvoNone}, {"-", pvoNone}, {"Plane", pvoPlane}, {"Box", pvoBox},
    {"Sphere", pvoSphere}, {"Teapot", pvoTeapot}, {"-", pvoNone}, {"Custom...", pvoCustom}, {0, 0}};
...
bool CSHEngineTools::OnPreviewObjectRefChange(PropValue* sender, u32& new_val)
{
    ...
    case pvoCustom:
        ...
        if (!TfrmChoseItem::SelectItem(smObject, fn)) return false;
...
LPCSTR CSHEngineTools::AppendItem(LPCSTR folder_name, LPCSTR parent_name)
{
    ...
    if (!parent) {
        ...
        if (!TfrmChoseItem::SelectItem(
                smCustom, M, 1, 0, fastdelegate::bind<TOnChooseFillItems>(this, &CSHEngineTools::FillChooseTemplate)) ||
            !M)
            return 0;
        for (TemplateIt it = m_TemplatePalette.begin(); it != m_TemplatePalette.end(); it++)
...
{
    if (type == TYPE_OBJECT) RemoteRenameBlender(old_full_name, new_full_name);
}
