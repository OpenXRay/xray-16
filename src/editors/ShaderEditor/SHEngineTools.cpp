        ...
        // Load blenders
        {
            ...
            while ((chunk = fs->open_chunk(chunk_id)) != NULL)
            {
                ...
                if (B)
                {
                    ...
                    std::pair<BlenderPairIt, bool> I = m_Blenders.insert(make_pair(blender_name, B));
...
void CSHEngineTools::RealRenameItem(LPCSTR old_full_name, LPCSTR new_full_name)
{
    ...
    std::pair<BlenderPairIt, bool> RES = m_Blenders.insert(make_pair(xr_strdup(new_full_name), B));
...
LPCSTR CSHEngineTools::AppendConstant(CConstant *src, CConstant **dest)
{
    ...
    C->dwReference = 1;
    char name[128];
    std::pair<ConstantPairIt, bool> I = m_Constants.insert(make_pair(xr_strdup(GenerateConstantName(name)), C));
    VERIFY(I.second);
    if (dest)
        *dest = C;
...
LPCSTR CSHEngineTools::AppendMatrix(CMatrix *src, CMatrix **dest)
{
    ...
    M->dwReference = 1;
    char name[128];
    std::pair<MatrixPairIt, bool> I = m_Matrices.insert(make_pair(xr_strdup(GenerateMatrixName(name)), M));
    VERIFY(I.second);
    if (dest)
        *dest = M;
    return I.first->first;
...
void CSHEngineTools::AppendItem(LPCSTR path, CLASS_ID cls_id, IBlender *parent)
{
    ...
    // insert blender
    std::pair<BlenderPairIt, bool> I = m_Blenders.insert(make_pair(xr_strdup(m_LastSelection.c_str()), B));
    R_ASSERT2(I.second, "shader.xr - found duplicate name!!!");
    // insert to TreeView
    ExecCommand(COMMAND_UPDATE_LIST);
    ExecCommand(COMMAND_UPDATE_PROPERTIES);
