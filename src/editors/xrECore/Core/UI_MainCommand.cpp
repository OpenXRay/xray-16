...
#include "editors/xrECore/Props/NumericVector.h"
#include "editors/xrECore/Props/TextEdit.h"
...
SECommand* FindCommandByName(LPCSTR nm)
{
    ...
    {
        SECommand*& CMD = cmds[cmd_idx];
        if (CMD && (0 == xr_stricmp(CMD->name, nm)))
            return CMD;
    }
    return 0;
}
...
SESubCommand* FindSubCommandByName(SECommand* CMD, LPCSTR nm)
{
    ...
    {
        SESubCommand* SUB_CMD = CMD->sub_commands[sub_cmd_idx];
        if (0 == xr_stricmp(SUB_CMD->desc.c_str(), nm))
            return SUB_CMD;
    }
    return 0;
}
        ...
        if (CMD && CMD->editable)
        {
            for (u32 sub_cmd_idx = 0; sub_cmd_idx < CMD->sub_commands.size(); sub_cmd_idx++)
            {
                ...
                if (SUB->p0.IsString() && SUB->p1.IsString())
                    sprintf(tmp, "%d, \"%s\",\"%s\"", SUB->shortcut.hotkey, xr_string(SUB->p0).c_str(),
...
{
    if (mrYes ==
        ELog.DlgMsg(mtConfirmation, TMsgDlgButtons() << mbYes << mbNo, "Are you sure to break current action?"))
    {
        NeedBreak();
