...
#define munmap_chunk(mst, p)                                                                                           \
    ...
    {                                                                                                                  \
        ...
        if (CALL_MUNMAP((char*)(p)-prevsize, (int)psize) == 0) ((struct malloc_state*)(mst))->footprint -= psize;      \
    } while (0)
...
#define arena_for_chunk(ptr)                                                                                           \
    (chunk_non_main_arena(ptr) ?                                                                                       \
            *(struct malloc_arena**)((char*)(ptr) + chunksize(ptr) - (FOOTER_OVERHEAD - SIZE_T_SIZE)) :                \
            &main_arena)
...
#define set_non_main_arena(mem, ar_ptr)                                                                                \
    ...
    {                                                                                                                  \
        ...
        size_t SZ = chunksize(P) - (is_mmapped(P) ? sizeof(struct malloc_arena*) : (FOOTER_OVERHEAD - SIZE_T_SIZE));   \
...
static void free_atfork(void* mem, const void* caller)
{
    ...
    tsd_getspecific(arena_key, vptr);
    if (vptr != ATFORK_ARENA_PTR) (void)mutex_lock(&ar_ptr->mutex);
    mspace_free(arena_to_mspace(ar_ptr), mem);
    if (vptr != ATFORK_ARENA_PTR) (void)mutex_unlock(&ar_ptr->mutex);
}
...
/*if (mp_.pagesize == 0)
  ptmalloc_init_minimal();*/
    ...
    if (__pthread_initialize != NULL) __pthread_initialize();
#endif /* !defined _LIBC */
#endif /* USE_STARTER & 1 */
#endif /* !defined NO_THREADS */
    mutex_init(&main_arena.mutex);
    main_arena.next = &main_arena;
    mspace = create_mspace_with_base((char*)&main_arena + MSPACE_OFFSET, sizeof(main_arena) - MSPACE_OFFSET, 0, 0, 0);
  ...
  if (! secure) {
    ...
    /*if(s[0]) mALLOPt(M_CHECK_ACTION, (int)(s[0] - '0'));
      __malloc_check_init();*/
    //  }
    if (__malloc_initialize_hook != NULL) (*__malloc_initialize_hook)();
...
void* public_mALLOc(size_t bytes)
{
    ...
    if (!ar_ptr) return 0;
    if (ar_ptr != &main_arena) bytes += FOOTER_OVERHEAD;
    victim = mspace_malloc(arena_to_mspace(ar_ptr), bytes);
    if (victim && ar_ptr != &main_arena) set_non_main_arena(victim, ar_ptr);
    (void)mutex_unlock(&ar_ptr->mutex);
    assert(!victim || is_mmapped(mem2chunk(victim)) || ar_ptr == arena_for_chunk(mem2chunk(victim)));
    return victim;
}
    ...
    void* public_rEALLOc(void* oldmem, size_t bytes)
{
    ...
    if (hook != NULL) return (*hook)(oldmem, bytes, RETURN_ADDRESS(0));
    ...
    if (newp && ar_ptr != &main_arena) set_non_main_arena(newp, ar_ptr);
    (void)mutex_unlock(&ar_ptr->mutex);
    ...
    void* public_mEMALIGn(size_t alignment, size_t bytes)
{
    ...
    if (hook != NULL) return (*hook)(alignment, bytes, RETURN_ADDRESS(0));
    ...
    if (p && ar_ptr != &main_arena) set_non_main_arena(p, ar_ptr);
    (void)mutex_unlock(&ar_ptr->mutex);
...
int public_pMEMALIGn(void** memptr, size_t alignment, size_t size)
{
    ...
    if (alignment % sizeof(void*) != 0 || !my_powerof2(alignment / sizeof(void*)) != 0 || alignment == 0) return EINVAL;
...
#define HALF_INTERNAL_SIZE_T (((size_t)1) << (8 * sizeof(size_t) / 2))
    ...
    if (mem && ar_ptr != &main_arena) set_non_main_arena(mem, ar_ptr);
    (void)mutex_unlock(&ar_ptr->mutex);
...
void** public_iCALLOc(size_t n, size_t elem_size, void* chunks[])
{
    ...
    if (ar_ptr != &main_arena) elem_size += FOOTER_OVERHEAD;
...
/*unsigned long in_use_b, system_b, avail_b;*/
    ...
    for (i = 0, ar_ptr = &main_arena;; ++i)
    {
        ...
        if (MALLOC_DEBUG > 1) {
            ...
            while (mseg)
            {
                ...
                fprintf(stderr, " seg %08lx-%08lx\n", *(unsigned long*)&mseg->base, *(unsigned long*)(&temp));
                mseg = mseg->next;
            }
        }
        ar_ptr = ar_ptr->next;
    ...
    if (main_arena.stat_starter > 0) fprintf(stderr, "starter hooks    = %10ld\n", main_arena.stat_starter);
#endif
}

struct virtual_alloc_arena g_ptmalloc3_arena;

void initialize_virtual_alloc_arena(struct virtual_alloc_arena* result, void* buffer, size_t buffer_size,
    size_t granularity, char const* arena_id, out_of_memory_handler_type handler,
