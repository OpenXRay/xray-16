...
int main(void)
{
    ...
    malloc_stats();
    p1 = malloc(18);
    if (p1 == NULL) merror("malloc (10) failed.");
    p2 = (char**)independent_calloc(25, CSIZE, 0);
    if (p2 == NULL) merror("independent_calloc (25, ...) failed.");
    p3 = (char**)independent_comalloc(10, sizes, 0);
    if (p3 == NULL) merror("independent_comalloc (10, ...) failed.");

    for (i = 0; i < 25; ++i)
    {
        for (j = 0; j < CSIZE; ++j)
            if (p2[i][j] != '\0') merror("independent_calloc memory not zeroed.");
        p2[i][CSIZE - 1] = 'x';
        if (i < 10) {
            p3[i][sizes[i] - 1] = 'y';
            if (i % 4 == 0) {
                p3[i] = realloc(p3[i], i * 11 + sizes[9 - i]);
                if (p3[i] == NULL) merror("realloc (i*11 + sizes[9-i]) failed.");
            }
            else if (i % 4 == 1)
            {
                free(p3[i]);
        ...
        if (i % 4 == 0) {
            ...
            if (p2[i] == NULL) merror("realloc (i*7 + 3) failed.");
        }
        else if (i % 4 == 1)
        {
            free(p2[i]);
