...
#define transform_dir(idx, res, SX, SY, SZ, T1)                                                                        \
    __asm movzx eax, WORD PTR[esi][idx * (TYPE u16)] S.m __asm movaps res, SX __asm sal eax, 5 __asm lea eax,          \
        [eax + eax * 4] __asm movaps T1, SY __asm mulps res, XMMWORD PTR[edx][eax][64] __asm mulps T1,                 \
        XMMWORD PTR[edx][eax][80] __asm addps res, T1 __asm movaps T1, SZ __asm mulps T1,                              \
        XMMWORD PTR[edx][eax][96] __asm addps res, T1
...
#define transform_dir2(idx, res, SX, SY, SZ, T1)                                                                       \
    __asm movzx eax, WORD PTR[esi] S.matrix##idx __asm movaps res, SX __asm sal eax, 5 __asm lea eax,                  \
        [eax + eax * 4] __asm movaps T1, SY __asm mulps res, XMMWORD PTR[edx][eax][64] __asm mulps T1,                 \
        XMMWORD PTR[edx][eax][80] __asm addps res, T1 __asm movaps T1, SZ __asm mulps T1,                              \
        XMMWORD PTR[edx][eax][96] __asm addps res, T1
