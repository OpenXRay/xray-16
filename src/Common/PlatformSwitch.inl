...
// XXX: Very basic impl to get it built, must be enchanced or reworked 
inline void *mmap(void */*addr*/, size_t length, int /*prot*/, int /*flags*/, int fd, off_t offset)
{
    FILE* mappedFile = fdopen(fd, "rb");
    if (mappedFile != nullptr)
    {
        //printf("~~~~ FILE OPENED (fd %d) len: %d off: %d\n", fd, length, offset);

        void* data = malloc(length);
        fseek(mappedFile, offset, SEEK_SET);
        fread(data, 1, length, mappedFile);
...
inline int	pthread_getname_np(pthread_t, char *, size_t) {return 0;}
...
inline long ptrace(enum __ptrace_request request, pid_t pid, void *addr, void *data)
{
        // XXX: TODO implement
        return 0;
}

inline size_t switch_mem_usage()
{
    // XXX: TODO implement
    return 42;
}
