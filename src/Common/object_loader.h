...
struct CLoader
{
    ...
    static void load_data(const char*& data, M& stream, const P& p) { NODEFAULT; }
    static void load_data(char*& data, M& stream, const P& p)
    {
        shared_str S;
        stream.r_stringZ(S);
        data = xr_strdup(*S);
    ...
    static void load_data(std::pair<T1, T2>& data, M& stream, const P& p)
    {
        ...
        {
            const bool value = object_type_traits::is_same<T1, const char*>::value;
            VERIFY(!value);
