...
- Make it a drop-in replacement for original engine
  - 100% compatibility and same behaviour
  - Compile engine into a single executable file that you can just drop into `bin` folder. (see [#210](https://github.com/OpenXRay/xray-16/issues/210))
- Support all three games in the series: SOC/CS/COP. (see [Supported games](#supported-games) below)
- Fix original S.T.A.L.K.E.R. series bugs
- Introduce a solid platform for modmakers
  - Add frame/render graph for those who want to add new graphics features.
  - Improve performance via refactoring the code, parallelizing the engine, making it multithreaded
  - Add new scripting, development and debugging features
- Clean up engine code, make it easily portable to new platforms, minimize platform-specific code
- Enhance player's experience with new graphics, gameplay and other features that can be enabled optionally. (by default, we stay close to vanilla)

## Main differences from original X-Ray
...
- We want to keep the game close to the vanilla, untouched state, so if you want to introduce new gameplay features, make sure it is optional, doesn't break compatibility with original game resources (i.e. everything in `gamedata` folder and `.db*`/`.xdb` archives)
